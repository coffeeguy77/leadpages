...
function HeroWebsiteShowcase(demo, opts) {
  ...
  return '<div class="prm-hero-showcase' + (opts.className ? ' ' + esc(opts.className) : '') + '">'
    ...
    + '<span class="prm-quote-toast-icon">' + renderIcon('check') + '</span>'
  ...
  groups.forEach(function(group, i) {
    ...
    const features = (c.features || []).map(function(f) {
      return '<li class="prm-feature-item">'
        + '<span class="prm-feature-ring">' + icon(f.icon) + '</span>'
...
function LeadFlowDiagram(steps, heading, eyebrow) {
  const list = steps || [];
  ...
  return '<div class="prm-lead-flow">'
    + (eyebrow ? '<p class="prm-eyebrow prm-eyebrow--lime">' + esc(eyebrow) + '</p>' : '')
    + (heading ? '<h2 class="prm-h2 prm-serif prm-on-dark">' + esc(heading) + '</h2>' : '')
    + connectedFlowArt()
    ...
    + (branchStep ? '<div class="prm-flow-branch">'
      + '<span class="prm-flow-branch-line" aria-hidden="true"></span>'
      + '<div class="prm-flow-step prm-flow-step--highlight">'
      + '<span class="prm-flow-node prm-flow-node--lime">' + icon(branchStep.icon) + '</span>'
  ...
  return '<ol class="prm-timeline" data-prm-timeline>'
    + list.map(function(s, i) {
      return '<li class="prm-timeline-step" style="--prm-step:' + i + '">'
        + '<span class="prm-timeline-icon">' + icon(s.icon || 'plan') + '</span>'
...
function ContactPanel(content, copy) {
  ...
  return '<div class="prm-contact-panel prm-contact-panel--dark">'
    + '<h3 class="prm-h3 prm-on-dark">' + esc(cc.formTitle || 'Let\u2019s plan your website') + '</h3>'
    + '<form class="prm-form prm-form--dark pt-enquiry-form" data-pl-lead-form data-pl-kind="partner-consultation" data-pl-extended="1" method="post" action="#">'
...
function FinalCTA(content, copy) {
  ...
  return '<section class="prm-final-cta" id="cta">'
    ...
    + '<span class="prm-final-icon">' + renderIcon('rocket') + '</span>'
