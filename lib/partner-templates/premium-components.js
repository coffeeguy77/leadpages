...
function DemoFaqPanel(faqs, copy) {
  ...
  return '<aside class="prm-demo-faq" id="faqs" aria-label="Frequently asked questions">'
    + (c.eyebrow ? '<p class="wc-eyebrow wc-eyebrow--pill">' + esc(c.eyebrow) + '</p>' : '')
    + (c.heading ? '<h3 class="prm-faq-heading prm-serif">' + esc(c.heading) + '</h3>' : '')
...
function IndustryTabs(groups, copy, opts) {
  ...
  const faqHtml = faqs.length ? DemoFaqPanel(faqs, opts.faqCopy || {}) : '';
  let panelsHtml = '';
  let tabsHtml = '';
  let indicatorsHtml = '';
...
function ReviewSlider(testimonials, opts) {
  ...
  return '<div class="prm-review-slider"'
    ...
    + list.map(function(t, i) {
      return ReviewCard(t, { slide: true, active: i === 0, index: i });
    }).join('')
    + '</div></div>'
    + (multi
