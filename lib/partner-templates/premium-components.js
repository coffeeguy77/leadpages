...
function HeroCustomerJourney(steps, opts) {
  ...
  return '<div class="prm-hero-journey"'
    ...
    + list.map(function(s, i) {
      return '<li class="prm-journey-step' + (i === 0 ? ' is-active' : '') + '" data-journey-step="' + i + '">'
        + '<span class="prm-journey-step-icon">' + renderIcon(s.icon) + '</span>'
        + '<span class="prm-journey-step-label">' + esc(s.label) + '</span>'
        + '</li>';
    }).join('')
    + '</ol>'
  ...
  function col(items, mod) {
    return '<ul class="prm-transform-list">'
      + items.map(function(item) {
        return '<li><span class="prm-transform-icon" aria-hidden="true">'
          + renderIcon(mod === 'before' ? 'square' : 'check')
          + '</span><span>' + esc(item) + '</span></li>';
      }).join('')
      + '</ul>';
  }
...
function LeadFlowDiagram(steps, opts) {
  ...
  return '<div class="prm-lead-flow">'
    ...
    + mainSteps.map(function(step, i) {
      return '<li class="prm-flow-step" data-prm-flow-step="' + i + '" style="--prm-step:' + i + '">'
