...
function screenSlidesHtml(demos, opts) {
  opts = opts || {};
  ...
  return demos.map(function(d, i) {
    ...
    const name = d.name || 'Live demo';
    return '<a class="prm-screen-slide' + active + '" data-prm-hero-slide="' + i + '"'
      + ' href="' + esc(d.url || '#demos') + '" target="_blank" rel="noopener"'
      + ' data-demo-host="' + esc(host) + '"'
      + ' data-demo-name="' + esc(name) + '"'
      + ' aria-hidden="' + (i === 0 ? 'false' : 'true') + '"'
      + (i === 0 ? '' : ' tabindex="-1"') + '>'
      + demoScreenHtml(d, variant === 'phone' ? Math.round(width * 0.55) : width)
      + '</a>';
  }).join('');
}
...
function DemoBrowser(demoOrDemos, opts) {
  ...
  const screenInner = rotating
    ? '<div class="prm-screen-stack" data-prm-hero-stack="browser">' + screenSlidesHtml(demos, { width: opts.width || 1200, variant: 'desktop' }) + '</div>'
  ...
  return '<div class="prm-browser' + cls + animate + '" data-prm-browser>'
    ...
    + '<span class="prm-browser-url"' + (rotating ? ' data-prm-hero-url' : '') + '>' + esc(host) + '</span>'
    + '</span>'
    + '</div>'
    + '</div>'
    + '<div class="prm-browser-body">'
    + '<div class="prm-browser-screen' + (rotating ? ' prm-browser-screen--stack' : '') + '">'
    + screenInner
    + '<span class="prm-screen-glare" aria-hidden="true"></span>'
    + '</div></div></div>';
}
...
function DemoPhone(demoOrDemos, opts) {
  ...
  const rotating = demos.length > 1;
  const screenInner = rotating
    ? '<div class="prm-screen-stack" data-prm-hero-stack="phone">' + screenSlidesHtml(demos, { width: opts.width || 480, variant: 'phone' }) + '</div>'
  ...
  return '<div class="prm-phone' + cls + animate + '" data-prm-phone>'
    + '<div class="prm-phone-bezel">'
    + '<span class="prm-phone-button prm-phone-button--silent" aria-hidden="true"></span>'
    + '<span class="prm-phone-button prm-phone-button--volume-up" aria-hidden="true"></span>'
    + '<span class="prm-phone-button prm-phone-button--volume-down" aria-hidden="true"></span>'
    + '<span class="prm-phone-button prm-phone-button--power" aria-hidden="true"></span>'
    + '<div class="prm-phone-island" aria-hidden="true"><span class="prm-phone-island-lens"></span></div>'
    + '<div class="prm-phone-screen' + (rotating ? ' prm-phone-screen--stack' : '') + '">'
    + screenInner
    + '<span class="prm-screen-glare prm-screen-glare--phone" aria-hidden="true"></span>'
    + '</div>'
    + '<span class="prm-phone-home" aria-hidden="true"></span>'
    + '</div></div>';
}

function heroPickerHtml(demos) {
  if (!demos || demos.length < 2) return '';
  return '<div class="prm-hero-picker" role="tablist" aria-label="Preview live website demos">'
    + demos.map(function(d, i) {
      const label = demoPickerLabel(d);
      return '<button type="button" role="tab" class="prm-hero-pick' + (i === 0 ? ' is-active' : '') + '"'
        + ' data-prm-hero-pick="' + i + '" aria-selected="' + (i === 0 ? 'true' : 'false') + '"'
        + ' aria-label="' + esc((d.industry || d.name || label) + ' demo') + '">'
        + '<span class="prm-hero-pick-dot" aria-hidden="true"></span>'
        + '<span class="prm-hero-pick-label">' + esc(label) + '</span>'
        + '</button>';
    }).join('')
    + '</div>';
}
...
function HeroWebsiteShowcase(demoOrDemos, opts) {
  ...
  return '<div class="prm-hero-showcase' + (opts.className ? ' ' + esc(opts.className) : '') + '"'
    + (rotating ? ' data-prm-hero-showcase data-prm-hero-interval="' + (opts.interval || 5500) + '"' : '')
    + '>'
    + '<div class="prm-hero-stage" aria-hidden="false">'
    + '<div class="prm-hero-stage-shadow" aria-hidden="true"></div>'
    + DemoBrowser(demos, { className: 'prm-hero-browser', width: 1400, animate: true })
    + DemoPhone(demos, { className: 'prm-hero-phone', width: 520, animate: true })
    + '</div>'
    + heroPickerHtml(demos)
