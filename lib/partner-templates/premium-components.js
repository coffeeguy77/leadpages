...
function IndustryTabs(groups, copy, opts) {
  ...
  groups.forEach(function(group, i) {
    ...
    const features = (opts.featuresForTab && opts.featuresForTab(group.tab.key)) || [];
    ...
    panelsHtml += '<div class="prm-demo-panel' + (active ? ' is-active' : '') + '"'
      ...
      + (features.length
        ? '<ul class="prm-demo-feature-list" data-prm-demo-features>'
...
function PartnerCard(content, ctx, copy) {
  ...
  return '<article class="prm-partner-card">'
    ...
    + (c.email ? '<a class="prm-btn prm-btn-ghost" href="#contact">Email</a>' : '')
    + '</div></article>';
}
...
function reviewSummaryHtml(opts) {
  ...
  if (rating && count) {
    const label = Number(rating).toFixed(1) + ' average · ' + String(count) + '+ verified reviews';
    const inner = '<div class="prm-review-stars" aria-hidden="true">'
      + [1, 2, 3, 4, 5].map(function() { return renderIcon('star'); }).join('')
      + '</div><p class="prm-review-rating">' + esc(label) + '</p>';
    if (link) {
  ...
  return '<div class="prm-review-summary prm-review-summary--fallback">'
    + '<p class="prm-review-rating">' + esc(fallback) + '</p></div>';
}
