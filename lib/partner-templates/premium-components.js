...
function LeadFlowDiagram(steps, heading) {
  ...
  return '<div class="prm-lead-flow">'
    ...
    + '<ol class="prm-flow-steps" data-prm-flow>'
    + list.map(function(step, i) {
      return '<li class="prm-flow-step" style="--prm-step:' + i + '">'
        ...
        + (i < list.length - 1 ? '<span class="prm-flow-connector" aria-hidden="true"></span>' : '')
        + '</li>';
    }).join('')
    + '</ol></div>';
}

function ServiceCard(pillar, cta) {
  if (!pillar) return '';
  return '<article class="prm-service-card" data-prm-lift>'
    ...
    + '<a class="prm-btn prm-btn-ghost" href="#contact">' + esc(cta || 'Plan my website') + '</a>'
    + '</article>';
}
...
function PartnerCard(content, ctx, copy) {
  ...
  return '<article class="prm-partner-card" data-prm-lift>'
    + '<div class="prm-partner-visual">' + visual + '</div>'
    + (pc.eyebrow ? '<p class="prm-eyebrow">' + esc(pc.eyebrow) + '</p>' : '')
    + '<h2 class="prm-h2">' + esc(pc.heading || ('Work with ' + p.displayName)) + '</h2>'
    + (b.shortIntro ? '<p class="prm-partner-intro">' + esc(b.shortIntro) + '</p>' : '')
    + (b.fullBio ? '<p class="prm-partner-bio">' + esc(String(b.fullBio).split('\n')[0]) + '</p>' : '')
...
function PlatformDiagram(content, copy) {
  ...
  return '<div class="prm-platform-diagram" data-prm-lift>'
    ...
    + '<span class="prm-platform-hub-label">' + esc(plc.connector || 'One connected service') + '</span>'
    + '</div>'
...
function ReviewCard(testimonial, contact, metrics) {
  const t = testimonial || {};
  const c = contact || {};
  const m = metrics || {};
  const stars = '★★★★★';
  return '<article class="prm-review-card" data-prm-lift>'
    ...
    + (m.typicalResponseTime ? '<p><strong>Response time:</strong> ' + esc(m.typicalResponseTime) + '</p>' : '')
    + (c.responseTime ? '<p><strong>Partner response:</strong> ' + esc(c.responseTime) + '</p>' : '')
    + (c.phone ? '<p><a href="tel:' + String(c.phone).replace(/[^+0-9]/g, '') + '">' + esc(c.phone) + '</a></p>' : '')
