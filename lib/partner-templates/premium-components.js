...
function heroDashboardEmbedded(copy) {
  ...
  return '<div class="prm-eco-dashboard" aria-label="LeadPages platform dashboard">'
    ...
    + widgets.map(function(widget, i) {
      ...
      return '<article class="wc-dashboard-widget" data-prm-eco-widget="' + i + '">'
...
function HeroProductEcosystem(demoOrDemos, opts) {
  ...
  const showTabs = rotating && opts.hideTabs !== true;
  const picker = showTabs ? heroDotNav(demos) : '';
  const hiddenPicks = rotating && opts.hideTabs
    ? '<div class="prm-hero-dots prm-hero-dots--sr" hidden aria-hidden="true">'
      + demos.map(function(d, i) {
        const thumb = demoThumbSrc(d, 1200);
        return '<button type="button" data-prm-hero-pick="' + i + '"'
          + ' data-demo-url="' + esc(d.url || '#demos') + '"'
          + ' data-demo-host="' + esc(demoHost(d)) + '"'
          + ' data-demo-name="' + esc(d.name || 'Live demo') + '"'
          + (thumb ? ' data-demo-thumb="' + thumb + '"' : '')
          + '></button>';
      }).join('')
      + '</div>'
    : '';
  const interval = opts.interval || 12000;
  const pipeline = [
  ...
  return '<div class="prm-hero-ecosystem" data-prm-hero-ecosystem'
    + (rotating ? ' data-prm-hero-showcase data-prm-hero-interval="' + interval + '"' : '')
    + (opts.hideTabs ? ' data-prm-hero-hide-tabs="1"' : '')
    ...
    + pipeline.map(function(step, i) {
      const bridge = i < pipeline.length - 1
        ...
        : '';
      return '<span class="prm-eco-pipeline-step" data-prm-eco-pipe="' + i + '" style="--prm-pipe:' + i + '">'
...
function IndustryTabs(groups, copy, opts) {
  ...
  const hideTabs = opts.hideTabs === true;
  const interval = opts.interval || 12000;
  ...
  const controls = showNav
    ? ('<div class="prm-demo-controls" data-prm-demo-controls>'
  ...
  const body = ''
    ...
    // Keep hidden tab buttons for carousel JS when named tabs are off.
    + (hideTabs && showNav
      ? '<div class="prm-tabs prm-tabs--sr" role="tablist" aria-label="Industry demos" hidden>' + tabsHtml + '</div>'
      : '')
    + controls
    + demoBody;

  if (opts.embedded) {
    return '<div class="wc-hero-demos prm-demo-showcase prm-demo-showcase--embedded' + (hideTabs ? ' prm-demo-showcase--no-tabs' : '') + '" id="' + esc(tabsId) + '"'
      + ' data-prm-demo-carousel data-prm-demo-interval="' + interval + '"'
      + (hideTabs ? ' data-prm-demo-hide-tabs="1"' : '') + '>' + body + '</div>';
  }

  return '<section class="prm-demo-showcase' + (hideTabs ? ' prm-demo-showcase--no-tabs' : '') + '" id="' + esc(tabsId) + '"'
    + ' data-prm-demo-carousel data-prm-demo-interval="' + interval + '"'
    + (hideTabs ? ' data-prm-demo-hide-tabs="1"' : '') + '>'
    + '<div class="prm-shell">'
    + body
    + '</div></section>';
}
...
function ContactPanel(content, copy) {
  ...
  return '<div class="prm-contact-panel">'
    ...
    + '<div class="lp-hp" aria-hidden="true"><label>Company website</label><input type="text" name="lp_hp" value="" tabindex="-1" autocomplete="off"></div>'
