...
function navBlock(c, ctx) {
  ...
  return '<header class="lwc-header">'
    + shell(
      ...
      + (phone ? '<a class="lwc-header-phone" href="' + esc(telHref(phone)) + '">' + esc(phone) + '</a>' : '')
  ...
  return '<section class="lwc-section lwc-hero" id="top">'
    + shell(
      ...
      + hero.chips.map(function(chip, i) {
        return '<span class="lwc-float-chip lwc-float-chip--' + (i + 1) + '">' + esc(chip) + '</span>';
      }).join('')
      + '</div>'
      + '</div>'
      + '</div>'
...
function demoCard(card, index) {
  return '<article class="lwc-demo-card" data-lwc-demo-industry="' + esc(card.industry || 'All') + '">'
    ...
    + '<img src="' + esc(card.image) + '" alt="' + esc(card.name) + ' website demo" loading="lazy" decoding="async">'
    + '</div>'
    + '</div>'
    ...
    + '</div>'
    + '<a class="lwc-link" href="' + esc(card.url || '#contact') + '" target="' + (card.url && card.url.charAt(0) !== '#' ? '_blank' : '_self') + '" rel="noopener">View live demo</a>'
    + '</div>'
  ...
  return '<section class="lwc-section lwc-demos" id="websites">'
    + shell(
      ...
      + COPY.tabs.map(function(tab, i) {
        return '<button type="button" class="lwc-pill' + (i === 0 ? ' is-active' : '') + '">' + esc(tab) + '</button>';
      }).join('')
      + '</div>'
...
function featuresSection(c) {
  ...
  return '<section class="lwc-section lwc-features" id="features">'
    + shell(
      ...
      + COPY.callouts.map(function(item, i) { return '<span class="lwc-pin lwc-pin--' + (i + 1) + '">' + esc(item.title) + '</span>'; }).join('')
      + '</div>'
      + '</div>'
...
function aboutSection(c, ctx) {
  ...
  return '<section class="lwc-section lwc-about" id="about">'
    + shell(
      ...
      + (contact.phone ? '<a class="lwc-btn lwc-btn--light-outline" href="' + esc(telHref(contact.phone)) + '">' + esc(COPY.callLabel) + '</a>' : '')
      + (contact.email ? '<a class="lwc-btn lwc-btn--light-outline" href="mailto:' + esc(contact.email) + '">' + esc(COPY.emailLabel) + '</a>' : '')
      + (!contact.phone && !contact.email ? '<a class="lwc-btn lwc-btn--light-outline" href="#contact">Contact ' + esc(firstName(c)) + '</a>' : '')
      + '</div>'
      + '</article>'
      + '</div>'
    )
    ...
    + shell(
      ...
      + COPY.plans.map(function(plan) {
        return '<article class="lwc-price-card' + (plan.badge ? ' is-featured' : '') + '">'
          ...
          + '<ul>' + plan.bullets.map(function(b) { return '<li>' + esc(b) + '</li>'; }).join('') + '</ul>'
...
function trustSection(c) {
  ...
  return '<section class="lwc-section lwc-trust" id="trust">'
    + shell(
      sectionIntro(COPY)
      + '<div class="lwc-trust-grid">'
      + COPY.items.map(function(item) {
        return '<article>' + iconDot(item.title) + '<h3>' + esc(item.title) + '</h3><p>' + esc(item.body) + '</p></article>';
      }).join('')
      + '</div>'
...
function finalCtaSection(c) {
  ...
  return '<section class="lwc-section lwc-final" id="contact">'
    + shell(
      ...
      + '</div>'
      + '</div>'
      + '<div class="lwc-bottom-contact">'
      + (COPY.phone ? '<a href="' + esc(telHref(COPY.phone)) + '">Phone ' + esc(COPY.phone) + '</a>' : '<span>Phone available after enquiry</span>')
      + '<a href="#contact">Book a quick website chat</a>'
      + '</div>'
    )
    + '</section>';
}
