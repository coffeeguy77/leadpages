...
const { resolvePartnerLogo } = require('../partner-website/logo');
...
function buildPartnerLandingHtml(prof, partner, demos, base, opts) {
  ...
  const mergedLogo = resolvePartnerLogo({
    showcaseConfig: profCopy.showcase_config,
    home: opts.home || null
  });
  if (mergedLogo && !profCopy.showcase_config.logo) {
    profCopy.showcase_config.logo = mergedLogo;
  }
