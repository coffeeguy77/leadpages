...
function buildContext(prof, partner, demos, base, opts) {
  ...
  const templateKey = cfg.templateKey || 'converge';
  ...
  return {
    ...
    email, phone, siteId, siteSlug, siteName, logoUrl, templateKey, ctaHref, year
  };
}
...
function footerBlock(ctx) {
  return '<footer class="pt-footer">'
    ...
    + '<a href="#demos">Demos</a><a href="#power">Platform</a><a href="#support">Support</a><a href="#contact">Contact</a>'
    + '</nav>'
    + '<div class="pt-footer-bottom">'
    + '<p>&copy; ' + ctx.year + ' ' + ctx.name + '</p>'
    + '<a href="https://www.leadpages.com.au" target="_blank" rel="noopener" class="pt-lp-badge">'
    + '<span class="leadpages-logo" data-lp-logo="auto" data-lp-logo-ink="auto" role="img" aria-label="LeadPages"></span>'
    + '<span>Powered by LeadPages</span></a>'
    + '</div></div></footer>';
}

function pageShell(ctx, bodyHtml, opts) {
  opts = opts || {};
  const cssHref = opts.css || '/assets/partner-templates/converge.css';
  ...
  return '<!DOCTYPE html><html lang="en"><head>'
    ...
    + '<title>' + ctx.name + ' — ' + ctx.headline + '</title>'
    + '<meta name="description" content="' + ctx.intro + '">'
    ...
    + ' data-pt-template="' + esc(opts.templateId || ctx.templateKey) + '">'
