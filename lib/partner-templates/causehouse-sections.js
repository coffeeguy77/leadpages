...
function brandLogoUrl(c) {
  const url = c.partner && c.partner.logoUrl;
  return url && isValidImageUrl(url) ? String(url).trim() : '';
}

function brandLinkHtml(c) {
  const brand = brandName(c);
  const logo = brandLogoUrl(c);
  if (logo) {
    return '<a href="#top" class="ch-brand ch-brand-lockup">'
      + '<img src="' + esc(logo) + '" alt="" class="ch-brand-img" aria-hidden="true">'
...
function heroVisual(c) {
  ...
  if (p.logoUrl && isValidImageUrl(p.logoUrl)) {
...
function footerBlock(ctx, c) {
  ...
  const logo = brandLogoUrl(content);
  ...
  const brandHtml = logo
    ? '<div class="ch-footer-brand-lockup">'
      + '<img src="' + esc(logo) + '" alt="" class="ch-footer-logo" aria-hidden="true">'
