...
const { isUsableLogoUrl } = require('../partner-website/logo');
...
function brandLogoUrl(c, ctx) {
  const raw = (c.partner && c.partner.logoUrl) || '';
  if (raw && isUsableLogoUrl(raw)) return String(raw).trim();
  if (ctx && ctx.logoUrl) {
    const fromCtx = String(ctx.logoUrl).replace(/&amp;/g, '&');
    if (isUsableLogoUrl(fromCtx)) return fromCtx.trim();
  }
  return '';
}
...
function brandLinkHtml(c, ctx) {
  ...
  const logo = brandLogoUrl(c, ctx);
  if (logo) {
    return '<a href="#top" class="ch-brand ch-brand-lockup ch-brand-logo-only">'
      + '<span class="ch-brand-img-plate">'
      + '<img src="' + esc(logo) + '" alt="' + esc(brand) + '" class="ch-brand-img">'
      + '</span>'
      + '</a>';
  }
...
function heroVisual(c) {
  ...
  if (p.logoUrl && isUsableLogoUrl(p.logoUrl)) {
    return '<div class="ch-hero-visual ch-hero-frame ch-hero-logo-wrap">'
      ...
      + '<img src="' + esc(p.logoUrl) + '" alt="' + esc(brand) + ' logo" class="ch-hero-logo" loading="eager">'
      + '</span>'
      + '</div>';
  }
...
function footerBlock(ctx, c) {
  ...
  const logo = brandLogoUrl(content, ctx);
  ...
  const brandHtml = logo
    ? '<div class="ch-footer-brand-lockup">'
      ...
      + '<img src="' + esc(logo) + '" alt="' + esc(brand) + '" class="ch-footer-logo">'
      + '</span>'
