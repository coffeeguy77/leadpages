...
function heroSection(c, ctx) {
  const hero = c.hero || {};
  const brand = brandName(c);
  const heroSrc = logoSrc(ctx, 280);
  const heroLogo = heroSrc
    ? '<div class="ch-hero-logo-mark"><img src="' + heroSrc + '" alt="' + esc(brand) + ' logo" class="ch-hero-mark-logo" loading="eager" decoding="async"></div>'
    : '';
  return chapter('bg', ''
