...
function heroSection(c, ctx) {
  ...
  const journeySteps = heroJourneySteps();
  const proofStats = prm.ProofStats(c.metrics);
  ...
  return '<section class="wc-section wc-hero" id="top">'
    + shell(
      ...
      + (proofStats ? '<div class="wc-hero-proof">' + proofStats + '</div>' : '')
      + '</div>'
      + '<div class="wc-hero-visual">'
      + (journeySteps.length
        ? prm.HeroCustomerJourney(journeySteps, { toast: COPY.quoteToast, interval: 7000 })
        : '')
      + prm.HeroPlatformDashboard(COPY.heroDashboard)
      + '</div>'
      + '</div>'
    )
    + '</section>';
