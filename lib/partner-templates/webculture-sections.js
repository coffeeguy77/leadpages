...
function brandLinkHtml(c, ctx) {
  ...
  const wordmark = '<span class="wc-wordmark">' + esc(String(brand).toUpperCase()) + '</span>';
  return '<a href="#top" class="wc-brand wc-brand-text">' + wordmark + '</a>';
}
...
function heroSection(c, ctx) {
  ...
  return '<section class="wc-section wc-hero" id="top">'
    + shell(
      ...
      + '<p class="wc-eyebrow wc-eyebrow--pill">' + esc(hero.eyebrow || 'YOUR WEBSITE PARTNER') + '</p>'
      + '<h1 class="wc-display prm-serif">' + highlightHeadline(hero.headline, hero.highlightedPhrase) + '</h1>'
      ...
      + '<a class="wc-btn wc-btn-ghost wc-btn-white" href="#contact">' + esc(hero.primaryCta) + '</a>'
      + '</div>'
      + (trust ? '<ul class="wc-trust-row">' + trust + '</ul>' : '')
      + '</div>'
      + prm.HeroWebsiteShowcase(heroDemo, { toast: COPY.quoteToast })
      + '</div>'
      + (c.demos && c.demos.length ? prm.LiveDemoGallery(c.demos, copyFor(c).gallery) : '')
    )
    + '</section>';
}
...
function connectedSection(c) {
  ...
  const cards = COPY.proofCards.map(function(card) {
    return '<article class="prm-proof-card">'
      + prm.iconRing(card.icon, 'prm-icon-ring--proof')
...
function servicesSection(c) {
  ...
  return '<section class="wc-section wc-services" id="services">'
    + shell(
      ...
      + pillars.map(function(p) { return prm.ServiceCard(p); }).join('')
      + '</div>'
    )
    + '</section>';
}
  ...
  return '<section class="wc-section wc-partner-process" id="process">'
    + shell(
      ...
      + (showProcess ? '<div class="wc-process-panel">'
        + '<p class="prm-eyebrow prm-eyebrow--lime">' + esc(prc.eyebrow) + '</p>'
        + '<h2 class="wc-h2 prm-serif prm-on-dark">' + esc(prc.heading) + '</h2>'
...
function platformSection(c) {
  ...
  return '<section class="wc-section wc-platform" id="support">'
    + shell(
      ...
      + '<h2 class="wc-h1 prm-serif">' + esc(plc.heading) + '</h2>'
    ...
    + shell(
      ...
      + (showTestimonials && featured
        ? '<div class="wc-testimonial-col">'
          + prm.ReviewCard(featured, c.contact, c.metrics, c.social, c.serviceArea)
          + '</div>'
