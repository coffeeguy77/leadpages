...
function heroSection(c, ctx) {
  ...
  const hero = heroFields(c);
  const COPY = copyFor(c);
  const heroDemos = pickHeroDemos(c.demos, 1);
  ...
  return '<section class="wc-section wc-hero" id="top">'
    + shell(
      ...
      + prm.HeroProductEcosystem(heroDemos, { dashboard: COPY.heroDashboard, toast: COPY.quoteToast })
      + '</div>'
      + '</div>'
    )
    + '</section>';
...
function demoShowcaseSection(c) {
  if (!c.visibility || c.visibility.demos === false) return '';
  const groups = groupDemosByIndustryTab(c.demos, c.industryShowcase);
