...
const { CULTURE_COLOR_PRESETS } = require('../partner-website/webculture-color-presets');
...
function heroSection(c, ctx) {
  ...
  const hero = heroFields(c);
  const COPY = copyFor(c);
  const hs = c.heroShowcase || {};
  const heroDemos = pickHeroDemos(c.demos, Math.max(1, (hs.siteIds && hs.siteIds.length) || 1), hs);
  ...
  return '<section class="wc-section wc-hero" id="top">'
    + shell(
      ...
      + '</div>'
      + prm.HeroTrustMeta(COPY.heroTrust, c.metrics, COPY.heroResponse)
      + '</div>'
      + '<div class="wc-hero-visual">'
      + prm.HeroProductEcosystem(heroDemos, {
        dashboard: COPY.heroDashboard,
        toast: COPY.quoteToast,
        hideTabs: !!hs.hideTabs,
        interval: hs.intervalMs || 12000
      })
      + '</div>'
      + '</div>'
    )
...
function demoShowcaseSection(c) {
  if (!c.visibility || c.visibility.demos === false) return '';
  const showcase = c.industryShowcase || {};
  const groups = groupDemosByIndustryTab(c.demos, showcase);
  if (!groups.length) return '';
  const faqs = (!c.visibility || c.visibility.faqs !== false) ? (c.faqs || []) : [];
  return prm.IndustryTabs(groups, copyFor(c).demos, {
    id: 'demos',
    faqs: faqs,
    faqCopy: copyFor(c).faqs,
    faqForTab: industryFaqsForTab,
    trustItems: demoTrustItems(),
    hideTabs: !!showcase.hideTabs,
    interval: showcase.intervalMs || 12000
  });
}
...
function testimonialsContactSection(c) {
  ...
  return '<section class="wc-section wc-section--ink wc-testimonials-contact" id="contact">'
    + shell(
      ...
      + (showTestimonials && list.length
        ? '<div class="wc-testimonial-col">'
          + prm.ReviewSlider(list, {
            interval: 10000,
            googleLink: (c.social && c.social.googleBusiness) || null,
            googleRating: (c.metrics && c.metrics.googleRating) || null,
            googleReviewCount: (c.metrics && c.metrics.googleReviewCount) || null,
            reviewFallback: copyFor(c).testimonials.googleFallback
          })
          + '</div>'
...
/**
 * Visitor colour playground — live CSS-variable preview of Culture presets.
 * Session-only; does not change the partner's saved theme.
 */
function colourLabBlock(palette) {
  const pal = palette || {};
  const presets = CULTURE_COLOR_PRESETS.map(function(p) {
    return {
      id: p.id,
      name: p.name,
      blurb: p.blurb,
  ...
  const siteDefault = {
    id: pal.custom ? 'site-default' : (pal.presetId || 'culture'),
    name: pal.custom ? 'Site default' : (pal.name || 'Culture Lime'),
    custom: !!pal.custom,
    primary: pal.primary,
    ink: pal.ink,
    bg: pal.bg,
  ...
  const payload = { presets: presets, siteDefault: siteDefault };
  const payloadJson = JSON.stringify(payload).replace(/</g, '\\u003c');

  const swatches = presets.map(function(p) {
    const selected = !pal.custom && p.id === (pal.presetId || 'culture');
    return '<button type="button" class="wc-colour-lab__swatch' + (selected ? ' is-active' : '') + '"'
      + ' data-wc-colour-preset="' + esc(p.id) + '"'
      + ' role="radio" aria-checked="' + (selected ? 'true' : 'false') + '"'
      + ' aria-label="' + esc(p.name) + '">'
      + '<span class="wc-colour-lab__swatch-chips" aria-hidden="true">'
      + '<span style="background:' + esc(p.bg) + '"></span>'
      + '<span style="background:' + esc(p.primary) + '"></span>'
      + '<span style="background:' + esc(p.ink) + '"></span>'
      + '</span>'
      + '<span class="wc-colour-lab__swatch-meta">'
      + '<span class="wc-colour-lab__swatch-name">' + esc(p.name) + '</span>'
      + '<span class="wc-colour-lab__swatch-blurb">' + esc(p.blurb || '') + '</span>'
      + '</span>'
      + '</button>';
  }).join('');

  return '<aside class="wc-colour-lab" data-wc-colour-lab aria-label="Colour scheme playground">'
    + '<script type="application/json" data-wc-colour-lab-data>' + payloadJson + '</script>'
    + '<button type="button" class="wc-colour-lab__fab" data-wc-colour-lab-fab aria-expanded="false" aria-controls="wc-colour-lab-panel">'
    + '<span class="wc-colour-lab__fab-orb" aria-hidden="true">'
    + '<span class="wc-colour-lab__fab-dot" style="background:' + esc(pal.primary || '#C5E13F') + '"></span>'
    + '<span class="wc-colour-lab__fab-dot" style="background:' + esc(pal.ink || '#0B2114') + '"></span>'
    + '<span class="wc-colour-lab__fab-dot" style="background:' + esc(pal.bg || '#FDFCF0') + '"></span>'
    + '</span>'
    + '<span class="wc-colour-lab__fab-label">Try colours</span>'
    + '</button>'
    + '<div class="wc-colour-lab__panel" id="wc-colour-lab-panel" data-wc-colour-lab-panel hidden role="dialog" aria-modal="false" aria-labelledby="wc-colour-lab-title">'
    + '<div class="wc-colour-lab__panel-inner">'
    + '<header class="wc-colour-lab__head">'
    + '<p class="wc-colour-lab__eyebrow">Live colour studio</p>'
    + '<h2 id="wc-colour-lab-title" class="wc-colour-lab__title">Pick a colour scheme</h2>'
    + '<p class="wc-colour-lab__lede">The whole page updates instantly — the kind of control built into every LeadPages site.</p>'
    + '<button type="button" class="wc-colour-lab__close" data-wc-colour-lab-close aria-label="Close colour studio">&times;</button>'
    + '</header>'
    + '<div class="wc-colour-lab__grid" role="radiogroup" aria-label="Colour presets">'
    + swatches
    + '</div>'
    + '<footer class="wc-colour-lab__foot">'
    + '<button type="button" class="wc-colour-lab__reset" data-wc-colour-lab-reset>Site default</button>'
    + '<a href="#contact" class="wc-colour-lab__cta" data-wc-colour-lab-cta>Love this look? Plan My Website →</a>'
    + '</footer>'
    + '</div>'
    + '</div>'
    + '<div class="wc-colour-lab__scrim" data-wc-colour-lab-scrim hidden></div>'
    + '</aside>';
}
