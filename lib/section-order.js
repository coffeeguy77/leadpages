...
var OPTIONAL_SECTIONS = [
  ...
  'videoReels', 'activityTimeline', 'customerReactions', 'onlineQuote', 'customHtml',
...
var OFF_BY_DEFAULT = [
  ...
  'mobileBar', 'emergencyAvailability', 'onlineQuote', 'customHtml', 'estimateBuilder', 'finance',
...
function resolveSectionOrder(cfg, layoutSections) {
  ...
  // Repair SearchCanvas when it landed under Hero before How It Works / Services.
  if (filtered.indexOf('searchCanvas') >= 0 && sectionIsOn(cfg, 'searchCanvas')) {
    filtered = repairSearchCanvasPlacement(filtered);
  }
...
/**
 * If SearchCanvas sits before How It Works / Services, move it to the default slot.
 * Leaves later custom placements alone.
 */
function repairSearchCanvasPlacement(order) {
  ...
  if (sci > target) return list; // user moved it later — keep
  list.splice(sci, 1);
