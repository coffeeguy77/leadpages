...
async function discoverFromSerpSeeds(admin, site, opts) {
  ...
  for (let i = 0; i < seeds.length; i++) {
    ...
    results.forEach(function (r) {
      ...
      if (!d || d === own) return;
      // Customer path blocks fixtures + *.example; mock tests only block hard-coded plumber demos.
  ...
  return {
    ...
    labelClass: 'estimated',
    note:
      'Rivals inferred from top organic results for YOUR seed keywords (' +
      seeds.join(', ') +
...
async function loadCompetitionSnapshot(admin, site, opts) {
  ...
  return {
    ...
    note: liveReady
      ? 'Competition Analysis uses DataForSEO Labs for YOUR domain and keywords only. Demo/mock plumber fixtures are blocked.'
      : 'Set DATAFORSEO_LOGIN + DATAFORSEO_PASSWORD to discover real rivals. Mock demo data will not be shown as your competitors.'
  };
}
