...
function locationFromSite(site, override) {
  ...
  const candidates = [
    ...
    cfg.business && (cfg.business.region || cfg.business.serviceArea || cfg.business.location)
  ];
  function looksLikeGeo(s) {
    const t = String(s || '')
      .toLowerCase()
      .trim();
    if (!t || t.length > 90) return false;
    // Product/niche strings are not SERP locations
    if (/xero|quick\s*books|reconcil|bookkeep|account|crm|saas|software|matcher/i.test(t)) {
      return false;
    }
    if (
      /australia|canberra|sydney|melbourne|brisbane|perth|adelaide|hobart|darwin|\bnsw\b|\bvic\b|\bqld\b|\bact\b|\bwa\b|\bsa\b|\btas\b|\bnt\b|\bau\b/.test(
        t
      )
    ) {
      return true;
  ...
  for (let i = 0; i < candidates.length; i++) {
    if (candidates[i] && looksLikeGeo(candidates[i])) return String(candidates[i]).trim();
  }
  return 'Australia';
}
...
function isSerpRivalType(typ) {
  ...
  if (!t) return true;
  // Exclude pure chrome / non-website blocks
  if (
    t === 'people_also_ask' ||
    t === 'related_searches' ||
    t === 'knowledge_graph' ||
...
/** Domains that count as "you" for this site (custom domain, config, or slug match). */
function ownHostsFromSite(site, overrideDomain) {
  const hosts = [];
  function push(h) {
    const d = cleanDomain(h);
    if (!d || hosts.indexOf(d) >= 0) return;
    hosts.push(d);
  }
  push(overrideDomain);
  push(hostFromSite(site));
  const cfg = (site && site.config) || {};
  push(cfg.domain);
  push(cfg.website);
  push(cfg.customDomain);
  if (cfg.business) {
    push(cfg.business.website);
    push(cfg.business.domain);
  }
  if (cfg.seo && Array.isArray(cfg.seo.domains)) {
    cfg.seo.domains.forEach(push);
  }
  return hosts;
}

function domainIsOwn(domain, site, ownHosts) {
  const d = cleanDomain(domain);
  if (!d) return false;
  if ((ownHosts || []).indexOf(d) >= 0) return true;
  const slug = String((site && site.slug) || '')
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '');
  // Match beanculture.com.au when site slug is beanculture even if custom_domain is mistyped
  if (slug && slug.length >= 4) {
    const bare = d
      .replace(/\.(com\.au|co\.uk|com|net|org|au)$/i, '')
      .replace(/[^a-z0-9]/g, '');
    if (bare === slug || bare.indexOf(slug) >= 0 || slug.indexOf(bare) >= 0) return true;
  }
  return false;
}
...
async function lookupKeywordSerp(admin, site, opts) {
  ...
  const location = locationFromSite(site, o.location);
  const ownHosts = ownHostsFromSite(site, o.domain);
  const own = ownHosts[0] || cleanDomain(o.domain || hostFromSite(site));
  ...
  const snap = await gw.serp({
    ...
    siteDomain: own,
    ownDomain: own,
    mode: o.mode || o.serpMode || 'advanced'
  });
  if (!snap || !snap.ok) {
    return {
      ok: false,
      ...
      keyword: keyword,
      location: location,
      locationUsed: snap && snap.locationUsed,
      rawItemCount: snap && snap.rawItemCount,
      rawTypes: snap && snap.rawTypes
    };
  }
  ...
  const results = ((snap.snapshot && snap.snapshot.results) || []).filter(function (r) {
    return r && r.domain && isSerpRivalType(r.type);
  });
  ...
  results.forEach(function (r) {
    ...
    if (gate.provider === 'mock' ? isHardcodedFixtureDomain(d) : isForbiddenCompetitorDomain(d)) return;
    const isYou = domainIsOwn(d, site, ownHosts);
    const row = {
  ...
  if (!serpRows.length) {
    return {
      ...
      error: 'serp_empty',
      message:
        'No website rows after filtering for "' +
        keyword +
        '". Provider returned data but nothing usable — check DataForSEO location/balance.',
      keyword: keyword,
      location: location,
      locationUsed: snap.locationUsed || (snap.snapshot && snap.snapshot.location),
      rawItemCount: snap.rawItemCount,
  ...
  return {
    ...
    locationUsed: snap.locationUsed || (snap.snapshot && snap.snapshot.location),
    device: device,
    serpMode: snap.mode || o.mode || o.serpMode || 'advanced',
    domain: own,
    ownHosts: ownHosts,
      ...
      ownPosition != null
        ...
        : own
          ? 'You (' +
            ...
            keyword +
            '". Rivals below currently occupy this SERP. Tip: if your live domain is wrong above (e.g. typo), fix the site custom domain.'
...
/**
 * Labs SERP Competitors — domains ranking for YOUR keyword seeds (visibility / rating / etv).
 */
async function discoverSerpCompetitors(admin, site, opts) {
  const o = opts || {};
  const gate = competitionGateway(o);
  if (!gate.ok) {
    return { ok: false, error: gate.error, message: gate.message };
  ...
  const rawSeeds = Array.isArray(o.seeds)
    ...
    : Array.isArray(o.keywords)
      ? o.keywords
      : String(o.seeds || o.keywords || o.keyword || '').split(/[,\n]/);
  const keywords = rawSeeds
    .map(function (s) {
      return String(s || '').trim();
    })
    .filter(Boolean)
  ...
  if (!keywords.length) {
    return {
      ...
      message: 'Enter 1–20 keywords to find domains that rank for them (SERP Competitors).'
    };
  }

  const own = cleanDomain(o.domain || hostFromSite(site));
  const res = await gw.serpCompetitors({
    keywords: keywords,
    location: location,
    limit: o.limit || 20
  });
  const blocked = rejectMockProvider(res, gate);
  if (blocked) return Object.assign(blocked, { keywords: keywords });
  if (!res || !res.ok) {
    return {
      ok: false,
      error: (res && res.error) || 'serp_competitors_failed',
      message: (res && res.message) || 'Could not load SERP competitors for these keywords.',
      keywords: keywords,
      provider: (res && res.provider) || gw.preferred
    };
  }
  ...
  const competitors = (res.competitors || []).filter(function (c) {
    if (!c || !c.domain) return false;
    if (c.domain === own) return false;
    if (gate.provider === 'mock' || res.provider === 'mock') {
      return !isHardcodedFixtureDomain(c.domain);
    }
    if (isForbiddenCompetitorDomain(c.domain)) return false;
    if (isPlatformNoiseDomain(c.domain)) return false;
    return true;
  });
  const domains = competitors.map(function (c) {
    return c.domain;
  ...
  if (canPersist && admin && site && site.id && domains.length) {
    ...
    await meterUsage(admin, site.id, 'serp_competitors', 1, {
      provider: res.provider,
      keywords: keywords,
      count: domains.length
    });
  ...
  return {
    ...
    configSave: configSave,
    note:
      'Domains ranked by SERP presence (rating / visibility) for YOUR keywords. Labs estimate — not a live SERP scrape.',
    analysedAt: new Date().toISOString()
  };
}

/**
 * You vs one competitor — Domain Intersection (shared + keywords they rank for that you don't).
 */
async function runDomainIntersection(admin, site, opts) {
  const o = opts || {};
  const gate = competitionGateway(o);
  if (!gate.ok) {
    return { ok: false, error: gate.error, message: gate.message };
  ...
  if (!own) {
    return {
      ok: false,
      error: 'domain_required',
      message: 'Connect a custom domain so we can compare keyword overlap with a rival.'
    };
  }
  await purgeForbiddenCompetitors(admin, site);

  let rival = cleanDomain(o.competitor || o.target2);
  if (!rival) {
    const list = filterCompetitorDomains(
      Array.isArray(o.competitors) ? o.competitors : competitorDomainsFromSite(site, [])
    );
    rival = list[0] || null;
  }
  if (!rival && admin && site && site.id) {
  ...
  if (!rival) {
    return {
      ...
      message: 'Pick a competitor domain for the you-vs-rival comparison.'
    };
  }
  if (isForbiddenCompetitorDomain(rival) && gate.provider !== 'mock') {
    return {
      ok: false,
      error: 'forbidden_fixture',
      message: 'That domain is a demo fixture and cannot be analysed.'
    };
  }

  const limit = Math.min(100, Math.max(10, o.limit || 40));
  const sharedRes = await gw.domainIntersection({
    target1: own,
    target2: rival,
    location: location,
    intersections: true,
  ...
  // Rival-only keywords (they rank, you don't): target1 = rival, intersections false
  const gapRes = await gw.domainIntersection({
    target1: rival,
    target2: own,
    location: location,
    intersections: false,
  ...
  if (sharedRes && sharedRes.provider === 'mock' && gate.provider !== 'mock') {
    return {
      ok: false,
      error: 'mock_blocked',
      message: 'Refused mock intersection fixtures. Configure DataForSEO for live comparison.',
      domain: own,
      competitor: rival
    };
  }
  if ((!sharedRes || !sharedRes.ok) && (!gapRes || !gapRes.ok)) {
    return {
      ok: false,
      error: (sharedRes && sharedRes.error) || (gapRes && gapRes.error) || 'intersection_failed',
      message:
        (sharedRes && sharedRes.message) ||
        (gapRes && gapRes.message) ||
        'Could not load domain intersection.',
      domain: own,
      competitor: rival
    };
  }

  const shared = ((sharedRes && sharedRes.keywords) || []).map(function (k) {
    return {
      keyword: k.keyword,
      volume: k.volume,
      cpc: k.cpc,
      ownPosition: k.position1,
      rivalPosition: k.position2,
      ownUrl: k.url1,
      rivalUrl: k.url2
    };
  });
  const missing = ((gapRes && gapRes.keywords) || []).map(function (k) {
    return {
      keyword: k.keyword,
      volume: k.volume,
      cpc: k.cpc,
      rivalPosition: k.position1,
      rivalUrl: k.url1,
      ownPosition: null
    };
  });
  ...
  if (admin && site && site.id) {
    await meterUsage(admin, site.id, 'domain_intersection', 2, {
      provider: (sharedRes && sharedRes.provider) || (gapRes && gapRes.provider) || gw.preferred,
      domain: own,
      competitor: rival
    });
  }
  ...
  return {
    ...
    note:
      'You vs ' +
      rival +
      ': Shared = both rank. Missing = they rank and you do not (Labs domain intersection).',
    analysedAt: new Date().toISOString()
  };
}
