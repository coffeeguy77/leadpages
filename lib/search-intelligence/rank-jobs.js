...
async function resolveSiteTargetUrl(admin, siteId) {
  const { data: site } = await admin
    .from('sites')
    .select('id,slug,custom_domain,config,business_name')
    .eq('id', siteId)
    .maybeSingle();
  if (!site) return null;
...
function resolveSiteGeo(site) {
  const cfg = (site && site.config) || {};
  const tokens = (cfg.sections && cfg.sections.seoTokens) || {};
  const city = String(tokens.city || tokens.suburb || cfg.region || '').trim();
  const region = String(tokens.region || '').trim();
  if (city && region) return city + ', ' + region;
  if (city) return city + ', AU';
  if (region) return region + ', AU';
  return 'Canberra, ACT';
}
...
async function runRankJobs(admin, opts) {
  ...
  for (const item of batch) {
    try {
      if (!siteCache[item.siteId]) {
        const { data: site } = await admin
          .from('sites')
          .select('id,slug,custom_domain,config,business_name')
          .eq('id', item.siteId)
          .maybeSingle();
        siteCache[item.siteId] = site || null;
        targetCache[item.siteId] = await resolveSiteTargetUrl(admin, item.siteId);
      }
      const site = siteCache[item.siteId];
      const targetUrl = o.url || targetCache[item.siteId] || null;
      const geo = item.geo || resolveSiteGeo(site);
