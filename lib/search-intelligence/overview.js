...
async function buildOverview(opts) {
  ...
  const openIssues = audit.issueCount + crawlIssues + pageFindings.length + listingFindings.length + rankedWithPos.filter(function (r) {
  ...
  const cards = [
    ...
    },
    {
      ...
      value: listings && listings.listingsScore != null
        ...
        : null,
      state: listingFindings.length ? 'attention' : 'ok',
      labelClass: 'modelled',
      hint: listingFindings.length
        ? listingFindings.length + ' listings/NAP issue(s). See Local tab.'
        : 'First-party NAP looks complete. GBP OAuth is optional Phase 3.'
    }
  ];
