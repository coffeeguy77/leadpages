...
async function buildOverview(opts) {
  ...
  let listings = { findings: [], listingsScore: null, nap: null, gbpStatus: null };
  try {
    listings = auditListings({
      id: siteId,
      business_name: o.businessName || (site && site.business_name),
      config: cfg
    });
  } catch (_e) { /* keep empty */ }
  let localPages = { findings: [] };
  try {
    localPages = detectLocalPageIssues({
      id: siteId,
      business_name: o.businessName || (site && site.business_name),
      config: cfg
    });
  ...
  try {
    adsUniverse = buildAdsKeywordUniverse(o.trackedKeywords || [], o.adsKeywords || []);
  } catch (_e) { /* keep empty */ }
  let keywordCoverage = { findings: [] };
  try {
    keywordCoverage = detectKeywordNoPage(
      {
        id: siteId,
        business_name: o.businessName || (site && site.business_name),
        config: cfg
