...
/**
 * Search Intelligence sync — GSC query×page → si_query_page_stats.
 */
...
/**
 * Sync one site's GSC property into si_query_page_stats.
 */
  ...
  try {
    ...
    if (rows.length) {
      ...
      for (let i = 0; i < batch.length; i += chunk) {
        ...
        const { error: insErr } = await admin.from('si_query_page_stats').insert(slice);
