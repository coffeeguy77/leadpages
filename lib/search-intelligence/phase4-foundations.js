...
/**
 * AI visibility placeholder — uses SERP ai_overview feature when a brand query is sampled.
 */
async function probeAiVisibility(site, opts) {
  ...
  const features = (res.snapshot && res.snapshot.features) || [];
  const hasAi = features.indexOf('ai_overview') >= 0;
  const findings = [];
  if (hasAi) {
    const recipe = getRecipe('ai_overview_absent');
    // Presence of AI overview without our domain in organic top is a soft signal
    const hosts = [];
    if (site.custom_domain) hosts.push(String(site.custom_domain).toLowerCase());
    if (site.slug) hosts.push(String(site.slug).toLowerCase());
    const organic = ((res.snapshot && res.snapshot.results) || []).filter(function (r) {
      return r.type === 'organic';
    });
    const owned = organic.some(function (r) {
      const d = String(r.domain || '').toLowerCase();
      return hosts.some(function (h) {
        return h && d.indexOf(h.replace(/^www\./, '')) >= 0;
      });
    });
    if (!owned) {
      findings.push({
        ...
        autoFixAllowed: false,
        evidence: { source: 'serp_ai_overview', keyword: keyword, features: features },
  ...
  return {
    ok: true,
    available: true,
    score: hasAi ? (findings.length ? 0.35 : 0.7) : 0.5,
    state: hasAi ? (findings.length ? 'ai_overview_competitor_risk' : 'ai_overview_present') : 'no_ai_overview',
    keyword: keyword,
    features: features,
    findings: findings,
    provider: res.provider,
    labelClass: (res.snapshot && res.snapshot.labelClass) || 'estimated',
    note: 'Phase 4 foundation — citation ownership expands later. DataForSEO SERP only.'
  };
}
