...
/**
 * Light Phase 4 foundations — Ads↔SEO keyword universe + AI visibility probes.
 * Market data: DataForSEO (+ mock). Answer probes: OpenAI + Perplexity when keyed.
 */

const { getRecipe } = require('./recipes/registry');
const { createGateway } = require('./providers/gateway');
const { analyseAiCitations, platformCatalogue, PLATFORM_CATALOGUE } = require('./ai-citations');
const { probeChatGptAnswers, probePerplexity } = require('./ai-answer-probes');
...
/**
 * AI visibility — Google AI Overview citations + ChatGPT / Perplexity answer probes.
 */
async function probeAiVisibility(site, opts) {
  ...
  const answerProbes = {};
  if (o.skipAnswerProbes !== true) {
    try {
      answerProbes.chatgpt = await probeChatGptAnswers(site, {
        keyword: keyword,
        fetchImpl: o.fetchImpl
      });
    } catch (e) {
      answerProbes.chatgpt = {
        ok: false,
        available: false,
        status: 'error',
        provider: 'openai',
    ...
    try {
      answerProbes.perplexity = await probePerplexity(site, {
        keyword: keyword,
        fetchImpl: o.fetchImpl
      });
    } catch (e) {
      answerProbes.perplexity = {
        ok: false,
        available: false,
        status: 'error',
        provider: 'perplexity',
  ...
  if (!res || !res.ok) {
    return {
      ok: true,
      available: !!(
        (answerProbes.chatgpt && answerProbes.chatgpt.available) ||
        (answerProbes.perplexity && answerProbes.perplexity.available)
      ),
      score: null,
      state: 'provider_unavailable',
      provider: res && res.provider,
      platforms: analyseAiCitations(site, { features: [], results: [] }, {
        keyword: keyword,
      ...
      note:
        (res && res.message) ||
        'Google SERP probe unavailable — ChatGPT / Perplexity still run when API keys are set.'
    };
  }

  const analysis = analyseAiCitations(site, res.snapshot, {
    keyword: keyword,
    answerProbes: answerProbes
  });
  const findings = [];
  ...
  // Surface answer-probe misses as soft findings when keys are live.
  ['chatgpt', 'perplexity'].forEach(function (key) {
    const probe = answerProbes[key];
    if (!probe || !probe.available) return;
    if (probe.cited || probe.brandMentioned) return;
    findings.push({
      id: 'ai:answer_absent_' + key,
      code: 'ai_answer_absent',
      recipeId: 'ai_overview_absent',
      title: (key === 'chatgpt' ? 'ChatGPT' : 'Perplexity') + ' did not cite your brand',
      plainLanguage:
        (key === 'chatgpt' ? 'ChatGPT' : 'Perplexity') +
        ' answered about “' +
        keyword +
        '” without citing your domain or clearly naming your business. Strengthen entity proof and local pages.',
      severity: 'low',
      status: 'open',
      actions: ['create_task', 'page_optimiser'],
      autoFixAllowed: false,
      evidence: { source: key + '_answers', keyword: keyword, domains: probe.domains || [] },
      labelClass: probe.labelClass || 'measured'
    });
  });

  const liveAnswers = [];
  if (answerProbes.chatgpt && answerProbes.chatgpt.status === 'live') liveAnswers.push('ChatGPT');
  if (answerProbes.perplexity && answerProbes.perplexity.status === 'live') liveAnswers.push('Perplexity');
  ...
  return {
    ...
    note:
      ...
      (liveAnswers.length ? ' · answer probes: ' + liveAnswers.join(' + ') : ' · set OPENAI_API_KEY / PERPLEXITY_API_KEY for ChatGPT and Perplexity probes') +
      '.'
  };
}
