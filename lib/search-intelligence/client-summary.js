...
async function buildClientSummary(admin, site, opts) {
  ...
  const [gsc, ga4, organic, pages, tracked, connections] = await Promise.all([
  ...
  const openActions = (pages.findings || []).concat(audit.findings || []).slice(0, 8);
