...
async function serp(input) {
  ...
  // Include a site host when provided so lookup can detect "you".
  const ownHost = cleanDomain((input && (input.siteDomain || input.ownDomain)) || '');
  const results = [
    types.serpResult({
      rank: 1,
      type: 'local_pack',
      url: 'https://comp-1-' + slug + '.example/local',
      domain: 'comp-1-' + slug + '.example',
      title: titleBase + ' — local A'
    }),
    types.serpResult({
      rank: 2,
      type: 'local_pack',
      url: 'https://comp-2-' + slug + '.example/local',
      domain: 'comp-2-' + slug + '.example',
      title: titleBase + ' — local B'
    }),
    types.serpResult({
      rank: 1,
      type: 'organic',
      url: ownHost ? 'https://' + ownHost + '/' : 'https://comp-1-' + slug + '.example/',
      domain: ownHost || 'comp-1-' + slug + '.example',
      title: ownHost ? titleBase + ' — your site' : titleBase + ' — competitor A'
    }),
    types.serpResult({
      rank: 2,
      type: 'organic',
  ...
  return {
    ok: true,
    provider: 'mock',
    mode: String((input && (input.mode || input.serpMode)) || 'advanced').toLowerCase() === 'regular'
      ? 'regular'
      : 'advanced',
...
async function serpCompetitors(input) {
  const raw =
    Array.isArray(input && input.keywords)
      ? input.keywords
      : String((input && (input.keywords || input.keyword || input.seeds)) || 'local service').split(
          /[,\n]/
        );
  const keywords = raw
    .map(function (k) {
    ...
    .filter(Boolean)
    .slice(0, 10);
  const seed = keywords[0] || 'local service';
  const tokens = serviceTokensFromDomain('', seed);
  const slug = (tokens.slice(0, 2).join('-') || 'local').slice(0, 32);
  const rivals = [1, 2, 3, 4].map(function (i) {
    return Object.assign(
      types.competitorDomain({
        domain: 'comp-' + i + '-' + slug + '.example',
        overlapCount: Math.max(1, keywords.length - (i - 1)),
        competitionLevel: 400 - i * 60,
        visibilityEstimate: 0.8 - i * 0.12,
        organicKeywords: Math.max(1, keywords.length - (i - 1)),
        organicTraffic: 900 - i * 140,
        avgPosition: 2 + i,
        competitorType: 'search',
        provider: 'mock',
        labelClass: 'estimated'
      ...
      {
        rating: 400 - i * 60,
        visibility: 0.8 - i * 0.12,
        medianPosition: 2 + i,
        relevantSerpItems: Math.max(1, keywords.length),
  ...
  return {
    ok: true,
    provider: 'mock',
    keywords: keywords.length ? keywords : [seed],
    competitors: rivals,
    totalCount: rivals.length,
    labelClass: 'estimated',
    note: 'Mock SERP competitors derived from your keyword seeds only.'
  };
}
