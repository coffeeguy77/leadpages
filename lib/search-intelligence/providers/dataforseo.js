...
function resolveItemDomain(it) {
  ...
  return hostFromUrl(it.url || it.link || '') || null;
}
...
function mapSerpItems(items) {
  ...
  (items || []).forEach(function (it) {
    ...
    if (t === 'organic') {
      ...
      // Previously we collapsed the whole pack to maps.google.com and rival discovery
      // filtered that out → "Found 0 domains" for local keywords like coffee cart hire.
      const nested =
      ...
      if (nested.length) {
        nested.slice(0, 20).forEach(function (el, idx) {
          const domain = resolveItemDomain(el);
          if (!domain) return;
          results.push(
            types.serpResult({
              rank: el.rank_group != null ? el.rank_group : idx + 1,
              url: el.url || el.link || null,
              domain: domain,
              title: el.title || el.domain || 'Local result',
              snippet: el.description || el.snippet || null,
              type: 'local_pack'
            })
          );
        });
      } else {
        const domain = resolveItemDomain(it) || 'maps.google.com';
        results.push(
          types.serpResult({
            ...
            type: domain === 'maps.google.com' ? 'maps' : 'local_pack'
          })
        );
      }
