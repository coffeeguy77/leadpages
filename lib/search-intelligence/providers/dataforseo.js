...
/**
 * Build DataForSEO location fields.
 * Prefer location_name (canonical DFS strings) over guessed city codes —
 * invalid codes (e.g. a wrong Canberra integer) return empty SERPs with task errors.
 */
function resolveSerpLocation(input) {
  if (input && input.locationCode != null) {
    return {
      location_code: Number(input.locationCode),
      se_domain: input.seDomain || 'google.com.au',
      label: 'code:' + input.locationCode
    };
  }
  if (input && input.locationName && String(input.locationName).indexOf(',') >= 0) {
    return {
      location_name: String(input.locationName).trim(),
      se_domain: input.seDomain || 'google.com.au',
      label: String(input.locationName).trim()
    };
  }
  const loc = String((input && (input.location || input.geo || input.locationName)) || '')
    .toLowerCase()
    .trim();
  if (/canberra|\bact\b/.test(loc)) {
    return {
      location_name: 'Canberra,Australian Capital Territory,Australia',
      se_domain: 'google.com.au',
      label: 'Canberra,Australian Capital Territory,Australia'
    };
  }
  if (/sydney|\bnsw\b/.test(loc)) {
    return {
  ...
  if (/perth|\bwa\b/.test(loc)) {
    return {
      location_name: 'Perth,Western Australia,Australia',
      se_domain: 'google.com.au',
      label: 'Perth,Western Australia,Australia'
    };
  }
  if (/adelaide|\bsa\b/.test(loc)) {
    return {
      location_name: 'Adelaide,South Australia,Australia',
      se_domain: 'google.com.au',
      label: 'Adelaide,South Australia,Australia'
    };
  }
  // Explicit DFS-format name passed through
  if (loc && loc.indexOf(',') >= 0) {
    return {
      location_name: String((input && (input.location || input.locationName)) || '').trim(),
      se_domain: 'google.com.au',
      label: String((input && (input.location || input.locationName)) || '').trim()
    };
  }
  const envCode = parseInt(process.env.DATAFORSEO_LOCATION_CODE || '2036', 10);
  return {
    location_code: Number.isFinite(envCode) ? envCode : 2036,
    se_domain: 'google.com.au',
    label: 'Australia'
  };
}

/** @deprecated use resolveSerpLocation — kept for callers that only need a numeric code */
function locationCode(input) {
  const resolved = resolveSerpLocation(input);
  if (resolved.location_code != null) return resolved.location_code;
  return 2036;
}
...
function resolveItemDomain(it) {
  if (!it) return null;
  const fromField = it.domain ? String(it.domain).replace(/^www\./, '').toLowerCase() : '';
  if (fromField && fromField.indexOf('.') >= 0) return fromField;
  const website = it.website || it.url || it.link || it.breadcrumb || '';
  return hostFromUrl(website) || (fromField || null);
}

function pushSerpRow(results, partial) {
  const domain = resolveItemDomain(partial);
  if (!domain) return;
  results.push(
    types.serpResult({
      rank: partial.rank_group != null ? partial.rank_group : partial.rank_absolute != null ? partial.rank_absolute : partial.rank,
      url: partial.url || partial.link || partial.website || null,
      domain: domain,
      title: partial.title || partial.domain || null,
      snippet: partial.description || partial.snippet || null,
      type: partial.type || 'organic'
    })
  );
}
...
function mapSerpItems(items) {
  ...
  (items || []).forEach(function (it) {
    ...
    if (t === 'organic' || t === 'featured_snippet' || t === 'answer_box') {
      pushSerpRow(results, Object.assign({}, it, { type: t === 'organic' ? 'organic' : t }));
      return;
    }
    if (
      t === 'local_pack' ||
      ...
      // DataForSEO nests local businesses under local_pack.items — expand them.
      const nested =
        (Array.isArray(it.items) && it.items) ||
        (Array.isArray(it.local_pack_items) && it.local_pack_items) ||
        [];
      if (nested.length) {
        nested.slice(0, 20).forEach(function (el, idx) {
          pushSerpRow(
            results,
            Object.assign({}, el, {
              type: 'local_pack',
              rank_group: el.rank_group != null ? el.rank_group : idx + 1
            })
          );
        });
      } else {
        pushSerpRow(
          results,
          Object.assign({}, it, {
            type: resolveItemDomain(it) ? 'local_pack' : 'maps',
            domain: resolveItemDomain(it) || 'maps.google.com'
          })
        );
      }
      return;
    ...
    if (t === 'ai_overview' || t === 'chatgpt') {
      ...
      if (refs.length) {
        refs.slice(0, 12).forEach(function (ref, idx) {
          pushSerpRow(results, Object.assign({}, ref, { type: 'ai_overview', rank_group: idx + 1 }));
        });
      } else {
        pushSerpRow(results, Object.assign({}, it, { type: 'ai_overview' }));
      }
      return;
    }
    // Catch-all: any row with a real website (DFS adds new types over time)
    if (it.url || it.domain || it.website || it.link) {
      pushSerpRow(results, Object.assign({}, it, { type: t || 'organic' }));
    }
  });
  return {
...
async function serp(input) {
  ...
  try {
    ...
    // regular = organic/paid/featured_snippet only; advanced = full SERP features (default)
    const mode =
      String((input && (input.mode || input.serpMode)) || 'advanced').toLowerCase() === 'regular'
        ? 'regular'
        : 'advanced';
    const loc = resolveSerpLocation(input);
    const payload = {
      keyword: keyword,
      language_code: languageCode(input),
      device: device,
    ...
    if (loc.location_code != null) payload.location_code = loc.location_code;
    if (loc.se_domain) payload.se_domain = loc.se_domain;

    const json = await dfsPost('serp/google/organic/live/' + mode, payload);
    const task = firstTask(json);
    if (!task) {
      return {
        ok: false,
        ...
        error: 'serp_no_task',
        operation: 'serp',
        message: 'DataForSEO returned no SERP task for this keyword.'
      };
    }
    if (task.status_code && Number(task.status_code) !== 20000) {
      return {
        ok: false,
        provider: 'dataforseo',
        error: 'serp_task_error',
        operation: 'serp',
        message:
          task.status_message ||
          'DataForSEO SERP task failed (' + task.status_code + '). Check location / API balance.',
        locationUsed: loc.label,
    ...
    const mapped = mapSerpItems(rawItems);
    if (!mapped.results.length) {
      return {
        ok: false,
        provider: 'dataforseo',
        error: 'serp_empty',
        ...
        message:
          'DataForSEO returned ' +
          rawItems.length +
          ' SERP element(s) but no usable website rows for "' +
          keyword +
          '" @ ' +
          loc.label +
          '. Try Australia-wide or check API response types.',
        locationUsed: loc.label,
    ...
    return {
      ...
      snapshot: types.serpSnapshot({
        keyword: keyword,
        location: loc.label || (input && input.location) || 'Australia',
        device: device,
        provider: 'dataforseo',
...
/**
 * Domains that rank for a keyword set — Labs SERP Competitors (visibility / etv / positions).
 */
async function serpCompetitors(input) {
  if (!configured()) return notConfigured('serpCompetitors');
  const raw =
    Array.isArray(input && input.keywords)
      ? input.keywords
      : String((input && (input.keywords || input.keyword || input.seeds)) || '').split(/[,\n]/);
  const keywords = raw
    .map(function (k) {
  ...
  const limit = Math.min(100, Math.max(3, (input && input.limit) || 20));
  try {
    const payload = {
      keywords: keywords,
      language_code: languageCode(input),
      item_types: Array.isArray(input && input.itemTypes)
        ? input.itemTypes
        : ['organic', 'local_pack', 'featured_snippet'],
      limit: limit,
      order_by: ['rating,desc'],
      location_code: locationCode(input)
    };
    // Prefer explicit city location_name when provided (Labs accepts either)
    if (input && input.locationName && String(input.locationName).indexOf(',') >= 0) {
      delete payload.location_code;
      payload.location_name = String(input.locationName).trim();
    } else if (input && input.locationCode != null) {
      payload.location_code = Number(input.locationCode);
    }

    const json = await dfsPost('dataforseo_labs/google/serp_competitors/live', payload);
    const task = firstTask(json);
    if (task && task.status_code && Number(task.status_code) >= 40000) {
      throw new Error(task.status_message || 'dataforseo_task_' + task.status_code);
    }
    ...
    const competitors = items.map(function (it) {
      ...
      const etv = it.etv != null ? Number(it.etv) : null;
      const kwCount = it.keywords_count != null ? Number(it.keywords_count) : null;
      return Object.assign(
        types.competitorDomain({
          domain: cleanDomain(it.domain),
          overlapCount: kwCount,
          competitionLevel: rating != null ? rating : kwCount,
          visibilityEstimate: it.visibility != null ? Number(it.visibility) : etv,
        ...
        {
          rating: rating,
          visibility: it.visibility != null ? Number(it.visibility) : null,
          medianPosition: it.median_position != null ? Number(it.median_position) : null,
          relevantSerpItems: it.relevant_serp_items != null ? Number(it.relevant_serp_items) : null,
          keywordsPositions: it.keywords_positions || null,
    ...
    return {
      ...
      totalCount: result && result.total_count != null ? Number(result.total_count) : competitors.length,
      labelClass: 'estimated'
    };
  } catch (e) {
    return {
...
module.exports = {
  ...
  mapSerpItems: mapSerpItems,
  resolveItemDomain: resolveItemDomain,
  resolveSerpLocation: resolveSerpLocation,
