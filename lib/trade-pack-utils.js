...
function hydrateSparsePack(parsed, trade, category) {
  ...
  s.quote = s.quote || {
    ...
    successSub: "A team member from {{businessName}} will call you on the number you gave us. If it's an emergency, please call {{phoneText}} so we can get moving now.",
...
function finalizePack(parsed, prompt) {
  let pack = parsed;
  let validErr = validatePack(pack);
  if (validErr) {
    const tradeMatch = String(prompt).match(/trade:\s*"([^"]+)"/i);
    const catMatch = String(prompt).match(/"category":\s*"([^"]+)"/);
    pack = hydrateSparsePack(pack, tradeMatch && tradeMatch[1], catMatch && catMatch[1]);
    validErr = validatePack(pack);
  }
  if (validErr) throw new Error('AI pack validation failed: ' + validErr);
  return pack;
}

async function callClaudeViaBrain(prompt, attempt) {
  const { getPlatformBrain } = require('./brain/platform');
    ...
    messages: [
      {
        ...
        content:
          ...
          'Return ONLY one complete raw JSON object — no markdown, no code fences, no commentary.'
      },
  ...
  if (!result.ok) {
    throw new Error(
      (result.error && result.error.message) || 'Brain trade pack generation failed'
    );
  }
  const rawText = result.output && result.output.text != null ? String(result.output.text) : '';
  let parsed = null;
  let parseErr = null;
  try {
  ...
  if (!parsed) {
    ...
    throw new Error('AI returned invalid JSON' + hint + (parseErr ? ' (' + parseErr.message + ')' : ''));
  }
  return finalizePack(parsed, prompt);
}

async function callClaudeViaAnthropic(prompt, attempt) {
...
async function callClaude(prompt, attempt = 1) {
  let useBrain = false;
  try {
    const { isTradePackEnabled } = require('./brain/platform');
    useBrain = isTradePackEnabled();
  } catch (_e) {
    useBrain = false;
  }
  if (useBrain) return callClaudeViaBrain(prompt, attempt);
  return callClaudeViaAnthropic(prompt, attempt);
}
