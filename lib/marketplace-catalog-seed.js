...
function isPlatformFeature(meta) {
  return !!(meta && meta.platformFeature === true);
}
...
async function upsertCatalogFeature(sb, sectionKey, meta, sellTpl, categoryId, sortOrder) {
  ...
  const platform = isPlatformFeature(meta);
  const hero = platform ? null : mv.getCategoryHero(sectionKey, meta);
  const demoSite = platform ? null : demoSites.getDemoSiteForApp(sectionKey);
  const row = {
    name: meta.name || sellTpl.name || sectionKey,
    slug,
    tagline: meta.tagline || sellTpl.tagline || null,
    summary: meta.summary || sellTpl.summary || null,
    section_key: platform ? null : sectionKey,
    category_id: categoryId,
    hero_image_url: (hero && hero.url) || (sellTpl.hero_image_url || null),
    demo_url: (demoSite && demoSite.url) || null,
    badge: platform ? 'Platform' : null,
...
async function seedMarketplaceCatalog(sb, options) {
  ...
  for (const sectionKey of sectionKeys) {
    ...
    try {
      ...
      // Platform tools (SEO Command, Advertising) are marketing explainers only —
      // do not register installable section apps.
      if (!isPlatformFeature(meta)) {
