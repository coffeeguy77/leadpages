...
function buildServices(brief, foundation) {
  ...
  if (foundation.category === 'retail') {
    return [
      { title: 'Signature pieces', text: 'Curated ' + spec + ' with private viewings in ' + loc + '.' },
      { title: 'Bespoke design', text: 'Made-to-order settings guided by your taste and stone.' },
      { title: 'Private appointments', text: 'Quiet consultations for collectors and couples.' }
    ];
  }
  if (foundation.category === 'hospitality') {
    return [
      { title: 'Specialty menu', text: spec + ' prepared with care in ' + loc + '.' },
      { title: 'Daily favourites', text: 'Seasonal plates and drinks locals return for.' },
      { title: 'Takeaway', text: 'Quality to go when you cannot stay.' }
    ];
  }
  if (foundation.category === 'professional') {
    return [
      { title: 'Core advisory', text: spec + ' for growing businesses in ' + loc + '.' },
      { title: 'Clarity sessions', text: 'Practical guidance without jargon.' },
      { title: 'Ongoing support', text: 'A steady partner through each reporting cycle.' }
    ];
  }
  ...
  if (foundation.category === 'trade' || foundation.category === 'property') {
    return [
      { title: 'Primary service', text: spec + ' delivered by a professional local team.' },
      { title: 'Quoted clearly', text: 'Upfront pricing before work starts in ' + loc + '.' },
      { title: 'Reliable follow-through', text: 'Tidy workmanship and clear communication.' }
    ];
  }
  return [
    { title: 'Featured service', text: spec + ' tailored for ' + (brief.audience || 'your clients') + '.' },
    { title: 'Guided experience', text: 'A clear path from first enquiry to outcome.' },
    { title: 'Local presence', text: 'Based in ' + loc + '.' }
  ];
}
  ...
  switch (key) {
    case 'hero':
      return {
        ...base,
        variant: 'hero',
        content: { heading: ctx.heading, subheading: ctx.subheading, cta: ctx.cta }
      };
    case 'emerg':
      return {
        ...base,
