  ...
  try {
    ...
    function wipe(el){
      ...
      for (var i=0;i<nodes.length;i++){
        ...
        if(!t) continue;
        if(/blocked\\s*drain|plumber|plumbing|we'll clear it today|burst pipes|emergency\\s*call|licensed\\s*canberra\\s*plumber|hedge\\s*trimm|master\\s*plumber|Gungahlin|Belconnen|Tuggeranong|pergola|landscap(e|ing)\\s+(design|crew)|drain\\s*clear/i.test(t)){
          nodes[i].textContent = '';
        }
      }
    }
...
const TRADE_RESIDUAL =
  /we'll clear it today|burst pipes|blocked drain|licensed canberra plumber|24\/7 emergency plumber/i;
...
function hideInactiveSections(html, cfg) {
  ...
  return String(html || '').replace(
    /<section\b([^>]*\bdata-sec="([^"]+)"[^>]*)>/gi,
    function (full, attrs, key) {
      const s = sections[key];
      if (s && s.on !== false) return full;
      if (/\bhidden\b/i.test(attrs) || /display\s*:\s*none/i.test(attrs)) return full;
      return '<section' + attrs + ' hidden style="display:none!important">';
    }
  );
}
