...
/**
 * Render a Website Studio draft config through the real trade template HTML.
 * Preview-only: noindex, no-store, forms/analytics sandboxed.
 * Legacy module path: lib/theme-studio/ (see docs/website-studio/MIGRATION.md).
 */
...
/**
 * Strip operational / tracking fields and mark preview sandbox.
...
/** Neutralize trade-shell hardcoded copy before applyCfg for Website Studio drafts. */
  ...
  try {
    ...
    function wipe(el){
      ...
      for (var i=0;i<nodes.length;i++){
        ...
        if(!t) continue;
        if(/blocked\\s*drain|plumber|plumbing|hi-?vis|emergency\\s*call|licensed\\s*canberra\\s*plumber|hedge\\s*trimm|master\\s*plumber|@type\\s*:\\s*Plumber|Gungahlin|Belconnen|Tuggeranong|pergola|landscap(e|ing)\\s+(design|crew)|drain\\s*clear/i.test(t)){
          nodes[i].textContent = '';
        }
      }
    }
    document.querySelectorAll('[data-sec]').forEach(function(sec){
      ...
      if(!s || s.on === false){
        ...
        // Clear known trade fallback strings even on active sections before applyCfg fills them
        wipe(sec);
      }
    });
  } catch (e) {}
...
/**
 * @param {object} draftConfig
 * @param {{ mode?: 'desktop'|'mobile' }} [opts]
 * @returns {string} HTML
 */
function renderDraftPreviewHtml(draftConfig, opts) {
  ...
  const isComposerDraft =
    ...
    cfg.__websiteComposer.contentInheritance === 'none';
  const headInject = PREVIEW_GUARD + (isComposerDraft ? SHELL_NEUTRALIZE : '');
  if (html.includes('</head>')) {
    html = html.replace('</head>', headInject + '</head>');
  } else {
    html = headInject + html;
  }
  // Ensure neutralize runs as early as possible in body too
  if (isComposerDraft && html.includes('<body')) {
