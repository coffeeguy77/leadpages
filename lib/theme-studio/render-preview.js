...
/**
 * Render a Theme Studio draft config through the real trade template HTML.
...
<div class="ts-preview-banner">Theme Studio preview · draft only · not published · forms &amp; tracking disabled<br><small>Desktop/mobile via parent toolbar</small></div>
...
function renderDraftPreviewHtml(draftConfig, opts) {
  ...
  if (html.includes('</head>')) {
    html = html.replace('</head>', PREVIEW_GUARD + '</head>');
  } else {
    html = PREVIEW_GUARD + html;
  }
  if (mode === 'mobile') {
    const mobileCss =
