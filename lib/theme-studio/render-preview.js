...
function applyIdentityTokens(html, cfg) {
  ...
  let out = html;
  for (const [k, v] of Object.entries(tokens)) out = out.replaceAll(k, () => String(v ?? ''));
  if (!mark) {
    out = out.replace(
      '</head>',
...
function sandboxConfig(draftConfig) {
  ...
  cfg.__noindex = true;
  rewriteDeadMockImageUrls(cfg);
  return cfg;
}

/** Dead mock hosts never resolve and keep Safari/iPad tab spinners alive in iframes. */
function isDeadMockImageUrl(url) {
  const u = String(url || '');
  return /https?:\/\/images\.example\.com\//i.test(u) || /https?:\/\/example\.com\/photos\//i.test(u);
}

function inlinePlaceholderDataUri(label) {
  const safe = String(label || 'Preview image').replace(/[<>&]/g, '').slice(0, 40);
  const svg =
    '<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="900" viewBox="0 0 1600 900">' +
    '<rect width="100%" height="100%" fill="#1e293b"/>' +
    '<text x="48" y="820" fill="rgba(255,255,255,.8)" font-family="system-ui,sans-serif" font-size="32">' +
    safe +
    '</text></svg>';
  return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg);
}

function rewriteDeadMockImageUrls(node, label) {
  if (!node || typeof node !== 'object') return;
  if (Array.isArray(node)) {
    node.forEach(function (item, idx) {
      rewriteDeadMockImageUrls(item, label || String(idx));
    });
    return;
  }
  for (const key of Object.keys(node)) {
    const val = node[key];
    if (typeof val === 'string' && isDeadMockImageUrl(val)) {
      node[key] = inlinePlaceholderDataUri(label || key);
    } else if (val && typeof val === 'object') {
      rewriteDeadMockImageUrls(val, label || key);
    }
  }
}
...
/**
 * Server-side paint so Composer previews show real section copy/images even when
 * client applyCfg is slow, partial, or missing field mappings (e.g. services
 * headings, footer serviceLinks vs services).
 */
function escHtml(s) {
  return String(s == null ? '' : s)
    .replace(/&/g, '&amp;')
...
function paintComposerShellHtml(html, cfg) {
  let out = String(html || '');
  const sections = (cfg && cfg.sections) || {};
  const hero = sections.hero || {};
  const servicesSec = sections.services || {};
  const featured = sections.featuredProjects || {};
  const why = sections.why || {};
  const footer = sections.footer || {};
  const services = Array.isArray(cfg.services)
    ? cfg.services
    : Array.isArray(servicesSec.items)
      ? servicesSec.items
      : [];

  if (hero.on !== false) {
    const eyebrow = hero.eyebrow != null ? String(hero.eyebrow) : '';
    const title = String(hero.title || hero.heading || '').trim();
    const sub = String(hero.sub || hero.subheading || '').trim();
    const heroImg = String(hero.imageUrl || hero.image || '').trim();
    out = out.replace(
      /(<section class="hero" data-sec="hero">[\s\S]*?<span class="eyebrow">)[\s\S]*?(<\/span>)/i,
      '$1' + escHtml(eyebrow) + '$2'
    );
    if (title) {
      out = out.replace(
        /(<section class="hero" data-sec="hero">[\s\S]*?<h1>)[\s\S]*?(<\/h1>)/i,
        '$1' + escHtml(title) + '$2'
      );
    }
    out = out.replace(
      /(<section class="hero" data-sec="hero">[\s\S]*?<p class="hero-sub">)[\s\S]*?(<\/p>)/i,
      '$1' + escHtml(sub) + '$2'
    );
    if (heroImg) {
      const safeImg = heroImg.replace(/"/g, '');
      out = out.replace(
        /(<section class="hero" data-sec="hero")([^>]*)>/i,
        '$1$2 style="background-image:linear-gradient(180deg,rgba(8,12,20,.55),rgba(8,12,20,.72)),url(&quot;' +
          safeImg +
          '&quot;);background-size:cover;background-position:center" data-hero-ssr="1">'
      );
    }
  }

  // Hero inline proof badges (static shell defaults include "Same-day service")
  out = out.replace(
    /(<div class="badges">)[\s\S]*?(<\/div>\s*<\/div>\s*<\/section>\s*<section data-sec="textBox")/i,
    function (_m, start, end) {
      const trust = sections.trustBar || {};
      const badges = Array.isArray(trust.badges) ? trust.badges : [];
      const painted = (
        badges.length
          ? badges.filter((b) => b && b.on !== false && b.label).slice(0, 4)
          : [
              { label: 'Local & trusted' },
        ...
        .map(
          (b) =>
            '<div class="badge"><span class="b-ic">✓</span>' + escHtml(b.label) + '</div>'
        )
        .join('');
      return start + painted + end;
    }
  );

  if (servicesSec.on !== false) {
    const eyebrow = servicesSec.eyebrow != null ? String(servicesSec.eyebrow) : 'What we offer';
    const heading = String(servicesSec.heading || servicesSec.title || 'Services');
    const intro = servicesSec.intro != null ? String(servicesSec.intro) : '';
    out = out.replace(
      /(<section data-sec="services"[\s\S]*?<div class="section-head"><span class="eyebrow">)[\s\S]*?(<\/span><h2>)[\s\S]*?(<\/h2><p>)[\s\S]*?(<\/p><\/div>)/i,
      '$1' + escHtml(eyebrow) + '$2' + escHtml(heading) + '$3' + escHtml(intro) + '$4'
    );
    if (services.length) {
      const cards = services
        .filter((s) => s && s.on !== false && (s.title || s.body || s.text || s.image))
        .slice(0, 8)
        .map((s) => {
          const img = String(s.image || '').trim();
          const media = img
            ? '<div class="ic has-img"><img src="' +
              escHtml(img) +
              '" alt="" style="object-fit:cover"></div>'
            : '<div class="ic"></div>';
          return (
            '<div class="svc align-left">' +
            media +
            '<h3>' +
            escHtml(s.title || '') +
            '</h3><p>' +
            escHtml(s.body || s.text || '') +
            '</p></div>'
          );
        })
        .join('');
      out = out.replace(
        /(<section data-sec="services"[\s\S]*?<div class="svcs">)([\s\S]*?)(<\/div>\s*<\/div>\s*<\/section>)/i,
        '$1' + cards + '$3'
      );
    }
  }

  if (featured.on !== false) {
    const projects = Array.isArray(featured.projects) ? featured.projects : [];
    if (featured.eyebrow != null || featured.heading) {
      out = out.replace(
        /(<section data-sec="featuredProjects"[\s\S]*?<span class="eyebrow">)[\s\S]*?(<\/span><h2>)[\s\S]*?(<\/h2>)/i,
        '$1' +
          escHtml(featured.eyebrow != null ? featured.eyebrow : 'Recent work') +
          '$2' +
          escHtml(featured.heading != null ? featured.heading : 'Featured work') +
          '$3'
      );
    }
    if (projects.length) {
      const cards = projects
        ...
        .map((p) => {
          ...
          return (
            ...
            (img
              ? '<div class="fp-media"><img src="' + escHtml(img) + '" alt=""></div>'
              : '') +
            '<div class="fp-body"><h3>' +
            escHtml(p.title || '') +
            '</h3><p>' +
            escHtml(p.text || '') +
            '</p></div></article>'
        ...
        .join('');
      out = out.replace(
        /(<section data-sec="featuredProjects"[\s\S]*?<div class="fp-grid">)([\s\S]*?)(<\/div>\s*<\/section>)/i,
        '$1' + cards + '$3'
      );
    }
  }

  if (why.on !== false && why.heading) {
    out = out.replace(
      /(<section data-sec="why"[\s\S]*?<h2>)[\s\S]*?(<\/h2>)/i,
      '$1' + escHtml(why.heading) + '$2'
    );
  }

  const footLinks = Array.isArray(footer.serviceLinks)
    ? footer.serviceLinks
    : Array.isArray(footer.services)
      ? footer.services
      : [];
  if (footer.on !== false && footLinks.length) {
    const heading = footer.servicesHeading || 'Services';
    const linksHtml =
      '<h4>' +
      escHtml(heading) +
      '</h4>' +
      footLinks
        .map(
          (L) =>
            '<a href="' +
            escHtml(L.href || '#quote') +
            '">' +
            escHtml(L.label || L.title || '') +
            '</a>'
        )
        .join('');
    out = out.replace(
      /<h4>Services<\/h4>\s*(?:<a[^>]*>[\s\S]*?<\/a>\s*)+/i,
      linksHtml
    );
  }

  const logo = (cfg && cfg.logo) || {};
  if (logo.mode === 'image' && logo.imageUrl) {
    const logoImg =
      '<img class="lp-logo" src="' +
      escHtml(logo.imageUrl) +
      '" alt="' +
      escHtml(logo.text || cfg.name || '') +
      '">';
    out = out.replace(
...
<script id="ws-composer-paint">
(function(){
  try {
    var C = window.__SITE_CONFIG__ || window.SITE_CONFIG || null;
    if(!C || !C.__websiteComposer || C.__websiteComposer.contentInheritance !== 'none') return;
    var SV = (C.sections && C.sections.services) || {};
    var svNode = document.querySelector('[data-sec="services"]');
    if(svNode){
      var head = svNode.querySelector('.section-head');
      if(head){
        var eb = head.querySelector('.eyebrow');
        if(eb && SV.eyebrow != null) eb.textContent = String(SV.eyebrow);
        var h2 = head.querySelector('h2');
        if(h2 && (SV.heading != null || SV.title != null)) h2.textContent = String(SV.heading || SV.title || '');
        var p = head.querySelector('p');
        if(p && SV.intro != null) p.textContent = String(SV.intro);
      }
    }
    var F = (C.sections && C.sections.footer) || {};
    if(Array.isArray(F.serviceLinks) && F.serviceLinks.length && !Array.isArray(F.services)){
      F.services = F.serviceLinks;
    }
  } catch (e) {}
})();
</script>
...
function injectSiteConfig(html, cfg) {
  ...
  // Function replacer — string replacement would interpret $', $`, $& in JSON
  // (e.g. fmtMoney `?'-$':'$'`) and splice the template into SITE_CONFIG.
...
function renderDraftPreviewHtml(draftConfig, opts) {
  ...
  if (isComposer) {
    html = paintComposerShellHtml(html, cfg);
    ...
    const headInject = PREVIEW_GUARD + SHELL_NEUTRALIZE + COMPOSER_PAINT;
    ...
    if (html.includes('<body')) {
      html = html.replace(/<body([^>]*)>/i, '<body$1>' + SHELL_NEUTRALIZE + COMPOSER_PAINT);
    }
  ...
  // Belt-and-suspenders: never leave dead mock hosts in the iframe document.
  if (/images\.example\.com/i.test(html)) {
    html = html.replace(/https?:\/\/images\.example\.com\/[^"'\\\s)]+/gi, function () {
      return inlinePlaceholderDataUri('Preview image');
    });
  }
  return html;
}
...
module.exports = {
  ...
  paintComposerShellHtml,
  heroContentOk,
  rewriteDeadMockImageUrls,
  isDeadMockImageUrl,
  inlinePlaceholderDataUri,
  safeJson,
  resolveShellAsset,
  loadShellHtml,
  TRADE_RESIDUAL
