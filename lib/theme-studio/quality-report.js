...
function buildQualityReport(concept, draftConfig) {
  ...
  const gate = runQualityGate(concept, draftConfig || null, {});
  for (const issue of gate.issues || []) {
    notes.push({
      code: issue.code,
      severity: issue.severity === 'critical' ? 'error' : issue.severity === 'warning' ? 'warn' : 'info',
  ...
  return {
    score,
    ok: gate.status !== 'blocked' && notes.every((n) => n.severity !== 'error'),
    status: gate.status,
    gateSummary: gate.summary,
    issues: gate.issues,
    notes,
    disclaimer:
      'Deterministic quality gate + advisory checks. Not a WCAG certification.',
