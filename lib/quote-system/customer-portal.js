...
async function getCustomerPortalByEmail(siteId, email) {
  const normalized = normalizeEmail(email);
  ...
  const { data, error } = await admin.from('quote_customer_portals')
    ...
    .maybeSingle();
  if (error) {
    console.warn('quote_customer_portals by email:', error.message);
    return null;
  }
  return data || null;
}
...
/**
 * Find an existing jobs portal, or create one if this email already has
 * SMS-verified quote sessions for the site (never create empty portals for strangers).
 */
async function resolvePortalForAccess(siteId, email) {
  const normalized = normalizeEmail(email);
  if (!siteId || !normalized || normalized.indexOf('@') < 3) {
    return { ok: false, error: 'valid_email_required' };
  }

  const existing = await getCustomerPortalByEmail(siteId, normalized);
  if (existing && existing.access_token) {
    await touchCustomerPortal(existing);
    return { ok: true, portal: existing, created: false };
  }

  const admin = getAdmin();
  const { data: sessions } = await admin.from('quote_sessions')
    .select('id,contact_phone,sms_verified_at')
    .eq('site_id', siteId)
  ...
  const hit = sessions && sessions[0];
  if (!hit) return { ok: false, error: 'no_portal' };

  const ensured = await ensureCustomerPortal(siteId, normalized, hit.contact_phone || null);
  if (!ensured.ok || !ensured.portal) return { ok: false, error: ensured.error || 'no_portal' };
  return { ok: true, portal: ensured.portal, created: true };
}
