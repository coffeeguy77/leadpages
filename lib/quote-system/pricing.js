...
function normalizeCartRow(c, products) {
  ...
  var quantity = Math.max(1, parseInt(c.quantity, 10) || 1);
  if (product && product.allowQuantity) {
    var maxQ = parseInt(product.maxQuantity, 10);
    if (!isNaN(maxQ) && maxQ > 0) quantity = Math.min(maxQ, quantity);
  }

  var cartHours = null;
  var parsedHours = parseInt(c.hours, 10);
  if (!isNaN(parsedHours) && parsedHours > 0) cartHours = Math.min(48, parsedHours);
...
function beverageMinQuantity(beverage) {
  var n = parseInt(beverage && beverage.minQuantity, 10);
  return (!isNaN(n) && n > 0) ? Math.min(50000, n) : 0;
}

function clampBeverageQuantity(beverage, qty) {
  var q = Math.max(0, Math.round(Number(qty) || 0));
  if (q <= 0) return 0;
  var min = beverageMinQuantity(beverage);
  if (min > 0 && q < min) return min;
  return Math.min(50000, q);
}

/**
 * Resolve beverage/package lines for pricing.
 * Prefer beverageLines [{ beverageId, quantity }]; fall back to legacy beverageId + guestCount/unitCount.
 * Optional beverages list enforces each package's minQuantity when qty > 0.
 */
function resolveBeverageLines(inp, beverages) {
  inp = inp || {};
  var lines = [];
  if (Array.isArray(inp.beverageLines) && inp.beverageLines.length) {
    inp.beverageLines.forEach(function(l) {
      if (!l || !l.beverageId) return;
      var bev = findById(beverages, l.beverageId);
      var q = clampBeverageQuantity(bev, l.quantity);
      if (q <= 0) return;
      lines.push({ beverageId: String(l.beverageId), quantity: q });
    });
    return lines;
  }
  var id = String(inp.beverageId || '').trim();
  if (!id) return [];
  var bev = findById(beverages, id);
  var qty = clampBeverageQuantity(bev, packageQuantity(inp));
  if (qty <= 0) return [];
  return [{ beverageId: id, quantity: qty }];
}
...
function cartEventHours(cart, inp, labour) {
  var planning = String(inp.labourPlanning || labour.planningMode || 'hours');
  var allowShifts = labour.allowShiftPlanner !== false && planning === 'shifts';
  if (allowShifts) return eventBillableHours(inp, labour);
  var min = Math.max(1, Number(labour.minimumHours) || 3);
  if (inp.eventConfigMode === 'custom' && cart && cart.hours != null && cart.hours !== '') {
    return Math.max(min, Number(cart.hours) || min);
  }
  return eventBillableHours(inp, labour);
}
