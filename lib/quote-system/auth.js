...
function canReadFullConfig(quoteSystem, { isSuper, isAdminRequest }) {
...
function canWriteConfig(quoteSystem, { isSuper }) {
  if (!isSuper) return false;
  if (!quoteSystem) return true;
  const cls = quoteSystem.configuration_classification;
  if (cls === CONFIG_CLASSIFICATION.BLANK) return true;
  if (cls === CONFIG_CLASSIFICATION.PRIVATE_SUPERUSER) return true;
  if (cls === CONFIG_CLASSIFICATION.PUBLIC) return true;
  return false;
}
