...
async function sendEmailVerifiedTotalEmail({ to, businessName, totalFormatted, siteSlug }) {
  const key = process.env.RESEND_API_KEY;
  if (!key || !to) return { sent: false, reason: !key ? 'no_key' : 'no_recipient' };
  ...
  const html =
    ...
    (siteSlug ? '<p style="font-size:13px;color:#8a93a3">Return to <a href="https://' + esc(siteSlug) + '.leadpages.com.au/' + esc(siteSlug) + '">your quote</a> to finish verification.</p>' : '') +
    '</div>';
