...
async function sendPortalLinkEmail({ to, businessName, portalUrl, totalFormatted, pdfBuffer }) {
  ...
  const html =
    ...
    '<a href="' + esc(portalUrl) + '" style="display:inline-block;background:#1f7a63;color:#fff;text-decoration:none;font-weight:700;padding:14px 24px;border-radius:10px">Open your quotes portal</a>' +
    '<p style="color:#566;font-size:14px;margin:16px 0 0">View all your jobs, adjust quantities, accept a quote, or download a PDF anytime.</p>' +
    (pdfBuffer ? '<p style="color:#566;font-size:14px;margin:8px 0 0">The itemised quote PDF is attached to this email.</p>' : '') +
...
async function sendPortalAccessEmail({ to, businessName, portalUrl }) {
  const key = process.env.RESEND_API_KEY;
  if (!key || !to) return { sent: false, reason: !key ? 'no_key' : 'no_recipient' };

  const html =
    '<div style="font-family:system-ui,sans-serif;max-width:520px;margin:0 auto;color:#1c2330">' +
    '<h2 style="margin:0 0 8px">Access your quotes — ' + esc(businessName || 'your provider') + '</h2>' +
    '<p style="color:#566;margin:0 0 16px">Use this private link to open your quotes portal. You can view past jobs, adjust quantities, accept a quote, or download a PDF.</p>' +
    '<a href="' + esc(portalUrl) + '" style="display:inline-block;background:#1f7a63;color:#fff;text-decoration:none;font-weight:700;padding:14px 24px;border-radius:10px">Open my quotes portal</a>' +
    '<p style="color:#8a93a3;font-size:12px;margin:20px 0 0">This link is private — do not share it. If you did not request access, you can ignore this email.</p>' +
    '</div>';
  ...
  try {
    const r = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: FROM,
        to: [to],
        subject: 'Access your quotes — ' + (businessName || 'Quote portal'),
        html
      })
    });
    if (!r.ok) return { sent: false, reason: 'resend_' + r.status };
