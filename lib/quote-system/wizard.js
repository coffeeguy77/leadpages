...
/**
 * Advance/retreat by step identity so index shifts (conditional travel, etc.)
 * cannot skip or stall on the wrong step.
 */
function stepIndexAfterMove(stepsBefore, currentIndex, delta, stepsAfter) {
  const before = Array.isArray(stepsBefore) ? stepsBefore : [];
  const after = Array.isArray(stepsAfter) ? stepsAfter : before;
  const curIdx = Math.max(0, Math.min(before.length - 1, Number(currentIndex) || 0));
  const curKey = before[curIdx];
  const targetIdx = curIdx + (delta < 0 ? -1 : 1);
  const targetKey = before[targetIdx];

  if (targetKey) {
    const mapped = after.indexOf(targetKey);
    if (mapped >= 0) return mapped;
  }

  const curAfter = after.indexOf(curKey);
  if (curAfter >= 0) {
