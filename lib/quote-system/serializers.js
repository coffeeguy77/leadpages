...
function publicProductLabels(config) {
  ...
  return {
    products: products.map(function(p) {
      return { id: p.id, label: p.label, description: p.description || null, type: p.type || 'equipment' };
    }),
    addons: addons.map(function(a) {
      return { id: a.id, label: a.label, description: a.description || null };
    }),
    beverages: ((config && config.beverages) || []).map(function(b) {
      return { id: b.id, label: b.label, description: b.description || null };
    }),
    wizard: (config && config.wizard) || { steps: ['event', 'products', 'addons', 'contact'] },
    business: {
