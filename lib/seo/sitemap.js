...
export function primaryHosts() {
  return (process.env.PRIMARY_HOSTS || 'app.leadpages.com.au,leadpages.webculture.au,leadpages.com.au,www.leadpages.com.au')
    .split(',')
...
/** Max sitemap locs Google accepts in one index file (use a cushion below 50k). */
export const SITEMAP_INDEX_PAGE_SIZE = 40000;

/**
 * Build a sitemap index XML document.
 * @param {Array<string|{loc:string,lastmod?:string}>} locs
 */
export function buildSitemapIndexXml(locs) {
  const esc = (u) =>
    String(u).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const rows = (Array.isArray(locs) ? locs : []).map((e) => {
    const loc = typeof e === 'string' ? e : e && e.loc;
    if (!loc) return '';
    const lastmod = typeof e === 'string' ? '' : (e && e.lastmod) || '';
    let row = '  <sitemap><loc>' + esc(loc) + '</loc>';
    if (lastmod) row += '<lastmod>' + esc(lastmod) + '</lastmod>';
    row += '</sitemap>';
    return row;
  }).filter(Boolean);
...
/**
 * Decide whether the root /seo-sitemap.xml should list tenant sitemaps directly
 * or shard into /seo-sitemaps/{page}.xml child indexes.
 */
export function sitemapIndexPlan(total, pageSize) {
  const size = pageSize || SITEMAP_INDEX_PAGE_SIZE;
  const n = Math.max(0, parseInt(total, 10) || 0);
  if (n <= size) {
    return { mode: 'direct', pages: 1, pageSize: size };
  }
  return { mode: 'sharded', pages: Math.ceil(n / size), pageSize: size };
}
