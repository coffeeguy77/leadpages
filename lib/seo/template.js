...
function normalizeGoogleVerification(v) {
  v = String(v == null ? '' : v).trim();
  if (!v) return '';
  // Accept token, google-site-verification=token, or full <meta ... content="token" ...>
  let m = v.match(/google-site-verification=([^\s"'>]+)/i);
  if (m && m[1]) return m[1].trim();
  m = v.match(/content\s*=\s*["']([^"']+)["']/i);
  if (m && m[1]) return m[1].trim();
  return v;
}
...
export function applySeoHead(html, { title, description, canonical, image, robots, googleVerification }) {
  ...
  const _g = normalizeGoogleVerification(googleVerification);
  if (_g) {
    const gmeta =
      '<meta name="google-site-verification" content="' + esc(_g) + '">';
