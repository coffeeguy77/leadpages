...
function buildDraftConfig(input) {
  ...
  // Optional logo upload from Website Studio brief (data URL or hosted URL).
  const logoUrl = String(
    (concept.logo && (concept.logo.imageUrl || concept.logo.url)) ||
      bp.logoImageUrl ||
      bp.logoUrl ||
      ''
  ).trim();
  if (logoUrl && (/^https?:\/\//i.test(logoUrl) || /^data:image\//i.test(logoUrl))) {
    draft.logo.mode = 'image';
    draft.logo.imageUrl = logoUrl;
    draft.logo.text = String(bp.businessName || draft.name || '').trim();
    writtenPaths.push('logo.mode', 'logo.imageUrl', 'logo.text');
  } else if (!draft.logo.mode) {
    draft.logo.mode = 'text';
    draft.logo.text = String(bp.businessName || draft.name || '').trim();
    writtenPaths.push('logo.mode', 'logo.text');
  }
