 ...
 * @param {object} input.diagnostics partial
 */
function buildDraftConfig(input) {
  ...
  const draft = {
    ...
    __websiteComposer: {
      ...
      rendererShellAsset: RENDERER_SHELL_ASSETS[RENDERER_SHELL_LANDING_V1],
  ...
  if (concept.sections && typeof concept.sections === 'object') {
    for (const key of order) {
      ...
      // Flatten content onto section for renderer/manage field expectations
      const next = {
