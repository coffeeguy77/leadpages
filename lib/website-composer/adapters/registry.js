...
const ADAPTERS = {
  hero(input) {
    ...
    const badges = Array.isArray(i.badges)
      ? i.badges.map((b) => ({
          icon: (b && b.icon) || 'check',
          text: (b && (b.text || b.label)) || '',
          on: !b || b.on !== false
        }))
      : [];
    return {
  ...
  services(input) {
    ...
    return {
      ...
      config: baseOut('services', {
        // Always overwrite shell eyebrows ("What we fix") — empty string clears them.
        eyebrow: (input && input.eyebrow != null) ? String(input.eyebrow) : 'What we offer',
  ...
  premiumGallery(input) {
    const errors = [];
    const i = input || {};
    const images = i.images || i.items || [];
    requireArray({ images }, 'images', 2, errors, 'sections.premiumGallery.images');
    if (errors.length) return { ok: false, errors };
    let smartPreset = String(i.smartPreset || i.preset || 'dynamic').toLowerCase();
    const allowed = ['clean', 'editorial', 'dynamic', 'minimal', 'cinematic', 'collage'];
    if (allowed.indexOf(smartPreset) < 0) smartPreset = 'dynamic';
    let applied = {};
    try {
      const presets = require('../../premium-gallery-presets');
      applied = presets.applySmartPreset({}, smartPreset) || {};
    } catch (_e) {
      applied = { smartPreset: smartPreset, layout: 'smart-mosaic' };
    }
    return {
      ok: true,
      sectionKey: 'premiumGallery',
      config: baseOut(
        'premiumGallery',
        Object.assign({}, applied, {
          on: true,
          smartPreset: smartPreset,
          heading: i.heading || 'A gallery of completed projects',
          eyebrow: i.eyebrow || 'Our work',
          intro: i.intro || '',
          structureMode: i.structureMode || 'simple',
          urlMode: i.urlMode || 'hash',
          analyticsOn: i.analyticsOn !== false,
          images: images.slice(0, 24).map((img, idx) => ({
            on: true,
            id: img.id || 'pg-studio-' + idx,
            src: img.src || img.image || img.url || '',
            alt: img.alt || img.title || '',
            title: img.title || '',
            caption: img.caption || '',
            tags: img.tags || [],
            size: img.size || 'standard',
            featured: !!img.featured,
            orientation: img.orientation || '',
            imageBrief: img.imageBrief || null
          }))
        }),
        i.provenance
      ),
      writtenPaths: ['sections.premiumGallery'],
      install: { sectionKey: 'premiumGallery', position_slot: 'upper' }
    };
  },
  ...
  reviews(input) {
    ...
    return {
      ...
      config: baseOut('reviews', {
        eyebrow: (input && input.eyebrow != null) ? String(input.eyebrow) : 'Client stories',
        heading: (input && input.heading) || 'What clients say',
        intro: (input && input.intro) || '',
    ...
    return {
      ...
      config: baseOut(
        'footer',
        {
          ...
          callTitle: i.callTitle || i.callLabel || 'Call us',
          servicesHeading: i.servicesHeading || 'Services',
          emergencyLabel: i.emergencyLabel != null ? i.emergencyLabel : '',
          // Composer field
          serviceLinks: mappedLinks,
          // Trade shell applyCfg reads sections.footer.services — keep both in sync
          services: mappedLinks,
          on: true
        },
  ...
  searchCanvas(input) {
    const errors = [];
    const i = input || {};
    const tabsIn = i.tabs || i.items || i.topics || [];
    requireArray({ tabs: tabsIn }, 'tabs', 3, errors, 'sections.searchCanvas.tabs');
    const meaningful = tabsIn.filter(function (t) {
      const intro = String((t && (t.intro || t.text || t.body)) || '').trim();
      const heading = String((t && (t.heading || t.label || t.title)) || '').trim();
      return heading && intro.length >= 40;
    });
    if (meaningful.length < 3) {
      errors.push({
        code: 'item_count_invalid',
        message: 'searchCanvas needs ≥3 meaningful topics (not one-sentence filler)',
        path: 'sections.searchCanvas.tabs'
      });
    }
    if (errors.length) return { ok: false, errors };
    const { applyAiDraftToSearchCanvas, defaultSearchCanvasConfig } = require('../../search-canvas');
    const base = defaultSearchCanvasConfig();
    const draft = {
      eyebrow: i.eyebrow || (i.header && i.header.eyebrow) || 'Our expertise',
      heading: i.heading || (i.header && i.header.heading) || 'Solutions designed around your business',
      intro: i.intro || (i.header && i.header.intro) || '',
      tabs: meaningful.slice(0, 12).map(function (t) {
        return {
          label: t.label || t.title || t.heading,
          iconSuggestion: t.iconKey || t.icon || t.iconSuggestion || 'check',
          heading: t.heading || t.title || t.label,
          intro: t.intro || t.text || t.body || '',
          supportingParagraphs: Array.isArray(t.supportingParagraphs)
            ? t.supportingParagraphs
            : t.content
              ? [String(t.content)]
              : [],
          bullets: Array.isArray(t.bullets) ? t.bullets : [],
          linkLabel: (t.link && t.link.label) || t.linkLabel || '',
          imageSearchQuery: t.imageSearchQuery || '',
          imageAltText: (t.image && t.image.alt) || t.imageAltText || ''
        };
      }),
      cta: i.cta || null
    };
    const cfg = applyAiDraftToSearchCanvas(base, draft, {
      mode: 'replace',
      source: 'website-composer',
      replaceImages: false
    });
    cfg.on = true;
    if (i.layout && typeof i.layout === 'object') {
      cfg.layout = Object.assign({}, cfg.layout, i.layout);
    }
    return {
      ok: true,
      sectionKey: 'searchCanvas',
      config: baseOut('searchCanvas', cfg, i.provenance),
      writtenPaths: ['sections.searchCanvas'],
      install: { sectionKey: 'searchCanvas', position_slot: 'main' }
    };
  }
};
