...
const ADAPTERS = {
  ...
  splitHero(input) {
    const errors = [];
    requireString(input, 'title', errors, 'sections.splitHero.title');
    if (errors.length) return { ok: false, errors };
    return {
      ok: true,
  ...
  why(input) {
    const errors = [];
    requireString(input || {}, 'heading', errors, 'sections.why.heading');
    if (errors.length) return { ok: false, errors };
    return {
      ok: true,
      sectionKey: 'why',
  ...
  trustBar(input) {
    ...
    return {
      ...
      config: baseOut('trustBar', {
        heading: (input && input.heading) || 'Trusted locally',
        badges: badges.slice(0, 6).map((b) => ({ label: b.label || b.title || String(b) }))
      }, input && input.provenance),
  ...
  reviewHighlights(input) {
    return {
      ...
      config: baseOut('reviewHighlights', {
        heading: (input && input.heading) || 'Client highlights',
        items: ((input && input.items) || []).slice(0, 4)
      }, input && input.provenance),
  ...
  quote(input) {
    const errors = [];
    const heading = (input && (input.heading || input.title || input.cta)) || '';
    ...
    return {
      ...
      config: baseOut('quote', {
        eyebrow: (input && input.eyebrow) || '',
        heading,
        title: heading,
        intro: (input && input.intro) || '',
        buttonText: (input && (input.buttonText || input.cta)) || heading,
        successMessage: (input && input.successMessage) || 'Thanks — we will be in touch shortly.'
      }, input && input.provenance),
  ...
  footer(input) {
    return {
      ...
      config: baseOut(
        'footer',
        {
          blurb: (input && input.blurb) || '',
          on: true
        },
        (input && input.provenance) || PROVENANCE.BUSINESS_PROFILE
      ),
