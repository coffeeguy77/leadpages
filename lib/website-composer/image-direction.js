...
function buildStructuredImageBriefs({ brief, profile, foundation, sectionOrder, direction, contentHints }) {
  ...
  if (sectionOrder.includes('premiumGallery')) {
    const premiumSubjects = {
      'wedding-photographer': [
        'editorial wedding portrait landscape',
        'ceremony detail rings florals portrait',
        'reception dance floor wide',
        'intimate couple portrait golden hour'
      ],
      landscaping: [
        'finished garden courtyard landscape',
        'timber deck outdoor living portrait',
        'stone path planted beds square',
        'evening entertaining area panorama'
      ],
      jewellery: [
        'engagement ring macro square',
        'boutique display editorial landscape',
        'stacked bracelets portrait',
        'atelier workbench detail'
      ]
    };
    const list =
      premiumSubjects[profile.profileId] ||
      [
        'completed project wide landscape',
        'detail craftsmanship portrait',
        'finished room square',
        'site atmosphere landscape'
      ];
    list.forEach((subject, idx) => {
      const orientation =
        /portrait/i.test(subject) ? 'portrait' : /square/i.test(subject) ? 'square' : 'landscape';
      briefs.push(
        createImageBrief({
          imageBriefId: 'premium_gallery_' + idx,
          sectionId: 'premiumGallery:' + idx,
          appId: 'premiumGallery',
          purpose: 'gallery',
          subject,
          setting: hero.setting,
          industry,
          ...
          targetAspectRatio: orientation === 'portrait' ? '3:4' : orientation === 'square' ? '1:1' : '3:2',
          minimumWidth: orientation === 'portrait' ? 900 : 1200,
          minimumHeight: orientation === 'portrait' ? 1200 : 800,
          humanPresence: dir.humanPresence,
          composition: dir.composition,
          avoidTerms: avoid,
          altTextIntent: subject
