...
async function composeOne({
  ...
  for (const r of imageResolution.results || []) {
    ...
    if (sid === 'premiumGallery' || String(sid).startsWith('premiumGallery:')) {
      premiumGalleryImages.push(r.selection);
    }
  ...
  if (premiumGalleryImages.length && generated.sections.premiumGallery) {
    const images = generated.sections.premiumGallery.images || [];
    images.forEach((im, idx) => {
      const keyed = imagesBySection['premiumGallery:' + idx];
      const pick = keyed || premiumGalleryImages[idx];
      if (pick) {
        im.src = pick.selectedVariantUrl || pick.sourceImageUrl;
        if (!im.alt) im.alt = pick.altText || im.title || '';
      }
    });
    generated.sections.premiumGallery.images = images;
    imagesBySection.premiumGallery = premiumGalleryImages[0];
  }
