...
const { PROVENANCE, RENDERER_SHELL_LANDING_V1, PROTECTED_FIELDS } = require('./constants');
...
async function composeOne({
  ...
  const diagnostics = buildDiagnostics({
    ...
    rendererWarnings: [
      {
        code: 'shell_technical_only',
        message:
          'Preview uses landing-shell-v1 (trade.template.json) as technical HTML. Website Studio drafts neutralize fallback copy and set unused sections on:false.'
      }
    ]
  });
