...
function titleCaseService(raw) {
  const s = String(raw || '').trim();
  if (!s) return s;
  const lower = s.toLowerCase();
  const rewrites = {
    service: 'Servicing & repairs',
    services: 'Servicing & repairs',
    parts: 'Parts & spares',
    upgrades: 'Upgrades & hop-ups',
    'track days': 'Track days',
    sales: 'Sales & advice'
  };
  if (rewrites[lower]) return rewrites[lower];
  // Keep known acronyms (RC) uppercase; title-case the rest.
  return s
    .split(/\s+/)
    .map((word) => {
      if (/^(rc|r\/c|fpv|esc|lipo)$/i.test(word)) return word.toUpperCase().replace('R/C', 'RC');
      if (/^(and|or|of|the|a|an|&)$/i.test(word)) return word.toLowerCase();
...
function servicesFromBrief(brief, profile) {
  ...
  return raw.slice(0, 8).map((title, idx) => {
    const nice = titleCaseService(title);
    return {
      title: nice,
      text:
        idx === 0 && spec
          ? spec + (loc ? ' across ' + loc + '.' : '.')
          : (spec ? spec + ' — ' : '') + nice + (loc ? ' in ' + loc : '') + '.'
    };
  });
}
...
function serviceCatalog(brief, profile, recipe) {
  ...
  const catalogs = {
    ...
    'hobby-retail': [
      {
        title: 'RC cars & ready-to-run',
        text: 'Buggies, trucks and touring cars ready for ' + loc + ' tracks and backyard fun.'
      },
      {
        title: 'Parts & hop-ups',
        text: 'Spares, electronics and performance upgrades for popular RC platforms.'
      },
      {
        title: 'Servicing & tuning',
        text: 'Bench setup, repairs and tuning so your car handles the way it should.'
      },
      {
        title: 'Track days & club nights',
        text: 'In-store track time and community sessions for hobbyists of every level.'
      }
    ],
    'trade-services': [
      {
...
function imageBriefsFor(brief, profile, recipe) {
  ...
  const map = {
    ...
    'hobby-retail': {
      heroImage: 'remote control RC car racing on outdoor dirt track',
      galleryImage1: 'hobby shop shelf with RC cars and spare parts',
      galleryImage2: 'person tuning an RC buggy on a workbench',
      galleryImage3: 'RC truck jumping on multi-terrain track'
    }
  };
...
function faqItems(brief, profile) {
  ...
  if (pid === 'hobby-retail') {
    return [
      {
        q: 'Do you stock parts as well as cars?',
        a: 'Yes — ' + name + ' carries platforms, spares and upgrades so you can keep racing.'
      },
      {
        q: 'Can you help me choose a first RC car?',
        a: 'Tell us your terrain and budget — we will match a kit that suits how you want to drive.'
      },
      {
        q: 'Do you offer service and repairs?',
        a: 'Bring your buggy or truck in — we diagnose, advise and get you back on the track.'
      }
    ];
  }
  if (pid === 'jewellery') {
...
function reviewItems(brief, profile) {
  ...
  const samples = {
    'hobby-retail': [
      {
        name: 'Alex',
        text: 'Knew exactly which RC platform suited my terrain — left with the right kit, not an upsell.'
      },
      {
        name: 'Mia',
        text: 'Parts advice was honest and the service counter got my buggy sorted quickly in ' + loc + '.'
      },
      {
        name: 'Chris',
        text: 'A proper hobby shop feel — stocked shelves and people who actually run the cars.'
      }
    ],
    jewellery: [
      {
...
function whyItems(brief, profile) {
  ...
  const catalogs = {
    ...
    'hobby-retail': [
      { title: 'Enthusiast advice', text: 'Guidance from people who actually run the cars.' },
      { title: 'Parts on the shelf', text: 'Platforms, spares and upgrades ready when you need them.' },
      { title: 'Service counter', text: 'Diagnostics and setup help so you spend more time driving.' },
      { title: 'Local hobby hub', text: name + ' is built for the ' + loc + ' RC community.' }
    ]
  };
...
function quoteFields(brief, profile, cta, slot) {
  ...
  const b = bases[pid] || (pid === 'hobby-retail'
    ? {
        sub: 'Tell us what you drive — we will help you pick the right kit or book a service.',
        lblJob: 'What do you need?',
        jobOptions: [
          { text: 'New RC car advice', on: true },
          { text: 'Parts & upgrades', on: true },
          { text: 'Service / repairs', on: true },
          { text: 'Visit the shop', on: true }
        ],
        points: [
          { text: 'Enthusiast advice in store', on: true },
          { text: 'Parts ready when you need them', on: true },
          { text: 'Local hobby shop in ' + loc, on: true }
        ],
        detailPh: 'Platform, terrain and what you want help with',
        suburbPh: loc
      }
    : {
  ...
  return {
    eyebrow: pid === 'hobby-retail' ? 'Plan your visit' : 'Get in touch',
    heading: cta,
    title: cta,
    sub: b.sub,
    intro: b.sub,
...
function generateSectionContent(brief, profile, foundation, recipe, ctx) {
  ...
  for (const key of ctx.sectionOrder) {
    switch (key) {
          ...
          key,
          {
            ...
            eyebrow:
              profile.profileId === 'jewellery'
                ...
                : profile.profileId === 'hobby-retail'
                  ? 'Hobby retail · parts · service'
                  : '',
            imageBrief: imageBriefs.heroImage,
            badges:
              profile.profileId === 'hobby-retail'
                ? [
                    { icon: 'check', text: 'Parts & advice in store' },
                    { icon: 'star', text: 'Run by enthusiasts' },
                    { icon: 'map', text: loc ? 'Based in ' + loc : 'Local specialty shop' }
                  ]
                : [
                    { icon: 'check', text: 'Clear next steps' },
                    { icon: 'star', text: 'Local & trusted' },
                    { icon: 'map', text: loc ? 'Serving ' + loc : 'Easy to reach' }
                  ]
          },
        ...
        put(
          key,
          {
            eyebrow:
              profile.profileId === 'hobby-retail'
                ? 'In store & ready'
                : recipe.contentHints?.serviceShape === 'practice-areas'
                  ? 'How we help'
                  : 'What we offer',
            heading:
              recipe.contentHints?.serviceShape === 'practice-areas'
      ...
      case 'premiumGallery': {
        const smart =
          profile.profileId === 'wedding-photographer' || profile.profileId === 'jewellery'
            ? 'cinematic'
            : profile.profileId === 'landscaping' || profile.profileId === 'coffee-event'
              ? 'editorial'
              : 'dynamic';
        put(
          key,
          {
            ...
            heading:
              profile.profileId === 'wedding-photographer'
            ...
            intro:
              brief.notes
                ? String(brief.notes).slice(0, 160)
                : 'Browse recent work from ' + name + (loc ? ' across ' + loc : ''),
            urlMode: 'hash',
            analyticsOn: true,
            images: (services.length ? services : [{ title: 'Gallery image', text: '' }])
              .slice(0, 8)
              .map((svc, idx) => ({
                on: true,
                id: 'pg-gen-' + idx,
                title: svc.title || 'Gallery image ' + (idx + 1),
                alt: svc.title || 'Gallery image',
                caption: '',
                tags: svc.title ? [String(svc.title).split(/[&/]/)[0].trim()] : [],
                size: idx === 0 ? 'wide' : idx === 1 ? 'tall' : 'standard',
                featured: idx < 2,
                imageBrief:
                  imageBriefs['galleryImage' + (idx + 1)] ||
                  imageBriefs.galleryImage1 ||
      ...
      case 'reviews':
        put(
          key,
          {
            ...
            intro: 'Recent notes from people who chose ' + name + '.',
            items: reviews
          },
          PROVENANCE.AI_GENERATED
        );
          ...
          key,
          {
            ...
            steps:
              profile.profileId === 'coffee-event'
                ...
                : profile.profileId === 'hobby-retail'
                  ? [
                      { title: 'Tell us what you drive', text: 'Share your platform, skill level and terrain.' },
                      { title: 'We match the kit', text: 'Cars, parts and upgrades suited to how you ride.' },
                      { title: 'Visit or collect', text: 'Get set up in store — or we prepare your order for pickup.' }
                    ]
                  : [
      ...
      case 'beforeAfter':
        put(
          key,
          {
            ...
            intro: 'Proof of craftsmanship from recent work with ' + name + '.',
            items: (services.length ? services : [{ title: 'Project', text: 'Recent work' }])
              .slice(0, 2)
              .map((svc, idx) => ({
                title: svc.title || 'Project ' + (idx + 1),
                text: svc.text || 'A clear transformation for a local client.',
                beforeLabel: 'Before',
                afterLabel: 'After',
                imageBrief: imageBriefs['galleryImage' + (idx + 1)] || imageBriefs.heroImage
              }))
          },
          PROVENANCE.AI_GENERATED
        );
