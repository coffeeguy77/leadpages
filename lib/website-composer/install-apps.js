...
function selectAppsForConcept({ foundation, recipe, profile, brief, slot }) {
  ...
  // Photo-led construction / landscaping showcase (Yass-style visual homepage)
  if (
    recipe.id === 'recipe-landscaping-showcase' ||
    recipe.id === 'recipe-builder-showcase' ||
    (profile && profile.profileId === 'landscaping')
  ) {
    const visualHero =
      slot === 0 && supported.has('heroSlider')
        ? 'heroSlider'
        : slot === 1 && supported.has('heroSlider')
          ? 'heroSlider'
          : slot === 2 && supported.has('splitHero')
            ? 'splitHero'
            : supported.has('heroSlider')
              ? 'heroSlider'
              : 'hero';
    order = uniqueOrder(
      [
        visualHero,
        supported.has('trustBar') ? 'trustBar' : null,
        'featuredProjects',
        'services',
        supported.has('beforeAfter') ? 'beforeAfter' : null,
        supported.has('brandStory') ? 'brandStory' : 'why',
        'serviceProcess',
        'crew',
        slot === 2 ? 'reviewHighlights' : null,
        'reviews',
  ...
  const heroAppId = heroPicked || 'hero';
  return {
    ...
    layoutHints: {
      ...
      preferredLayoutId: heroAppId === 'heroSlider' ? 'hero-image-slider' : null
    }
  };
}
...
function installAppsIntoDraft({ sectionOrder, sectionContentMap, imagesBySection, provenanceMap }) {
  ...
  for (const key of sectionOrder) {
    ...
    if (imagesBySection) {
      ...
      if (key === 'heroSlider' && Array.isArray(structured.slides)) {
        structured.slides = structured.slides.map((slide, idx) => {
          const slideImg =
            imagesBySection[key + ':' + idx] ||
            imagesBySection[key] ||
            null;
          const url =
            (slide && (slide.imageUrl || slide.image)) ||
            (slideImg && (slideImg.selectedVariantUrl || slideImg.sourceImageUrl)) ||
            null;
          return { ...slide, imageUrl: url, image: url };
        });
      }
      if (key === 'trustBar' && Array.isArray(structured.badges)) {
        structured.mode = structured.mode || 'badges';
        structured.badges = structured.badges.map((badge, idx) => {
          const badgeImg = imagesBySection[key + ':' + idx] || null;
          const url =
            (badge && (badge.image || badge.imageUrl)) ||
            (badgeImg && (badgeImg.selectedVariantUrl || badgeImg.sourceImageUrl)) ||
            null;
          return { ...badge, image: url, imageUrl: url, on: badge.on !== false };
        });
        if (structured.badges.some((b) => b.image)) structured.mode = 'images';
      }
      if (key === 'featuredProjects' && Array.isArray(structured.projects)) {
        structured.projects = structured.projects.map((p, idx) => {
          if (p.image) return p;
          const pImg = imagesBySection[key + ':' + idx] || img;
          return {
            ...p,
            image: (pImg && (pImg.selectedVariantUrl || pImg.sourceImageUrl)) || null
          };
        });
      }
      if (key === 'featuredProjects' && Array.isArray(structured.items) && !structured.projects) {
        structured.projects = structured.items.map((it, idx) => ({
          ...
          image:
            (imagesBySection[key + ':' + idx] &&
              (imagesBySection[key + ':' + idx].selectedVariantUrl ||
                imagesBySection[key + ':' + idx].sourceImageUrl)) ||
            it.image ||
            (img && (img.selectedVariantUrl || img.sourceImageUrl)) ||
            null,
          imageBrief: it.imageBrief
        }));
      }
      if (key === 'premiumGallery' && Array.isArray(structured.images)) {
        structured.images = structured.images.map((im, idx) => {
          if (im && im.src) return im;
          const pImg = imagesBySection[key + ':' + idx] || img;
          const url = (pImg && (pImg.selectedVariantUrl || pImg.sourceImageUrl)) || (im && im.image) || null;
          return Object.assign({}, im || {}, {
            on: true,
            src: url,
            alt: (im && im.alt) || (im && im.title) || '',
            title: (im && im.title) || ''
          });
        });
      }
