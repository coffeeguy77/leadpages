...
const APP_METADATA = Object.freeze({
  ...
  why: meta({
    ...
    compatiblePreviousApps: ['services', 'featuredProjects', 'premiumGallery'],
    qualityScore: 0.88
  }),
  trustBar: meta({
    pagePurposes: ['home'],
  ...
  aboutUs: meta({
    pagePurposes: ['home', 'landing', 'about'],
    contentDensity: 'editorial',
    conversionGoals: ['inform', 'enquire'],
    imageDependency: 'optional',
    recommendedPagePosition: 'upper',
  ...
  searchCanvas: meta({
    ...
    suitableForProductBusiness: true,
    suitableForServiceBusiness: true,
    suitableForLocalSeo: true,
    supportedVariants: ['default', 'vertical-tabs-image-right', 'horizontal-tabs', 'cards'],
    qualityScore: 0.9,
    mobilePriority: 'high'
  }),
  featureStrip: meta({
...
function scoreAppForContext(appId, ctx) {
  ...
  const flagMap = {
    ...
    // Construction / landscaping are photo-led showcases — not emergency trades.
    construction: 'suitableForConstruction'
  };
