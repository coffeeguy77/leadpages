...
function buildLeadNotifyEmail(opts) {
  ...
  const st = normalizeStyle(opts.style || DEFAULT_STYLE);
  ...
  if (st.showLogo && st.showAnimatedLogo) {
    logoParts.push(
      ...
      (st.showWordmarkLogo ? '12px' : '0') +
      '">' +
      '<img src="' +
      esc(st.logoAnimated) +
      '" width="' +
      esc(st.logoMarkWidth) +
      '" height="' +
      esc(st.logoMarkHeight) +
      '" alt="LeadPages" style="display:block;border-radius:12px;border:0;width:' +
      esc(st.logoMarkWidth) +
      'px;height:' +
      esc(st.logoMarkHeight) +
      'px">' +
  ...
  if (st.showLogo && st.showWordmarkLogo) {
    logoParts.push(
      ...
      esc(st.logoWordmark) +
      '" height="' +
      esc(st.logoWordmarkHeight) +
      '" alt="leadpages" style="display:block;border:0;height:' +
      esc(st.logoWordmarkHeight) +
      'px;width:auto">' +
      '</td>'
    );
