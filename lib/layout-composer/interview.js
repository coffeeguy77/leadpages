...
function suggestApps(understanding, blueprintKeys) {
  ...
  function add(key, title, reason, seo) {
    out.push({
      key: key,
      title: title,
      reason: reason,
      inLayout: !!have[key],
      status: have[key] ? 'already_in_layout' : 'suggested_add',
      defaultSelected: !have[key],
      sampleSeo: seo || null
    });
  }
  ...
  if (step.terminal) {
    ...
    if (opt && opt.followUp) {
      ...
      if (!options.length && fu.optionsFrom === 'services') {
        options = defaultServices(session.understanding.trade).map(function (label, i) {
    ...
    if (opt && opt.jumpTo) {
      ...
      const followId = 'followup-' + Date.now();
      const options = defaultServices(session.understanding.trade).map(function (label, i) {
        return { id: 'fu-svc-' + i, label: label, recommended: i < 2 };
      });
      const followStep = {
        id: followId,
