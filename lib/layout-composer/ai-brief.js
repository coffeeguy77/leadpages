...
async function generateInterviewTurn(input) {
  ...
  const system =
    ...
    'CRITICAL: chip labels MUST match THIS business’s real products/services from businessName + trade + understanding. ' +
    'If they manufacture or supply roof ventilators / ventilation products, chips must be product lines ' +
    '(e.g. metal corrugated roof ventilators, ridge vents, turbine vents, flashings) — NEVER roof repairs, re-roofing, guttering, leak detection, or skylights. ' +
    'Never reuse the same chip set across turns; each turn’s options must reflect prior free-text answers. ' +
