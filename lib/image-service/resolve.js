    ...
    for (const query of queries) {
      ...
      if (!search) {
        if (pexels.isConfigured()) {
          ...
          // Prefer curated CDN sample photos over abstract SVG mocks so Studio
          // previews look populated when PEXELS_API_KEY is not configured.
          const sampleStock = require('./providers/sample-stock');
          search = sampleStock.searchSampleStock({
            query,
            orientation: brief.orientation,
            perPage: 8,
            usedUrls: options.usedSampleUrls || []
          });
...
async function resolveImageBriefs(briefs, opts) {
  ...
  const usedSampleUrls = new Set((opts && opts.usedSampleUrls) || []);
  const out = [];
  const diagnosticsList = [];
  for (const b of briefs || []) {
    const resolved = await resolveImageBrief(b, {
      ...(opts || {}),
      usedProviderAssetIds: [...used],
      usedSampleUrls: [...usedSampleUrls]
    });
    ...
    if (resolved.selection && resolved.selection.selectedVariantUrl) {
      usedSampleUrls.add(resolved.selection.selectedVariantUrl);
    }
    out.push(resolved);
