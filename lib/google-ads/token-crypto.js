 ...
 * Key: GOOGLE_ADS_OAUTH_ENCRYPTION_KEY — any secret ≥32 characters; hashed with SHA-256 to 32 bytes.
 ...
 * Plaintext storage is refused — encryptSecret throws if the key is missing/short.
 */
const crypto = require('crypto');
const { encryptionKey, encryptionConfigured } = require('./config');
...
function encryptSecret(plain) {
  ...
  const key = keyBuf();
  if (!key) {
    throw new Error('GOOGLE_ADS_OAUTH_ENCRYPTION_KEY required (min 32 chars) — refusing plaintext token storage');
  }
