 ...
 *  3) DataForSEO via SI gateway — optional fallback only (costs money)
 ...
 * @param {object} plan
 * @param {{ measured?: object, ideas?: object[], provider?: string|null, liveMarket?: boolean, marketError?: string|null }} data
 */
function applyKeywordMetrics(plan, data) {
  ...
  if (!parts.length) {
    if (d.marketError) {
      parts.push(
        'Search volume / CPC unavailable — Keyword Planner: ' + String(d.marketError).slice(0, 160)
      );
    } else {
      parts.push(
        liveMarket
