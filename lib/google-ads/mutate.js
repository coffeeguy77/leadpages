...
async function createPausedSearchCampaign(admin, conn, plan, opts) {
  ...
  // Campaign-level negatives from plan (jobs/diy/free etc.) — advanced manager default
  const negTerms = [];
  (plan.negatives || []).forEach((cat) => {
    (cat.terms || []).forEach((t) => {
      const term = String(t || '').trim().slice(0, 80);
      if (term && negTerms.indexOf(term) < 0) negTerms.push(term);
    });
  });
  negTerms.slice(0, 40).forEach((term, i) => {
    operations.push({
