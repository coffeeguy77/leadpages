...
const { analyzePageFit, suggestRsaFromPage, keywordsFromPage } = require('./page-fit');
...
function publishedPages(site) {
  ...
  // Only explicitly published pages — never treat missing status or drafts as live.
  return pages.filter(
    (p) => p && (p.status === 'published' || p.published === true) && p.status !== 'draft'
  );
}
...
function buildRsa(site, service, geo, finalUrl, page) {
  const brand = clip((site && site.business_name) || 'Local business', 30);
  const svc = clip(service, 30);
  const g = clip(geo, 20);
  // Prefer page-grounded RSA when we have a real landing page
  if (page && (page.h1 || page.title || page.body)) {
    const fromPage = suggestRsaFromPage(
      page,
      [{ keyword: svc, approved: true }],
      g,
      brand
    );
    const rsa = normalizeRsaCopy(
      Object.assign({}, fromPage, { finalUrlHint: finalUrl })
    );
    return Object.assign({}, rsa, {
      finalUrl: finalUrl,
      provenance: { source: 'landing_page', edited: false }
    });
  }
  const raw = {
    headlines: [
...
function planForPage(site, page, opts) {
  ...
  const service =
    o.service ||
    clip(page && (page.primaryKeyword || page.h1 || page.title), 80) ||
    'Local services';
  ...
  const fromPage = keywordsFromPage(page, geo, brand);
  const baseKws = keywordIdeas(service, geo, brand);
  // Merge page-derived keywords first (better relevance), then starters
  const seen = {};
  const kws = [];
  fromPage.concat(baseKws).forEach((k) => {
    const key = String(k.keyword || '').toLowerCase();
    if (!key || seen[key]) return;
    seen[key] = true;
    kws.push(k);
  });
  const rsa = buildRsa(site, service, geo, finalUrl, page);
  const plan = {
    mode: 'page',
    campaignName: clip(
    ...
    budgetDaily: o.budgetDaily != null && o.budgetDaily !== '' ? Number(o.budgetDaily) : null,
    budgetNote:
      o.budgetDaily != null && o.budgetDaily !== ''
        ? null
        : 'Enter a daily budget before create — no invented figure.',
    conversionGoals: {
    ...
    provenance: {
      ...
      sources: ['site_config', 'page_content', 'page_fit']
    },
    warnings: domainCheck.ok ? [] : [domainCheck.message || domainCheck.error],
    metricsNote:
      'Search volume / CPC filled from Google Ads Keyword Planner (connected account) or measured Ads keyword CPC — never invented.'
  };

  const pageFit = analyzePageFit(page, {
    plan: plan,
    geo: geo,
    brand: brand
  });
  plan.pageFit = pageFit;
  if (pageFit && pageFit.score < 55) {
    plan.warnings = (plan.warnings || []).concat([
      'Landing page fit grade ' + pageFit.grade + ' (' + pageFit.score + '/100) — fix issues before spending for better relevance / lower CPC.'
    ]);
  }
  return plan;
}
