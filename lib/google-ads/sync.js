...
const { ensureConnectionTagId } = require('./tag-id');
const { purgeStaleAdsMetrics, clearCustomerMetricsInRange } = require('./metrics-scope');
...
async function syncSiteMetrics(admin, site, conn, { fromDay, toDay } = {}) {
  ...
  // Auto-fill Google tag (AW-…) from conversion tracking id when not set.
  try {
    await ensureConnectionTagId(admin, conn, access);
  } catch (_e) {
    /* non-fatal */
  }
  ...
  // Drop metrics from any previous Ads customer still attached to this site.
  // Empty new accounts must not keep showing Bean Culture / old spend.
  const purged = await purgeStaleAdsMetrics(admin, site.id, cid);

  // Upsert never deletes. If Google returned nothing for this window, clear
  // this customer's cached rows in-range (deleted campaigns / empty account).
  let clearedWindow = null;
  if (upserts.length === 0) {
    clearedWindow = await clearCustomerMetricsInRange(admin, site.id, cid, from, to);
  }
  ...
  // Drop stale unmatched rows from a previous Ads account / older destinations.
  // Keep only URLs seen in this sync window (empty set → clear all for this site).
  try {
    if (unmatched.size === 0) {
      await admin.from('ads_unmatched_urls').delete().eq('site_id', site.id);
    } else {
      const keep = Array.from(unmatched.keys());
      const { data: existing } = await admin.from('ads_unmatched_urls').select('final_url').eq('site_id', site.id);
      const stale = (existing || []).filter((r) => keep.indexOf(r.final_url) < 0).map((r) => r.final_url);
      for (let i = 0; i < stale.length; i++) {
        await admin.from('ads_unmatched_urls').delete().eq('site_id', site.id).eq('final_url', stale[i]);
      }
    }
  } catch (_e) {
    /* non-fatal */
