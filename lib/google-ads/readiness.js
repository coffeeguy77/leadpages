...
function runReadinessAudit(ctx) {
  ...
  checks.push(
    ads && (ga4 || gaId) && ads.ads_ga4_link_confirmed_at
      ? check(
          'ads_ga4_link',
          'Ads ↔ GA4 link',
          'pass',
          'Confirmed ' +
            String(ads.ads_ga4_link_confirmed_at).slice(0, 19).replace('T', ' ') +
            ' — verify in Google Ads → Tools → Linked accounts if account changes'
        )
      : ads && (ga4 || gaId)
        ? check(
            'ads_ga4_link',
            'Ads ↔ GA4 link',
            'warn',
            'Confirm in Google Ads → Tools → Linked accounts (or GA4 Admin → Google Ads links), then mark confirmed in Connection & Health. LeadPages cannot verify this automatically.'
          )
