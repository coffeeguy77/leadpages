...
function runReadinessAudit(ctx) {
  ...
  checks.push(
    plan && plan.adGroups && plan.adGroups[0] && plan.adGroups[0].finalUrl
      ? check('landing', 'Landing page URL set', 'pass', plan.adGroups[0].finalUrl)
      : check('landing', 'Landing page URL set', 'fail', 'Select a published landing page.')
  );
  ...
  checks.push(
    ads && ga4
      ? check('ads_ga4_link', 'Ads ↔ GA4 link', 'warn', 'Confirm link inside Google Ads / GA4 Admin — LeadPages cannot always verify.')
  ...
  checks.push(
    ads && ads.tag_id
      ...
      : check('google_tag', 'Google tag (AW) configured', 'warn', 'Set Ads tag ID on the Connection tab.')
  );
  ...
  checks.push(
    plan && plan.conversionGoals && plan.conversionGoals.primary && plan.conversionGoals.primary.length
      ? check('primary_conversion', 'Primary conversion selected', 'pass', plan.conversionGoals.primary.join(', '))
      : check('primary_conversion', 'Primary conversion selected', 'fail', 'Choose primary conversions (form / qualified call).')
  );
  ...
  if (planDomain && domain && planDomain !== domain) {
    ...
    checks.push(check('domain_match', 'Final URL domain matches site', planDomain || domain ? 'pass' : 'warn', planDomain || domain || '—'));
  }
...
module.exports = { runReadinessAudit, check };
