const { adsFetch, digits, ensureAccessToken } = require('./client');
...
async function ensureConversionActions(admin, conn) {
  ...
  const existing = await listConversionActions(access, conn.customer_id, conn.login_customer_id || conn.customer_id);
  const byName = {};
  ...
  if (creates.length) {
    ...
    const res = await adsFetch(`customers/${cid}/conversionActions:mutate`, {
      method: 'POST',
      accessToken: access,
      loginCustomerId: conn.login_customer_id || cid,
      body: { operations }
    });
