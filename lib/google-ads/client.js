...
async function adsFetch(path, { method, accessToken, loginCustomerId, body } = {}) {
  ...
  if (!r.ok) {
    // Never surface Google HTML error pages (sunset API versions return 404 HTML).
    let msg = (json && json.error && (json.error.message || json.error.status)) || '';
    if (!msg || /<!DOCTYPE|<html/i.test(msg) || /<!DOCTYPE|<html/i.test(text || '')) {
      msg = 'google_ads_api_http_' + r.status + (r.status === 404
        ? ' (API version may be sunset — set GOOGLE_ADS_API_VERSION, default v22)'
        : '');
    }
