...
function cleanIndustryShowcase(input) {
  ...
  const tabs = cleanArray(input.tabs, 12, function(t, i) {
    ...
    if (t.enabled === false) {
      // Keep disabled built-in tabs so partners can re-enable; drop empty custom drafts.
...
function mergeWebsiteProfilePatch(existing, patch) {
  ...
  [
    ...
    'social', 'seo', 'leadOffer', 'enquiryForm', 'visibility', 'metrics', 'industryShowcase'
  ].forEach(function(section) {
    if (patch[section]) {
      if (section === 'industryShowcase') {
