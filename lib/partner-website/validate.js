...
function slugifyTabKey(label, fallback) {
  const base = cleanStr(label, 40) || fallback || 'tab';
  return base.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 40) || 'tab';
}

function cleanIntervalMs(v, fallback) {
  const n = Number(v);
  if (!Number.isFinite(n)) return fallback;
  // 4s–60s — partners asked for slower autoplay than the old 8s default.
  return Math.min(60000, Math.max(4000, Math.round(n)));
}

function resolveIntervalMs(input, fallback) {
  input = input || {};
  if (input.intervalMs != null && input.intervalMs !== '') return cleanIntervalMs(input.intervalMs, fallback);
  if (input.intervalSec != null && input.intervalSec !== '') return cleanIntervalMs(Number(input.intervalSec) * 1000, fallback);
  return fallback;
}

function cleanIndustryShowcase(input) {
  input = input && typeof input === 'object' ? input : {};
  const seen = {};
  const tabs = cleanArray(input.tabs, 12, function(t, i) {
    if (!t || typeof t !== 'object') return null;
    if (t.enabled === false) {
      const key = cleanStr(t.key, 40) || slugifyTabKey(t.label, 'tab-' + (i + 1));
      if (!key) return null;
      return {
        key: key,
        label: cleanStr(t.label, 40) || key,
        siteId: cleanStr(t.siteId, 40),
        slug: cleanStr(t.slug, 80),
        enabled: false,
        sortOrder: Number.isFinite(Number(t.sortOrder)) ? Number(t.sortOrder) : i
      };
    }
    let key = cleanStr(t.key, 40) || slugifyTabKey(t.label, 'tab-' + (i + 1));
    if (seen[key]) key = key + '-' + (i + 1);
    seen[key] = true;
    return {
      key: key,
      label: cleanStr(t.label, 40) || key,
  ...
  tabs.sort(function(a, b) { return (a.sortOrder || 0) - (b.sortOrder || 0); });
  return {
    hideTabs: !!input.hideTabs,
    intervalMs: resolveIntervalMs(input, 12000),
    tabs: tabs
  };
}

function cleanHeroShowcase(input) {
  input = input && typeof input === 'object' ? input : {};
  const siteIds = [];
  const seen = {};
  function pushId(id) {
    const s = cleanStr(id, 40);
    if (!s || seen[s]) return;
    seen[s] = true;
    siteIds.push(s);
  }
  if (Array.isArray(input.siteIds)) {
    input.siteIds.forEach(pushId);
  }
  pushId(input.siteId);
  pushId(input.featuredSiteId);
  return {
    siteId: siteIds[0] || null,
    siteIds: siteIds.slice(0, 8),
    hideTabs: !!input.hideTabs,
    intervalMs: resolveIntervalMs(input, 12000)
  };
}
...
function validateWebsiteProfile(input) {
  ...
  return {
    identity: {
      ...
      logoUrl: cleanUrl(identity.logoUrl),
      badgeStatus: cleanStr(identity.badgeStatus, 40) || 'leadpages-partner',
      displaySurnamePublicly: identity.displaySurnamePublicly === true
    },
    contact: {
    ...
    industryShowcase: cleanIndustryShowcase(input.industryShowcase),
    heroShowcase: cleanHeroShowcase(input.heroShowcase),
    metrics: {
...
function mergeWebsiteProfilePatch(existing, patch) {
  existing = validateWebsiteProfile(existing || {});
  patch = patch && typeof patch === 'object' ? patch : {};
  const merged = Object.assign({}, existing);

  [
    'identity', 'contact', 'location', 'biography', 'experience', 'positioning',
    'social', 'seo', 'leadOffer', 'enquiryForm', 'visibility', 'metrics',
    'industryShowcase', 'heroShowcase'
  ].forEach(function(section) {
    if (patch[section]) {
      if (section === 'industryShowcase' || section === 'heroShowcase') {
        merged[section] = patch[section];
      } else {
        merged[section] = mergePlainObjects(existing[section], patch[section]);
      }
    }
  });

  [
    'serviceSelections', 'testimonials', 'caseStudies', 'partnerFaqs'
  ].forEach(function(listKey) {
    if (patch[listKey] !== undefined) merged[listKey] = patch[listKey];
  });

  return validateWebsiteProfile(merged);
}
