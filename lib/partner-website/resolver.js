...
function resolvePartnerThemeContent(input) {
  ...
  const showcaseTabs = (wp.industryShowcase && wp.industryShowcase.tabs) || [];
  const configuredIds = {};
  const configuredSlugs = {};
  showcaseTabs.forEach(function(t) {
    if (!t || t.enabled === false) return;
    if (t.siteId) configuredIds[String(t.siteId)] = true;
    if (t.slug) configuredSlugs[String(t.slug)] = true;
  });
  const hs = wp.heroShowcase || {};
  (hs.siteIds || []).concat(hs.siteId ? [hs.siteId] : []).forEach(function(id) {
    if (id) configuredIds[String(id)] = true;
  });
  const demosForCards = (demos || []).map(function(d) {
    if (!d) return d;
    const forced = !!(d.id && configuredIds[String(d.id)]) || !!(d.slug && configuredSlugs[String(d.slug)]);
    return forced ? Object.assign({}, d, { forceInclude: true }) : d;
  });
  const demoCards = normaliseDemos(demosForCards, base);
  ...
  const rawShortIntro = wp.biography.shortIntro || '';
  const shortIntroFallback = rawShortIntro || (publicIntro.contactLine ? '' : heroSupporting.slice(0, 240));
  const biography = {
    shortIntro: publicPartnerBio(shortIntroFallback, publicIntro, {
      agencyName: agencyName,
  ...
  const content = {
    ...
    services: services,
    demos: demoCards,
    industryShowcase: wp.industryShowcase || { tabs: [], hideTabs: false, intervalMs: 12000 },
    heroShowcase: wp.heroShowcase || { siteId: null, siteIds: [], hideTabs: false, intervalMs: 12000 },
    ...
    meta: {
      ...
      templateKey: cfg.templateKey || 'webculture',
