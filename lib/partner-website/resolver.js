...
const { validateWebsiteProfile } = require('./validate');
...
function resolveShowcaseLogo(cfg) {
  if (!cfg || !cfg.logo) return null;
  if (typeof cfg.logo === 'string') {
    const trimmed = cfg.logo.trim();
    return trimmed || null;
  }
  if (typeof cfg.logo === 'object' && cfg.logo.imageUrl) {
    const trimmed = String(cfg.logo.imageUrl).trim();
    return trimmed || null;
  }
  return null;
}
...
function resolvePartnerThemeContent(input) {
  ...
  const heroSupporting = positioning.heroSupporting || cfg.intro || defaultPos.heroSupporting;
  ...
  const content = {
    partner: {
      ...
      logoUrl: resolveShowcaseLogo(cfg),
    ...
    trust: [
      { label: 'Local partner', value: displayName },
