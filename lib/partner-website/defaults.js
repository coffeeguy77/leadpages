...
function primaryRegion(location, prof, directory) {
  if (location && location.serviceRegionHeadline) return location.serviceRegionHeadline;
...
function buildDefaultPositioning(vars) {
  return {
    ...
    heroSupporting: applyTemplate(
      'Work directly with {{partnerDisplayName}}, your local website partner in {{primaryRegion}}. Get a professionally built website, live demos you can explore, and ongoing support from someone who understands your business.',
      vars
    ),
...
function buildDefaultBiography(vars, experience) {
  ...
  return applyTemplate(
    '{{partnerDisplayName}} works directly with businesses across {{serviceRegion}} to plan, launch, and improve professional websites.' + expPart + ' {{firstName}} helps business owners turn their services into clear, credible websites designed to generate enquiries.',
    vars
  );
}
