...
function primaryRegion(location, prof, directory) {
  ...
  const headline = String((location && location.serviceRegionHeadline) || '').trim();
  const headlineMatch = headline.match(/Serving\s+([^,\.]+?)(?:\s+and\b|,|\.|$)/i);
  if (headlineMatch && headlineMatch[1]) {
    return headlineMatch[1].trim();
  }
  const cityPattern = /canberra|sydney|melbourne|brisbane|perth|adelaide|hobart|darwin|gold coast|newcastle|wollongong|geelong|sunshine coast/i;
  if (areas.length) {
    const city = areas.find(function(a) { return a && a.label && cityPattern.test(a.label); });
    if (city) return city.label;
  }
  if (location && location.state && /^ACT$/i.test(String(location.state).trim())) {
    return 'Canberra';
  }
  if (areas.length) {
