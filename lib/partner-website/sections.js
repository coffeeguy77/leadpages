...
function biographySection(content) {
  ...
  return '<section class="pt-biography" id="partner"><div class="pt-wrap pt-bio-grid">'
    ...
    + (p.headshotUrl ? '<img src="' + esc(p.headshotUrl) + '" alt="' + esc(p.displayName) + '" class="pt-bio-headshot">' : '<div class="pt-bio-placeholder" aria-hidden="true"></div>')
    + '</div>'
    + '<div class="pt-bio-copy">'
    + '<p class="pt-eyebrow">Meet your local website partner</p>'
    + '<h2 class="pt-section-title">' + esc(p.displayName) + '</h2>'
    ...
    + (c.email ? '<a class="pt-btn pt-btn-outline" href="mailto:' + esc(c.email) + '">Email ' + esc(p.firstName || p.displayName) + '</a>' : '')
...
function platformBackingSection(content) {
  ...
  return '<section class="pt-support" id="support"><div class="pt-wrap">'
    ...
    + '<h3>' + esc(p.displayName) + ' — your direct contact</h3>'
    + '<p>' + esc(pb.personalStatement) + ' ' + esc(pb.localStatement) + '</p>'
    + (c.email ? '<a class="pt-btn pt-btn-outline" href="mailto:' + esc(c.email) + '">Email ' + esc(p.displayName) + '</a>' : '')
    + (c.phone ? '<a class="pt-btn pt-btn-outline" href="tel:' + c.phone.replace(/[^+0-9]/g, '') + '">' + esc(c.phone) + '</a>' : '')
    + '</article>'
...
function contactSection(content, opts) {
  ...
  return '<section class="pt-contact" id="contact"><div class="pt-wrap pt-contact-grid">'
    + '<div class="pt-contact-info">'
    + '<p class="pt-eyebrow">Get in touch</p>'
    + '<h2 class="pt-section-title">Work with ' + esc(p.displayName) + '</h2>'
    ...
    + '<p class="pt-lp-disclosure"><span class="leadpages-logo" data-lp-logo="auto" data-lp-logo-ink="auto" role="img" aria-label="LeadPages"></span> Powered by LeadPages — your website runs on established platform infrastructure with local partner support.</p>'
    + '</div>'
