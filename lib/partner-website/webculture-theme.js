...
const WEBCULTURE_PROCESS = [
  { step: 1, title: 'Talk', description: 'Understand your services, customers and what a strong enquiry looks like.' },
  { step: 2, title: 'Plan', description: 'Shape structure, messaging and the customer journey before anything is built.' },
  { step: 3, title: 'Build', description: 'Create the website on LeadPages and connect forms, tools and lead capture.' },
  { step: 4, title: 'Review', description: 'Walk through the site live, refine details and confirm you are ready to launch.' },
  { step: 5, title: 'Launch', description: 'Go live with hosting in place and ongoing local support behind the platform.' }
];
...
const INDUSTRY_TABS = [
  ...
  { key: 'professional', label: 'Professional', match: /professional|legal|account|consult|medical|dental/i },
...
const LEAD_FLOW_STEPS = [
  ...
  { key: 'quote', label: 'Quote', icon: 'form' },
  { key: 'crm', label: 'CRM', icon: 'crm' },
  { key: 'customer', label: 'Customer', icon: 'check' }
];
...
function buildWebcultureCopy(content) {
  ...
  return {
    ...
    connected: {
      ...
      proofCards: [
        { icon: 'pin', title: 'Local partner support', body: regionLabel + '-based planning and personal guidance.' },
    ...
    demos: {
      ...
      features: [
        { icon: 'target', label: 'Built to convert' },
        { icon: 'device', label: 'Looks great on all devices' },
        { icon: 'edit', label: 'Easy to update' }
      ],
    ...
    platform: {
      ...
      agencyItems: ['Strategy', 'Content', 'Planning', 'Support'],
      platformItems: ['Hosting', 'CRM', 'Backups', 'Platform', 'Security']
    },
    testimonials: {
