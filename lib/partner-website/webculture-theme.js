...
function findDemoForTab(demos, tab) {
  if (!tab || !demos || !demos.length) return null;
  if (tab.siteId) {
    const byId = demos.find(function(d) { return d && d.id && String(d.id) === String(tab.siteId); });
    if (byId) return byId;
  }
  if (tab.slug) {
    const bySlug = demos.find(function(d) { return d && d.slug && String(d.slug) === String(tab.slug); });
