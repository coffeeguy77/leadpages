...
function findDemoByIdOrSlug(demos, siteId, slug) {
  const list = demos || [];
  if (siteId) {
    const byId = list.find(function(d) { return d && d.id && String(d.id) === String(siteId); });
    if (byId) return byId;
  }
  if (slug) {
    return list.find(function(d) { return d && d.slug && String(d.slug) === String(slug); }) || null;
  }
  return null;
}

function findDemoForTab(demos, tab) {
  if (!tab) return null;
  return findDemoByIdOrSlug(demos, tab.siteId, tab.slug);
}

/**
 * Build industry scroller groups.
 * When industryShowcase.tabs is configured (with site picks / custom labels),
 * those tabs win. Otherwise demos are auto-grouped by config.trade regex.
 */
function groupDemosByIndustryTab(demos, industryShowcase) {
  const list = demos || [];
  const configured = industryShowcase && Array.isArray(industryShowcase.tabs)
    ? industryShowcase.tabs.slice()
    : [];
  const configuredActive = configured.filter(function(t) {
    return t && t.enabled !== false && (t.siteId || t.slug);
  });

  if (configuredActive.length) {
    const ordered = [];
    const used = {};
    configured
      .slice()
      ...
      .forEach(function(t) {
        ...
        const demo = findDemoForTab(list, t);
        if (!demo) return;
        const key = String(t.key || 'tab');
        if (used[key]) return;
        used[key] = true;
        ordered.push({
          tab: { key: key, label: t.label || demo.industry || demo.name || key },
          demos: [demo]
        });
      });
  ...
  if (!ordered.length && list.length) {
    ordered.push({ tab: INDUSTRY_TABS[INDUSTRY_TABS.length - 1], demos: list.slice() });
  }
  return ordered;
}

function pickHeroDemos(demos, limit, heroShowcase) {
  const max = limit || 5;
  const list = demos || [];
  const hs = heroShowcase || {};
  const configuredIds = Array.isArray(hs.siteIds) && hs.siteIds.length
    ? hs.siteIds
    : (hs.siteId ? [hs.siteId] : []);
  ...
  if (configuredIds.length) {
    ...
    configuredIds.forEach(function(id) {
      const demo = findDemoByIdOrSlug(list, id, null);
      if (!demo) return;
      const key = demo.id || demo.slug || demo.url || demo.name;
...
function pickHeroDemo(demos, heroShowcase) {
  const list = pickHeroDemos(demos, 1, heroShowcase);
  return list[0] || null;
}
