...
function pushSlot(out, w, dateStr, isOverride) {
  ...
  out.push({
    ...
    window_id: w.id && w.id !== 'default' ? w.id : null,
    date: dateStr,
    window_start: startT,
    window_end: endT,
    ...
    is_default_hours: !isOverride && (!w.id || w.id === 'default')
  });
}

/**
 * Build bookable pickup slots for the booking range.
 * Priority per day: date override → weekday window row → schedule default hours.
 */
