...
async function sendDepositLink(opts) {
  ...
  const url = PUBLIC_BASE + '/order-portal?t=' + encodeURIComponent(tok.token);

  await notifyEvent({
    event_type: opts.event_type || 'deposit_required',
    template_category: opts.template_category,
    system: opts.system,
    site: opts.site,
    order: order,
    portal_link: url,
