...
async function buildVars(ctx) {
  ...
  const first = String(order.customer_name || cart.guest_name || 'there')
    .trim()
    .split(/\s+/)[0];
  return {
    first_name: first,
...
function defaultSubject(event, vars) {
  if (event === 'abandoned_cart') return 'Your order is still waiting — ' + (vars.business_name || '');
