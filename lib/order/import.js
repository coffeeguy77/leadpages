...
function rememberCustomer(cache, customer) {
  ...
  var e164 = customer.phone_e164 || normaliseAuPhone(customer.phone);
  if (e164) cache.byPhone[e164] = customer;
...
async function upsertCustomer(admin, system, site, mapped, cache) {
  ...
  if (e164) {
    ...
    if (existing) {
      ...
      // Always backfill e164 so portal OTP / history match spaced display phones.
      if (e164 && existing.phone_e164 !== e164) patch.phone_e164 = e164;
...
async function commitOrderHistory(opts) {
  ...
  for (var g = 0; g < slice.length; g++) {
    ...
    try {
      ...
      if (externalNumber) {
        // Skip if already imported — do not duplicate; do not append lines onto existing.
        var { data: byExternal } = await admin
          .from('order_orders')
          .select('id')
          .eq('order_system_id', system.id)
        ...
        var { data: byOrderNumber } = await admin
          .from('order_orders')
          .select('id')
          .eq('order_system_id', system.id)
          .eq('order_number', externalNumber)
          .maybeSingle();
        if (byOrderNumber) {
          stats.orders_skipped += 1;
          processed += 1;
      ...
      for (var li = 0; li < group.lines.length; li++) {
        ...
        var weightKg = parseSizeWeightKg(line.size_weight);
        var noteParts = [line.size_weight, line.line_notes].filter(Boolean);
        var priced = priceLineAtOrder(product, qty, weightKg);
        itemRows.push({
          site_id: site.id,
