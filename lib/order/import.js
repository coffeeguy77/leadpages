...
async function upsertCustomer(admin, system, site, mapped) {
...
async function upsertProduct(admin, system, site, mapped, createMissing) {
  ...
  var { data: existingList } = await admin
    ...
    .ilike('name', name)
    .limit(1);
  var existing = (existingList && existingList[0]) || null;
  if (!existing) {
...
async function commitCustomers(opts) {
  ...
  for (var i = start; i < rows.length; i++) {
    try {
      ...
      var res = await upsertCustomer(admin, system, site, mapped);
...
async function commitProducts(opts) {
  ...
  for (var i = start; i < rows.length; i++) {
    try {
      ...
      var res = await upsertProduct(admin, system, site, mapped, true);
      if (res.skipped) stats.skipped += 1;
      else if (res.created) stats.created += 1;
      else stats.updated += 1;
    } catch (e) {
...
async function commitOrderHistory(opts) {
  ...
  // Group line items by order number + phone
  var groups = {};
  var stats = {
    orders_created: 0,
    orders_skipped: 0,
  ...
  for (var i = start; i < rows.length; i++) {
    ...
    var e164 = normaliseAuPhone(mapped.phone);
    var key = (onum || ('row-' + i)) + '|' + (e164 || fullName(mapped) || 'anon');
    if (!groups[key]) groups[key] = { mappedMeta: mapped, lines: [], rowIndexes: [] };
    groups[key].lines.push(mapped);
    groups[key].rowIndexes.push(i + 1);
    // Prefer first non-empty meta
    ['first_name', 'surname', 'name', 'phone', 'email', 'pickup_date', 'order_date', 'deposit', 'order_number'].forEach(function (k) {
      if (mapped[k] && !groups[key].mappedMeta[k]) groups[key].mappedMeta[k] = mapped[k];
  ...
  for (var g = 0; g < keys.length; g++) {
    ...
    try {
      var custRes = await upsertCustomer(admin, system, site, meta);
      if (custRes.skipped) {
      ...
      var orderInsert = {
        ...
        balance_cents: null,
        internal_notes: 'Imported order history',
        created_at: createdDay ? createdDay + 'T12:00:00.000Z' : new Date().toISOString(),
        confirmed_at: createdDay ? createdDay + 'T12:00:00.000Z' : new Date().toISOString(),
        collected_at: pickup ? pickup + 'T12:00:00.000Z' : null,
        updated_at: new Date().toISOString()
      };
      ...
      stats.orders_skipped += 1;
      if (stats.errors.length > 80) break;
    }
  }
...
async function commitImport(opts) {
  ...
  var stats;
  if (kind === 'customers') stats = await commitCustomers(payload);
  else if (kind === 'products') stats = await commitProducts(payload);
  else stats = await commitOrderHistory(payload);
