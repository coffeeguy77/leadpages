...
const { aggregateSupply } = require('./supply');
...
async function createStaffOrder(opts) {
  ...
  const itemRows = linesIn.map(function (line, idx) {
    ...
    return {
      ...
      requested_weight_kg: line.requested_weight_kg != null ? line.requested_weight_kg : null,
      ...
      product_snapshot: product
        ...
        : { custom: true, name: snapProduct.name },
      sort_order: idx
    };
  });
...
async function supplyForDate(systemId, siteId, pickupDate) {
  ...
  const { data: orders, error } = await admin
    .from('order_orders')
    .select('id, status, order_number, pickup_date')
  ...
  if (!ids.length) return { order_count: 0, lines: [] };
