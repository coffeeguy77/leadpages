...
const { editingStateFor, resolveChangeDeadline } = require('./cutoff');
...
/** Recalculate totals; never increase deposit after customer has paid the required deposit. */
async function recalculateOrderPreservingDeposit(orderId) {
  const admin = getAdmin();
  const { data: orderBefore } = await admin.from('order_orders').select('*').eq('id', orderId).single();
  const result = await recalculateOrder(orderId);
  const paid = Number(orderBefore.deposit_paid_cents) || 0;
  const req = Number(orderBefore.deposit_required_cents) || 0;
  if (req > 0 && paid >= req) {
    return result;
  }
  if (!orderBefore.payment_rule_snapshot) return result;
  const totals = {
    known_subtotal_cents: result.order.known_subtotal_cents,
    has_unknown_prices: result.order.has_unknown_prices
  };
  const dep = computeDepositRequired(orderBefore.payment_rule_snapshot, totals);
  if (dep.deposit_required_cents === req) return result;
  const { data: updated, error } = await admin
    .from('order_orders')
    .update({
      deposit_required_cents: dep.deposit_required_cents,
      updated_at: new Date().toISOString()
    })
    .eq('id', orderId)
    .select('*')
  ...
  const balance = balanceRemaining(
    Object.assign({}, updated, {
      final_subtotal_cents: updated.final_subtotal_cents,
      known_subtotal_cents: updated.known_subtotal_cents
    })
  );
  if (balance !== updated.balance_cents) {
    const { data: balOrder } = await admin
      .from('order_orders')
      .update({ balance_cents: balance, updated_at: new Date().toISOString() })
      .eq('id', orderId)
      .select('*')
      .single();
    return { order: balOrder, items: result.items };
  }
  return { order: updated, items: result.items };
}
...
async function createStaffOrder(opts) {
  ...
  let schedule = {};
  try {
    const { parsePickupSchedule } = require('./pickup-schedule');
    schedule = parsePickupSchedule(system) || {};
  } catch (_e) {
    schedule = (system.settings && system.settings.pickup_schedule) || {};
  }
  const cutoff = resolveChangeDeadline(
    (products || []).length
      ? products
      : linesIn.map(function () {
...
module.exports = {
  slugify,
  recalculateOrder,
  recalculateOrderPreservingDeposit,
  createStaffOrder,
  lockOrdersForDate,
  finaliseItemPrice,
  supplyForDate,
