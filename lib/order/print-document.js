...
function printCss(format) {
  ...
  return (
    ...
    '.day-head{margin:18px 0 8px;padding:8px 10px;background:#1f5c3a;color:#fff;border-radius:6px;font-size:14px;font-weight:800}' +
...
function productLineDetail(item) {
  var parts = [];
  ...
  if (isTbcItem(item) && item.price_status !== 'final') parts.push('TBC');
  return parts.join(' · ');
}

function renderProductSheet(groups, business, meta) {
  meta = meta || {};
  var biz = (business && business.business_name) || 'Orders';
  var query = meta.product_query || '';
  var modeLabel = meta.product_mode === 'exact' ? 'exact match' : 'contains';
  var html = '<div class="head"><p class="biz">' + esc(biz) + '</p>';
  html += '<h1 class="title">Product sheet';
  if (query) html += ' — ' + esc(query);
  html += '</h1>';
  html +=
    '<p class="sub">' +
    esc(meta.match_count || 0) +
    ' line' +
    (meta.match_count === 1 ? '' : 's') +
    ' · ' +
    esc(meta.order_count || 0) +
    ' order' +
    (meta.order_count === 1 ? '' : 's') +
    (query ? ' · ' + esc(modeLabel) : '') +
    '</p></div>';

  if (!(groups || []).length) {
    html +=
      '<p class="sub">No matching products' +
      (query ? ' for “' + esc(query) + '”' : '') +
      (meta.date_label ? ' on ' + esc(meta.date_label) : '') +
      '.</p>';
    return html;
  }

  groups.forEach(function (group, gIdx) {
    if (group.date && (groups.length > 1 || meta.show_date_heading)) {
      html += '<h2 class="day-head">' + esc(formatDateLabel(group.date)) + '</h2>';
    }
    html += '<div class="pick-list"><table><thead><tr>';
    html += '<th>Order</th><th>Customer</th><th>Phone</th><th>Product</th><th>Qty / weight</th><th>Options</th>';
    html += '</tr></thead><tbody>';
    (group.rows || []).forEach(function (row) {
      var ord = row.order || {};
      var it = row.item || {};
      var optTxt = itemOptionsText(it);
      html += '<tr>';
      html += '<td><strong>' + esc(ord.order_number || '—') + '</strong></td>';
      html += '<td>' + esc(ord.customer_name || '—') + '</td>';
      html += '<td>' + esc(ord.customer_phone || ord.customer_email || '—') + '</td>';
      html += '<td><strong>' + esc(it.product_name || '—') + '</strong>';
      if (it.notes) html += '<br><span style="font-size:11px;color:#666">' + esc(it.notes) + '</span>';
      html += '</td>';
      html += '<td>' + esc(productLineDetail(it) || '—') + '</td>';
      html += '<td>' + esc(optTxt || '—') + '</td>';
      html += '</tr>';
    });
    html += '</tbody></table></div>';
    if (gIdx < groups.length - 1) html += '<div style="height:12px"></div>';
  });
  return html;
}
...
function buildPrintDocument(payload) {
  ...
  } else if (format === 'product_sheet') {
    body = renderProductSheet(payload.product_groups || [], payload.business, payload.meta || {});
