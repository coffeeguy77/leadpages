...
var { displayFullName } = require('./customer-name');
...
/**
 * Parse butcher size/weight notes into kg for reorder.
 * Examples: "3.5 kg", "800g", "2 - 3 kg", "2.5kg - 3kg"
 * Ranges use the midpoint so customers can bump up/down from last year.
 */
function parseSizeWeightKg(raw) {
  var s = String(raw || '')
    .trim()
    .toLowerCase()
    .replace(/,/g, '');
  ...
  var range = s.match(
    /([\d.]+)\s*(?:kg|kgs)?\s*(?:-|–|—|to)\s*([\d.]+)\s*(?:kg|kgs)?/
  );
  if (range) {
    var a = parseFloat(range[1]);
    var b = parseFloat(range[2]);
    if (isFinite(a) && isFinite(b) && a > 0 && b > 0) {
      // If both look like grams (e.g. 800 - 1000), convert.
      if (!/kg/.test(s) && a >= 100 && b >= 100) {
        a = a / 1000;
        b = b / 1000;
      }
  ...
  var kg = s.match(/([\d.]+)\s*kgs?\b/);
  if (kg) {
    var kgn = parseFloat(kg[1]);
    return isFinite(kgn) && kgn > 0 ? Math.round(kgn * 1000) / 1000 : null;
  }

  var g = s.match(/([\d.]+)\s*g\b/);
  if (g) {
    var gn = parseFloat(g[1]);
    return isFinite(gn) && gn > 0 ? Math.round((gn / 1000) * 1000) / 1000 : null;
  }

  var bare = s.match(/^([\d.]+)$/);
  if (bare) {
    var n = parseFloat(bare[1]);
    if (!isFinite(n) || n <= 0) return null;
    // Bare numbers: treat as kg when small, grams when large (>= 100).
    if (n >= 100) return Math.round((n / 1000) * 1000) / 1000;
...
function fullName(mapped) {
  var raw = mapped.name
    ...
    : [mapped.first_name, mapped.surname].filter(Boolean).join(' ').trim();
...
function groupOrderHistoryRows(rows, mapping, hasHeader, phoneNormaliser) {
  ...
  for (var i = start; i < rows.length; i++) {
    ...
    // Same CSV order number = one order (multi product lines), even if phone formatting differs.
    // Rows without an order number fall back to phone + name so they do not collapse together.
    var key = onum
      ? 'ord:' + onum
      : 'ph:' + (e164 || fullName(mapped) || 'row-' + i);
    if (!groups[key]) {
