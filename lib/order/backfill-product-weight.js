...
async function backfillProductWeightSettings(opts) {
  ...
  for (var i = 0; i < (products || []).length; i++) {
    ...
    if (isPoultryWholeBirdProduct(p)) {
      ...
      var prevPrompt =
        p.options && p.options.quantity_prompt != null && String(p.options.quantity_prompt).trim() !== ''
          ? String(p.options.quantity_prompt).trim()
          : null;
      var nextPrompt =
        copy.quantity_prompt != null && String(copy.quantity_prompt).trim() !== ''
          ? String(copy.quantity_prompt).trim()
          : null;
      optsObj.size_mode = 'each';
      delete optsObj.pack_weight_kg;
      delete optsObj.pack_label;
      delete optsObj.minimum_kg;
      if (nextPrompt) optsObj.quantity_prompt = nextPrompt;
      else delete optsObj.quantity_prompt;
      if (
        p.short_description !== copy.short_description ||
        optsObj.size_mode !== (p.options && p.options.size_mode) ||
