...
function formatOptionsForPortal(item, answerRows) {
  ...
  if (Array.isArray(sel)) {
    sel.forEach(function (o) {
      ...
      if (o.key) seenKeys[String(o.key)] = true;
      var bit = o.question
        ? String(o.question).trim() + ': ' + String(o.label || o.value || '').trim()
  ...
  (answerRows || [])
    ...
    .forEach(function (a) {
      if (a.question_key && seenKeys[String(a.question_key)]) return;
      var v = a.value;
      if (v && typeof v === 'object' && v.label) addLabel(a.question_label + ': ' + v.label);
      else if (v != null && v !== '') addLabel(a.question_label + ': ' + String(v));
    });
  return labels;
}
