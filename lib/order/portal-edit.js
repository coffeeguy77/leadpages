...
function formatOptionsForPortal(item, answerRows) {
  ...
  if (Array.isArray(sel)) {
    sel.forEach(function (o) {
      ...
      if (o.key) seenKeys[norm(o.key)] = true;
      if (o.question) seenQuestions[norm(o.question)] = true;
      var labelVal = String(o.label || o.value || '').trim();
      if (labelVal) seenLabels[norm(labelVal)] = true;
      var bit = o.question ? String(o.question).trim() + ': ' + labelVal : labelVal;
      addLabel(bit);
    });
  }
  ...
  (answerRows || [])
    ...
    .forEach(function (a) {
      if (a.question_key && seenKeys[norm(a.question_key)]) return;
      if (a.question_label && seenQuestions[norm(a.question_label)]) return;
      var v = a.value;
      var valStr =
        v && typeof v === 'object' && v.label != null
          ? String(v.label)
          : v != null
            ? String(v)
            : '';
      if (valStr && seenLabels[norm(valStr)]) return;
      if (v && typeof v === 'object' && v.label) addLabel(a.question_label + ': ' + v.label);
      else if (v != null && v !== '') addLabel(a.question_label + ': ' + valStr);
    });
  return labels;
}
