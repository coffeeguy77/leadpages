...
function twilioOtpConfigured() {
  ...
  if (process.env.TWILIO_FROM_NUMBER || process.env.TWILIO_SMS_FROM) return true;
  return false;
}
...
async function queueAndSend(opts) {
  const admin = getAdmin();
  const { recordSmsUsage, estimateSegments } = require('./sms-usage');
  ...
  if (channel === 'sms' && opts.order_system_id && opts.site_id) {
    await recordSmsUsage({
      ...
      order_id: opts.order_id || null,
      message_id: msg.id,
      kind: opts.sms_kind || opts.event_type || 'transactional',
      destination: opts.destination,
