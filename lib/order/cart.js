...
async function syncCartTotals(cartId) {
  ...
  var nextStatus = packed.cart.status;
  // Do not revive carts that already received an abandoned-cart reminder —
  // that was causing repeat SMS every idle window.
  var reminded =
    packed.cart.recovery_state &&
    (packed.cart.recovery_state.reminder_sent || Number(packed.cart.recovery_state.stage) >= 1);
  if (packed.cart.status === 'abandoned' && !reminded) {
    nextStatus = 'active';
  }
