...
async function addOrUpdateItem(cart, product, opts) {
  ...
  if (opts.cart_item_id) {
    const priced = priceLineAtOrder(product, qtyAdd, weight);
    const { data, error } = await admin
      .from('order_cart_items')
      .update({
        quantity: qtyAdd,
  ...
  const existing = (packed && packed.items ? packed.items : []).find(function (it) {
    ...
    if (!sameWeight(it.requested_weight_kg, weight)) return false;
    if (String(it.notes || '') !== String(notes || '')) return false;
    return answersKey(it.answers) === answersKey(answers);
  });

  if (existing) {
    const newQty = Number(existing.quantity || 0) + qtyAdd;
    const priced = priceLineAtOrder(product, newQty, weight);
    const { data, error } = await admin
      .from('order_cart_items')
      .update({
        quantity: newQty,
