...
function buildProductSnapshot(product, extras, gstSettings) {
  ...
  return {
    ...
    includes_gst: productHasGst(product, gstSettings || { enabled: false, category_ids: [] })
  };
}
...
async function createCart(system, siteId, guest) {
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      site_id: siteId,
      guest_name: displayFullName((guest && guest.name) || '') || (guest && guest.name) || null,
...
async function addOrUpdateItem(cart, product, opts) {
  ...
  const weightIn = opts.requested_weight_kg != null ? Number(opts.requested_weight_kg) : null;
  if (!isPackSize(product) && !isEachSize(product)) {
    const minW = minimumKg(product);
    if (minW != null && weightIn != null && weightIn > 0 && weightIn + 0.0001 < minW) {
      throw Object.assign(new Error('weight_below_minimum'), {
        code: 400,
        minimum_kg: minW
      });
    }
  ...
  const gstSettings = opts.gst_settings || parseGstSettings(opts.system);
  const snapshot = buildProductSnapshot(product, {}, gstSettings);
  ...
  if (opts.cart_item_id) {
    ...
    const { data, error } = await admin
      ...
      .single();
    if (error) throw error;
    const items = (opts.existing_items || []).map(function (it) {
      return it.id === opts.cart_item_id ? data : it;
    });
    const updatedCart = await syncCartTotals(cart.id, { cart: cart, items: items });
    return { cart: updatedCart, items: items, item: data };
  }
  ...
  const packedItems = opts.existing_items;
  const existingList =
    packedItems != null
      ? packedItems
      : (await getCart(cart.id)).items;
  const existing = (existingList || []).find(function (it) {
  ...
  if (existing) {
    ...
    const { data, error } = await admin
      ...
      .single();
    if (error) throw error;
    const mergedItems = (existingList || []).map(function (it) {
      return it.id === existing.id ? data : it;
    });
    const updatedCart = await syncCartTotals(cart.id, { cart: cart, items: mergedItems });
    return { cart: updatedCart, items: mergedItems, item: data };
  }
  ...
  const { data, error } = await admin
    ...
    .single();
  if (error) throw error;
  const nextItems = (existingList || []).concat([data]);
  const updatedCart = await syncCartTotals(cart.id, { cart: cart, items: nextItems });
  return { cart: updatedCart, items: nextItems, item: data };
}

async function removeItem(cartId, itemId, opts) {
  opts = opts || {};
  const admin = getAdmin();
  await admin.from('order_cart_items').delete().eq('id', itemId).eq('cart_id', cartId);
  if (opts.packed && opts.packed.cart) {
    const items = (opts.packed.items || []).filter(function (it) {
      return it.id !== itemId;
    });
    const cart = await syncCartTotals(cartId, { cart: opts.packed.cart, items: items });
    return { cart: cart, items: items };
  }
 ...
 */
async function addReorderLines(cart, lines, gstSettings) {
  const admin = getAdmin();
  const gst = gstSettings || { enabled: false, category_ids: [] };
  const rows = [];
  for (var i = 0; i < (lines || []).length; i++) {
    ...
    var snapshot = buildProductSnapshot(product, extras, gst);
    snapshot.reordered = true;
    rows.push({
      cart_id: cart.id,
      site_id: cart.site_id,
      product_id: product.id,
...
async function syncCartTotals(cartId, opts) {
  opts = opts || {};
  let items = opts.items;
  let cartMeta = opts.cart;
  if (!items) {
    const packed = await getCart(cartId);
    if (!packed) return null;
    items = packed.items;
    cartMeta = packed.cart;
  }
  const totals = recalcCartTotals(items);
  var nextStatus = cartMeta.status;
  // Do not revive carts that already received an abandoned-cart reminder —
  // that was causing repeat SMS every idle window.
  var reminded =
    cartMeta.recovery_state &&
    (cartMeta.recovery_state.reminder_sent || Number(cartMeta.recovery_state.stage) >= 1);
  if (cartMeta.status === 'abandoned' && !reminded) {
    nextStatus = 'active';
  }
...
async function convertCartToOrder(opts) {
  ...
  const body = {
    customer_name: (function () {
      const { displayFullName } = require('./customer-name');
      const raw = opts.customer_name || cart.guest_name;
      return displayFullName(raw) || raw;
    })(),
