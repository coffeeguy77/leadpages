...
function buildPoultryWholeBirdCopy(product) {
  ...
  if (!range) {
    return {
      short_description: 'Sold per bird — choose quantity. We will match the closest size at pickup.',
      quantity_prompt: poultryQuantityPrompt(kind)
    };
  }
  var sizeLabel =
  ...
  return {
    short_description:
      ...
      ' per ' +
      birdWord +
      '). Choose quantity — we pick the closest bird at pickup.',
    quantity_prompt: poultryQuantityPrompt(kind)
  };
}
