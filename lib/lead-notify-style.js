...
const DEFAULT_STYLE = {
  ...
  // Empty = no tint. Cloudinary PNGs only (not GIFs / non-Cloudinary URLs).
  logoTint: '',
  showLogo: true,
...
function normalizeStyle(input) {
  ...
  STYLE_KEYS.forEach(function (k) {
    if (k === 'logoTint') {
      if (!Object.prototype.hasOwnProperty.call(src, 'logoTint')) return;
      if (src.logoTint == null || src.logoTint === '') {
        out.logoTint = '';
        return;
      }
      const tint = hexOk(src.logoTint);
      out.logoTint = tint && tint.charAt(0) === '#' ? tint : '';
      return;
    }
