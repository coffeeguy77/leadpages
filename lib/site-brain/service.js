...
/**
 * Patch fields on an existing recommendation (same id) — used by Discuss.
 * mutator may be an object of fields or a function(rec) => patch object.
 */
async function patchRecommendation(siteId, recommendationId, mutator, opts) {
  const o = opts || {};
  const store = storage.adapter();
  const got = await store.getRecommendation(recommendationId);
  if (!got.ok) return got;
  ...
  const patch = typeof mutator === 'function' ? mutator(got.recommendation) || {} : mutator || {};
  const updated = {
    ...got.recommendation,
    ...patch,
    id: got.recommendation.id,
    site_id: got.recommendation.site_id,
    brain_id: got.recommendation.brain_id,
    updated_by: o.actorUserId || null
  };
  if (patch.proposed_change || patch.proposedChange) {
    updated.proposed_change = patch.proposed_change || patch.proposedChange;
  }
  const saved = await store.upsertRecommendation(updated);
  if (!saved.ok) return saved;
  await store.appendEvent({
    site_id: siteId,
    brain_id: got.recommendation.brain_id,
    event_type: 'discuss_recommendation',
    path: 'recommendations.' + recommendationId,
    after_value: {
      hasDiscussion: !!(
        saved.recommendation.proposed_change && saved.recommendation.proposed_change.discussion
      )
    },
    source: o.source || 'specialist_inference',
    actor_user_id: o.actorUserId || null,
