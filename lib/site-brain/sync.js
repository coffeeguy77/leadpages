...
async function enrichSearchIntelligenceFromWarehouse(snapshot, siteId, admin) {
  if (!snapshot || !snapshot.searchIntelligence || !admin || !siteId) return snapshot;
  try {
    const { data } = await admin
      .from('si_keyword_clusters')
      .select('id,name,primary_keyword_id,intent,meta')
      .eq('site_id', siteId)
      .limit(40);
    snapshot.searchIntelligence.clusters = (data || []).map(function (c) {
      const meta = c.meta || {};
      return {
        id: c.id,
        primaryKeyword: meta.primaryKeyword || c.name,
        secondary: meta.secondaryKeywords || [],
        pageRef: null,
...
async function syncSiteBrainFromSite(site, opts) {
  const o = opts || {};
  const siteId = String(site.id);
  let snapshot = buildSnapshotFromSite(site);
  if (o.admin) {
    snapshot = await enrichSearchIntelligenceFromWarehouse(snapshot, siteId, o.admin);
  }
...
module.exports = {
  ...
  enrichSearchIntelligenceFromWarehouse
};
