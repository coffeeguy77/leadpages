...
function formatSeoTextHtml(raw) {
  ...
  blocks.forEach(function (block) {
    ...
    // Entire block is a markdown heading
...
function clientSource() {
  return (
    ...
    'if(lines.length===1&&/^###\\s+/.test(lines[0].trim())){out.push("<h3 class=\\"seotxt-h3\\">"+inlineFmt(esc(lines[0].trim().replace(/^###\\s+/,"")))+"</h3>");return;}' +
    'if(lines.length===1&&/^##\\s+/.test(lines[0].trim())){out.push("<h2 class=\\"seotxt-subh\\">"+inlineFmt(esc(lines[0].trim().replace(/^##\\s+/,"")))+"</h2>");return;}' +
    'if(lines.length===1&&looksLikeHeading(lines[0])&&!/^##/.test(lines[0].trim())){out.push("<h3 class=\\"seotxt-h3\\">"+inlineFmt(esc(lines[0].trim().replace(/:$/,"")))+"</h3>");return;}' +
    ...
    'if(/^###\\s+/.test(t)){flushPara();out.push("<h3 class=\\"seotxt-h3\\">"+inlineFmt(esc(t.replace(/^###\\s+/,"")))+"</h3>");return;}' +
    'if(/^##\\s+/.test(t)){flushPara();out.push("<h2 class=\\"seotxt-subh\\">"+inlineFmt(esc(t.replace(/^##\\s+/,"")))+"</h2>");return;}' +
    'var next=(lines[i+1]||"").trim();' +
    'if(looksLikeHeading(t)&&next&&next.length>Math.min(40,t.length)&&!/^##/.test(next)){flushPara();out.push("<h3 class=\\"seotxt-h3\\">"+inlineFmt(esc(t.replace(/:$/,"")))+"</h3>");return;}' +
