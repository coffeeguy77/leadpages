...
/**
 * LeadPages Brain — Phase 1–5 foundation.
 *
 * Feature → Brain → prompt/context → adapter (mock / Anthropic / OpenAI / Gemini).
 * Retries, task flags, soft budgets. No provider SDKs.
 * Default routes use mock (CI-safe). Set BRAIN_PROVIDER to switch.
 *
...
const { costFromUsage, resolveModelRate } = require('./pricing');
const {
  persistUsageEvent,
  loadDurableUsage,
  eventToRow
} = require('./usage-persist');
...
function createBrain(opts) {
  ...
  function withFetch(extra) {
    const out = Object.assign({}, extra);
    if (options.fetchImpl && !out.fetchImpl) out.fetchImpl = options.fetchImpl;
    return out;
  }

  const adapters = options.adapters || createAdapters({
    mock: options.mock,
    anthropic: withFetch(options.anthropic),
    openai: withFetch(options.openai),
    gemini: withFetch(options.gemini)
  });
