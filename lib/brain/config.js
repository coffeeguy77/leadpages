 ...
 * @property {{ disabledTasks?: string[], landingDraft?: boolean, igEnrich?: boolean, suburbIntro?: boolean, helpAssist?: boolean, tradePack?: boolean, themeStudio?: boolean, marketingHub?: boolean }} [flags]
...
function defaultBrainConfig() {
  ...
  const cfg = {
    ...
    flags: {
      ...
      /** Remaining migrations — default OFF until soak. */
      igEnrich: String(process.env.BRAIN_IG_ENRICH || '').toLowerCase() === '1',
      suburbIntro: String(process.env.BRAIN_SUBURB_INTRO || '').toLowerCase() === '1',
      helpAssist: String(process.env.BRAIN_HELP_ASSIST || '').toLowerCase() === '1',
      tradePack: String(process.env.BRAIN_TRADE_PACK || '').toLowerCase() === '1',
      /** Phase 8–9 product surfaces — default ON (set =0 to disable). */
      themeStudio: String(process.env.BRAIN_THEME_STUDIO || '1').toLowerCase() !== '0',
      marketingHub: String(process.env.BRAIN_MARKETING_HUB || '1').toLowerCase() !== '0',
      /** Theme Studio V2 full product — default ON (THEME_STUDIO_V2=0 to disable). */
      themeStudioV2: String(process.env.THEME_STUDIO_V2 || '1').toLowerCase() !== '0',
      /** Per-app section copy. Default ON — BRAIN_APP_CONTENT=0 to disable. */
      appContent: String(process.env.BRAIN_APP_CONTENT || '1').toLowerCase() !== '0',
      searchCanvasDraft:
        String(process.env.BRAIN_SEARCH_CANVAS || '').toLowerCase() === '1' ||
        String(process.env.BRAIN_LANDING_DRAFT || '').toLowerCase() === '1'
    },
    budgets: {
  ...
  route('content.landing_draft', 'mock-default', reason, {
    structured: true,
    maxTokens: 8192,
    timeoutMs: 90000
  });
  route('content.app_section', 'mock-default', reason, {
    structured: true,
    maxTokens: 8192,
    timeoutMs: 90000
  });
  /** SearchCanvas prefers OpenAI as primary when BRAIN_PROVIDER=openai or keys allow override. */
  route('content.search_canvas_draft', 'mock-default', {
    anthropic: anthropicSonnet,
    openai: openaiModel,
    gemini: geminiModel
  }, {
  ...
  // When global provider is not openai, still register openai as preferred primary for this task.
  if (activeProvider !== 'openai' && process.env.OPENAI_API_KEY) {
    cfg.routes['content.search_canvas_draft'] = {
      taskId: 'content.search_canvas_draft',
      primary: { provider: 'openai', model: openaiModel },
      fallback: [
        { provider: activeProvider === 'mock' ? 'mock' : activeProvider, model: activeProvider === 'mock' ? 'mock-default' : (activeProvider === 'anthropic' ? anthropicSonnet : (activeProvider === 'gemini' ? geminiModel : openaiModel)) },
  ...
  route('theme.generate', 'mock-default', reason, {
    structured: true,
    maxTokens: 1024,
    timeoutMs: 45000
  });
  route('theme.refine', 'mock-default', reason, {
    structured: true,
    maxTokens: 1024,
    timeoutMs: 45000
  });
  route('ads.campaign_plan', 'mock-default', reason, {
    structured: true,
    maxTokens: 4096,
    timeoutMs: 90000
  });
  route('ads.rsa_copy', 'mock-default', reason, {
    structured: true,
    maxTokens: 2048,
    timeoutMs: 60000
  });
  route('theme_studio.business_analysis', 'mock-default', reason, {
    structured: true,
    maxTokens: 2048,
    timeoutMs: 60000
  });
  route('theme_studio.foundation_selection', 'mock-default', reason, {
    structured: true,
    maxTokens: 2048,
    timeoutMs: 60000
  });
  route('theme_studio.concept_generation', 'mock-default', reason, {
    structured: true,
    maxTokens: 8192,
    timeoutMs: 120000
  });
  route('theme_studio.content_generation', 'mock-default', reason, {
    structured: true,
    maxTokens: 8192,
    timeoutMs: 120000
  });
  route('theme_studio.image_direction', 'mock-default', reason, {
    structured: true,
    maxTokens: 2048,
    timeoutMs: 60000
  });
  route('theme_studio.refinement', 'mock-default', reason, {
    structured: true,
    maxTokens: 4096,
    timeoutMs: 90000
  });
  route('theme_studio.quality_review', 'mock-default', reason, {
    structured: true,
    maxTokens: 2048,
    timeoutMs: 60000
  });
  // Domain Finder — prefer OpenAI when key present (product requirement).
  // Keep timeouts well under Vercel maxDuration (60s) so searches don't 504.
  route('domain_finder.generate', 'mock-default', reason, {
    structured: true,
    maxTokens: 1600,
    timeoutMs: 14000
  });
  route('domain_finder.rank', 'mock-default', reason, {
    structured: true,
    maxTokens: 1600,
    timeoutMs: 12000
  });
  if (process.env.OPENAI_API_KEY) {
    cfg.routes['domain_finder.generate'] = {
