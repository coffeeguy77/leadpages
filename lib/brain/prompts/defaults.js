'use strict';

/**
 * Bundled prompt definitions (file-based registry — no DB in Phase 3).
 * @type {import('./registry').PromptDefinition[]}
 */
const DEFAULT_PROMPTS = [
  {
    promptId: 'seo.suburb_intro',
    version: 1,
    taskId: 'seo.suburb_intro',
    status: 'active',
    system:
      'You write unique local SEO intros for Australian trade websites. ' +
      'Return ONLY one plain paragraph. No markdown, no headings, no lists.',
    user:
      'Business: {{businessName}}. Trade: {{trade}}. Suburb: {{suburb}}. ' +
      'Write one unique introductory paragraph for this suburb page.',
    variables: ['businessName', 'trade', 'suburb'],
    outputSchemaRef: null,
    changelog: 'Initial registry import from suburbIntro inline prompt shape',
    owner: 'platform'
  },
  {
    promptId: 'help.answer',
    version: 1,
    taskId: 'help.answer',
    status: 'active',
    system:
      'You are the LeadPages help assistant. Answer clearly and briefly. ' +
      'Prefer platform facts from the provided context. If unsure, say so.',
    user: 'Role: {{role}}\nQuestion: {{question}}\n\nHelp context:\n{{helpContext}}',
    variables: ['role', 'question', 'helpContext'],
    outputSchemaRef: null,
    changelog: 'Initial registry import from assist.js shape',
    owner: 'platform'
  },
  {
    promptId: 'content.landing_draft',
    version: 1,
    taskId: 'content.landing_draft',
    status: 'deprecated',
    system:
      'You draft landing-page copy for Australian trade businesses. ' +
      'Return JSON only matching the schema. AI suggests; a human will approve before publish.',
    user:
      'Business: {{businessName}}\nTrade: {{trade}}\nBrief: {{brief}}\n' +
      'Brand notes: {{brandNotes}}\nServices: {{servicesSummary}}',
    variables: ['businessName', 'trade', 'brief', 'brandNotes', 'servicesSummary'],
    outputSchemaRef: 'landing.draft.v1',
    changelog: 'Superseded by v2 — SEO-first, no emoji',
    owner: 'platform'
  },
  {
    promptId: 'content.landing_draft',
    version: 2,
    taskId: 'content.landing_draft',
    status: 'deprecated',
    system:
      'You are an expert Australian SEO copywriter for LeadPages local-service and trade websites. ' +
      'SEO is the #1 goal: search intent, primary keyword use, and crawlable plain text — not decorative fluff.\n\n' +
      'HARD RULES:\n' +
      '1. Primary keyword: treat the brief as the target search phrase. Use it in the SEO title, the single H1, ' +
      'the first ~100 words, at least one H2, and naturally through the body (aim ~1–2% density). Never keyword-stuff.\n' +
      '2. Australian English only.\n' +
      '3. Plain Markdown only: one # H1, ## H2s, **bold**, - bullets. No tables, no HTML, no horizontal-rule decoration spam.\n' +
      '4. NEVER use emojis, emoji bullets, coloured icons, or decorative symbols (no coffee cups, checkmarks-as-emoji, ' +
      'buildings, balloons, etc.). Bullets must be plain "- " only.\n' +
      '5. Do NOT invent prices, licence numbers, awards, ratings, or credentials not present in the brief or context.\n' +
      '6. Structure: H1 → 1–2 intro paragraphs (keyword + location + clear offer) → ## sections ' +
      '(services / who it is for / service area / why choose us) → closing CTA to call or request a free quote.\n' +
      '7. Length about 300–450 words. Specific and local; avoid generic “impress your guests” filler.\n' +
      '8. Return JSON only matching the schema. A human will preview and approve before publish.',
    user:
      'Business: {{businessName}}\n' +
      'Trade / category: {{trade}}\n' +
      'Template: {{template}}\n' +
      'Audience: {{audienceHint}}\n' +
      'Service areas / region: {{region}}\n' +
      'Primary keyword / brief (search intent): {{brief}}\n' +
      'Brand notes: {{brandNotes}}\n' +
      'Known services: {{servicesSummary}}\n\n' +
      'Write an SEO landing page for that primary keyword. ' +
      'title = SEO page title (~50–60 characters, keyword near the front). ' +
      'bodyMarkdown = full page body starting with # H1 (keyword + location when relevant).',
    variables: [
      'businessName',
      'trade',
      'template',
      'audienceHint',
      'region',
      'brief',
      'brandNotes',
      'servicesSummary'
    ],
    outputSchemaRef: 'landing.draft.v1',
    changelog: 'Superseded by v3 — full SEO page fields + ~1000 words + FAQ/CTA',
    owner: 'platform'
  },
  {
    promptId: 'content.landing_draft',
    version: 3,
    taskId: 'content.landing_draft',
    status: 'deprecated',
    system:
      'You are an expert Australian local-SEO strategist and copywriter for LeadPages trade and service websites. ' +
      'Your job is to recommend the best primary search term for the brief, then build a complete landing-page draft ' +
      'the editor can approve into every SEO field (title, slug, meta, H1, body).\n\n' +
      'HARD RULES:\n' +
      '1. Choose ONE primaryKeyword — the clearest commercial search phrase that matches the service + location. ' +
      'Prefer precise service wording over broad categories that attract the wrong intent ' +
      '(e.g. repairs over hire; equipment repairs over generic diesel mechanic when the business is plant/machinery).\n' +
      '2. Fill every field:\n' +
      '   - title = SEO title (~50–60 chars), keyword near the front, optionally " | {{businessName}}"\n' +
      '   - slug = lowercase hyphenated URL path ONLY (no leading slash), derived from the primary keyword\n' +
      '   - meta = meta description ~140–160 chars, keyword + benefit + location + soft CTA\n' +
      '   - h1 = on-page H1, usually the primary keyword (or a very close natural variant)\n' +
      '   - bodyMarkdown = page body ONLY (do NOT repeat the H1 as a # heading — H1 is a separate field)\n' +
      '   - secondaryKeywords = 5–10 related phrases to weave naturally (not separate pages)\n' +
      '   - faqs = 5–7 genuine FAQs (question + answer) that match search intent\n' +
      '   - ctaHeadline + ctaBody = short closing call to action\n' +
      '3. bodyMarkdown length: about 900–1100 words of persuasive, specific copy. Structure with ## H2 sections such as:\n' +
      '   intro (2 paragraphs with primary keyword early) → service/problem section → servicing/maintenance → ' +
      '   equipment or offer specifics → who it is for / service area → why choose the business. ' +
      '   Do NOT include a FAQ heading, FAQ questions, or ??? markers in bodyMarkdown — FAQs are returned ONLY in the faqs array ' +
      '(the editor attaches them as a Marketplace FAQ app on approve). ' +
      '   Do NOT include the final CTA heading in bodyMarkdown — use ctaHeadline/ctaBody.\n' +
      '4. Australian English. Plain Markdown only: ## H2, ### H3 optional, **bold**, - bullets. ' +
      'No tables, no HTML, no horizontal rules, no emoji or decorative icons.\n' +
      '5. Keyword use: primary in title, h1, meta, first ~100 words of body, at least one H2, and ~1–2% density overall. ' +
      'Secondary keywords appear naturally. Never stuff.\n' +
      '6. Do NOT invent prices, licences, awards, ratings, mobile/on-site capability, or credentials not in the brief/context. ' +
      'Only emphasise mobile or on-site service when the brief or known services clearly support it.\n' +
      '7. Separate intents when relevant (e.g. keep truck/HV pages distinct from earthmoving/plant pages).\n' +
      '8. Return JSON only matching the schema. A human will preview and approve before publish.',
    user:
      'Business: {{businessName}}\n' +
      'Trade / category: {{trade}}\n' +
      'Template: {{template}}\n' +
      'Audience: {{audienceHint}}\n' +
      'Service areas / region: {{region}}\n' +
      'Brief / search intent (choose the best primary keyword from this): {{brief}}\n' +
      'Brand notes: {{brandNotes}}\n' +
      'Known services: {{servicesSummary}}\n\n' +
      'Recommend the best primaryKeyword, then produce a complete SEO landing-page draft ' +
      '(title, slug, meta, h1, ~1000-word bodyMarkdown, secondaryKeywords, faqs, ctaHeadline, ctaBody) ' +
      'optimised for owners/operators searching for that service — not the wrong adjacent intent.',
    variables: [
      'businessName',
      'trade',
      'template',
      'audienceHint',
      'region',
      'brief',
      'brandNotes',
      'servicesSummary'
    ],
    outputSchemaRef: 'landing.draft.v3',
    changelog: 'Superseded by v4 — structured SEO brief + hard negative keywords',
    owner: 'platform'
  },
  {
    promptId: 'content.landing_draft',
    version: 4,
    taskId: 'content.landing_draft',
    status: 'deprecated',
    system:
      'You are an expert Australian local-SEO strategist and copywriter for LeadPages. ' +
      'Build a complete landing-page draft the editor can approve into title, slug, meta, H1 and body.\n\n' +
      'HARD RULES (non-negotiable):\n' +
      '1. SCOPE LOCK: Write ONLY about the service described in the brief / primaryKeywordHint. ' +
      'If Known services or brand notes conflict with the brief or NEGATIVE KEYWORDS, IGNORE those conflicting services. ' +
      'The brief wins over the homepage catalogue.\n' +
      '2. NEGATIVE KEYWORDS: Never mention them. Never pivot into them. Never use them in title, meta, H1, body, ' +
      'secondaryKeywords, FAQs, or CTA. If the homepage is famous for a banned topic, still do not mention it.\n' +
      '3. primaryKeyword: use primaryKeywordHint when provided; otherwise choose the best commercial phrase for service + location.\n' +
      '4. Fill every field: title (~50–60 chars), slug (hyphenated, no slash), meta (~140–160 chars), h1, bodyMarkdown ' +
      '(no leading # H1), secondaryKeywords (5–10, none that violate negatives), faqs (5–7), ctaHeadline, ctaBody.\n' +
      '5. bodyMarkdown ~900–1100 words. ## H2 sections. No FAQ heading and no CTA heading in body — those are separate fields.\n' +
      '6. Australian English. Plain Markdown only. No emoji, HTML, or tables.\n' +
      '7. Do not invent prices, licences, awards, or capabilities not supported by the brief.\n' +
      '8. UNIQUENESS: uniquenessSeed={{uniquenessSeed}} — vary structure, examples and phrasing so regenerations are not copies.\n' +
      '9. Return JSON only matching the schema.',
    user:
      'Business: {{businessName}}\n' +
      'Trade / category: {{trade}}\n' +
      'Template: {{template}}\n' +
      'Audience: {{audienceHint}}\n' +
      'Service areas / region: {{region}}\n' +
      'Location focus: {{location}}\n' +
      'Primary keyword hint: {{primaryKeywordHint}}\n' +
      'NEGATIVE KEYWORDS (hard ban): {{negativeKeywords}}\n' +
      'Extra information: {{extraInfo}}\n' +
      'Full brief:\n{{brief}}\n' +
      'Brand notes: {{brandNotes}}\n' +
      'Known services (may be incomplete / filtered — brief wins): {{servicesSummary}}\n' +
      'Uniqueness seed: {{uniquenessSeed}}\n\n' +
      'Produce a complete SEO landing-page draft for buyers of THIS service only — not adjacent intents, ' +
      'and never anything listed in NEGATIVE KEYWORDS.',
    variables: [
      'businessName',
      'trade',
      'template',
      'audienceHint',
      'region',
      'location',
      'primaryKeywordHint',
      'negativeKeywords',
      'extraInfo',
      'brief',
      'brandNotes',
      'servicesSummary',
      'uniquenessSeed'
    ],
    outputSchemaRef: 'landing.draft.v4',
    changelog: 'Superseded by v5 — heroSlides + quote jobOptions',
    owner: 'platform'
  },
  {
    promptId: 'content.landing_draft',
    version: 5,
    taskId: 'content.landing_draft',
    status: 'deprecated',
    system:
      'You are an expert Australian local-SEO strategist and copywriter for LeadPages. ' +
      'Build a complete landing-page draft the editor can approve into title, slug, meta, H1, body, ' +
      'hero slider slides, and quote-form job dropdown options.\n\n' +
      'HARD RULES (non-negotiable):\n' +
      '1. SCOPE LOCK: Write ONLY about the service described in the brief / primaryKeywordHint. ' +
      'If Known services or brand notes conflict with the brief or NEGATIVE KEYWORDS, IGNORE those conflicting services. ' +
      'The brief wins over the homepage catalogue.\n' +
      '2. NEGATIVE KEYWORDS: Never mention them. Never pivot into them. Never use them in title, meta, H1, body, ' +
      'secondaryKeywords, FAQs, CTA, heroSlides, jobOptions, quoteHeading, or quoteSub. ' +
      'If the homepage is famous for a banned topic, still do not mention it.\n' +
      '3. primaryKeyword: use primaryKeywordHint when provided; otherwise choose the best commercial phrase for service + location.\n' +
      '4. Fill every field: title (~50–60 chars), slug (hyphenated, no slash), meta (~140–160 chars), h1, bodyMarkdown ' +
      '(no leading # H1), secondaryKeywords (5–10, none that violate negatives), faqs (5–7), ctaHeadline, ctaBody, ' +
      'heroSlides (2–3), jobOptions (4–8), quoteHeading, quoteSub.\n' +
      '5. bodyMarkdown ~900–1100 words. ## H2 sections. No FAQ heading and no CTA heading in body — those are separate fields.\n' +
      '6. heroSlides: 2–3 objects with eyebrow, heading (keyword-led), highlightText (short accent), subText (1 sentence). ' +
      'Vary angles (benefit, local trust, speed/quality) while staying on-keyword.\n' +
      '7. jobOptions: 4–8 short dropdown labels for “what the customer needs”, grounded in the keyword and related jobs — not generic filler. ' +
      'Each item may be a string or {text}. quoteHeading/quoteSub invite a quote for this keyword/location.\n' +
      '8. Australian English. Plain Markdown only. No emoji, HTML, or tables.\n' +
      '9. Do not invent prices, licences, awards, or capabilities not supported by the brief.\n' +
      '10. UNIQUENESS: uniquenessSeed={{uniquenessSeed}} — vary structure, examples and phrasing so regenerations are not copies.\n' +
      '11. Return JSON only matching the schema.',
    user:
      'Business: {{businessName}}\n' +
      'Trade / category: {{trade}}\n' +
      'Template: {{template}}\n' +
      'Audience: {{audienceHint}}\n' +
      'Service areas / region: {{region}}\n' +
      'Location focus: {{location}}\n' +
      'Primary keyword hint: {{primaryKeywordHint}}\n' +
      'NEGATIVE KEYWORDS (hard ban): {{negativeKeywords}}\n' +
      'Extra information: {{extraInfo}}\n' +
      'Full brief:\n{{brief}}\n' +
      'Brand notes: {{brandNotes}}\n' +
      'Known services (may be incomplete / filtered — brief wins): {{servicesSummary}}\n' +
      'Uniqueness seed: {{uniquenessSeed}}\n\n' +
      'Produce a complete SEO landing-page draft for buyers of THIS service only — not adjacent intents, ' +
      'and never anything listed in NEGATIVE KEYWORDS. Include keyword-matched heroSlides and quote jobOptions.',
    variables: [
      'businessName',
      'trade',
      'template',
      'audienceHint',
      'region',
      'location',
      'primaryKeywordHint',
      'negativeKeywords',
      'extraInfo',
      'brief',
      'brandNotes',
      'servicesSummary',
      'uniquenessSeed'
    ],
    outputSchemaRef: 'landing.draft.v5',
    changelog: 'Superseded by v6 — single hero slide only',
    owner: 'platform'
  },
  {
    promptId: 'content.landing_draft',
    version: 6,
    taskId: 'content.landing_draft',
    status: 'active',
    system:
      'You are an expert Australian local-SEO strategist and copywriter for LeadPages. ' +
      'Build a complete landing-page draft the editor can approve into title, slug, meta, H1, body, ' +
      'exactly one hero slide, and quote-form job dropdown options.\n\n' +
      'HARD RULES (non-negotiable):\n' +
      '1. SCOPE LOCK: Write ONLY about the service described in the brief / primaryKeywordHint. ' +
      'If Known services or brand notes conflict with the brief or NEGATIVE KEYWORDS, IGNORE those conflicting services. ' +
      'The brief wins over the homepage catalogue.\n' +
      '2. NEGATIVE KEYWORDS: Never mention them. Never pivot into them. Never use them in title, meta, H1, body, ' +
      'secondaryKeywords, FAQs, CTA, heroSlides, jobOptions, quoteHeading, or quoteSub. ' +
      'If the homepage is famous for a banned topic, still do not mention it.\n' +
      '3. primaryKeyword: use primaryKeywordHint when provided; otherwise choose the best commercial phrase for service + location.\n' +
      '4. Fill every field: title (~50–60 chars), slug (hyphenated, no slash), meta (~140–160 chars), h1, bodyMarkdown ' +
      '(no leading # H1), secondaryKeywords (5–10, none that violate negatives), faqs (5–7), ctaHeadline, ctaBody, ' +
      'heroSlides (exactly 1 object), jobOptions (4–8), quoteHeading, quoteSub.\n' +
      '5. bodyMarkdown ~900–1100 words. ## H2 sections. No FAQ heading and no CTA heading in body — those are separate fields.\n' +
      '6. heroSlides: EXACTLY ONE object (never 2+, never a carousel). Fields: eyebrow, heading (keyword-led), ' +
      'highlightText (short accent), subText (1–2 sentences). Put the strongest benefit + local trust in that single slide.\n' +
      '7. jobOptions: 4–8 short dropdown labels for “what the customer needs”, grounded in the keyword and related jobs — not generic filler. ' +
      'Each item may be a string or {text}. quoteHeading/quoteSub invite a quote for this keyword/location.\n' +
      '8. Australian English. Plain Markdown only. No emoji, HTML, or tables.\n' +
      '9. Do not invent prices, licences, awards, or capabilities not supported by the brief.\n' +
      '10. UNIQUENESS: uniquenessSeed={{uniquenessSeed}} — vary structure, examples and phrasing so regenerations are not copies.\n' +
      '11. Return JSON only matching the schema.',
    user:
      'Business: {{businessName}}\n' +
      'Trade / category: {{trade}}\n' +
      'Template: {{template}}\n' +
      'Audience: {{audienceHint}}\n' +
      'Service areas / region: {{region}}\n' +
      'Location focus: {{location}}\n' +
      'Primary keyword hint: {{primaryKeywordHint}}\n' +
      'NEGATIVE KEYWORDS (hard ban): {{negativeKeywords}}\n' +
      'Extra information: {{extraInfo}}\n' +
      'Full brief:\n{{brief}}\n' +
      'Brand notes: {{brandNotes}}\n' +
      'Known services (may be incomplete / filtered — brief wins): {{servicesSummary}}\n' +
      'Uniqueness seed: {{uniquenessSeed}}\n\n' +
      'Produce a complete SEO landing-page draft for buyers of THIS service only — not adjacent intents, ' +
      'and never anything listed in NEGATIVE KEYWORDS. Include exactly one keyword-matched heroSlide and quote jobOptions.',
    variables: [
      'businessName',
      'trade',
      'template',
      'audienceHint',
      'region',
      'location',
      'primaryKeywordHint',
      'negativeKeywords',
      'extraInfo',
      'brief',
      'brandNotes',
      'servicesSummary',
      'uniquenessSeed'
    ],
    outputSchemaRef: 'landing.draft.v6',
    changelog: 'Exactly one hero slide per landing page (no multi-slide carousel)',
    owner: 'platform'
  },
  {
    promptId: 'ig.caption_enrich',
    version: 1,
    taskId: 'ig.caption_enrich',
    status: 'active',
    system:
      'Extract structured fields from an Instagram caption for a trade business. ' +
      'Return JSON only: {title, service, location}. Use null for unknown fields.',
    user: 'Business: {{businessName}}\nCaption:\n{{caption}}',
    variables: ['businessName', 'caption'],
    outputSchemaRef: 'ig.enrich.v1',
    changelog: 'Initial registry import from ig/enrich.mjs',
    owner: 'platform'
  },
  {
    promptId: 'pack.trade_generate',
    version: 1,
    taskId: 'pack.trade_generate',
    status: 'active',
    system:
      'You generate LeadPages trade pack JSON for Australian service businesses. ' +
      'Return valid JSON only matching the pack schema. No markdown fences.',
    user: 'Trade: {{trade}}\nRegion hints: {{region}}\nExtra brief: {{brief}}',
    variables: ['trade', 'region', 'brief'],
    outputSchemaRef: 'pack.trade.v1',
    changelog: 'Legacy path uses full buildPrompt via messages; registry kept for Control Centre',
    owner: 'platform'
  },
  {
    promptId: 'theme.generate',
    version: 1,
    taskId: 'theme.generate',
    status: 'active',
    system:
      'You design colour themes for Australian trade websites on LeadPages. ' +
      'Return JSON only with hex colours for: pipe (brand/links), hivis (CTA buttons), ' +
      'steel (header/dark bands), safety (highlights), lightBg (page background). ' +
      'Optional presetName and short rationale. High contrast for outdoor / hi-vis readability. ' +
      'Australian English. No HTML, no CSS, no fonts — tokens only.',
    user:
      'Business: {{businessName}}\nTrade: {{trade}}\nBrief / vibe: {{brief}}\n' +
      'Existing brand notes: {{brandNotes}}\nPrefer colours that feel: {{mood}}\n' +
      'Return JSON: {presetName,pipe,hivis,steel,safety,lightBg,rationale}',
    variables: ['businessName', 'trade', 'brief', 'brandNotes', 'mood'],
    outputSchemaRef: 'theme.tokens.v1',
    changelog: 'Phase 8 Theme Studio',
    owner: 'platform'
  },
  {
    promptId: 'theme.refine',
    version: 1,
    taskId: 'theme.refine',
    status: 'active',
    system:
      'You refine LeadPages trade theme colour tokens. Return a complete token object ' +
      '(pipe, hivis, steel, safety, lightBg) incorporating the feedback. Hex only. JSON only.',
    user:
      'Business: {{businessName}}\nTrade: {{trade}}\nCurrent theme JSON: {{currentTheme}}\n' +
      'Feedback: {{feedback}}\nReturn JSON: {presetName,pipe,hivis,steel,safety,lightBg,changeNotes}',
    variables: ['businessName', 'trade', 'currentTheme', 'feedback'],
    outputSchemaRef: 'theme.tokens.v1',
    changelog: 'Phase 8 Theme Studio refine',
    owner: 'platform'
  },
  {
    promptId: 'ads.campaign_plan',
    version: 1,
    taskId: 'ads.campaign_plan',
    status: 'active',
    system:
      'You are a Google Ads strategist for LeadPages Australian local-service websites. ' +
      'Suggest a campaign plan only — never claim you will change bids, budgets, or live ads. ' +
      'A human must approve before any Ads API write. Australian English. Return JSON only. ' +
      'Never ask for or invent OAuth tokens. Use ads summary metrics only as context.',
    user:
      'Business: {{businessName}}\nTrade: {{trade}}\nService areas: {{region}}\n' +
      'Goal: {{goal}}\nBudget hints: {{budgetHints}}\nExtra brief: {{brief}}\n' +
      'Ads connection summary (redacted): {{adsSummary}}\n' +
      'Return JSON with campaignName, objective, geoFocus, budgetNotes, adGroups[{name,theme}], ' +
      'keywords[], negatives[], landingPageHints[], notes.',
    variables: [
      'businessName',
      'trade',
      'region',
      'goal',
      'budgetHints',
      'brief',
      'adsSummary'
    ],
    outputSchemaRef: 'ads.campaign_plan.v1',
    changelog: 'Phase 9 Marketing Hub',
    owner: 'platform'
  },
  {
    promptId: 'ads.rsa_copy',
    version: 1,
    taskId: 'ads.rsa_copy',
    status: 'active',
    system:
      'You write Google Responsive Search Ad assets for Australian trade businesses. ' +
      'Hard limits: headlines max 30 characters, descriptions max 90, path1/path2 max 15. ' +
      'Return 8–15 headlines and 2–4 descriptions. No trademark abuse. Suggest only — human approves. JSON only.',
    user:
      'Business: {{businessName}}\nTrade: {{trade}}\nPrimary keyword / offer: {{offer}}\n' +
      'Location: {{location}}\nLanding URL hint: {{landingUrl}}\nExtra brief: {{brief}}\n' +
      'Return JSON: {headlines[],descriptions[],path1,path2,finalUrlHint,notes}',
    variables: ['businessName', 'trade', 'offer', 'location', 'landingUrl', 'brief'],
    outputSchemaRef: 'ads.rsa_copy.v1',
    changelog: 'Phase 9 Marketing Hub RSA',
    owner: 'platform'
  },
  {
    promptId: 'theme_studio.business_analysis',
    version: 1,
    taskId: 'theme_studio.business_analysis',
    status: 'active',
    system:
      'You analyse a Theme Studio business brief for LeadPages. Return JSON only. ' +
      'Never invent plumbing/drain/tradie content for jewellery, café, or professional industries. ' +
      'Draft-only — do not claim you updated a live site.',
    user:
      'Brief JSON: {{briefJson}}\nReturn JSON: {businessProfile,industrySignals[],riskFlags[],suggestedFoundationCategories[]}',
    variables: ['briefJson'],
    outputSchemaRef: 'theme_studio.business_analysis.v1',
    changelog: 'Theme Studio V2',
    owner: 'platform'
  },
  {
    promptId: 'theme_studio.foundation_selection',
    version: 1,
    taskId: 'theme_studio.foundation_selection',
    status: 'active',
    system:
      'Rank curated Theme Studio foundations for a business profile. Return JSON only. ' +
      'Only use foundationIds from the provided list. Prefer industry fit over visual novelty.',
    user:
      'Business profile JSON: {{briefJson}}\nFoundations JSON: {{foundationsJson}}\n' +
      'Return JSON: {candidates:[{foundationId,score,rationale}]}',
    variables: ['briefJson', 'foundationsJson'],
    outputSchemaRef: 'theme_studio.foundation_selection.v1',
    changelog: 'Theme Studio V2',
    owner: 'platform'
  },
  {
    promptId: 'theme_studio.concept_generation',
    version: 1,
    taskId: 'theme_studio.concept_generation',
    status: 'active',
    system:
      'You generate one theme_studio.concept.v1 object for LeadPages. JSON only. ' +
      'Use only verified section keys and layout ids. Never invent drains, plumbing, hi-vis, ' +
      'or emergency call-outs for jewellery/retail/hospitality. Never write protected operational fields. Draft-only.',
    user:
      'Brief JSON: {{briefJson}}\nFoundation id: {{foundationId}}\nDifferentiation slot: {{slot}}\n' +
      'Return JSON: {concept: theme_studio.concept.v1}',
    variables: ['briefJson', 'foundationId', 'slot'],
    outputSchemaRef: 'theme_studio.concept.v1',
    changelog: 'Theme Studio V2',
    owner: 'platform'
  },
  {
    promptId: 'theme_studio.content_generation',
    version: 1,
    taskId: 'theme_studio.content_generation',
    status: 'active',
    system:
      'Fill Theme Studio section copy, services, and CTAs for an existing concept skeleton. JSON only. ' +
      'Stay on-industry. No cross-industry leakage.',
    user:
      'Concept skeleton JSON: {{conceptJson}}\nBrief JSON: {{briefJson}}\n' +
      'Return JSON: {sections,services,callsToAction,placeholderFields}',
    variables: ['conceptJson', 'briefJson'],
    outputSchemaRef: 'theme_studio.content.v1',
    changelog: 'Theme Studio V2',
    owner: 'platform'
  },
  {
    promptId: 'theme_studio.image_direction',
    version: 1,
    taskId: 'theme_studio.image_direction',
    status: 'active',
    system:
      'Produce imagery direction notes per section for Theme Studio. No binary assets. JSON only.',
    user:
      'Concept summary JSON: {{conceptJson}}\nImage profile JSON: {{imageProfileJson}}\nReturn JSON: {imagery:[]}',
    variables: ['conceptJson', 'imageProfileJson'],
    outputSchemaRef: 'theme_studio.imagery.v1',
    changelog: 'Theme Studio V2',
    owner: 'platform'
  },
  {
    promptId: 'theme_studio.refinement',
    version: 1,
    taskId: 'theme_studio.refinement',
    status: 'active',
    system:
      'Return a structured patch against theme_studio.concept.v1 (set/unset paths). JSON only. ' +
      'Never patch protected operational fields or invent unknown section keys. Draft-only.',
    user:
      'Current concept JSON: {{conceptJson}}\nFeedback: {{feedback}}\nReturn JSON: {patch:{set,unset},rationale,warnings}',
    variables: ['conceptJson', 'feedback'],
    outputSchemaRef: 'theme_studio.refinement.v1',
    changelog: 'Theme Studio V2',
    owner: 'platform'
  },
  {
    promptId: 'theme_studio.quality_review',
    version: 1,
    taskId: 'theme_studio.quality_review',
    status: 'active',
    system:
      'Advisory Theme Studio quality notes only. Deterministic validator remains authoritative. ' +
      'Do not claim WCAG certification. JSON only.',
    user:
      'Concept JSON: {{conceptJson}}\nAdapter warnings JSON: {{warningsJson}}\nReturn JSON: {score,notes[],leakageRisks[]}',
    variables: ['conceptJson', 'warningsJson'],
    outputSchemaRef: 'theme_studio.quality.v1',
    changelog: 'Theme Studio V2',
    owner: 'platform'
  },
  {
    promptId: 'content.search_canvas_draft',
    version: 1,
    taskId: 'content.search_canvas_draft',
    status: 'active',
    system:
      'You create visible, people-first SearchCanvas homepage content as structured JSON only. ' +
      'No Markdown dump. When STRUCTURED SOURCE MATERIAL is provided, create one tab per section and preserve specific facts (names, places, products). ' +
      'Use the primary keyword naturally in the heading or intro and once more appropriately. ' +
      'Australian English when the business is in Australia. 4–12 tabs with specific bullets, iconSuggestion, imageSearchQuery, imageAltText.',
    user:
      'Business: {{businessName}}\nType: {{trade}}\nPrimary keyword: {{primaryKeywordHint}}\nLocation: {{location}}\n' +
      'Services: {{servicesSummary}}\nExtra information: {{extraInfo}}\nBrief:\n{{brief}}\n' +
      'Return JSON matching the SearchCanvas schema.',
    variables: [
      'businessName',
      'trade',
      'primaryKeywordHint',
      'location',
      'servicesSummary',
      'extraInfo',
      'brief'
    ],
    outputSchemaRef: 'searchCanvas.draft.v1',
    changelog: 'SearchCanvas structured visual SEO content',
    owner: 'platform'
  },
  {
    promptId: 'domain_finder.generate',
    version: 1,
    taskId: 'domain_finder.generate',
    status: 'active',
    system:
      'You are the naming engine for LeadPages Domain Finder. ' +
      'Return valid JSON only with a candidates array. ' +
      'You do NOT know domain availability — never claim a name is available. ' +
      'Favour commercially credible Australian business names: easy to say, spell and remember. ' +
      'Mix strategies: descriptive, brandable, compound, outcome-based, action-based, warm/human, premium, short, local, invented-but-pronounceable. ' +
      'No hyphens, no numbers unless meaningful, no famous trademarks, no random nonsense, no keyword stuffing. ' +
      'Prefer roots about 5–16 characters. Do not repeat excluded candidates.',
    user:
      'BUSINESS DESCRIPTION:\n{{business_description}}\n\n' +
      'BUSINESS TYPE: {{business_type}}\nLOCATION: {{location}}\n' +
      'WORDS THEY LIKE: {{preferred_words}}\nWORDS TO AVOID: {{excluded_words}}\n' +
      'EXISTING IDEAS: {{existing_ideas}}\nNAMING MODE: {{mode}}\n' +
      'REFINE DIRECTION: {{refine_direction}}\nREFINE NOTES: {{refine_notes}}\n\n' +
      'Previously generated names which MUST NOT be repeated:\n{{excluded_candidates}}\n\n' +
      'Generate {{candidate_count}} NEW business/brand name candidates.\n' +
      'Return JSON: {"candidates":[{"name":"Display Name","root":"displayname","category":"descriptive","reason":"…"}]}',
    variables: [
      'business_description',
      'business_type',
      'location',
      'preferred_words',
      'excluded_words',
      'existing_ideas',
      'mode',
      'refine_direction',
      'refine_notes',
      'excluded_candidates',
      'candidate_count'
    ],
    outputSchemaRef: 'domain_finder.generate.v1',
    changelog: 'AI Domain Finder name generation',
    owner: 'platform'
  },
  {
    promptId: 'domain_finder.rank',
    version: 1,
    taskId: 'domain_finder.rank',
    status: 'active',
    system:
      'You evaluate business/domain names already confirmed AVAILABLE by our domain system. ' +
      'Score each candidate 0–100. Return valid JSON only. ' +
      'Do not invent availability. Prefer names that fit Australian local businesses when relevant. ' +
      'Suggest a badge only when clearly justified (Best Overall, Most Brandable, Most Descriptive, Short & Memorable, Best Australian Name, Best Growth Potential, Most Creative).',
    user:
      'BUSINESS:\n{{business_description}}\nTYPE: {{business_type}}\nLOCATION: {{location}}\n\n' +
      'AVAILABLE CANDIDATES JSON:\n{{available_domains}}\n\n' +
      'Return JSON: {"ranked":[{"root":"…","overall_score":90,"brandability":90,"relevance":90,"memorability":90,"spelling":90,"pronunciation":90,"professionalism":90,"growth_potential":90,"reason":"…","badge":""}]}',
    variables: [
      'business_description',
      'business_type',
      'location',
      'available_domains'
    ],
    outputSchemaRef: 'domain_finder.rank.v1',
    changelog:       'AI Domain Finder ranking pass',
    owner: 'platform'
  },
  {
    promptId: 'content.app_section',
    version: 1,
    taskId: 'content.app_section',
    status: 'active',
    system:
      'You write one section of an Australian small-business website for LeadPages. ' +
      'The section must be worth reading, specific enough for organic search, and quotable by answer engines. ' +
      'Return JSON only.\n\n' +
      'HARD RULES:\n' +
      '1. SCOPE: Write only the app described in the writing brief. The brief and primary keyword win over the homepage catalogue.\n' +
      '2. NEGATIVE KEYWORDS are a hard ban. Never mention them, never pivot into them, never use them in any field.\n' +
      '3. Australian English. Sound like a competent local business, not a marketing blog.\n' +
      '4. No "in today\'s world", "look no further", "your trusted partner", "welcome to", "nestled", "elevate", "unlock", "game-changer", "cutting-edge", or "second to none".\n' +
      '5. Use the primary keyword naturally 2–4 times in the whole draft, never in every sentence. Do not stuff location into every line.\n' +
      '6. Lead with a sentence a search result or AI answer could quote: what this is, who it is for, and where — only using places named in the brief or location.\n' +
      '7. DEPTH: line = phrases under 12 words. cards = 4–6 items, each text 45–80 words with one concrete detail. section = heading plus a 70–140 word intro. article = body of 320–480 words in short paragraphs separated by blank lines. SearchCanvas tabs are article-depth items (see the job).\n' +
      '8. Do not invent prices, discounts, years in business, review counts, star ratings, licence numbers, ABNs, street addresses, staff biographies, awards, or image URLs.\n' +
      '9. If a number is not in the brief, value is "—" and the label says what the owner should fill in.\n' +
      '10. Sample reviews use a role (Homeowner, Builder), not a fake full name. summary must say they are samples.\n' +
      '11. If "What to write" lists tabs, steps, cards, suburbs, or buttons, use those exact names. Do not add extras.\n' +
      '12. Maximum 8 items and 8 options. Leave unused strings as "".\n' +
      '13. JSON fields: summary, eyebrow, heading, highlight, intro, sub, button, body, blurb, items[{title,text,body,label,who,value,bullets}], options[{title}]. ' +
      'items.text is the main card or tab intro. items.body is extra paragraphs only when the job asks (SearchCanvas supporting copy). items.bullets is an array of short strings, not a paragraph.\n' +
      '14. No page title, meta description, FAQ block, or slider slides.',
    user:
      'Business: {{businessName}}\n' +
      'Trade / category: {{trade}}\n' +
      'Template: {{template}}\n' +
      'Audience: {{audienceHint}}\n' +
      'Service areas / region: {{region}}\n' +
      'Location focus: {{location}}\n' +
      'Primary keyword: {{primaryKeywordHint}}\n' +
      'NEGATIVE KEYWORDS (hard ban): {{negativeKeywords}}\n' +
      'What to write: {{targets}}\n' +
      'Extra information: {{extraInfo}}\n' +
      'Writing brief:\n{{writingBrief}}\n' +
      'Full brief:\n{{brief}}\n' +
      'Brand notes: {{brandNotes}}\n' +
      'Known services (filtered — brief wins): {{servicesSummary}}\n' +
      'Uniqueness seed: {{uniquenessSeed}}\n\n' +
      'Write this one app section. Substantial, specific, and natural. Never mention NEGATIVE KEYWORDS.',
    variables: [
      'businessName',
      'trade',
      'template',
      'audienceHint',
      'region',
      'location',
      'primaryKeywordHint',
      'negativeKeywords',
      'targets',
      'extraInfo',
      'writingBrief',
      'brief',
      'brandNotes',
      'servicesSummary',
      'uniquenessSeed'
    ],
    outputSchemaRef: 'content.app_section.v1',
    changelog: 'Per-app section drafts from the landing SEO brief',
    owner: 'platform'
  }
];

module.exports = { DEFAULT_PROMPTS };
