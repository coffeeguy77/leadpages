 ...
 * Awaited from platform onUsage (with gateway timeout) so Vercel isolates
 * do not freeze before the insert completes.
 */
...
/** @type {{ at: string|null, error: string|null, skipped: boolean|null }} */
let lastPersistError = { at: null, error: null, skipped: null };

function recordPersistResult(result) {
  if (!result) return;
  if (result.ok) {
    lastPersistError = { at: new Date().toISOString(), error: null, skipped: !!result.skipped };
    return;
  }
  lastPersistError = {
...
function getLastPersistStatus() {
  return {
    at: lastPersistError.at,
    error: lastPersistError.error,
    skipped: lastPersistError.skipped
  };
}
...
function isDuplicateError(err) {
  ...
  // Postgres unique_violation = 23505; PostgREST often surfaces it in message.
  return (
    code === '23505' ||
...
/**
 * Persist one usage event (simple insert; duplicates are treated as ok).
 * @param {object} event
...
async function persistUsageEvent(event) {
  ...
  if (!admin) {
    const result = { ok: false, skipped: true, error: 'no_supabase' };
  ...
  try {
    ...
    const result = { ok: false, error: (err && err.message) || 'persist_failed' };
    recordPersistResult(result);
    return result;
  }
}
...
async function loadDurableUsage(opts) {
  ...
  try {
    ...
    if (rows.length === 0) {
      notice =
        'ledger empty — new Brain calls after deploy will appear here';
    }
...
function resetUsagePersistClient() {
  _admin = null;
  _adminFailed = false;
  lastPersistError = { at: null, error: null, skipped: null };
}
