...
/** AI Domain Finder. Default ON — BRAIN_DOMAIN_FINDER=0 to disable. */
function isDomainFinderEnabled(brain) {
  const b = brain || getPlatformBrain();
  const flags = (b.config && b.config.flags) || {};
  if (flags.domainFinder === false) return false;
  if (Array.isArray(flags.disabledTasks) && flags.disabledTasks.includes('domain_finder.generate')) {
    return false;
  }
  const env = String(process.env.BRAIN_DOMAIN_FINDER || '1').toLowerCase();
  return env !== '0' && env !== 'false' && env !== 'off';
}

/** Per-app section copy. Default ON — BRAIN_APP_CONTENT=0 to disable. */
function isAppContentEnabled(brain) {
  const b = brain || getPlatformBrain();
  const flags = (b.config && b.config.flags) || {};
  if (flags.appContent === false) return false;
  if (Array.isArray(flags.disabledTasks) && flags.disabledTasks.includes('content.app_section')) {
    return false;
  }
  const env = String(process.env.BRAIN_APP_CONTENT || '1').toLowerCase();
  return env !== '0' && env !== 'false' && env !== 'off';
}
