...
function buildSearchCanvasSystemPrompt() {
  return [
    ...
    '',
    'TAB RULES (CRITICAL):',
    '- Each tab MUST be a real customer-facing SERVICE this business offers (e.g. Landscape Design, Retaining Walls, Garden Maintenance).',
    ...
    '- If Services is empty, invent 4–6 specific services that fit the business type, primary keyword and location.',
    ...
    '- 4–6 tabs (prefer the requested count). Never create filler tabs.',
...
function buildSearchCanvasUserPrompt(brief) {
  ...
  return [
    ...
    'Location: ' + (b.location || ''),
    'Services (use these as tab labels when present): ' +
      (services.length ? services.join('; ') : '(none supplied — invent specific services for this trade/keyword)'),
    ...
    '',
    'Create the SearchCanvas JSON now. Tabs = this business’s services, not Planning/Delivery/Support/Maintenance.'
  ].join('\n');
}
...
function mockSearchCanvasDraft(brief) {
  ...
  const fromBrief = Array.isArray(brief && brief.services)
    ...
    : [];
  const services = (fromBrief.length ? fromBrief : servicesFromKeyword(kw, trade)).slice(0, Math.max(3, Math.min(6, Number(brief && brief.tabCount) || 5)));
