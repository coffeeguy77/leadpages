 ...
 * Fills title / slug / meta / H1 / body, and appends FAQ + CTA sections
 * that can collapse on the public page via `??? question` markdown.
 */
...
function composeBodyMarkdown(output) {
  ...
  const faqs = normalizeFaqs(output.faqs);
  const hasFaqMarkers =
    /##\s*Frequently Asked Questions/i.test(body) ||
    /^\?\?\?\s+/m.test(body);

  if (faqs.length && !hasFaqMarkers) {
  ...
  const hasCta = /##\s*(Contact|Get in touch|Ready to|Call |Book |Request )/i.test(body);
  if ((ctaHeadline || ctaBody) && !hasCta) {
    body += '\n\n## ' + (ctaHeadline || 'Get in touch') + '\n\n';
    if (ctaBody) body += ctaBody.trim() + '\n';
  }
