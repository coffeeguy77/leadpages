...
const SECTION_SCALAR_KEYS = new Set([
  'theme', 'business', 'trade', 'phone', 'phoneText', 'layout', 'sectionOrder', 'logo', 'pages', 'slug'
]);
...
function flatDemoToSiteConfig(flat, sectionKey) {
  ...
  if (src.slug) out.slug = src.slug;
  Object.keys(src).forEach(function(k) {
    if (SECTION_SCALAR_KEYS.has(k)) return;
    if (k === 'servicesMeta') return;
    // Root service cards are an array on site_config.services — not a section object.
    if (k === 'services' && Array.isArray(src[k])) return;
    if (Array.isArray(src[k])) return;
    if (src[k] && typeof src[k] === 'object') {
      out.sections[k] = Object.assign({ on: true }, src[k]);
    }
  });
  if (sectionKey && src[sectionKey] && !Array.isArray(src[sectionKey]) && !out.sections[sectionKey]) {
    out.sections[sectionKey] = Object.assign({ on: true }, src[sectionKey]);
  }
  if (Array.isArray(src.services)) {
    out.services = clone(src.services);
  }
  if (src.servicesMeta && typeof src.servicesMeta === 'object' && !Array.isArray(src.servicesMeta)) {
    out.sections.services = Object.assign({ on: true }, clone(src.servicesMeta));
  }
  return out;
}
...
function siteConfigToFlatDemo(siteConfig) {
  ...
  if (sc.sections) {
    Object.keys(sc.sections).forEach(function(k) {
      // Keep root services[] for cards; section chrome goes to servicesMeta.
      if (k === 'services' && Array.isArray(sc.services)) {
        flat.servicesMeta = clone(sc.sections.services);
        return;
      }
      flat[k] = clone(sc.sections[k]);
    });
  }
  if (Array.isArray(sc.services)) flat.services = clone(sc.services);
  return flat;
}
