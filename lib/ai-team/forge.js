...
function stepsFromRecommendation(recommendation, snapshot, config) {
  ...
  if (outcome === 'plan_seo_landing') {
    const outline = Array.isArray(change.planOutline) && change.planOutline.length
      ? change.planOutline
      : [
          'Confirm the offer, audience, and location for this landing page.',
          'Lock primary search phrase and location.',
          'Write a short brief (headline, proof, objections, CTA).',
          'Create a draft landing page in Landing pages.',
          'Review, then you Publish Live Site yourself.'
        ];
    outline.forEach(function (text, i) {
      steps.push(
        makeStep({
          title: String(text),
          label: String(text),
    ...
    return { steps, knowledge, advisory: true };
  }
