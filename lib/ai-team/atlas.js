...
async function discussRecommendation(input) {
  ...
  const patched = await siteBrain.patchRecommendation(
    ...
    function (rec) {
      ...
      if (!outline.length) {
        outline = defaultPlanOutline(outcome, { topic: rec.title, requestText: message });
      }
      if (message) {
        outline = refinePlanOutline(outline, message, outcome, { topic: rec.title });
      }
      ...
      if (message) {
        ...
        messages.push({
          role: 'atlas',
          body: discussReply(rec, message, outline),
          at: new Date().toISOString()
        });
      }
