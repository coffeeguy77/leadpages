  ...
  if (!goal) {
    recs.push(
      attachGuardian({
        id: randomUUID(),
        specialist: 'atlas',
        title: 'Confirm the primary business goal',
        problem: 'Site Brain does not yet have a verified primary goal.',
        evidence: ['Site Brain goals.primary is empty or unverified'],
        proposedChange: {
          type: 'site_brain_update',
          path: 'goals.primary',
          note: 'Complete bootstrap review or Site Knowledge'
        },
        reason: 'Specialists need a clear outcome to prioritise improvements.',
  ...
  if (!cta || /get in touch|contact us|learn more/i.test(String(cta))) {
    recs.push(
      attachGuardian({
        ...
        problem: 'CTA copy is generic or missing.',
        evidence: ['goals.preferredCta is empty or generic'],
        proposedChange: {
          type: 'advisory',
          capabilityId: 'hero',
          sectionKey: 'hero',
          field: 'cta',
          note: 'Echo/Forge can rewrite in a later phase; confirm preferred CTA in Site Brain now'
        },
        reason: 'Specific CTAs convert better than generic contact language.',
        estimatedEffort: 'small',
        affectedAreas: ['hero', 'conversion'],
        requiredPermissions: ['editor'],
        risk: 'low',
        status: 'proposed',
        executable: false,
  ...
  if (services.length && services.length < 3) {
    recs.push(
      attachGuardian({
        id: randomUUID(),
        specialist: 'atlas',
        title: 'Expand the service list in Site Knowledge',
        problem: 'Fewer than three main services are recorded.',
        evidence: ['offers.mainServices length < 3'],
        proposedChange: {
          type: 'site_brain_update',
          path: 'offers.mainServices'
        },
        reason: 'Clearer service inventory improves SEO and homepage structure advice.',
        estimatedEffort: 'small',
        affectedAreas: ['Site Brain', 'services'],
        requiredPermissions: ['mutate_brain'],
        risk: 'low',
        status: 'proposed',
        executable: false,
