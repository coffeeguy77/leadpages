...
async function snapshotSite(site) {
  ...
  // Prefer existing smart site_backups table when available (no second backup system)
  if (!memoryEnabled()) {
    try {
      const { admin } = require('../brain/http');
      const { createBackup } = require('../site-backups/service');
      const bk = await createBackup(admin, {
        siteId: site.id,
        config: snap.config,
        source: 'website_studio',
        label: 'Website Studio replacement snapshot'
      });
      if (bk && bk.ok && bk.backup) {
        snap.site_backup_id = bk.backup.id;
