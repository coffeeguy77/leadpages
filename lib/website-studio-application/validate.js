...
/** Basic email shape — rejects values like `a@.com` that only contain `@`. */
...
function validateForApplication(input) {
  ...
  if (mode === MODES.CREATE_SITE || mode === MODES.REPLACEMENT_DRAFT) {
    if (!looksLikeEmail(contact.leadRecipientEmail)) {
      issues.push({
        severity: 'critical',
        code: 'lead_recipient_required',
        message: 'Confirm a valid lead recipient email before application',
        path: 'contact.leadRecipientEmail'
      });
    }
    if (!looksLikeEmail(contact.businessEmail)) {
      issues.push({
        severity: 'critical',
        code: 'business_email_required',
        message: 'Confirm a valid business email before application',
