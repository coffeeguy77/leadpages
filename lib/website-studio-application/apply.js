...
async function planApplication(input) {
  ...
  const imageFinalisation = finaliseImagesForApplication(prepared.version.draft_config_json, {
    siteId: 'pending',
    draftId: prepared.draft.id,
    mockImport: memoryEnabled() || input.mockImages === true,
...
async function commitApplication(input) {
  ...
  if (idempotencyKey) {
    const prior = getIdempotentResult(idempotencyKey);
  ...
  if (!validation.ok) {
    const audit = recordApplicationAudit({
    ...
    return {
      ...
      message: 'Pre-application validation failed',
      plan,
      validation,
      audit: audit.audit
    };
  ...
  if (idempotencyKey) setIdempotentResult(idempotencyKey, payload);
  return payload;
}
...
async function commitCreateSite({ input, plan, imageFinalisation, actor }) {
  ...
  const images = finaliseImagesForApplication(input.version.draft_config_json, {
    siteId: slug,
...
async function commitReplacementDraft({ input, plan, imageFinalisation, actor }) {
  ...
  const snap = snapshotSite(targetSite);
