...
const MARKETING_ALIASES = {
  ...
  'email-campaigns': {
    ...
    explainer: [
      ...
      { title: 'Try the demo', text: 'Play with fake clients, sample campaign messages, preset hero images (no upload), and unsubscribed contacts. Nothing is emailed or saved.' },
      { title: 'Where you find it', text: 'Open any site in the App Command Centre → Email clients / Newsletter tab. Campaigns are per-site, not a public page widget.' },
      { title: 'Safety', text: 'Recipients who opt out are skipped on real sends. This marketplace demo never calls the campaign API.' }
    ],
    cta: {
...
function premiumShowcaseFeature(slug, alias) {
  const cta = alias.cta || {};
  /* No benefits / process cards — hero + live example only (avoids duplicating copy). */
  const blocks = [];
  if (alias.showcaseDemoUrl) {
    blocks.push({
      id: 'prem-' + slug + '-demo',
