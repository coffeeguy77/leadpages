...
const MARKETING_ALIASES = {
  'quote-lead-capture': { sectionKey: 'onlineQuote', name: 'Quote & Lead Capture', tagline: 'Enquiries that never get lost', summary: 'Quote forms and click-to-call wired into a lead inbox, so every enquiry lands where your client will see it.', badge: 'Popular' },
  ...
  promotions: { sectionKey: 'specialOffer', name: 'Promotions & Offers', tagline: 'Seasonal offers, front and centre', summary: 'Highlight timed deals and specials so visitors act now — without a separate marketing tool.' },
  'email-campaigns': {
    sectionKey: null,
    platform: true,
    ...
    explainer: [
      { title: 'What it does', text: 'Compose a subject and message, pick leads from your inbox, and send or schedule — delivery runs through LeadPages mail, with opt-outs honoured automatically.' },
      { title: 'Who it’s for', text: 'Site owners who already capture quotes and want repeat work without exporting CSVs to Mailchimp.' },
      { title: 'Where you find it', text: 'Open any site in the App Command Centre → Email clients tab. Campaigns are per-site, not a public page widget.' },
      { title: 'Safety', text: 'Recipients who opt out are skipped. Nothing on this marketplace page sends mail — it’s an explainer only.' }
    ]
  }
};
...
function ensurePlayground(blocks, sectionKey) {
  ...
  if (hasPlayground(list)) {
    return list.map(function (b) {
      ...
      if (!payload.section_key) payload.section_key = sectionKey;
      if (!payload.presets || !payload.presets.length) payload.presets = ['default'];
...
function platformFeature(slug, alias) {
  ...
  const blocks = points.map(function (p, i) {
    return {
      id: 'plat-' + slug + '-' + i,
      sort_order: (i + 1) * 10,
      block_type: 'benefits',
      payload: {
        heading: i === 0 ? 'How it works' : '',
        items: [{ title: p.title, text: p.text }]
      }
    };
  });
  // Flatten explainers into one benefits block for cleaner UI
  ...
  return {
    ...
    blocks: [{
      ...
      sort_order: 10,
      block_type: 'benefits',
      payload: { heading: 'How it works', items: flatItems }
    }, {
      id: 'plat-' + slug + '-cta',
      sort_order: 20,
      block_type: 'cta',
      payload: {
        ...
        text: 'Email campaigns live under Email clients for each site — not as a page section demo.',
        button_label: 'Go to partners',
        button_url: '/partners'
      }
    }]
  };
...
function enrichCatalogPayload(feature, blocks, slug) {
  ...
  let sectionKey = (feature && feature.section_key) || (alias && alias.sectionKey) || null;
  if (!sectionKey && slug) {
  ...
  if (!feat && sectionKey) {
    ...
    if (!feat.section_key && sectionKey) feat.section_key = sectionKey;
    if (alias) {
