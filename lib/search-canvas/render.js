...
function renderSearchCanvasHtml(rawConfig, opts) {
  ...
  const widthClass = 'sc-width-' + (cfg.layout.contentWidth || 'site');
  ...
  if (cfg.cta && cfg.cta.enabled && (cfg.cta.heading || cfg.cta.text || cfg.cta.primaryLabel)) {
    const pHref = destHref(cfg.cta.primaryDestination);
      ...
      (cfg.cta.primaryLabel
        ? pHref
          ? '<a class="sc-btn" href="' +
            escAttr(pHref) +
            '" data-sc-cta="primary">' +
            esc(cfg.cta.primaryLabel) +
            '</a>'
          : '<span class="sc-btn" data-sc-cta="primary">' + esc(cfg.cta.primaryLabel) + '</span>'
        : '') +
