...
function normalizeTab(raw, i) {
  ...
  return {
    ...
    bulletIconKey: t.bulletIconKey == null || t.bulletIconKey === '' ? null : String(t.bulletIconKey).trim(),
    ...
    image: {
      ...
      fit: (function () {
        var f = String(img.fit || '').toLowerCase();
        if (f === 'stretch') f = 'fill';
...
function normalizeSearchCanvas(raw) {
  ...
  return {
    ...
    style: {
      ...
      bulletIconKey: styleIn.bulletIconKey == null || styleIn.bulletIconKey === ''
        ? 'check'
        : String(styleIn.bulletIconKey).trim(),
...
/**
 * Place SearchCanvas after How It Works (serviceProcess), else Services.
 * Only inserts when missing. Relocates early placements only when opts.force=true
 * so manual Position (including above How It Works) is respected.
 * @param {string[]} sectionOrder
 * @param {string[]} [layoutSections]
 * @param {{ force?: boolean }} [opts] force=true re-pins after How It Works/Services
 */
function ensureSearchCanvasInOrder(sectionOrder, layoutSections, opts) {
  ...
  // Already present — keep user's Position unless force re-pin requested.
  if (sci >= 0 && !force) return ord;
