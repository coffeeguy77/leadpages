...
function normalizeSearchCanvas(raw) {
  ...
  tabs = tabs.filter(function (t) { return t && (t.label || t.heading || t.intro); }).slice(0, 8);
  if (!tabs.length) tabs = base.tabs.map(function (t) { return normalizeTab(t, 0); });
  ...
  return {
    ...
    layout: {
      ...
      contentWidth: ['site', 'narrow', 'wide'].indexOf(layoutIn.contentWidth) >= 0 ? layoutIn.contentWidth : 'site'
    },
    cta: {
      ...
      primaryLabel: String(ctaIn.primaryLabel || '').trim(),
...
function applyAiDraftToSearchCanvas(existing, aiDraft, opts) {
  ...
  if (aiTabs.length) {
    if (mode === 'replace' || !out.tabs.length) {
      out.tabs = aiTabs.map(function (t, i) {
        return normalizeTab(
          {
            id: (out.tabs[i] && out.tabs[i].id) || newId('tab'),
            label: t.label,
            iconKey: (out.tabs[i] && out.tabs[i].iconKey) || t.iconSuggestion || t.iconKey || 'check',
            heading: t.heading || t.label,
            intro: t.intro,
            content: Array.isArray(t.supportingParagraphs) ? t.supportingParagraphs.join('\n\n') : t.content,
            bullets: t.bullets,
            image: replaceImages || !(out.tabs[i] && out.tabs[i].image && out.tabs[i].image.url)
              ? {
                  url: (out.tabs[i] && out.tabs[i].image && out.tabs[i].image.url) || null,
                  publicId: null,
                  alt: t.imageAltText || '',
                  fit: 'cover',
              ...
              : out.tabs[i].image,
            link: {
              label: t.linkLabel || (out.tabs[i] && out.tabs[i].link && out.tabs[i].link.label) || '',
              destination: (out.tabs[i] && out.tabs[i].link && out.tabs[i].link.destination) || null
            },
      ...
      aiTabs.slice(0, 8).forEach(function (t, i) {
        if (!out.tabs[i]) {
...
function ensureSearchCanvasInOrder(sectionOrder, layoutSections) {
  const ord = Array.isArray(sectionOrder) ? sectionOrder.slice() : [];
  if (ord.indexOf('searchCanvas') >= 0) return ord;
  const after = ['serviceProcess', 'services', 'hero', 'heroSlider', 'splitHero'];
  for (let i = 0; i < after.length; i++) {
    const ix = ord.indexOf(after[i]);
    if (ix >= 0) {
