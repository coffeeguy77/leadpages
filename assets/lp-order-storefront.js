...
(function () {
  ...
  function missingRequiredQuestions(questions, answers) {
    var missing = [];
    (questions || []).forEach(function (q) {
      if (!q || !q.required || q.staff_only) return;
      var key = q.key || q.id;
      var raw = answers && (answers[key] != null ? answers[key] : answers[q.id]);
      if (!answerValues(raw).length) missing.push(q);
    });
    return missing;
  }
  ...
  function firstName(name) {
    // Title-cased given name. Keep in sync with lib/order/customer-name.js
    return displayFullNameClient(name).split(/\s+/)[0] || '';
  }

  function displayFullNameClient(name) {
    ...
    if (raw.indexOf(',') >= 0) {
      ...
      if (commaBits.length >= 2) {
        ...
        return commaBits.map(function (b) {
          return b.split(/\s+/).map(titleCaseWord).join(' ');
        }).join(' ');
      }
    }
    ...
    if (isAllCapsWord(parts[0]) && !isAllCapsWord(parts[parts.length - 1])) {
      return titleCaseWord(parts[parts.length - 1]) + ' ' + parts.slice(0, -1).map(titleCaseWord).join(' ');
    }
  ...
  OrderStorefront.prototype.customerDisplayName = function () {
    ...
    else if (b.split(/\s+/).length > a.split(/\s+/).length) pick = b;
    else if (/[a-z]/.test(b) && !/[a-z]/.test(a)) pick = b;
  ...
  OrderStorefront.prototype.prefillCheckoutFromCustomer = function () {
    ...
    if (!d.name && c.name) d.name = displayFullNameClient(c.name);
    else if (d.name) d.name = displayFullNameClient(d.name);
  ...
  OrderStorefront.prototype.renderLoadingShell = function () {
    if (!this.root) return;
    this.root.innerHTML =
      '<div class="lp-oe lp-oe-loading"><p class="lp-oe-sub" style="margin:0;padding:24px 0">Loading menu…</p></div>';
  };

  OrderStorefront.prototype.refreshCartChrome = function () {
    var aside = document.getElementById('oe-aside');
    if (aside) {
      aside.innerHTML = this.renderLiveCart();
    }
    var n = qtyTotal(this.state.items);
    this.root.querySelectorAll('.lp-oe-mobile-cart').forEach(function (btn) {
      btn.textContent = 'Cart (' + n + ')';
    });
    this.bindCartActions();
  };

  OrderStorefront.prototype.bindCartActions = function () {
    var self = this;
    var aside = document.getElementById('oe-aside');
    if (!aside) return;
    aside.querySelectorAll('[data-act]').forEach(function (el) {
  ...
  OrderStorefront.prototype.updateProductAddButton = function () {
    if (this.state.view !== 'product' || !this.state.selected) return;
    var product = this.state.selected;
    var btn = this.root.querySelector('[data-act="add-product"]');
    if (!btn) return;
    var answers = this.collectAnswers(this.root);
    var missing = missingRequiredQuestions(product.questions || [], answers);
    var label = fastAddButtonLabel(product.questions || [], answers, false);
    btn.textContent = label === 'Add' ? 'Add to cart' : label;
    btn.classList.toggle('needs-options', missing.length > 0);
    btn.disabled = !!this.state.busy;
  };

  OrderStorefront.prototype.updateFastAddButtons = function () {
    var self = this;
    this.captureFastDrafts();
    this.root.querySelectorAll('[data-fast-row]').forEach(function (row) {
      var id = row.getAttribute('data-fast-row');
      ...
      if (!product) return;
      var btn = row.querySelector('[data-act="add-inline"]');
      if (!btn) return;
      var draft = self.state.fastDrafts[id] || {};
      var answers = draft.answers || {};
      var missing = missingRequiredQuestions(product.questions || [], answers);
      var label = fastAddButtonLabel(product.questions || [], answers, false);
      btn.textContent = label;
      btn.classList.toggle('needs-options', missing.length > 0);
      btn.disabled = !!self.state.busy;
    });
  };

  OrderStorefront.prototype.fastRowAnswers = function (row, productId) {
    var draft = this.state.fastDrafts[productId] || {};
    if (row) {
      var collected = this.collectAnswers(row);
      var out = {};
      Object.keys(collected).forEach(function (k) {
        out[k] = collected[k];
      });
    ...
    var ans = draft.answers || {};
    var mapped = {};
    Object.keys(ans).forEach(function (k) {
      mapped[k] = { value: ans[k], label: k };
    });
    return mapped;
  };
  ...
  OrderStorefront.prototype.applyAppearance = function () {
    ...
    function sectionBg() {
      ...
      try {
        var bg = section.style.background || section.style.backgroundColor || '';
        bg = String(bg || '').trim();
        if (/^#[0-9a-fA-F]{6}$/.test(bg)) return bg;
        var m = /^rgba?\((\d+)\s*,\s*(\d+)\s*,\s*(\d+)/i.exec(bg);
        if (m) {
          function hx(n) { n = Number(n); return ('0' + n.toString(16)).slice(-2); }
    ...
    var map = {
      ...
      '--lp-oe-page-bg': sectionVar('--lp-oe-page-bg') || sectionBg() || a.page_bg || ''
    };
    Object.keys(map).forEach(function (k) {
      if (map[k]) el.style.setProperty(k, map[k]);
      else el.style.removeProperty(k);
    });
    // Keep catalogue + cart aliases in lockstep with section/API colours so
    // render-inject rules (--accent/--card) and JS rules (--oe-*) match the CSS.
    var aliases = {
      '--oe-accent': map['--lp-oe-accent'],
      '--oe-card': map['--lp-oe-card'],
      '--oe-line': map['--lp-oe-line'],
      '--oe-ink': map['--lp-oe-ink'],
      '--oe-muted': map['--lp-oe-muted'],
      '--oe-btn': map['--lp-oe-btn-bg'] || map['--lp-oe-accent'],
      '--oe-btn-text': map['--lp-oe-btn-text'],
      '--accent': map['--lp-oe-accent'],
      '--card': map['--lp-oe-card'],
      '--line': map['--lp-oe-line'],
      '--ink': map['--lp-oe-ink'],
      '--muted': map['--lp-oe-muted']
    };
    Object.keys(aliases).forEach(function (k) {
      if (aliases[k]) el.style.setProperty(k, aliases[k]);
      else el.style.removeProperty(k);
    });
  };
  ...
  OrderStorefront.prototype.init = async function () {
    try {
      ...
      var catalogPromise = api('/api/order/storefront?slug=' + encodeURIComponent(slug));
      var sessionPromise = this.restoreCustomerSession();
      var cartPromise = cartId
        ? api(
            '/api/order/cart?slug=' +
      ...
      if (!this.state.cartId) {
        var warmSelf = this;
        this.ensureCart().catch(function () {});
      }
      await sessionPromise;
      var packed = await cartPromise;
      if (packed) {
        this.applyPacked(packed);
        this.refreshCartChrome();
      }
      try {
  ...
  OrderStorefront.prototype.render = function () {
    ...
    if (this.mode === 'embedded') {
      ...
      var secEy = this.root.getAttribute('data-eyebrow') || 'Order online';
      var secHead = this.root.getAttribute('data-heading') || ((c && c.site && c.site.business_name) || 'Order');
      var secIntro =
        this.root.getAttribute('data-intro') ||
        'Browse the menu, adjust your cart, and checkout on one page.';
      html +=
        '<header class="lp-oe-head"><div><p class="lp-oe-ey">' +
        esc(secEy) +
        '</p><h2 class="lp-oe-brand">' +
        esc(secHead) +
        '</h2><p class="lp-oe-sub">' +
        esc(secIntro) +
        '</p></div>';
      html += '<div class="lp-oe-head-actions">';
      html +=
  ...
  OrderStorefront.prototype.renderFastRow = function (p) {
    ...
    Object.keys(draft.answers || {}).forEach(function (k) {
      answerMap[k] = { value: draft.answers[k] };
    });
    var addLabel = fastAddButtonLabel(p.questions || [], answerMap, false);
    ...
    html +=
      ...
      '">Notes</button>';
    html +=
      '<button type="button" class="lp-oe-fast-add' +
      (addLabel === 'Choose Options' ? ' needs-options' : '') +
      '" data-act="add-inline" data-id="' +
      esc(id) +
      '"' +
  ...
  OrderStorefront.prototype.renderProduct = function (p) {
    ...
    html += '</div>';
    var detailAddLabel = fastAddButtonLabel(p.questions || [], {}, false);
    html +=
      '<button type="button" class="lp-oe-primary' +
      (detailAddLabel === 'Choose Options' ? ' needs-options' : '') +
      '" data-act="add-product" data-id="' +
      esc(p.id) +
      '"' +
      (this.state.busy ? ' disabled' : '') +
      '>' +
      (detailAddLabel === 'Add' ? 'Add to cart' : detailAddLabel) +
      '</button>';
  ...
  OrderStorefront.prototype.renderLiveCart = function () {
    ...
    html +=
      '<label>Your name<input id="oe-name" required value="' +
      esc(displayFullNameClient(d.name || '') || d.name || '') +
      '"></label>';
    html +=
  ...
  OrderStorefront.prototype.onAct = async function (act, el) {
    ...
    try {
      ...
      if (act === 'confirm') {
        ...
        var name = displayFullNameClient(draft.name) || draft.name;
        draft.name = name;
  ...
  function ensureStyles() {
    ...
    st.textContent = [
      '.lp-oe{--oe-accent:var(--lp-oe-accent,var(--hivis,var(--accent,var(--pipe,#1f7a63))));--oe-card:var(--lp-oe-card,var(--card,var(--paper,#fffcf7)));--oe-line:var(--lp-oe-line,var(--line,#e4e0d8));--oe-ink:var(--lp-oe-ink,var(--ink,#1c241e));--oe-muted:var(--lp-oe-muted,var(--muted,#667066));--oe-btn:var(--lp-oe-btn-bg,var(--oe-accent));--oe-btn-text:var(--lp-oe-btn-text,#fff);--oe-radius:var(--lp-oe-radius,14px);--oe-pad:var(--lp-oe-pad,16px);--oe-maxw:var(--lp-oe-maxw,1440px);--accent:var(--oe-accent);--card:var(--oe-card);--line:var(--oe-line);--ink:var(--oe-ink);--muted:var(--oe-muted);color:var(--oe-ink);font:400 14px/1.45 system-ui,-apple-system,Segoe UI,sans-serif;max-width:var(--oe-maxw);margin:0 auto;padding:var(--oe-pad);box-sizing:border-box;background:var(--lp-oe-page-bg,transparent)}',
      '.lp-oe *,.lp-oe *::before,.lp-oe *::after{box-sizing:border-box}',
      '.lp-oe-ey{margin:0 0 6px;font-size:13px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:var(--oe-accent)}',
      '.lp-oe-brand{font-size:clamp(32px,4.4vw,52px);margin:10px 0 0;font-weight:650;letter-spacing:-.02em;line-height:1.15}',
      '.lp-oe-sub{margin:13px 0 0;color:var(--oe-muted);font-size:18px;font-weight:500;line-height:1.45;max-width:62ch}',
      ...
      '.lp-oe-client-tab.on{color:var(--oe-accent);border-color:color-mix(in srgb,var(--oe-accent) 30%,var(--oe-line));background:var(--oe-card);box-shadow:0 4px 12px rgba(28,36,30,.05)}',
      ...
      '.lp-oe-layout{display:grid;grid-template-columns:minmax(0,2fr) minmax(0,1fr);gap:22px;align-items:start}',
      ...
      '.lp-oe-aside h3{margin:0 0 10px;font-size:22px}',
      '.lp-oe-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:16px}',
      '.lp-oe-list{display:flex;flex-direction:column;gap:12px;padding:8px 4px 16px;margin:0}',
      '.lp-oe-list .lp-oe-card{display:grid;grid-template-columns:88px minmax(0,1fr) auto;gap:14px;align-items:center;padding:16px 18px}',
      ...
      '.lp-oe-stepper-btn{appearance:none;border:0;background:color-mix(in srgb,var(--oe-accent) 8%,var(--oe-card));color:var(--oe-ink);width:44px;min-width:44px;font:700 22px/1 system-ui,sans-serif;cursor:pointer;padding:0}',
      '.lp-oe-stepper-btn:hover{background:color-mix(in srgb,var(--oe-accent) 16%,var(--oe-card))}',
      '.lp-oe-stepper-btn:active{background:color-mix(in srgb,var(--oe-accent) 22%,var(--oe-card))}',
      ...
      '.lp-oe-option-chip.on{border-color:var(--oe-accent);background:color-mix(in srgb,var(--oe-accent) 10%,var(--oe-card));color:var(--oe-accent)}',
      '.lp-oe-list .lp-oe-img{aspect-ratio:1;width:88px;border-radius:12px;margin:0;flex:none}',
      '.lp-oe-list .lp-oe-body{padding:0;min-width:0}',
      '.lp-oe-list .lp-oe-body h3{font-size:30px;margin:0 0 4px;line-height:1.2;letter-spacing:-.02em}',
      '.lp-oe-list .lp-oe-body p{margin:0;font-size:15px}',
      '.lp-oe-list .lp-oe-price{margin-top:6px;font-size:16px}',
      ...
      '.lp-oe-img.text span{font-size:48px;color:var(--oe-accent);opacity:.7}',
      '.lp-oe-body{padding:16px}',
      '.lp-oe-body h3{margin:0 0 8px;font-size:32px;line-height:1.2;letter-spacing:-.02em;font-weight:650}',
      '.lp-oe-body p,.lp-oe-desc{margin:0;color:var(--oe-muted);font-size:15px;line-height:1.45}',
      '.lp-oe-price{margin-top:10px;font-weight:700;color:var(--oe-accent);font-size:16px}',
      ...
      '.lp-oe-qty-btn{appearance:none;border:0;background:color-mix(in srgb,var(--oe-accent) 8%,var(--oe-card));width:44px;height:44px;font:700 22px/1 system-ui,sans-serif;cursor:pointer;color:var(--oe-ink)}',
      '.lp-oe-qty-btn:hover{background:color-mix(in srgb,var(--oe-accent) 16%,var(--oe-card))}',
      ...
      '.lp-oe-fast-info h3{margin:0 0 4px;font-size:30px;line-height:1.2;letter-spacing:-.02em;font-weight:650}',
      '.lp-oe-fast-desc{margin:0;color:var(--oe-muted);font-size:14px;line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}',
      '.lp-oe-fast-info .lp-oe-price{margin-top:6px;font-size:15px}',
      ...
      '.lp-oe-fast-add.needs-options{background:transparent;color:var(--oe-accent);border-color:color-mix(in srgb,var(--oe-accent) 45%,var(--oe-line))}',
      '.lp-oe-primary.needs-options{background:transparent;color:var(--oe-accent);border-color:color-mix(in srgb,var(--oe-accent) 45%,var(--oe-line))}',
