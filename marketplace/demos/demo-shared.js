...
(function(){
  ...
  function applyCfg(C){
    C=C||{}; var rs=document.documentElement.style, th=C.theme||{};
    ...
    if(Array.isArray(C.services)){ var grid=document.querySelector('.svcs'); if(grid){ var on=C.services.filter(function(s){return s&&s.on!==false&&(s.title||s.body||s.icon);}); grid.innerHTML=on.map(function(s){return '<div class="svc"><div class="ic">'+lpIcon(s.icon)+'</div><h3>'+esc(s.title||'')+'</h3><p>'+esc(s.body||'')+'</p></div>';}).join(''); } }
    if(C.logo){ var L=C.logo, brands=document.querySelectorAll('a.brand'); var _lhd=Math.max(20,Math.min(320,(L.heightPx?L.heightPx:Math.round(40*(L.scale||1)))||44)); var _lhm=Math.max(16,Math.min(200,(L.heightPxM?L.heightPxM:Math.round(_lhd*0.72)))); var _lbg=(L.bg&&L.bg!=='transparent')?L.bg:'transparent'; var _lpad=(L.pad!=null?L.pad:(_lbg!=='transparent'?8:0)); var _lrad=(L.radius!=null?L.radius:(_lbg!=='transparent'?12:0)); var _lp=L.position||'left'; var _hs=L.headerStyle||(L.float?'float':'solid-sticky'); var _hbg=(L.headerBg&&L.headerBg!=='')?L.headerBg:''; var _pos='sticky',_top=';top:0',_bg=(_hbg?(';background:'+_hbg):''); if(_hs==='solid-scroll'){ _pos='relative'; _top=''; } else if(_hs==='float'){ _pos='absolute'; _top=';top:0;left:0;right:0'; _bg=';background:transparent;box-shadow:none'; } else if(_hs==='shrink'){ _pos='sticky'; _bg=';background:transparent;box-shadow:none'; } var _hdrBase='header.site{position:'+_pos+_top+';z-index:60'+_bg+'}'; var _shrinkCss=(_hs==='shrink')?('header.site{transition:background .25s,box-shadow .25s}header.site img.lp-logo{transition:height .25s}'+'header.site.lp-shrunk{background:'+(_hbg||'rgba(10,12,18,.94)')+';box-shadow:0 6px 24px rgba(0,0,0,.28)}'+'header.site.lp-shrunk .bar{min-height:0;padding-top:6px;padding-bottom:6px}'+'header.site.lp-shrunk a.brand{margin-left:0;margin-right:auto}'+'header.site.lp-shrunk img.lp-logo{height:'+Math.min(_lhd,52)+'px}'):''; var _hdrCss=_hdrBase+_shrinkCss; (function(){ var _h=document.querySelector('header.site'); if(!_h) return; if(_hs==='shrink'){ if(!_h.__lpScroll){ _h.__lpScroll=function(){ if(window.scrollY>60) _h.classList.add('lp-shrunk'); else _h.classList.remove('lp-shrunk'); }; window.addEventListener('scroll',_h.__lpScroll,{passive:true}); } _h.__lpScroll(); } else { _h.classList.remove('lp-shrunk'); if(_h.__lpScroll){ window.removeEventListener('scroll',_h.__lpScroll); _h.__lpScroll=null; } } })(); var _st=document.getElementById('lp-logo-css'); if(!_st){ _st=document.createElement('style'); _st.id='lp-logo-css'; document.head.appendChild(_st); } _st.textContent='header.site .bar{height:auto;min-height:70px;padding-top:10px;padding-bottom:10px}'+'a.brand{max-width:100%}'+_hdrCss+'a.brand .lp-logo-wrap{display:inline-flex;align-items:center;background:'+_lbg+';padding:'+_lpad+'px;border-radius:'+_lrad+'px;line-height:0}'+'a.brand img.lp-logo{height:'+_lhd+'px;width:auto;max-width:min(62vw,440px);object-fit:contain;display:block}'+'@media(max-width:760px){a.brand img.lp-logo{height:'+_lhm+'px;max-width:72vw}}'; for(var i=0;i<brands.length;i++){ var a=brands[i]; if(a.__orig==null) a.__orig=a.innerHTML; if(L.mode==='image'&&L.imageUrl){ a.innerHTML='<span class="lp-logo-wrap"><img class="lp-logo" src="'+esc(L.imageUrl)+'" alt="'+esc(L.text||'')+'"></span>'; a.style.transform=''; } else { a.innerHTML=a.__orig; var mark=a.querySelector('.mark'), nm=a.querySelector('.nm'); if(L.mode==='text'&&mark) mark.style.display='none'; if(nm&&L.text){ var sm=nm.querySelector('small'); nm.textContent=L.text; if(sm) nm.appendChild(sm); } a.style.transformOrigin='left center'; var _ls=_lhd/40; a.style.transform=(_ls!==1?'scale('+_ls+')':''); } var _inHN=(C.sections&&C.sections.navMenu&&C.sections.navMenu.on===true&&C.sections.navMenu.placement==='header'); if(_inHN){ a.style.marginLeft=''; a.style.marginRight=''; } else { a.style.marginLeft=((_lp==='right'||_lp==='center')?'auto':''); a.style.marginRight=((_lp==='center')?'auto':''); } } }
    var SEC=C.sections||{};
    ...
    try{
      ...
      var __hbStyle=function(el,bg,bd,fg){ if(!el)return; el.style.background=(bg&&String(bg).trim())?bg:''; el.style.color=(fg&&String(fg).trim())?fg:''; if(bd&&String(bd).trim()){ el.style.border='2px solid '+bd; } else { el.style.border=''; } };
      ...
      if(__call){ if(__H.callOn===false){ __call.style.display='none'; } else { __call.style.display=''; var __ct=__subT((__H.callText!=null&&__H.callText!=='')?__H.callText:'📞 Call {{phoneText}}'); __hbContent(__call,__H.callIcon,__H.callIconOnly===true,__ct,__ct); __hbStyle(__call,__H.callBg,__H.callBorder,__H.callFg); if(__H.callAction==='link'&&__H.callHref){ __call.setAttribute('href',__H.callHref); } else { __call.setAttribute('href','tel:'+__ph); } } }
      var __q=document.querySelector('.hero .btn-quote');
      if(__q){ if(__H.quoteOn===false){ __q.style.display='none'; } else { __q.style.display=''; var __qt=__subT((__H.quoteText!=null&&__H.quoteText!=='')?__H.quoteText:'Get a fast quote'); __hbContent(__q,__H.quoteIcon,__H.quoteIconOnly===true,__qt,__qt); __hbStyle(__q,__H.quoteBg,__H.quoteBorder,__H.quoteFg); if(__H.quoteAction==='call'){ __q.setAttribute('href','tel:'+__ph); } else if(__H.quoteAction==='link'&&__H.quoteHref){ __q.setAttribute('href',__H.quoteHref); } else { __q.setAttribute('href','#quote'); } } }
    ...
    try{ ['navMenu','beforeAfter','responseCards','projectStats','serviceAreas','reviewHighlights','featuredProjects','specialOffer','heroBeforeAfter','heroSlider','splitHero','activityCounter','proofStream','projectFeed','jobsFeed','beforeAfterFeed','videoReels','activityTimeline','customerReactions','textBox'].forEach(function(__id){ var __n=document.querySelector('[data-sec="'+__id+'"]'); if(!__n) return; if(SEC[__id] && SEC[__id].on===true){ __n.style.display='block'; } }); }catch(e){}
      ...
      if(fpNode){
        ...
        if(FP.on===false||!_fit.length){ fpNode.style.display='none'; }
          ...
          if(_fg){ _fg.innerHTML=_fit.map(function(it){
            ...
            var _thumbs=_multi?('<div class="fp-thumbs">'+_gal.slice(0,4).map(function(u,k){return '<button type="button" class="fp-thumb" data-fp-g="'+_gi+'" data-fp-i="'+k+'" aria-label="Photo '+(k+1)+'"><img src="'+esc(u)+'" alt="" loading="lazy"></button>';}).join('')+'</div>'):'';
            var _svc='';
            if(it.services&&String(it.services).trim()){ var _ss=String(it.services).split(',').map(function(s){return s.trim();}).filter(Boolean); if(_ss.length) _svc='<div class="fp-services">'+_ss.map(function(s){return '<span class="fp-chip">'+esc(s)+'</span>';}).join('')+'</div>'; }
            return '<div class="fp-card"><div class="fp-shot">'+_cover+_tag+_open+'</div>'+_thumbs+'<div class="fp-body">'+(it.title?'<h3 class="fp-title">'+esc(it.title)+'</h3>':'')+(it.location?'<div class="fp-loc">'+esc(it.location)+'</div>':'')+(it.description?'<p class="fp-desc">'+esc(it.description)+'</p>':'')+_svc+'</div></div>';
          }).join(''); }
        }
      }
      ...
      if(tbNode){
        ...
        var _tbck='<svg viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"3\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M20 6 9 17l-5-5\"/></svg>'; var _tbIc=function(v){ if(!v) return _tbck; v=String(v); var _i=(window.LP_ICONS&&window.LP_ICONS[v]); if(!_i) return _tbck; return '<svg viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2.4\" stroke-linecap=\"round\" stroke-linejoin=\"round\">'+_i+'</svg>'; }; var _tbr=tbNode.querySelector('.tb-row'); if(_tbr){ var _tbp=_tbit.filter(function(b){return b&&b.on!==false&&b.label&&String(b.label).trim();}).map(function(b){ return '<span class=\"tb-badge\">'+_tbIc(b.icon)+'<span class=\"tb-txt\">'+esc(b.label)+'</span></span>'; }); var _tbpipe=(TB.sep==='pipe'); var _tbsep=_tbpipe?'<span class=\"tb-pipe\" aria-hidden=\"true\">|</span>':''; _tbr.innerHTML=_tbsep?_tbp.join(_tbsep):_tbp.join(''); _tbr.style.columnGap=_tbpipe?'16px':''; } var _tbbg=(TB.bg||'').trim(); if(_tbbg){ tbNode.style.setProperty('--tb-bg',_tbbg); } else { tbNode.style.removeProperty('--tb-bg'); } var _tbfg=(TB.fg||'').trim(); if(_tbfg){ tbNode.style.setProperty('--tb-fg',_tbfg); tbNode.style.setProperty('--tb-icon',_tbfg); } else { tbNode.style.removeProperty('--tb-fg'); tbNode.style.removeProperty('--tb-icon'); } var _tbln=(TB.line||'').trim(); if(_tbln){ tbNode.style.setProperty('--tb-line',_tbln); } else { tbNode.style.removeProperty('--tb-line'); } if(TB.lineOn===false){ tbNode.classList.add('tb-noline'); } else { tbNode.classList.remove('tb-noline'); } tbNode.style.display=(TB.on===false)?'none':'';
      }
      ...
      (function(){
        ...
        function _prPad(){ var fl=document.getElementById('promo-float'); var st=document.getElementById('promo-sticky'); var fh=(fl&&fl.classList.contains('shown'))?fl.offsetHeight:0; var sh=(st&&st.classList.contains('shown'))?st.offsetHeight:0; if(_body){ _body.style.paddingTop=fh?fh+'px':''; _body.style.paddingBottom=sh?sh+'px':''; } if(_hdr){ _hdr.style.top=fh?fh+'px':''; } }
    ...
    (function(){ var S=C.sections||{};
      var nmEl=document.querySelector('a.brand .nm'); var _SCb=(typeof SITE_CONFIG!=='undefined'&&SITE_CONFIG)||{}; var BIZ=(_SCb.business||_SCb.name||_SCb.business_name||_SCb.businessName||(nmEl&&nmEl.textContent.trim())||'');
      function tok(s){ return String(s==null?'':s).replace(/\{\{\s*businessName\s*\}\}/g,BIZ); }
      var E=S.emerg||{}; var em=document.querySelector('.emerg'); if(em && E.text!=null){ var ea=em.querySelector('a'); em.textContent=E.text+' '; if(ea) em.appendChild(ea); }
    ...
    try{
      ...
      var __LB={emerg:'Top Bar',hero:'Hero',heroSlider:'Hero Slider',heroBeforeAfter:'Hero Before/After Slider',trustBar:'Trust Bar',services:'Services',serviceProcess:'Service Process',why:'Why Us',area:'Service Area',reviews:'Reviews',promotions:'Promotions',certifications:'Certifications',finance:'Finance Options',estimateBuilder:'Estimate Builder',serviceAreaMap:'Service Area Map',emergencyAvailability:'Emergency Availability',crew:'Team Members',quote:'Quote Form',faq:'FAQ',footer:'Footer',beforeAfter:'Before & After',responseCards:'Response Cards',projectStats:'Project Stats',serviceAreas:'Service Areas',reviewHighlights:'Review Highlights',featuredProjects:'Project Portfolio',specialOffer:'Special Offer',splitHero:'Split Hero',activityCounter:'Activity Counter',proofStream:'Proof Stream',projectFeed:'Project Feed',jobsFeed:'Jobs Feed',beforeAfterFeed:'Before/After Feed',videoReels:'Video Reels',activityTimeline:'Activity Timeline',customerReactions:'Customer Reactions',textBox:'Text Box'};
  ...
  function _lpActivePage(){ try{ var pages=(SITE_CONFIG&&SITE_CONFIG.pages)||[]; if(!pages.length) return null; var qp=''; try{ qp=new URLSearchParams(location.search).get('page')||''; }catch(e){} var seg=location.pathname.replace(/\/+$/,'').split('/').pop()||''; var slug=String(qp||seg||'').toLowerCase(); if(!slug) return null; for(var i=0;i<pages.length;i++){ var p=pages[i]; if(p&&p.status==='published'&&String(p.slug||'').toLowerCase()===slug) return p; } return null; }catch(e){ return null; } }
  function _lpRenderPage(p){ var main=document.getElementById('top')||document.querySelector('main'); if(!main) return; if(p.title) document.title=p.title; if(p.meta){ var m=document.querySelector('meta[name=\"description\"]'); if(!m){ m=document.createElement('meta'); m.setAttribute('name','description'); document.head.appendChild(m); } m.setAttribute('content',p.meta); } if(!document.getElementById('lp-page-style')){ var st=document.createElement('style'); st.id='lp-page-style'; st.textContent='.lp-page{padding:52px 0 72px;background:var(--light,#eef2f6)}.lp-wrap{max-width:var(--maxw,1440px);margin:0 auto;padding:0 22px;color:var(--ink,#141a24)}.lp-text{max-width:880px;margin:0 auto}.lp-img-wrap-left .lp-text,.lp-img-wrap-right .lp-text{max-width:none;margin:0;overflow:hidden}.lp-page h1{font-size:clamp(28px,5vw,44px);line-height:1.12;margin:0 0 20px}.lp-body{font-size:17px;line-height:1.7}.lp-body h2{font-size:25px;margin:30px 0 10px}.lp-body h3{font-size:19px;margin:22px 0 8px}.lp-body ul{padding-left:20px;margin:12px 0}.lp-body li{margin:6px 0}.lp-body p{margin:14px 0}.lp-figure{margin:0 0 22px}.lp-figure img{max-width:100%;height:auto;display:block;border-radius:12px}.lp-img-center .lp-figure img{margin-left:auto;margin-right:auto}.lp-img-right .lp-figure img{margin-left:auto}.lp-img-wrap-left .lp-figure{float:left;max-width:46%}.lp-img-wrap-right .lp-figure{float:right;max-width:46%}.lp-cta{display:flex;gap:12px;flex-wrap:wrap;margin:34px 0 0;clear:both}.lp-btn{display:inline-block;padding:14px 24px;border-radius:11px;font-weight:800;text-decoration:none;font-size:16px}.lp-call{background:var(--hivis,#ff6a1f);color:#fff}.lp-quote{background:transparent;border:2px solid var(--brand,#1f7bb8);color:var(--brand,#1f7bb8)}.lp-clear{clear:both;height:0}@media(max-width:760px){.lp-img-wrap-left .lp-figure,.lp-img-wrap-right .lp-figure{float:none;max-width:100%;margin:0 0 18px}.lp-text{max-width:none}}'; document.head.appendChild(st); } var phone=(SITE_CONFIG&&SITE_CONFIG.phone)||''; var _sc=(p.showCall!==false)&&!!phone; var _sq=(p.showQuote!==false); var _ctaIn=(_sc?'<a class=\"lp-btn lp-call\" href=\"tel:'+esc(String(phone).replace(/[^0-9+]/g,''))+'\">Call '+esc(phone)+'</a>':'')+(_sq?'<a class=\"lp-btn lp-quote\" href=\"'+esc(_siteBase()||'/')+'\">Get a free quote</a>':''); var cta=_ctaIn?('<div class=\"lp-cta\">'+_ctaIn+'</div>'):''; var _im=(p.imgMode||'center'); var _ip=(p.imgPad!=null?p.imgPad:16); var _fm=(_im==='wrap-left')?('0 '+_ip+'px '+_ip+'px 0'):(_im==='wrap-right')?('0 0 '+_ip+'px '+_ip+'px'):('0 0 '+_ip+'px'); var _fig=p.img?('<figure class=\"lp-figure\" style=\"margin:'+_fm+'\"><img src=\"'+esc(p.img)+'\" alt=\"'+esc(p.h1||p.title||'')+'\"></figure>'):''; var _cls='lp-img-'+(p.img?_im:'none'); main.innerHTML='<section data-sec=\"navMenu\"></section><section class=\"lp-page '+_cls+'\"><div class=\"lp-wrap\"><div class=\"lp-text\">'+(p.h1?'<h1>'+esc(p.h1)+'</h1>':'')+_fig+'<div class=\"lp-body\"><p>'+_lpMd(p.body||'')+'</p></div></div>'+cta+'<div class=\"lp-clear\"></div></div></section>'; try{ _navMenuRender(SITE_CONFIG); }catch(e){} try{ window.scrollTo(0,0); }catch(e){} }
  function _lpFooterApply(C){
    ...
    fn.style.display='';
    var WH='https://res.cloudinary.com/dzx6x1hou/image/upload/v1782665886/leadpages-logo-white.png';
    var BK='https://res.cloudinary.com/dzx6x1hou/image/upload/v1782665885/leadpages-logo-black.png';
    var src=(F.version==='custom'&&F.customUrl)?F.customUrl:(F.version==='black'?BK:WH);
    var img=fn.querySelector('.lp-foot-logo'); if(img){ if(img.getAttribute('src')!==src) img.src=src; var sz=(F.size!=null?+F.size:30); if(isNaN(sz))sz=30; img.style.height=Math.max(10,Math.min(200,sz))+'px'; }
  ...
  function _lpBoot(){ applyCfg(SITE_CONFIG); try{_lpFooterApply(SITE_CONFIG);}catch(_e){} try{ var _p=_lpActivePage(); if(_p) _lpRenderPage(_p); }catch(e){}  try{_lpRichWalk(document.body);}catch(e){} }
  ...
  function _navMenuRender(c){ var node=document.querySelector('[data-sec=\"navMenu\"]'); var nm=(c&&c.sections&&c.sections.navMenu)||{}; var _nmBar=document.querySelector('header.site .bar'); function _nmHeaderClear(){ if(!_nmBar) return; _nmBar.classList.remove('hnav-left','hnav-center','hnav-right'); var _ex=_nmBar.querySelectorAll('.head-nav'); for(var _k=_ex.length-1;_k>=0;_k--){ _ex[_k].parentNode.removeChild(_ex[_k]); } } try{ console.log('[navMenu JUN28-R] style:',nm.style,'icons:',nm.icons,'bg:',nm.bg); }catch(_e){} if(!node) return; if(nm.on!==true){ _nmHeaderClear(); node.style.setProperty('display','none','important'); node.innerHTML=''; return; } function _nmIc(v){ if(!v) return ''; v=String(v); var _i=(window.LP_ICONS&&window.LP_ICONS[v]); if(!_i) return ''; return '<svg class=\"lp-ic\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\">'+_i+'</svg>'; } var items=(Array.isArray(nm.items)?nm.items:[]); var base=_siteBase(); var _imode=(nm.icons||'text'); if((nm.placement||'bar')==='header'){ node.style.setProperty('display','none','important'); node.innerHTML=''; if(_nmBar){ _nmHeaderClear(); var _lpos=(c&&c.logo&&c.logo.position)||'left'; var _brand=_nmBar.querySelector('a.brand'); if(_brand){ _brand.style.marginLeft=''; _brand.style.marginRight=''; } var _hbld=function(arr){ return arr.map(function(it){ if(!it) return ''; var label=esc(it.label||''); var ic=(_imode!=='text')?_nmIc(it.icon):''; if(!label&&!ic) return ''; var href='#',ext=false; if(it.target==='external'){ href=esc(it.url||'#'); ext=true; } else if(it.target&&it.target.indexOf('page:')===0){ href=esc((base+'/'+it.target.slice(5)).replace(/\/+/g,'/')); } else { href=esc(base||'/'); } var txt=((_imode==='icon'&&ic)?'':'<span class=\"hn-txt\">'+label+'</span>'); return '<a class=\"hn-link\" href=\"'+href+'\" title=\"'+label+'\"'+(ext?' target=\"_blank\" rel=\"noopener\"':'')+'>'+ic+txt+'</a>'; }).join(''); }; var _hvis=items.filter(function(it){ return it && (it.label||it.icon); }); var _ico=(_imode==='icon')?' nm-icononly':''; _nmBar.classList.add('hnav-'+_lpos); if(_lpos==='center'){ var _mid=Math.ceil(_hvis.length/2); var _nl=document.createElement('nav'); _nl.className='head-nav head-nav-l'+_ico; _nl.innerHTML=_hbld(_hvis.slice(0,_mid)); var _nr=document.createElement('nav'); _nr.className='head-nav head-nav-r'+_ico; _nr.innerHTML=_hbld(_hvis.slice(_mid)); if(_brand){ _nmBar.insertBefore(_nl,_brand); if(_brand.nextSibling){ _nmBar.insertBefore(_nr,_brand.nextSibling); } else { _nmBar.appendChild(_nr); } } } else { var _nv=document.createElement('nav'); _nv.className='head-nav'+_ico; _nv.innerHTML=_hbld(_hvis); if(_lpos==='right'){ _nmBar.insertBefore(_nv,_nmBar.firstChild); } else if(_brand){ if(_brand.nextSibling){ _nmBar.insertBefore(_nv,_brand.nextSibling); } else { _nmBar.appendChild(_nv); } } else { _nmBar.insertBefore(_nv,_nmBar.firstChild); } } } return; } _nmHeaderClear(); var html=items.map(function(it){ if(!it) return ''; var label=esc(it.label||''); var ic=(_imode!=='text')?_nmIc(it.icon):''; if(!label&&!ic) return ''; var href='#', ext=false; if(it.target==='external'){ href=esc(it.url||'#'); ext=true; } else if(it.target&&it.target.indexOf('page:')===0){ href=esc((base+'/'+it.target.slice(5)).replace(/\/+/g,'/')); } else { href=esc(base||'/'); } var txt=((_imode==='icon'&&ic)?'':'<span class=\"nm-txt\">'+label+'</span>'); return '<a class=\"nm-link\" href=\"'+href+'\" title=\"'+label+'\"'+(ext?' target=\"_blank\" rel=\"noopener\"':'')+'>'+ic+txt+'</a>'; }).join(''); var _io=(_imode==='icon'?' nm-icononly':''); node.className='nav-menu-sec nm-'+(nm.style||'pills')+' nm-al-'+(nm.align||'left')+_io; var _bg=(nm.bg||'').trim(); if(_bg){ node.style.setProperty('background',_bg,'important'); } else { node.style.removeProperty('background'); } var _lt=false; if(_bg){ var _hx=_bg.replace('#',''); if(_hx.length===3){ _hx=_hx.charAt(0)+_hx.charAt(0)+_hx.charAt(1)+_hx.charAt(1)+_hx.charAt(2)+_hx.charAt(2); } if(/^[0-9a-fA-F]{6}$/.test(_hx)){ var _r=parseInt(_hx.slice(0,2),16),_g=parseInt(_hx.slice(2,4),16),_b=parseInt(_hx.slice(4,6),16); _lt=(0.2126*_r+0.7152*_g+0.0722*_b)>165; } } node.style.setProperty('--nm-fg',_lt?'#1a2230':'#fff'); node.style.setProperty('--nm-fill',(nm.fill&&nm.fill.trim())?nm.fill.trim():(_lt?'rgba(0,0,0,.07)':'rgba(255,255,255,.13)')); var _stk=(nm.stroke||'').trim(); node.style.setProperty('--nm-stroke',_stk?_stk:(_lt?'rgba(0,0,0,.22)':'rgba(255,255,255,.4)')); node.innerHTML='<nav class=\"nm-inner\">'+html+'</nav>'; node.style.setProperty('display','block','important'); }
    ...
    window.__applyTradeConfig=function(_c){ applyCfg(_c); try{_lpFooterApply(_c);}catch(_e){} _lpRichWalk(document.body); };
