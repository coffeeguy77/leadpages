...
function applyCfg(C){
  ...
  function _lpArticleBlock(p){ _lpPageStyleOnce(); var _im=(p.imgMode||'center'); var _ip=(p.imgPad!=null?p.imgPad:16); var _fm=(_im==='wrap-left')?('0 '+_ip+'px '+_ip+'px 0'):(_im==='wrap-right')?('0 0 '+_ip+'px '+_ip+'px'):('0 0 '+_ip+'px'); var _fig=p.img?('<figure class=\"lp-figure\" style=\"margin:'+_fm+'\"><img src=\"'+esc(p.img)+'\" alt=\"'+esc(p.h1||p.title||'')+'\"></figure>'):''; var _cls='lp-img-'+(p.img?_im:'none'); function _lpHex(v){ v=String(v||'').trim(); if(/^#?[0-9a-fA-F]{3}$/.test(v)){ v=v.charAt(0)==='#'?v:'#'+v; return '#'+v.charAt(1)+v.charAt(1)+v.charAt(2)+v.charAt(2)+v.charAt(3)+v.charAt(3); } if(/^#?[0-9a-fA-F]{6}$/.test(v)) return v.charAt(0)==='#'?v:'#'+v; return ''; } var _st=[]; var _h1=_lpHex(p.h1Color); if(_h1) _st.push('--lp-h1:'+_h1); var _h2=_lpHex(p.h2Color); if(_h2) _st.push('--lp-h2:'+_h2); var _pc=_lpHex(p.paragraphColor); if(_pc) _st.push('--lp-p:'+_pc); var _style=_st.length?(' style=\"'+_st.join(';')+'\"'):''; return '<section class=\"lp-page '+_cls+'\"'+_style+' data-sec=\"__lpArticle\"><div class=\"lp-wrap\"><div class=\"lp-text\">'+(p.h1?'<h1>'+esc(p.h1)+'</h1>':'')+_fig+'<div class=\"lp-body\"><p>'+_lpMd(p.body||'')+'</p></div></div>'+'<div class=\"lp-clear\"></div></div></section>'; }
