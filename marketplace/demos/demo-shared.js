...
function applyCfg(C){
  ...
  function _navMenuRender(c){ var node=document.querySelector('[data-sec=\"navMenu\"]'); var nm=(c&&c.sections&&c.sections.navMenu)||{}; var _nmBar=document.querySelector('header.site .bar'); function _nmHeaderClear(){ if(!_nmBar) return; _nmBar.classList.remove('hnav-left','hnav-center','hnav-right','lp-logo-left','lp-logo-center','lp-logo-right','lp-logo-solo'); var _ex=_nmBar.querySelectorAll('.head-nav,.head-menu-btn'); for(var _k=_ex.length-1;_k>=0;_k--){ _ex[_k].parentNode.removeChild(_ex[_k]); } Array.prototype.forEach.call(document.querySelectorAll('.hnm-backdrop,.hnm-panel'),function(_oe){ if(_oe.parentNode) _oe.parentNode.removeChild(_oe); }); try{ document.documentElement.classList.remove('lp-compact-nav'); }catch(_e){} } try{ console.log('[navMenu JUN28-R] style:',nm.style,'icons:',nm.icons,'bg:',nm.bg); }catch(_e){} if(!node) return; if(nm.on!==true){ _nmHeaderClear(); node.style.setProperty('display','none','important'); node.innerHTML=''; return; } function _nmIc(v){ if(!v) return ''; v=String(v); var _i=(window.LP_ICONS&&window.LP_ICONS[v]); if(!_i) return ''; return '<svg class=\"lp-ic\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\">'+_i+'</svg>'; } function _nmChev(){ var _c=(window.LP_ICONS&&(window.LP_ICONS['chevron-down']||window.LP_ICONS.chevronDown))||'<polyline points=\"6 9 12 15 18 9\"></polyline>'; return '<svg class=\"lp-ic hn-chev\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\">'+_c+'</svg>'; } function _nmIsParent(it){ if(!it) return false; if(it.kind==='link') return false; if(it.kind==='parent') return true; return !!(Array.isArray(it.children)&&it.children.length); } function _nmKids(it){ return (it&&Array.isArray(it.children))?it.children.filter(function(ch){ return ch&&(ch.label||ch.icon); }):[]; } function _nmHref(it){ var href='#', blank=false; if(!it) return {href:href,blank:blank}; if(it.target==='external'){ href=it.url||'#'; blank=(it.opens!=='_self'); } else if(it.target&&String(it.target).indexOf('page:')===0){ href=(base+'/'+String(it.target).slice(5)).replace(/\/+/g,'/'); blank=(it.opens==='_blank'); } else if(it.target==='orderStorefront'){ href='#orderStorefront'; } else if(it.target==='orderShop'){ href=(base+'/order').replace(/\/+/g,'/')||'/order'; blank=(it.opens==='_blank'); } else if(it.target==='orderPortal'){ href=(base+'/orders').replace(/\/+/g,'/')||'/orders'; blank=(it.opens==='_blank'); } else { href=base||'/'; blank=(it.opens==='_blank'); } return {href:href,blank:blank}; } var items=(Array.isArray(nm.items)?nm.items:[]); var base=_siteBase(); var _imode=(nm.icons||'both'); if((nm.placement||'header')==='header'){ node.style.setProperty('display','none','important'); node.innerHTML=''; if(_nmBar){ _nmHeaderClear(); var _lpos=(c&&c.logo&&c.logo.position)||'left'; var _brand=_nmBar.querySelector('a.brand'); if(_brand){ _brand.style.marginLeft=''; _brand.style.marginRight=''; } var _hbld=function(arr){ return arr.map(function(it){ if(!it) return ''; var label=esc(it.label||''); var ic=(_imode!=='text')?_nmIc(it.icon):''; if(!label&&!ic) return ''; if(_nmIsParent(it)){ var kids=_nmKids(it); var sub=kids.map(function(ch){ var cl=esc(ch.label||''); var cic=(_imode!=='text')?_nmIc(ch.icon):''; if(!cl&&!cic) return ''; var hr=_nmHref(ch); var ctxt=((_imode==='icon'&&cic)?'':'<span class=\"hn-txt\">'+cl+'</span>'); return '<a class=\"hn-sub-link\" href=\"'+esc(hr.href)+'\" title=\"'+cl+'\"'+(hr.blank?' target=\"_blank\" rel=\"noopener\"':'')+'>'+cic+ctxt+'</a>'; }).join(''); var ptxt=((_imode==='icon'&&ic)?'':'<span class=\"hn-txt\">'+label+'</span>'); return '<div class=\"hn-drop\"><button type=\"button\" class=\"hn-link hn-parent\" aria-haspopup=\"true\" aria-expanded=\"false\" title=\"'+label+'\">'+ic+ptxt+_nmChev()+'</button><div class=\"hn-sub\" role=\"menu\">'+sub+'</div></div>'; } var hr=_nmHref(it); var txt=((_imode==='icon'&&ic)?'':'<span class=\"hn-txt\">'+label+'</span>'); return '<a class=\"hn-link\" href=\"'+esc(hr.href)+'\" title=\"'+label+'\"'+(hr.blank?' target=\"_blank\" rel=\"noopener\"':'')+'>'+ic+txt+'</a>'; }).join(''); }; var _hvis=items.filter(function(it){ return it && (it.label||it.icon||_nmIsParent(it)); }); var _ico=(_imode==='icon')?' nm-icononly':''; var _stCls=' nm-'+(nm.style||'pills'); _nmBar.classList.add('hnav-'+_lpos); if(_lpos==='center'){ var _mid=Math.ceil(_hvis.length/2); var _nl=document.createElement('nav'); _nl.className='head-nav head-nav-l'+_stCls+_ico; _nl.innerHTML=_hbld(_hvis.slice(0,_mid)); var _nr=document.createElement('nav'); _nr.className='head-nav head-nav-r'+_stCls+_ico; _nr.innerHTML=_hbld(_hvis.slice(_mid)); if(_brand){ _nmBar.insertBefore(_nl,_brand); if(_brand.nextSibling){ _nmBar.insertBefore(_nr,_brand.nextSibling); } else { _nmBar.appendChild(_nr); } } } else { var _nv=document.createElement('nav'); _nv.className='head-nav'+_stCls+_ico; _nv.innerHTML=_hbld(_hvis); if(_lpos==='right'){ _nmBar.insertBefore(_nv,_nmBar.firstChild); } else if(_brand){ if(_brand.nextSibling){ _nmBar.insertBefore(_nv,_brand.nextSibling); } else { _nmBar.appendChild(_nv); } } else { _nmBar.insertBefore(_nv,_nmBar.firstChild); } } }
          (function(){ var _hdr=document.querySelector('header.site'); if(!_hdr) return; function _sc(v){ v=(v==null?'':String(v)).trim(); return (/^#[0-9a-fA-F]{3,8}$/.test(v)||/^rgba?\([0-9.,%\s]+\)$/.test(v))?v:''; } function _set(n,v){ if(v) _hdr.style.setProperty(n,v); else _hdr.style.removeProperty(n); } var _st=(nm.style||'pills'); var _navs=_nmBar.querySelectorAll('.head-nav'); for(var _ni=0;_ni<_navs.length;_ni++){   var _el=_navs[_ni];   _el.classList.remove('nm-pills','nm-solid','nm-outline','nm-shadow','nm-buttons','nm-tabs','nm-underline','nm-links','nm-ghost');   _el.classList.add('nm-'+_st); } _set('--hn-fg', _sc(nm.fg||nm.linkColor)); _set('--hn-hover-fg', _sc(nm.hoverFg)); _set('--hn-hover-bg', _sc(nm.hoverBg)); var _fill=_sc(nm.fill); var _stroke=_sc(nm.stroke); if(_fill) _set('--hn-fill', _fill); else _hdr.style.removeProperty('--hn-fill'); if(_stroke) _set('--hn-stroke', _stroke); else _hdr.style.removeProperty('--hn-stroke'); })();
