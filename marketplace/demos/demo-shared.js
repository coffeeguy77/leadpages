...
(function(){
  ...
  /* Site Button look — law for fill / outline / stroke thickness / text (config.buttonLook) */
  var LP_BTN_STROKE_PX={fine:'1px',thin:'1.5px',medium:'2px',normal:'2.5px'};
  function lpBtnStrokePx(key){ return LP_BTN_STROKE_PX[key]||LP_BTN_STROKE_PX.medium; }
  function applyButtonLook(C){
    C=C||{}; var BL=C.buttonLook||{}; var th=C.theme||{}; var rs=document.documentElement.style;
    function hex(v){ v=String(v==null?'':v).trim(); return (/^#[0-9a-fA-F]{3,8}$/.test(v)||/^rgba?\([0-9.,%\s]+\)$/.test(v))?v:''; }
    var fill=BL.fill!==false; var outline=BL.outline===true;
    var bg=hex(BL.bg)||hex(th.hivis)||'';
    var fg=hex(BL.fg)||'#ffffff';
    var stroke=hex(BL.strokeColor)||(outline?(fg||bg):'');
    var sw=lpBtnStrokePx(BL.strokeWidth||'medium');
    if(bg) rs.setProperty('--lp-btn-bg', fill?bg:'transparent'); else rs.removeProperty('--lp-btn-bg');
    if(fg) rs.setProperty('--lp-btn-fg', fg); else rs.removeProperty('--lp-btn-fg');
    if(outline||stroke){ rs.setProperty('--lp-btn-stroke', stroke||fg||'#ffffff'); rs.setProperty('--lp-btn-stroke-w', outline?sw:(fill?'0px':sw)); }
    else { rs.removeProperty('--lp-btn-stroke'); rs.setProperty('--lp-btn-stroke-w', '0px'); }
    /* Secondary / outline default — white stroke on dark heroes */
    var sFill=BL.secondaryFill===true; var sOutline=BL.secondaryOutline!==false;
    var sBg=hex(BL.secondaryBg)||'#ffffff';
    var sFg=hex(BL.secondaryFg)||(hex(th.steel)||'#1a2230');
    var sStroke=hex(BL.secondaryStrokeColor)||sFg;
    var sSw=lpBtnStrokePx(BL.secondaryStrokeWidth||BL.strokeWidth||'medium');
    if(sFill&&sBg) rs.setProperty('--lp-btn-sec-bg', sBg); else rs.setProperty('--lp-btn-sec-bg', 'transparent');
    rs.setProperty('--lp-btn-sec-fg', sFg);
    rs.setProperty('--lp-btn-sec-stroke', sStroke||sFg);
    rs.setProperty('--lp-btn-sec-stroke-w', (sOutline||!sFill)?sSw:'0px');
    try{
      var de=document.documentElement;
      de.classList.toggle('lp-btn-fill', fill);
      de.classList.toggle('lp-btn-outline', outline);
      de.classList.toggle('lp-btn-sec-outline', sOutline);
    }catch(_e){}
  }
  function applySiteMaxWidth(cfg){
    ...
    var de=document.documentElement;
    var rs=de.style;
    var raw=cfg.maxSiteWidth;
    var mode=String(raw==null||raw===''?'1920':raw).trim().toLowerCase();
    if(mode==='full'||mode==='none'||mode==='100%'){
      ...
      clsF.push('site-width-full');
      de.className=clsF.join(' ');
      rs.removeProperty('--site-maxw');
      rs.setProperty('--maxw','none');
      return;
    }
    var px=parseInt(mode,10);
    if(!isFinite(px)||px<1) px=1920;
    var clsC=(de.className||'').split(' ').filter(function(x){ return x && x.indexOf('site-width-')!==0 && x.indexOf('site-max-')!==0; });
    clsC.push('site-width-capped');
    if(px<=1440) clsC.push('site-max-1440');
    de.className=clsC.join(' ');
  ...
  function __lpApplyColorOverrides(cfg){
    ...
    function walk(o){if(typeof o==='string')return rewrite(o,m);if(Array.isArray(o)){for(var i=0;i<o.length;i++)o[i]=walk(o[i]);return o;}if(o&&typeof o==='object'){Object.keys(o).forEach(function(k){o[k]=walk(o[k]);});return o;}return o;}
...
function applyCfg(C){
    ...
    try{ var __mn=document.querySelector('main#top'); if(__mn){ __mn.style.display='flex'; __mn.style.flexDirection='column';
      var __kids=[]; try{ __kids=[].slice.call(__mn.querySelectorAll(':scope > [data-sec]')); }catch(__e0){ __kids=[].filter.call(__mn.children||[],function(n){return n&&n.getAttribute&&n.getAttribute('data-sec');}); }
      __kids.forEach(function(__n){ __n.style.order=''; });
      var __seen={}, __ord=[];
      function __pushNode(__n){ if(!__n) return; var __id=__n.getAttribute('data-sec'); if(!__id||__seen[__id]) return; __seen[__id]=1; __ord.push(__n); }
      function __pushId(__id){ if(!__id||__seen[__id]) return; if(__id==='promotions'){ __pushNode(__mn.querySelector('[data-sec="promotions-hero"]')); __pushNode(__mn.querySelector('[data-sec="promotions-inline"]')); __seen[__id]=1; return; } __pushNode(__mn.querySelector('[data-sec="'+__id+'"]')); }
      if(Array.isArray(C.sectionOrder)) C.sectionOrder.forEach(__pushId);
      __kids.forEach(__pushNode);
      __ord.forEach(function(__n,__ix){ __n.style.order=String(__ix+1); });
    } }catch(e){} try{ _navMenuRender(C); }catch(e){}
    applyThemeVars(th);
    try{ applyButtonLook(C); }catch(_eBl){}
    ...
    if(Array.isArray(C.services)){ var grid=document.querySelector('.svcs'); if(grid){
      ...
      var SV0=(C.sections&&C.sections.services)||{};
      var split=(SV0.mode==='split'||SV0.mode==='columns');
      var cards=(SV0.mode==='cards'||SV0.mode==='homepage'||SV0.mode==='cap');
      var svSec=document.querySelector('[data-sec="services"]');
      if(svSec){
        svSec.classList.toggle('svcs-split', !!split);
        svSec.classList.toggle('svcs-cards', !!cards);
        if(cards){
          var secBg=_svcHex(SV0.sectionBg||SV0.bg); if(secBg) svSec.style.setProperty('--svc-sec-bg',secBg); else svSec.style.removeProperty('--svc-sec-bg');
          var icoSz=parseInt(SV0.iconCircleSize!=null?SV0.iconCircleSize:88,10); if(isNaN(icoSz)) icoSz=88; icoSz=Math.max(40,Math.min(140,icoSz));
          svSec.style.setProperty('--svc-ico-size',icoSz+'px');
          var ts=SV0.titleSize; if(ts!=null&&String(ts).trim()!=='') svSec.style.setProperty('--svc-title-size', (String(ts).match(/px|rem|em|%$/)?String(ts):(+ts+'px'))); else svSec.style.removeProperty('--svc-title-size');
          var bs=SV0.bodySize; if(bs!=null&&String(bs).trim()!=='') svSec.style.setProperty('--svc-body-size', (String(bs).match(/px|rem|em|%$/)?String(bs):(+bs+'px'))); else svSec.style.removeProperty('--svc-body-size');
          var icBg=_svcHex(SV0.iconCircleBg); if(icBg) svSec.style.setProperty('--svc-ico-bg',icBg); else svSec.style.removeProperty('--svc-ico-bg');
          var icFg=_svcHex(SV0.iconColor); if(icFg) svSec.style.setProperty('--svc-icon',icFg); else svSec.style.removeProperty('--svc-icon');
          var ctc=_svcHex(SV0.cardTitleColor); if(ctc) svSec.style.setProperty('--svc-title',ctc); else svSec.style.removeProperty('--svc-title');
          var cbc=_svcHex(SV0.cardBodyColor); if(cbc) svSec.style.setProperty('--svc-body',cbc); else svSec.style.removeProperty('--svc-body');
          var cbg=_svcHex(SV0.cardBg); if(cbg){ svSec.style.setProperty('--svc-bg',cbg); } else svSec.style.removeProperty('--svc-bg');
        }
        var head=svSec.querySelector('.section-head');
        if(head){
          var acc=head.querySelector('.svc-accent');
          if(split){
            if(!acc){ acc=document.createElement('span'); acc.className='svc-accent'; acc.setAttribute('aria-hidden','true'); var h2=head.querySelector('h2'); if(h2&&h2.nextSibling) head.insertBefore(acc,h2.nextSibling); else head.appendChild(acc); }
            acc.style.display=(SV0.accentOn===false)?'none':'';
            var ac=_svcHex(SV0.accentColor); if(ac) svSec.style.setProperty('--svc-accent',ac); else svSec.style.removeProperty('--svc-accent');
            svSec.style.setProperty('--svc-eyebrow-case', SV0.eyebrowCaps===false?'none':'uppercase');
            svSec.style.setProperty('--svc-heading-case', SV0.headingCaps===true?'uppercase':'none');
            svSec.style.setProperty('--lp-eyebrow-case', SV0.eyebrowCaps===false?'none':'uppercase');
            svSec.style.setProperty('--lp-heading-case', SV0.headingCaps===true?'uppercase':'none');
          } else if(acc){ acc.style.display='none'; }
        }
      }
      function _svcNormSec(v){ return String(v==null?'':v).replace(/^#/,'').replace(/[^\w-]/g,'')||'quote'; }
      function _svcIsSafeUrl(url){ if(url==null||url==='') return false; var s=String(url).trim(); if(!s) return false; if(/^javascript:/i.test(s)||/^data:/i.test(s)) return false; try{ var u=new URL(s,'https://example.invalid'); return u.protocol==='http:'||u.protocol==='https:'; }catch(_e){ return false; } }
      function _svcLinkHref(s){
        var cta=(s&&s.cta&&typeof s.cta==='object')?s.cta:s;
        var act=(cta&&cta.linkAction)||'none'; if(!act||act==='none') return '';
        if(act==='scroll') return '#'+_svcNormSec(cta.linkTarget||'quote');
        if(act==='page'){ var slug=String(cta.linkPage||'').trim().replace(/^\/+|\/+$/g,''); if(!slug) return ''; var base=typeof _siteBase==='function'?_siteBase():''; return (base+'/'+slug).replace(/\/+/g,'/'); }
        if(act==='url'){ var u=String(cta.linkUrl||'').trim(); return _svcIsSafeUrl(u)?u:''; }
        return '';
      }
      function _svcTick(){ return '<span class="svc-tick" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg></span>'; }
      function _svcArrow(){ return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="m13 6 6 6-6 6"/></svg>'; }
      var on=C.services.filter(function(s){return s&&s.on!==false&&(s.title||s.body||s.subtitle||s.icon||s.image||(Array.isArray(s.bullets)&&s.bullets.length));});
      grid.innerHTML=on.map(function(s,si){
        ...
        if(cards){
          var imgUrl=(s.image&&String(s.image).trim())||'';
          var mediaHtml='<div class="svc-cap-media'+(imgUrl?'':' is-empty')+'">'+(imgUrl?('<img src="'+esc(imgUrl)+'" alt="" loading="lazy">'):'')+'</div>';
          var showIco=s.icon!=null&&String(s.icon).trim()!==''&&s.iconOn!==false;
          var icoHtml=showIco?('<span class="svc-cap-ico" aria-hidden="true">'+lpIcon(s.icon)+'</span>'):'';
          var cardBg=_svcHex(s.bg); if(cardBg){ st.push('--svc-bg:'+cardBg); st.push('background:'+cardBg); }
          var stroke2=_svcHex(s.stroke); if(stroke2){ st.push('--svc-stroke:'+stroke2); }
          var tc2=_svcHex(s.titleColor); if(tc2) st.push('--svc-title:'+tc2);
          var fc2=_svcHex(s.color); if(fc2) st.push('--svc-body:'+fc2);
          return '<div class="'+cls.join(' ')+'"'+(st.length?' style="'+st.join(';')+'"':'')+'>'+mediaHtml+icoHtml+'<div class="svc-cap-body"><h3>'+esc(s.title||'')+'</h3><p>'+esc(s.body||'')+'</p></div></div>';
        }
        if(!split){
          return '<div class="'+cls.join(' ')+'"'+(st.length?' style="'+st.join(';')+'"':'')+'>'+media+'<h3>'+esc(s.title||'')+'</h3><p>'+esc(s.body||'')+'</p></div>';
        }
        var sub=(s.subtitle!=null&&String(s.subtitle).trim())?String(s.subtitle).trim():'';
        var bullets=Array.isArray(s.bullets)?s.bullets:[];
        if(!bullets.length && s.body && String(s.body).indexOf('\n')>=0){
          bullets=String(s.body).split(/\n+/).map(function(t){ return {on:true,text:t.replace(/^[-*•]\s*/,'').trim()}; }).filter(function(b){ return b.text; });
        }
        var bullHtml='';
        var visible=bullets.filter(function(b){ return b&&b.on!==false&&(b.text||b.title); });
        if(visible.length){
          bullHtml='<ul class="svc-bullets">'+visible.map(function(b){
            return '<li>'+_svcTick()+'<span>'+esc(b.text||b.title||'')+'</span></li>';
          }).join('')+'</ul>';
        }
        var cta=(s.cta&&typeof s.cta==='object')?s.cta:{};
        var ctaOn=cta.on!==false;
        var ctaLabel=(cta.label!=null&&String(cta.label).trim())?String(cta.label).trim():'Learn more';
        var href=_svcLinkHref(s);
        var act=(cta.linkAction||s.linkAction||'none');
        var ctaHtml='';
        if(ctaOn && (href || act==='none')){
          if(href){
            var extra=(act==='url')?' target="_blank" rel="noopener noreferrer"':'';
            var scrollAttr=(act==='scroll')?(' data-svc-scroll="'+_svcNormSec(cta.linkTarget||s.linkTarget||'quote')+'"'):'';
            ctaHtml='<a class="svc-cta" href="'+esc(href)+'"'+extra+scrollAttr+' data-svc-i="'+si+'">'+esc(ctaLabel)+_svcArrow()+'</a>';
          } else {
            ctaHtml='<span class="svc-cta" aria-hidden="true">'+esc(ctaLabel)+_svcArrow()+'</span>';
          }
        }
        var side=(s.sideImage&&String(s.sideImage).trim())||'';
        if(side){ cls.push('has-side'); }
        var sideHtml=side?('<div class="svc-side" style="background-image:url('+JSON.stringify(String(side))+')" aria-hidden="true"></div>'):'';
        var intro=(s.intro!=null&&String(s.intro).trim())?String(s.intro).trim():'';
        var listTitle=(s.listTitle!=null&&String(s.listTitle).trim())?String(s.listTitle).trim():'';
        if(s.titleCaps===true) st.push('--svc-title-case:uppercase');
        else if(s.titleCaps===false) st.push('--svc-title-case:none');
        if(s.subtitleCaps===false) st.push('--svc-sub-case:none');
        else st.push('--svc-sub-case:uppercase');
        if(s.listTitleCaps===true) st.push('--svc-list-title-case:uppercase');
        var introHtml=intro?('<p class="svc-intro">'+esc(intro)+'</p>'):'';
        var listTitleHtml=listTitle?('<div class="svc-list-title">'+esc(listTitle)+'</div>'):'';
        var bodyHidden=sub?'':'<p style="display:none"></p>';
        return '<div class="'+cls.join(' ')+'"'+(st.length?' style="'+st.join(';')+'"':'')+'>'+media+'<h3>'+esc(s.title||'')+'</h3>'+(sub?'<span class="svc-sub">'+esc(sub)+'</span>':'')+bodyHidden+introHtml+listTitleHtml+bullHtml+ctaHtml+sideHtml+'</div>';
      }).join('');
      if(split){
        grid.querySelectorAll('[data-svc-scroll]').forEach(function(el){
          var nm=el.getAttribute('data-svc-scroll'); if(!nm) return;
          el.onclick=function(ev){ try{ev.preventDefault();}catch(_e){} var tg=document.querySelector('[data-sec="'+nm+'"]')||document.getElementById(nm); if(tg&&tg.scrollIntoView) tg.scrollIntoView({behavior:'smooth',block:'start'}); try{ trackEvent('cta_click',{location:'services',action:'scroll',target:nm}); }catch(_e2){} };
        });
      }
    } }
    (function(){
    ...
    (function(){
      var SEC2=C.sections||{};
      Object.keys(SEC2).forEach(function(id){
        var node=document.querySelector('[data-sec="'+id+'"]'); if(!node) return;
        var S=SEC2[id]||{};
        if(S.eyebrowCaps===false) node.style.setProperty('--lp-eyebrow-case','none');
        else node.style.setProperty('--lp-eyebrow-case','uppercase');
        if(S.headingCaps===true) node.style.setProperty('--lp-heading-case','uppercase');
        else node.style.setProperty('--lp-heading-case','none');
      });
    })();

    if(C.logo){ var L=C.logo, brands=document.querySelectorAll('a.brand'); var _lhd=Math.max(20,Math.min(320,(L.heightPx?L.heightPx:Math.round(40*(L.scale||1)))||44)); var _lhm=Math.max(16,Math.min(200,(L.heightPxM!=null&&L.heightPxM!==''?+L.heightPxM:Math.round(_lhd*0.72)))); if(!isFinite(_lhm)) _lhm=Math.round(_lhd*0.72); var _cropY=Math.max(0,Math.min(50,+(L.cropY!=null?L.cropY:0)||0)); var _cropFocus=(L.cropFocus==='top'||L.cropFocus==='bottom')?L.cropFocus:'center'; var _cropYM=(L.cropYM!=null&&L.cropYM!=='')?Math.max(0,Math.min(50,+L.cropYM||0)):_cropY; var _cropFocusM=(L.cropFocusM==='top'||L.cropFocusM==='bottom'||L.cropFocusM==='center')?L.cropFocusM:_cropFocus; var _cropScale=_cropY>0?(100/(100-_cropY)):1; var _cropScaleM=_cropYM>0?(100/(100-_cropYM)):1; var _imgHd=Math.round(_lhd*_cropScale); var _imgHm=Math.round(_lhm*_cropScaleM); var _cropAlign=_cropFocus==='top'?'flex-start':(_cropFocus==='bottom'?'flex-end':'center'); var _cropAlignM=_cropFocusM==='top'?'flex-start':(_cropFocusM==='bottom'?'flex-end':'center'); var _cropPos=_cropFocus==='top'?'center top':(_cropFocus==='bottom'?'center bottom':'center center'); var _cropPosM=_cropFocusM==='top'?'center top':(_cropFocusM==='bottom'?'center bottom':'center center'); var _lbg=(L.bg&&L.bg!=='transparent')?L.bg:'transparent'; var _lpad=(L.pad!=null?L.pad:(_lbg!=='transparent'?8:0)); var _lrad=(L.radius!=null?L.radius:(_lbg!=='transparent'?12:0)); var _lp=L.position||'left'; var _hs=L.headerStyle||(L.float?'float':'solid-sticky'); var _hbg=(L.headerBg&&L.headerBg!=='')?L.headerBg:''; var _hop=L.headerBgOpacity; var _hopa=1; if(_hop!=null&&_hop!==''){ var _hopn=parseFloat(_hop); if(!isNaN(_hopn)) _hopa=Math.max(0,Math.min(100,_hopn))/100; } function _lgHexRgba(hex,a){ var m=/^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.exec(String(hex||'').trim()); if(!m) return ''; var h=m[1]; if(h.length===3) h=h[0]+h[0]+h[1]+h[1]+h[2]+h[2]; var n=parseInt(h,16); return 'rgba('+((n>>16)&255)+','+((n>>8)&255)+','+(n&255)+','+a+')'; } var _hbgBase=_hbg||'#0b1220'; var _hbgCss=_hopa<=0?'transparent':(_hopa<1?(_lgHexRgba(_hbgBase,_hopa)||_hbgBase):(_hbg||'')); var _barH=Math.max(40,Math.min(280,+(L.headerBarHeight!=null?L.headerBarHeight:0)||0)); var _hang=(L.headerHang==='below'||L.headerHang===true); var _barPad=Math.max(0,Math.min(48,+(L.headerBarPadding!=null?L.headerBarPadding:10)||0)); var _barPadM=(L.headerBarPaddingM!=null&&L.headerBarPaddingM!=='')?Math.max(0,Math.min(48,+L.headerBarPaddingM||0)):_barPad; var _offX=Math.max(-120,Math.min(120,+(L.offsetX!=null?L.offsetX:0)||0)); var _offY=Math.max(-120,Math.min(120,+(L.offsetY!=null?L.offsetY:0)||0)); var _offXM=(L.offsetXM!=null&&L.offsetXM!=='')?Math.max(-120,Math.min(120,+L.offsetXM||0)):0; var _offYM=(L.offsetYM!=null&&L.offsetYM!=='')?Math.max(-120,Math.min(120,+L.offsetYM||0)):0; var _lpM=(L.positionM==='center'||L.positionM==='right'||L.positionM==='left')?L.positionM:''; var _fitMin=Math.max(70,_lhd+_barPad*2); var _align=_hang?'flex-start':((L.headerLogoAlign==='end'||L.headerLogoAlign==='bottom')?'flex-end':((L.headerLogoAlign==='start'||L.headerLogoAlign==='top')?'flex-start':'center')); var _alignM=(L.headerLogoAlignM==='end'||L.headerLogoAlignM==='bottom')?'flex-end':((L.headerLogoAlignM==='start'||L.headerLogoAlignM==='top')?'flex-start':((L.headerLogoAlignM==='center')?'center':_align)); var _pos='sticky',_top=';top:var(--emerg-sticky-h,0px)',_bg=(_hbgCss?('background:'+_hbgCss):''); if(_hs==='solid-scroll'){ _pos='relative'; _top=''; } else if(_hs==='float'){ _pos='absolute'; _top=';top:0;left:0;right:0'; _bg='background:transparent'; } else if(_hs==='shrink'){ _pos='sticky'; _top=';top:var(--emerg-sticky-h,0px)'; _bg='background:transparent'; } var _stTopOn=L.headerStrokeTopOn===true; var _stBotOn=L.headerStrokeBottomOn===true; var _stTopW=Math.max(0,Math.min(16,+(L.headerStrokeTopWidth!=null?L.headerStrokeTopWidth:2)||0)); var _stBotW=Math.max(0,Math.min(16,+(L.headerStrokeBottomWidth!=null?L.headerStrokeBottomWidth:2)||0)); var _stTopC=(L.headerStrokeTopColor&&String(L.headerStrokeTopColor).trim())||'#ffffff'; var _stBotC=(L.headerStrokeBottomColor&&String(L.headerStrokeBottomColor).trim())||'#ffffff'; var _hdrBase='header.site{position:'+_pos+';'+_top+';z-index:60;overflow:visible;'+_bg+'}'; var _barCss='header.site .bar{height:'+(_barH?(_barH+'px'):'auto')+';min-height:'+(_barH?Math.max(_barH,_hang?_barH:_fitMin):_fitMin)+'px;padding-top:'+_barPad+'px;padding-bottom:'+_barPad+'px;align-items:'+_align+';overflow:visible}'; var _shrinkBg=_hbgCss||'rgba(10,12,18,.94)'; var _shrinkCss=(_hs==='shrink')?('header.site{transition:background .25s,box-shadow .25s}header.site img.lp-logo{transition:height .25s}'+'header.site.lp-shrunk{background:'+_shrinkBg+'!important;box-shadow:0 6px 24px rgba(0,0,0,.28)}'+'header.site.lp-shrunk .bar{min-height:0;height:auto;padding-top:6px;padding-bottom:6px}'+'header.site.lp-shrunk .lp-logo-wrap{height:'+Math.min(_lhd,52)+'px}'+'header.site.lp-shrunk img.lp-logo{height:'+Math.round(Math.min(_lhd,52)*_cropScale)+'px}'):''; var _logoPosCss='header.site .bar.lp-logo-left{justify-content:space-between}header.site .bar.lp-logo-left.lp-logo-solo{justify-content:flex-start}header.site .bar.lp-logo-center{justify-content:center}header.site .bar.lp-logo-right{justify-content:flex-end;gap:14px}header.site .bar.lp-logo-center .head-call{position:absolute;right:22px;top:50%;transform:translateY(-50%)}'; var _hdrCss=_hdrBase+_barCss+_shrinkCss+_logoPosCss; (function(){ var _h=document.querySelector('header.site'); if(!_h) return; if(_hs==='float'||_hs==='shrink'){ _h.style.setProperty('--hdr-bg','transparent'); } else if(_hbgCss){ _h.style.setProperty('--hdr-bg',_hbgCss); } else { _h.style.removeProperty('--hdr-bg'); } if(_barH){ _h.style.setProperty('--hdr-bar-h',_barH+'px'); } else { _h.style.removeProperty('--hdr-bar-h'); } _h.style.setProperty('--hdr-logo-align',_align); _h.classList.toggle('lp-hdr-float', _hs==='float'); _h.classList.toggle('lp-st-top', !!(!(_hs==='float')&&_stTopOn&&_stTopW>0)); _h.classList.toggle('lp-st-bot', !!(!(_hs==='float')&&_stBotOn&&_stBotW>0)); if(_stTopOn&&_stTopW>0){ _h.style.setProperty('--hdr-st-top-w',_stTopW+'px'); _h.style.setProperty('--hdr-st-top-c',_stTopC); } else { _h.style.removeProperty('--hdr-st-top-w'); _h.style.removeProperty('--hdr-st-top-c'); } if(_stBotOn&&_stBotW>0){ _h.style.setProperty('--hdr-st-bot-w',_stBotW+'px'); _h.style.setProperty('--hdr-st-bot-c',_stBotC); } else { _h.style.removeProperty('--hdr-st-bot-w'); _h.style.removeProperty('--hdr-st-bot-c'); } if(_hs==='shrink'){ if(!_h.__lpScroll){ _h.__lpScroll=function(){ if(window.scrollY>60) _h.classList.add('lp-shrunk'); else _h.classList.remove('lp-shrunk'); }; window.addEventListener('scroll',_h.__lpScroll,{passive:true}); } _h.__lpScroll(); } else { _h.classList.remove('lp-shrunk'); if(_h.__lpScroll){ window.removeEventListener('scroll',_h.__lpScroll); _h.__lpScroll=null; } } })(); var _st=document.getElementById('lp-logo-css'); if(!_st){ _st=document.createElement('style'); _st.id='lp-logo-css'; document.head.appendChild(_st); } try{ document.documentElement.style.setProperty('--hdr-logo-mh',_lhm+'px'); document.documentElement.style.setProperty('--hdr-logo-hd',_lhd+'px'); }catch(_eHm){} _st.textContent=':root{--hdr-logo-mh:'+_lhm+'px;--hdr-logo-hd:'+_lhd+'px}'+'header.site{overflow:visible;--hdr-logo-mh:'+_lhm+'px;--hdr-logo-hd:'+_lhd+'px}'+'header.site.lp-st-top::before,header.site.lp-st-bot::after{content:"";position:absolute;left:0;right:0;z-index:0;pointer-events:none}'+'header.site.lp-st-top::before{top:0;height:var(--hdr-st-top-w,0);background:var(--hdr-st-top-c,#fff)}'+'header.site.lp-st-bot::after{bottom:0;height:var(--hdr-st-bot-w,0);background:var(--hdr-st-bot-c,#fff)}'+'header.site .bar{position:relative;z-index:1;overflow:visible}'+'a.brand{max-width:100%;overflow:visible;position:relative;z-index:2}'+_hdrCss+'header.site a.brand .lp-logo-wrap{display:inline-flex;align-items:'+(_cropY?_cropAlign:'center')+';background:'+_lbg+';padding:'+_lpad+'px;border-radius:'+_lrad+'px;line-height:0;'+(_cropY?('height:'+_lhd+'px;overflow:hidden'):'overflow:visible')+'}'+'header.site a.brand img.lp-logo{height:'+(_cropY?_imgHd:_lhd)+'px;width:auto;max-width:min(62vw,440px);object-fit:'+(_cropY?'cover':'contain')+';'+(_cropY?('object-position:'+_cropPos+';'):'')+'display:block;image-rendering:-webkit-optimize-contrast}'+'@media(max-width:760px){header.site{--hdr-logo-mh:'+_lhm+'px;--hdr-bar-pad-m:'+_barPadM+'px;--hdr-logo-align-m:'+_alignM+'}header.site .bar{padding-top:var(--hdr-bar-pad-m,'+_barPadM+'px)!important;padding-bottom:var(--hdr-bar-pad-m,'+_barPadM+'px)!important;align-items:var(--hdr-logo-align-m,'+_alignM+')!important;min-height:0!important;height:auto!important}header.site a.brand{max-width:min(92vw,440px)}header.site a.brand .lp-logo-wrap{display:inline-flex!important;align-items:'+(_cropYM?_cropAlignM:'center')+'!important;'+(_cropYM?('height:'+_lhm+'px!important;overflow:hidden!important;'):'height:auto!important;max-height:none!important;overflow:visible!important;')+'}header.site a.brand img.lp-logo{height:'+(_cropYM?_imgHm:_lhm)+'px!important;max-height:'+(_cropYM?_imgHm:_lhm)+'px!important;width:auto!important;max-width:100%!important;object-fit:'+(_cropYM?'cover':'contain')+'!important;'+(_cropYM?('object-position:'+_cropPosM+';'):'')+'}header.site a.brand.lp-logo-text-scale{transform:scale(var(--hdr-logo-m-scale,1)) translate(var(--hdr-logo-ox-m-val,0px),var(--hdr-logo-oy-m-val,0px))!important;transform-origin:inherit}header.site a.brand.lp-logo-img-place{transform:translate(var(--hdr-logo-ox-m-val,0px),var(--hdr-logo-oy-m-val,0px))!important}header.site.lp-logo-m-left .bar:not(.hnav-left):not(.hnav-center):not(.hnav-right){justify-content:flex-start!important}header.site.lp-logo-m-center .bar:not(.hnav-left):not(.hnav-center):not(.hnav-right){justify-content:center!important}header.site.lp-logo-m-right .bar:not(.hnav-left):not(.hnav-center):not(.hnav-right){justify-content:flex-end!important;gap:14px}}'; function _lpLogoUrl(u,cssH,dpr){ u=String(u||''); if(!/^https?:\/\/res\.cloudinary\.com\//i.test(u)) return u; if(/\/image\/upload\/[^/]*\b(?:h_|w_)\d+/i.test(u)) return u; var h=Math.round((cssH||44)*(dpr||2)*1.5); h=Math.min(640,Math.max(64,h)); return u.replace('/image/upload/','/image/upload/h_'+h+',q_auto:best,f_auto,c_limit/'); } var _logoSrc=_lpLogoUrl(L.imageUrl,Math.max(_lhd,_lhm),1); var _logoSrc2=_lpLogoUrl(L.imageUrl,Math.max(_lhd,_lhm),2); var _inHN=(C.sections&&C.sections.navMenu&&C.sections.navMenu.on===true&&C.sections.navMenu.placement==='header'); var _barEl=document.querySelector('header.site .bar'); if(_barEl){ _barEl.classList.remove('lp-logo-left','lp-logo-center','lp-logo-right','lp-logo-solo'); if(!_inHN){ var _Hd=(C.sections&&C.sections.header)||{}; var _phoneOn=(_Hd.on!==false&&_Hd.showPhone!==false); var _ctaOn=(_Hd.on!==false); var _posEff=_phoneOn?'left':((_lp==='center'||_lp==='right')?_lp:'left'); _barEl.classList.add('lp-logo-'+_posEff); if(!_ctaOn) _barEl.classList.add('lp-logo-solo'); } } var _hdrEl=document.querySelector('header.site'); if(_hdrEl){ _hdrEl.classList.remove('lp-logo-m-left','lp-logo-m-center','lp-logo-m-right'); if(_lpM) _hdrEl.classList.add('lp-logo-m-'+_lpM); } for(var i=0;i<brands.length;i++){ var a=brands[i]; if(!a||a.classList.contains('foot-logo-link')||(a.closest&&a.closest('footer'))) continue; if(a.__orig==null) a.__orig=a.innerHTML; if(L.mode==='image'&&L.imageUrl){ a.innerHTML='<span class="lp-logo-wrap"><img class="lp-logo" src="'+esc(_logoSrc2)+'" srcset="'+esc(_logoSrc)+' 1x, '+esc(_logoSrc2)+' 2x" alt="'+esc(L.text||'')+'" decoding="async"></span>'; a.style.transform=''; } else { a.innerHTML=a.__orig; var mark=a.querySelector('.mark'), nm=a.querySelector('.nm'); if(L.mode==='text'&&mark) mark.style.display='none'; if(nm&&L.text){ var sm=nm.querySelector('small'); nm.textContent=L.text; if(sm) nm.appendChild(sm); } a.style.transformOrigin='left center'; var _ls=_lhd/40; a.style.transform=(_ls!==1?'scale('+_ls+')':''); } a.style.marginLeft=''; a.style.marginRight=''; if(a.closest&&a.closest('header.site')){ a.style.transformOrigin=(_lp==='center')?'center center':((_lp==='right')?'right center':'left center'); } var _isImg=!!(L.mode==='image'&&L.imageUrl); var _tx=[]; if(!_isImg){ var _ls2=_lhd/40; if(_ls2!==1) _tx.push('scale('+_ls2+')'); } if(_offX||_offY) _tx.push('translate('+_offX+'px,'+_offY+'px)'); a.classList.toggle('lp-logo-text-scale', !_isImg); a.classList.toggle('lp-logo-img-place', !!_isImg); a.style.setProperty('--hdr-logo-m-scale', String((_lhm/40)||1)); a.style.setProperty('--hdr-logo-ox', _offX+'px'); a.style.setProperty('--hdr-logo-oy', _offY+'px'); a.style.setProperty('--hdr-logo-ox-m-val', _offXM+'px'); a.style.setProperty('--hdr-logo-oy-m-val', _offYM+'px'); a.style.transform=_tx.join(' '); } }
    var SEC=C.sections||{};
    Object.keys(SEC).forEach(function(id){
      var node=document.querySelector('[data-sec="'+id+'"]'); if(!node) return;
      var s=SEC[id]||{};
      if(s.on===false){ node.setAttribute('hidden',''); node.style.setProperty('display','none','important'); return; } node.removeAttribute('hidden'); node.style.removeProperty('display');
    ...
    try{
      ...
      function __lpNormSec(v){ return String(v==null?'':v).replace(/^#/,'').replace(/[^\w-]/g,'')||'quote'; }
      function __lpBindScrollCta(el,loc,target){ if(!el) return; var nm=__lpNormSec(target); el.setAttribute('href','#'+nm); el.setAttribute('data-lp-scroll',nm); el.classList.add('lp-cta-scroll'); el.onclick=function(ev){ try{ev.preventDefault();}catch(_e){} var tg=document.querySelector('[data-sec="'+nm+'"]')||document.getElementById(nm); if(tg&&tg.scrollIntoView) tg.scrollIntoView({behavior:'smooth',block:'start'}); try{ trackEvent('cta_click',{location:loc||'hero',action:'scroll',target:nm}); }catch(_e2){} }; }
      function __lpClearScrollCta(el){ if(!el) return; el.classList.remove('lp-cta-scroll'); el.removeAttribute('data-lp-scroll'); el.onclick=null; }
      var __call=document.getElementById('heroCall');
      if(__call){ if(__H.callOn===false){ __call.style.display='none'; } else { __call.style.display=''; var __ct=__subT((__H.callText!=null&&__H.callText!=='')?__H.callText:'📞 Call {{phoneText}}'); __hbContent(__call,__H.callIcon,__H.callIconOnly===true,__ct,__ct); __hbStyle(__call,__H.callBg,__H.callBorder,__H.callFg,__H.callGlow===true,__H.callGlowColor); __lpClearScrollCta(__call); if(__H.callAction==='link'&&__H.callHref){ __call.setAttribute('href',__H.callHref); } else if(__H.callAction==='scroll'){ __lpBindScrollCta(__call,'heroCall',__H.callTarget||'customHtml'); } else { __call.setAttribute('href','tel:'+__ph); } } }
      var __q=document.querySelector('.hero .btn-quote');
      if(__q){ if(__H.quoteOn===false){ __q.style.display='none'; } else { __q.style.display=''; var __qt=__subT((__H.quoteText!=null&&__H.quoteText!=='')?__H.quoteText:'Get a fast quote'); __hbContent(__q,__H.quoteIcon,__H.quoteIconOnly===true,__qt,__qt); __hbStyle(__q,__H.quoteBg,__H.quoteBorder,__H.quoteFg,__H.quoteGlow===true,__H.quoteGlowColor); __lpClearScrollCta(__q); if(__H.quoteAction==='call'){ __q.setAttribute('href','tel:'+__ph); } else if(__H.quoteAction==='link'&&__H.quoteHref){ __q.setAttribute('href',__H.quoteHref); } else if(__H.quoteAction==='scroll'){ __lpBindScrollCta(__q,'heroCta',__H.quoteTarget||'customHtml'); } else { __q.setAttribute('href','#quote'); } } }
    ...
    try{ if(typeof lpApplyAboutUs==='function'){ var __AU=(C.sections&&C.sections.aboutUs)||{}; lpApplyAboutUs(__AU, document.querySelector('[data-sec="aboutUs"]'), {business:(C.business||C.name||C.business_name||C.businessName||''), theme:(C.theme||{})}); } }catch(e){}
    ...
    try{ var __ST=(C.sections&&C.sections.seoText)||{}; var __stn=document.querySelector('[data-sec="seoText"]');
      if(__stn){
        try{ function __stHex(v){ v=String(v||'').trim(); if(/^#?[0-9a-fA-F]{3}$/.test(v)){ v=v.charAt(0)==='#'?v:'#'+v; return '#'+v.charAt(1)+v.charAt(1)+v.charAt(2)+v.charAt(2)+v.charAt(3)+v.charAt(3); } if(/^#?[0-9a-fA-F]{6}$/.test(v)) return v.charAt(0)==='#'?v:'#'+v; return ''; } function __stSet(n,v){ var x=__stHex(v); if(x) __stn.style.setProperty(n,x); else __stn.style.removeProperty(n); } __stSet('--seotxt-eyebrow',__ST.eyebrowColor); __stSet('--seotxt-h1',__ST.h1Color); __stSet('--seotxt-h2',__ST.h2Color); __stSet('--seotxt-heading',__ST.headingColor); __stSet('--seotxt-text',__ST.textColor); }catch(_stc){}
        ...
        var __sc=__stn.querySelector('.seotxt-content'); if(__sc){
          ...
          __sc.innerHTML=(typeof __lpFormatSeoText==='function'?__lpFormatSeoText(__raw):(function(raw){
            function _esc(s){return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}
            function _inline(e){return String(e||'').replace(/\*\*([^*]+)\*\*/g,'<strong class="seotxt-strong">$1</strong>');}
            function _looks(line){var t=String(line||'').trim().replace(/:$/,'');if(!t||t.length>72)return false;if(/[.!?]/.test(t))return false;if(/^[-*•]/.test(t))return false;if(/^https?:\/\//i.test(t))return false;var w=t.split(/\s+/).filter(Boolean);return w.length>=1&&w.length<=10;}
            ...
            blocks.forEach(function(block){var lines=String(block||'').split('\n').map(function(l){return l.replace(/\s+$/g,'');});
              ...
              if(lines.length===1&&/^###?\s+/.test(lines[0].trim())){out.push('<h3 class="seotxt-subh">'+_inline(_esc(lines[0].trim().replace(/^###?\s+/,'')))+'</h3>');return;}
              if(lines.length===1&&_looks(lines[0])&&!/^##/.test(lines[0].trim())){out.push('<h3 class="seotxt-subh">'+_inline(_esc(lines[0].trim().replace(/:$/,'')))+'</h3>');return;}
              var paraBuf=[];function flush(){if(!paraBuf.length)return;out.push('<p class="seotxt-p">'+paraBuf.join('<br>\n')+'</p>');paraBuf=[];}
              lines.forEach(function(line,i){var t=line.trim();if(!t){flush();return;}
                if(/^###?\s+/.test(t)){flush();out.push('<h3 class="seotxt-subh">'+_inline(_esc(t.replace(/^###?\s+/,'')))+'</h3>');return;}
                var next=(lines[i+1]||'').trim();
                if(_looks(t)&&next&&next.length>Math.min(40,t.length)&&!/^##/.test(next)){flush();out.push('<h3 class="seotxt-subh">'+_inline(_esc(t.replace(/:$/,'')))+'</h3>');return;}
                paraBuf.push(_inline(_esc(t)));});flush();});
            return out.join('\n');
          })(__raw));
        }
        ...
        var __str=__stn.querySelector('.tb-row'); var __sttx=__stn.querySelector('.tb-text'); if(__str&&__sttx&&__stm){ var __swrap=(__ST.imageLayout==='wrap'); if(__swrap){ __stn.classList.add('tbx-wrap'); } else { __stn.classList.remove('tbx-wrap'); } if(__ST.imageSide==='left'){ __stn.classList.add('tbx-left'); } else { __stn.classList.remove('tbx-left'); } if(__swrap){ if(__str.firstElementChild!==__stm){ __str.insertBefore(__stm,__str.firstChild); } __str.style.flexDirection=''; __str.style.textAlign=''; } else { if(__stm.previousElementSibling!==__sttx){ __str.appendChild(__stm); } __str.style.flexDirection=(__ST.imageSide==='left'?'row-reverse':'row'); __str.style.textAlign=(__ST.textAlign==='center'?'center':''); } }
      }
    }catch(e){}
    ...
    try{ ['navMenu','beforeAfter','responseCards','projectStats','serviceAreas','reviewHighlights','featuredProjects','premiumGallery','featureStrip','specialOffer','heroBeforeAfter','heroSlider','splitHero','activityCounter','proofStream','projectFeed','jobsFeed','beforeAfterFeed','videoReels','activityTimeline','customerReactions','textBox','aboutUs','seoText','searchCanvas','onlineQuote','orderStorefront','bookingStorefront','customHtml'].forEach(function(__id){ var __n=document.querySelector('[data-sec="'+__id+'"]'); if(!__n) return; if(SEC[__id] && SEC[__id].on===true){ __n.style.display='block'; } else { __n.style.display='none'; } }); }catch(e){}
      ...
      (function(){
        var RV0=SEC.reviews||{};
        var rvSec=document.querySelector('[data-sec="reviews"]');
        var proof=(RV0.mode==='proof'||RV0.mode==='homepage'||RV0.mode==='tcard');
        if(rvSec) rvSec.classList.toggle('rv-proof', !!proof);
        if(!Array.isArray(RV0.items)) return;
        if(!proof){
          rlist('.reviews',RV0.items,function(r){return '<div class="review"><div class="stars">'+esc(r.stars||'\u2605\u2605\u2605\u2605\u2605')+'</div><p>'+esc(tok(r.text||''))+'</p><div class="rv-foot"><div class="who">'+esc(r.who||'')+'</div>'+_rvBadge(r.source)+'</div></div>';});
          return;
        }
        var host=document.querySelector('.reviews'); if(!host) return;
        var items=RV0.items.filter(function(r){return r&&r.on!==false&&(r.text||r.who||r.image||r.photo||r.metric);});
        host.innerHTML=items.map(function(r){
          var img=String(r.image||r.photo||'').trim();
          var stars=esc(r.stars||'\u2605\u2605\u2605\u2605\u2605');
          var raw=String(tok(r.text||'')||'').trim();
          var quote=raw;
          if(quote && quote.charAt(0)!=='"' && quote.charAt(0)!=='\u201c') quote='\u201c'+quote.replace(/^["\u201c]|["\u201d]$/g,'')+'\u201d';
          var who=String(r.who||'').trim();
          if(who && who.charAt(0)!=='\u2014' && who.charAt(0)!=='-') who='\u2014 '+who;
          var note=String(r.note||r.badge||'').trim();
          var noteHtml=note?(' <span class="demo-note">'+esc(note)+'</span>'):'';
          var metric=String(r.metric||'').trim();
          var mLabel=String(r.metricLabel||r.metricText||'').trim();
          var metricHtml='';
          if(metric||mLabel){
            var mShow=metric;
            if(mShow && mShow.charAt(0)!=='\u2191' && mShow.indexOf('\u2191')!==0 && mShow.indexOf('↑')!==0 && /\d/.test(mShow)) mShow='\u2191 '+mShow;
            metricHtml='<div class="t-metric">'+esc(mShow)+(mLabel?(' <small>'+esc(mLabel)+'</small>'):'')+'</div>';
          }
          return '<div class="review rv-tcard"><div class="avatar-col">'+(img?('<img class="avatar" src="'+esc(img)+'" alt="" loading="lazy">'):'')+'</div><div class="tcontent"><div class="stars">'+stars+'</div><blockquote class="rv-quote">'+esc(quote)+'</blockquote><p class="tname who">'+esc(who)+noteHtml+'</p>'+metricHtml+'</div></div>';
        }).join('');
      })();
      (function(){
        ...
        _rvSet('--rv-sec-bg', _rvHex(RV.sectionBg));
        _rvSet('--rv-content-bg', _rvHex(RV.contentBg));
        _rvSet('--rv-metric-bg', _rvHex(RV.metricBg));
        _rvSet('--rv-metric', _rvHex(RV.metricColor));
        _rvSet('--rv-metric-label', _rvHex(RV.metricLabelColor));
        _rvSet('--rv-note', _rvHex(RV.noteColor));
        if(RV.avatarWidth!=null&&String(RV.avatarWidth).trim()!==''){ var aw=parseInt(RV.avatarWidth,10); if(!isNaN(aw)) rvNode.style.setProperty('--rv-avatar-w',Math.max(80,Math.min(220,aw))+'px'); }
        else rvNode.style.removeProperty('--rv-avatar-w');
      })();
      ...
      if(rcNode){
        ...
        var _rd=[{icon:'clock',title:'24/7 Emergency',text:'Call any hour, day or night \u2014 someone answers.'},{icon:'zap',title:'Same Day Service',text:'Most jobs sorted the same day you call.'},{icon:'dollar-sign',title:'Upfront Pricing',text:'A fixed price agreed before any work starts.'},{icon:'shield-check',title:'Fully Licensed',text:'Licensed and insured \u2014 details on every invoice.'},{icon:'timer',title:'Rapid Response',text:'We dispatch fast and head straight to you.'},{icon:'headset',title:'No Call Centre',text:'Talk to a local tradie, never a script.'}];
      ...
      var PG=SEC.premiumGallery||{}; var pgNode=document.querySelector('[data-sec="premiumGallery"]');
      if(pgNode) try { _pgRenderPremiumGallery(PG, pgNode); } catch(_pgErr){ try{ console.error('premiumGallery',_pgErr); }catch(_e){} }
      ...
      if(hsNode){ var _hsl=hsNode.querySelector('.hsl');
        if(HS.on===true||__lay==='hero-image-slider'){
          ...
          function _hcta(s,k){ var t=s[k+'CtaText'], a=s[k+'CtaAction'], ic=s[k+'CtaIcon'], io=(s[k+'CtaIconOnly']===true); var ht=t&&String(t).trim(); if((!ht&&!(io&&ic))||a==='none') return ''; var _ip=(ic&&window.LP_ICONS&&window.LP_ICONS[ic])?window.LP_ICONS[ic]:''; var _ico=_ip?'<span class="hb-ic"><svg class="lp-ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'+_ip+'</svg></span>':''; var _only=(io&&_ico); var _inner=_only?_ico:(_ico+(ht?esc(t):'')); var _ec=_only?' btn-icononly':''; var _lbl=(_only&&ht)?(' aria-label="'+esc(t)+'"'):''; function _sc(v){ v=(v==null?'':String(v)).trim(); return (/^#[0-9a-fA-F]{3,8}$/.test(v)||/^[a-zA-Z]{2,20}$/.test(v)||/^rgba?\([0-9.,%\s]+\)$/.test(v))?v:''; } var _st='', _bg=_sc(s[k+'CtaBg']), _bd=_sc(s[k+'CtaBorder']), _fg=_sc(s[k+'CtaFg']); var _fill=s[k+'CtaFill']!==false; var _outline=s[k+'CtaOutline']===true; if(k==='secondary' && s.secondaryCtaOutline==null && s.secondaryCtaFill==null){ _fill=false; _outline=true; } var _sw=lpBtnStrokePx(s[k+'CtaStrokeWidth']||((C.buttonLook&&C.buttonLook.strokeWidth)||'medium')); function _hexRgba(hex,a){ var m=/^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.exec(String(hex||'').trim()); if(!m) return ''; var h=m[1]; if(h.length===3) h=h[0]+h[0]+h[1]+h[1]+h[2]+h[2]; var n=parseInt(h,16); return 'rgba('+((n>>16)&255)+','+((n>>8)&255)+','+(n&255)+','+a+')'; } var _op=s[k+'CtaOpacity']; var _opa=1; if(_op!=null&&_op!==''){ var _opn=parseFloat(_op); if(!isNaN(_opn)) _opa=Math.max(0,Math.min(100,_opn))/100; } var _bgUse=_bg; if(!_bgUse && _opa<1){ _bgUse=_sc((k==='primary'?((C.theme&&C.theme.hivis)||'#ff6a1f'):'#ffffff'))|| (k==='primary'?'#ff6a1f':'#ffffff'); } if(_outline){ _st+='background:transparent;'; if(_fg)_st+='color:'+_fg+';'; var _oc=_bd||_fg||'#ffffff'; _st+='border:'+_sw+' solid '+_oc+';'; } else { if(_bgUse && _fill){ var _bgOut=(_opa<1?(_hexRgba(_bgUse,_opa)||_bgUse):_bgUse); _st+='background:'+_bgOut+';'; } else if(!_fill){ _st+='background:transparent;'; } if(_fg)_st+='color:'+_fg+';'; if(_bd)_st+='border:'+_sw+' solid '+_bd+';'; else if(!_bg&&k==='secondary') _st+='border:'+_sw+' solid rgba(0,0,0,.14);'; else if(!_bg&&k==='primary') _st+='border:'+_sw+' solid transparent;'; } if(_outline) _ec+=' lp-btn-outline'; else if(!_fill) _ec+=' lp-btn-nofill'; if(s[k+'CtaGlow']===true){ var _gc=_sc(s[k+'CtaGlowColor'])||_sc((C.theme&&C.theme.hivis)||'')||'#000000'; var _gm=/^#([0-9a-fA-F]{6})$/.exec(_gc); if(_gm){ var _gn=parseInt(_gm[1],16); _st+='--btn-call-glow:rgba('+((_gn>>16)&255)+','+((_gn>>8)&255)+','+(_gn&255)+',.4);'; _ec+=' has-glow'; } } else { _st+='box-shadow:none;'; } var _sa=_st?(' style="'+_st+'"'):''; var _clsP=(k==='primary'||a==='call')?'btn btn-call':'btn btn-quote'; if(a==='call') return '<a class="'+_clsP+' hsl-cta-call'+_ec+'" href="'+esc(_tel)+'"'+_lbl+_sa+'>'+_inner+'</a>'; if(a==='quote') return '<a class="btn btn-quote'+_ec+'" href="#quote"'+_lbl+_sa+'>'+_inner+'</a>'; if(a==='scroll'){ var _tg=String(s[k+'CtaTarget']||'customHtml').replace(/^#/,'').replace(/[^\w-]/g,'')||'customHtml'; return '<a class="btn btn-quote hsl-cta-scroll'+_ec+'" href="#'+esc(_tg)+'" data-lp-scroll="'+esc(_tg)+'" data-lp-cta-loc="heroSlider"'+_lbl+_sa+'>'+_inner+'</a>'; } if(a==='page'){ var _slug=String(s[k+'CtaPage']||'').replace(/^\/+/,'').trim(); var _ph=_slug?('/'+_slug):'#'; return '<a class="btn btn-quote'+_ec+'" href="'+esc(_ph)+'"'+_lbl+_sa+'>'+_inner+'</a>'; } if(a==='url'||a==='link'){ var _u=String(s[k+'CtaUrl']||s[k+'CtaHref']||'').trim(); if(_u && !/^https?:\/\//i.test(_u) && _u.charAt(0)!=='/' && _u.charAt(0)!=='#') _u='https://'+_u; var _blank=(s[k+'CtaBlank']!==false); return '<a class="btn btn-quote'+_ec+'" href="'+esc(_u||'#')+'"'+(_blank?' target="_blank" rel="noopener noreferrer"':'')+_lbl+_sa+'>'+_inner+'</a>'; } return ''; }
          if(_trk){ _trk.innerHTML=_sl.map(function(s){ var _u=(s.imageUrl!=null?String(s.imageUrl).trim():''); var _us=_u.replace(/[)\s"<>]/g,''); var _fit=(s.imageFit||'cover'); var _pm={left:'left center',right:'right center',top:'center top',bottom:'center bottom',center:'center'}; var _pos=_pm[s.imagePos||'center']||'center'; var _bgc=(s.bgColour&&String(s.bgColour).trim())?String(s.bgColour).trim():''; var _sz=(_fit==='contain'||_fit==='blend')?'contain':((_fit==='fill'||_fit==='stretch')?'100% 100%':'cover'); var _rep=(_fit==='contain'||_fit==='blend')?';background-repeat:no-repeat':''; var _bg=_us?('<div class="hsl-bg" style="background-image:url('+_us+');background-size:'+_sz+';background-position:'+_pos+_rep+'"></div>'):('<div class="hsl-bg hsl-bg-grad"></div>'); var _blend=(_fit==='blend'&&_bgc)?('<div class="hsl-blend" style="background:linear-gradient(90deg,'+_bgc+' 0%,rgba(0,0,0,0) 16%,rgba(0,0,0,0) 84%,'+_bgc+' 100%)"></div>'):''; var _al=(s.slideTextAlign&&String(s.slideTextAlign).trim())?(' hsl-al-'+String(s.slideTextAlign).trim()):''; var _slStyle=_bgc?(' style="background-color:'+_bgc+'"'):''; function _hslCol(v){ v=(v==null?'':String(v)).trim(); return (/^#[0-9a-fA-F]{3,8}$/.test(v)||/^rgba?\([0-9.,%\s]+\)$/.test(v))?v:''; } var _ftc=_hslCol(s.featureTextColor), _hlc=_hslCol(s.highlightColor), _ebc=_hslCol(s.eyebrowColor), _hdc=_hslCol(s.headingColor); var _innerSt=_ftc?(' style="color:'+_ftc+'"'):''; var _ebSt=_ebc?(' style="color:'+_ebc+'"'):''; var _hdSt=_hdc?(' style="color:'+_hdc+'"'):''; var _hlSpan=(s.highlightText&&String(s.highlightText).trim())?(' <span class="hl"'+(_hlc?(' style="color:'+_hlc+'"'):'')+'>'+esc(s.highlightText)+'</span>'):''; var _hd=esc(s.heading||'')+_hlSpan; return '<div class="hsl-slide'+_al+'"'+_slStyle+'><div class="hsl-bgwrap">'+_bg+'</div><div class="hsl-ov"></div>'+_blend+'<div class="hsl-inner"'+_innerSt+'><div class="wrap">'+(s.eyebrow&&String(s.eyebrow).trim()?'<span class="eyebrow hsl-eyebrow"'+_ebSt+'>'+esc(s.eyebrow)+'</span>':'')+(_hd.trim()?'<h2 class="hsl-h"'+_hdSt+'>'+_hd+'</h2>':'')+(s.subText&&String(s.subText).trim()?'<p class="hsl-sub">'+esc(s.subText)+'</p>':'')+'<div class="hsl-cta">'+_hcta(s,'primary')+_hcta(s,'secondary')+'</div></div></div></div>'; }).join('');
            var _cl=_trk.querySelectorAll('.hsl-cta-call'); for(var _ci=0;_ci<_cl.length;_ci++){ (function(b){ b.addEventListener('click',function(){ try{ trackEvent('call_click',{location:'heroSlider'}); }catch(_e){} }); })(_cl[_ci]); } var _slc=_trk.querySelectorAll('.hsl-cta-scroll'); for(var _si=0;_si<_slc.length;_si++){ (function(b){ b.addEventListener('click',function(ev){ try{ev.preventDefault();}catch(_e0){} var nm=b.getAttribute('data-lp-scroll')||''; var tg=nm?(document.querySelector('[data-sec="'+nm+'"]')||document.getElementById(nm)):null; if(tg&&tg.scrollIntoView) tg.scrollIntoView({behavior:'smooth',block:'start'}); try{ trackEvent('cta_click',{location:b.getAttribute('data-lp-cta-loc')||'heroSlider',action:'scroll',target:nm}); }catch(_e){} }); })(_slc[_si]); } }
      ...
      if(acNode){
        ...
        var _acd=[{icon:'wrench',value:'7',label:'Jobs completed today'},{icon:'phone',value:'24',label:'Calls answered this week'},{icon:'smile',value:'387',label:'Happy customers this year'},{icon:'star',value:'4.9',label:'Google Rating'}];
        var _acit=(Array.isArray(AC.stats)&&AC.stats.length)?AC.stats:_acd;
        var _acg=acNode.querySelector('.ac-grid');
        if(_acg){ _acg.innerHTML=_acit.filter(function(it){return it&&it.on!==false&&((it.value!=null&&String(it.value).trim())||(it.label!=null&&String(it.label).trim()));}).map(function(it){ var _acic=(it.icon&&window.LP_ICONS&&window.LP_ICONS[it.icon])?('<span class="ac-ic" aria-hidden="true"><svg class="lp-ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'+window.LP_ICONS[it.icon]+'</svg></span>'):''; return '<div class="ac-stat"><div class="ac-value-row">'+_acic+'<div class="ac-value" data-raw="'+esc(String(it.value!=null?it.value:''))+'">'+esc(String(it.value!=null?it.value:''))+'</div></div><div class="ac-label">'+esc(it.label||'')+'</div></div>'; }).join('');
      ...
      ;(function(){ var CR=SEC.certifications||{}; var node=document.querySelector('[data-sec="certifications"]'); if(node&&CR.on!==false){ var host=node.querySelector('.cert-grid'); if(host){ var its=(CR.items||[]).filter(function(x){return x&&x.on!==false;}); host.innerHTML=its.map(function(x){ var img=(x.image!=null?String(x.image).trim():''); var logo=''; if(img){ logo='<img class="cert-logo" src="'+esc(img)+'" alt="'+esc(x.name||'')+'" loading="lazy">'; } else { var nm=String(x.name||'C').trim(); var initials=nm.split(/\s+/).filter(Boolean).slice(0,2).map(function(w){return w.charAt(0).toUpperCase();}).join('')||'C'; logo='<div class="cert-ph" aria-hidden="true">'+esc(initials)+'</div>'; } var meta=''; if(x.number) meta+='<span>No. '+esc(x.number)+'</span>'; if(x.expiry) meta+='<span>Exp '+esc(x.expiry)+'</span>'; return '<div class="cert-card">'+logo+(x.name?'<div class="cert-name">'+esc(x.name)+'</div>':'')+(x.body?'<div class="cert-body">'+esc(x.body)+'</div>':'')+(meta?'<div class="cert-meta">'+meta+'</div>':'')+'</div>'; }).join(''); if(!its.length) node.style.display='none'; } } })();
      ...
      ;(function(){ var OQ=SEC.onlineQuote||{}; var node=document.querySelector('[data-sec="onlineQuote"]'); if(!node) return;
        ...
        var _eh=node.querySelector('.ey'); if(_eh){ _eh.textContent=(OQ.eyebrow||''); if(OQ.eyebrowColor&&/^#[0-9a-fA-F]{6}$/.test(OQ.eyebrowColor)) _eh.style.color=OQ.eyebrowColor; else _eh.style.color=''; }
        var _hh=node.querySelector('h2'); if(_hh){ _hh.textContent=(OQ.heading||''); if(OQ.headingColor&&/^#[0-9a-fA-F]{6}$/.test(OQ.headingColor)) _hh.style.color=OQ.headingColor; else _hh.style.color=''; }
        var _pp=node.querySelector('.intro'); if(_pp){ var _pv=(OQ.intro||''); _pp.textContent=_pv; _pp.style.display=_pv?'':'none'; if(OQ.introColor&&/^#[0-9a-fA-F]{6}$/.test(OQ.introColor)) _pp.style.color=OQ.introColor; else _pp.style.color=''; }
        if(OQ.bg&&/^#[0-9a-fA-F]{6}$/.test(OQ.bg)) node.style.background=OQ.bg; else node.style.background='';
      ...
      if(vrNode){
        ...
        var _vrd=[{title:'Inside a full bathroom reno',tag:'0:42',thumbnail:'https://images.unsplash.com/photo-1584622650111-993a426fbf0a?w=900&h=1600&fit=crop&q=80'},{title:'How we clear a blocked drain',tag:'0:28',thumbnail:'https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?w=900&h=1600&fit=crop&q=80'},{title:'Hot water swap in under an hour',tag:'0:35',thumbnail:'https://images.unsplash.com/photo-1621905252507-b35492cc74b4?w=900&h=1600&fit=crop&q=80'}];
      ...
      if(tbNode){
        ...
        if(_tbMode==='images'){
          ...
          if(!_tiles.length) _tiles=_tbd;
                    if(_tbr){
            var _tbBgUrls=[];
            _tbr.innerHTML=_tiles.map(function(b,ti){
              var img=(b.image!=null?String(b.image).trim():'');
              var lab=(b.label!=null?String(b.label):'');
              var ic=_tbIc(b.icon);
              var _fit=(b.imageFit==='contain'||b.imageFit==='fill'||b.imageFit==='stretch'||b.imageFit==='cover')?b.imageFit:'cover'; if(_fit==='stretch') _fit='100% 100%'; else if(_fit==='fill') _fit='100% 100%'; var _pm={left:'left center',right:'right center',top:'center top',bottom:'center bottom',center:'center'}; var _pos=_pm[b.imagePos||'center']||'center';
              var bg;
              if(img){
                _tbBgUrls.push({i:ti,url:img,fit:_fit,pos:_pos});
                bg='<div class="tb-tile-bg" data-tb-bg="'+ti+'"></div>';
              } else {
      ...
      if(spNode){
        ...
        var _spSecStrokeOn=SP.sectionStrokeOn===true; var _spSecStroke=_spHex(SP.sectionStroke);
        spNode.classList.toggle('sp-sec-stroke-on', !!_spSecStrokeOn);
        if(_spSecStrokeOn&&_spSecStroke) spNode.style.setProperty('--sp-sec-stroke', _spSecStroke); else spNode.style.removeProperty('--sp-sec-stroke');
        spNode.style.display=(SP.on===false)?'none':'';
      }
      ...
      if(_prHero&&_prInlineSec){
        ...
        function _prActive(p){ var t=p.type;
          ...
          return true; }
        function _prDefCols(style){
          var th=(C&&C.theme)||{};
          var hv=th.hivis||'#ff7a00';
          if(style==='card') return {bg:'#ffffff',heading:'#1a2230',text:'#5b6878',btnBg:hv,btnText:'#ffffff',timeText:'#1a2230',timeBg:'#f3f1ec'};
          return {bg:'#1a2230',heading:'#ffffff',text:'#ffffff',btnBg:hv,btnText:'#ffffff',timeText:'#ffffff',timeBg:'#2a3545'};
        }
        function _prCol(p,key,defs){ var v=p.colors&&p.colors[key]; return (v&&String(v).trim())?String(v).trim():defs[key]; }
        function _prInnerStyle(p){
          var defs=_prDefCols(p.style||'banner');
          if(p.colors&&p.colors.bg) return 'background:'+_prCol(p,'bg',defs)+';';
          if((p.style||'banner')==='card') return 'background:#fff;';
          return 'background:linear-gradient(135deg,var(--steel-900,#1a2230),var(--steel-950,#0e131c));';
        }
        function _prCd(p){ if(p.type!=='weekly'&&p.type!=='deadline') return ''; var defs=_prDefCols(p.style||'banner'); var seg=function(cl,lb){return '<span class="cd-seg" style="background:'+esc(_prCol(p,'timeBg',defs))+';"><b class="'+cl+'" style="color:'+esc(_prCol(p,'timeText',defs))+';">00</b><i style="color:'+esc(_prCol(p,'timeText',defs))+';">'+lb+'</i></span>';}; var attrs; if(p.type==='deadline'){ var t=Date.parse(p.expiry); if(isNaN(t)) return ''; attrs='data-mode="fixed" data-target="'+t+'"'; } else { var d=(p.weeklyDay!=null?p.weeklyDay:4); attrs='data-mode="weekly" data-day="'+esc(String(d))+'" data-cut="'+esc(p.weeklyCutoff||'16:00')+'"'; } return '<div class="promo-cd" '+attrs+'>'+seg('cd-d','days')+seg('cd-h','hrs')+seg('cd-m','min')+seg('cd-s','sec')+'</div>'; }
        function _prExtra(p){ var t=p.type;
          ...
          return ''; }
        function _prCta(p){ var defs=_prDefCols(p.style||'banner'); var a=(p.cta&&p.cta.action)||'quote'; var txt=(p.cta&&p.cta.text)?p.cta.text:'Book now'; if(a==='none') return ''; var sty=' style="background:'+esc(_prCol(p,'btnBg',defs))+';color:'+esc(_prCol(p,'btnText',defs))+';"'; if(a==='call'){ var ph=(typeof SITE_CONFIG!=='undefined'&&SITE_CONFIG.phone)?SITE_CONFIG.phone:''; if(!ph) return ''; return '<a class="promo-cta promo-call" href="tel:'+esc(ph)+'"'+sty+'>'+esc(txt)+'</a>'; } if(a==='url'){ var u=(p.cta&&p.cta.url)?String(p.cta.url).trim():''; if(!u) return ''; if(!/^https?:\/\//i.test(u)&&u.charAt(0)!=='/'&&u.charAt(0)!=='#') u='https://'+u; return '<a class="promo-cta promo-url" href="'+esc(u)+'" target="_blank" rel="noopener noreferrer"'+sty+'>'+esc(txt)+'</a>'; } return '<a class="promo-cta" href="#quote"'+sty+'>'+esc(txt)+'</a>'; }
        function _prInner(p){ var defs=_prDefCols(p.style||'banner'); var before=''; if(p.type==='weekly'){ var d=(p.weeklyDay!=null?p.weeklyDay:4); before='<p class="promo-before" style="color:'+esc(_prCol(p,'text',defs))+';">Book before: '+esc(_DOW[d]||'')+' '+esc(_prFmtTime(p.weeklyCutoff||'16:00'))+'</p>'; } var txt='<div class="promo-text">'+(p.title?'<h3 class="promo-title" style="color:'+esc(_prCol(p,'heading',defs))+';">'+esc(p.title)+'</h3>':'')+(p.description?'<p class="promo-desc" style="color:'+esc(_prCol(p,'text',defs))+';">'+esc(p.description)+'</p>':'')+before+'</div>'; var act=_prCd(p)+_prCta(p); return txt+_prExtra(p)+(act?('<div class="promo-actions">'+act+'</div>'):''); }
        function _prCard(p,idx){ return '<div class="promo promo-'+((p.style==='card')?'card':'banner')+'" data-promo="'+idx+'"><div class="promo-inner" style="'+_prInnerStyle(p)+'">'+_prInner(p)+'</div></div>'; }
    ...
    (function(){ var S=C.sections||{};
      ...
      var Q=S.quote||{}; var qs=document.querySelector('[data-sec="quote"]'); if(qs){ var _qc=qs.querySelector('.qcard'); if(_qc){ if(Q.formStyle==='feature') _qc.classList.add('qcard-feature'); else _qc.classList.remove('qcard-feature'); }
        (function(){ function _qsc(v){ v=(v==null?'':String(v)).trim(); return (/^#[0-9a-fA-F]{3,8}$/.test(v)||/^[a-zA-Z]{2,20}$/.test(v)||/^rgba?\([0-9.,%\s]+\)$/.test(v))?v:''; } function _qhx(v){ var m=/^#([0-9a-fA-F]{6})$/.exec(String(v||'').trim()); if(!m) return null; var n=parseInt(m[1],16); return [(n>>16)&255,(n>>8)&255,n&255]; } function _qf(name,cls,val){ var c=_qsc(val); if(c){ qs.style.setProperty(name,c); qs.classList.add(cls); } else { qs.style.removeProperty(name); qs.classList.remove(cls); } return c; } _qf('--qf-bg','qf-bg',Q.bg); _qf('--qf-input-bg','qf-ibg',Q.inputBg); _qf('--qf-input-border','qf-ibd',Q.inputBorder); _qf('--qf-label','qf-lbl',Q.label); _qf('--qf-text','qf-txt',Q.text); _qf('--qf-success','qf-suc',Q.success); var _acc=_qf('--qf-accent','qf-acc',Q.accent); _qf('--qf-error','qf-err',Q.error); var _ico=_qf('--qf-icon','qf-icon',Q.iconColor); var _isc=parseInt(Q.iconScale,10); if(isNaN(_isc)) _isc=100; qs.style.setProperty('--qf-icon-size',Math.round(23*Math.max(50,Math.min(250,_isc))/100)+'px'); var _bgo=parseInt(Q.bgOpacity,10); if(isNaN(_bgo)) _bgo=100; qs.style.setProperty('--qf-bg-opacity',Math.max(0,Math.min(100,_bgo))+'%'); if(Q.formStrokeOn===true){ var _st=_qsc(Q.formStroke)||_qsc((C.theme&&C.theme.pipe)||'')||'#1f7bb8'; qs.style.setProperty('--qf-stroke',_st); qs.style.setProperty('--qf-stroke-w','2px'); qs.classList.add('qf-stroke'); } else { qs.style.removeProperty('--qf-stroke'); qs.style.removeProperty('--qf-stroke-w'); qs.classList.remove('qf-stroke'); } var _ag=_qhx(Q.accent); if(_ag) qs.style.setProperty('--qf-accent-glow','rgba('+_ag[0]+','+_ag[1]+','+_ag[2]+',.18)'); else qs.style.removeProperty('--qf-accent-glow'); var _eg=_qhx(Q.error); if(_eg) qs.style.setProperty('--qf-error-glow','rgba('+_eg[0]+','+_eg[1]+','+_eg[2]+',.16)'); else qs.style.removeProperty('--qf-error-glow'); var _bt=qs.querySelector('.btn-call'); if(_bt){ var _bbg=_qsc(Q.btnBg),_btx=_qsc(Q.btnText),_bhv=_qsc(Q.btnHover); _bt.style.background=''; _bt.style.color=''; if(_bbg){ qs.style.setProperty('--qf-btn-bg',_bbg); _bt.classList.add('qf-bbg'); } else { qs.style.removeProperty('--qf-btn-bg'); _bt.classList.remove('qf-bbg'); } if(_btx){ qs.style.setProperty('--qf-btn-text',_btx); _bt.classList.add('qf-btxt'); } else { qs.style.removeProperty('--qf-btn-text'); _bt.classList.remove('qf-btxt'); } if(Q.btnHoverOff===true){ var _ob=_bbg||(Q.formStyle==='feature'?'#ffffff':'var(--hivis)'); qs.style.setProperty('--qf-btn-hover',_ob); _bt.classList.add('qf-bhov'); } else if(_bhv){ qs.style.setProperty('--qf-btn-hover',_bhv); _bt.classList.add('qf-bhov'); } else { qs.style.removeProperty('--qf-btn-hover'); _bt.classList.remove('qf-bhov'); } if(Q.btnShadowOff===true||(Q.btnShadowOn!==true&&!_qhx(Q.btnShadow))){ _bt.classList.add('qf-noshadow'); _bt.classList.remove('qf-bsh','has-glow'); qs.style.removeProperty('--qf-btn-shadow'); _bt.style.removeProperty('--btn-call-glow'); } else { _bt.classList.remove('qf-noshadow'); var _bsh=_qhx(Q.btnShadow)||_qhx((C.theme&&C.theme.hivis)||''); if(_bsh){ qs.style.setProperty('--qf-btn-shadow','rgba('+_bsh[0]+','+_bsh[1]+','+_bsh[2]+',.4)'); _bt.style.setProperty('--btn-call-glow','rgba('+_bsh[0]+','+_bsh[1]+','+_bsh[2]+',.4)'); _bt.classList.add('qf-bsh','has-glow'); } else { qs.style.removeProperty('--qf-btn-shadow'); _bt.style.removeProperty('--btn-call-glow'); _bt.classList.remove('qf-bsh','has-glow'); } } } (function(){ function _qHex(v){ v=String(v||'').trim(); if(/^#?[0-9a-fA-F]{3}$/.test(v)){ v=v.charAt(0)==='#'?v:'#'+v; return '#'+v.charAt(1)+v.charAt(1)+v.charAt(2)+v.charAt(2)+v.charAt(3)+v.charAt(3); } if(/^#?[0-9a-fA-F]{6}$/.test(v)) return v.charAt(0)==='#'?v:'#'+v; return ''; } function _qSet(n,v){ if(v) qs.style.setProperty(n,v); else qs.style.removeProperty(n); } _qSet('--quote-eyebrow', _qHex(Q.eyebrowColor)||_acc||''); _qSet('--quote-heading', _qHex(Q.headingColor)); _qSet('--quote-intro', _qHex(Q.introColor)); var _eb=qs.querySelector('.eyebrow'); if(_eb){ var _ec=_qHex(Q.eyebrowColor)||_acc||''; if(_ec) _eb.style.color=_ec; else _eb.style.removeProperty('color'); } var _qh2=qs.querySelector('h2'); if(_qh2){ var _hc=_qHex(Q.headingColor); if(_hc) _qh2.style.color=_hc; else _qh2.style.removeProperty('color'); } var _ls=qs.querySelector('.lead-sub'); if(_ls){ var _ic=_qHex(Q.introColor); if(_ic) _ls.style.color=_ic; else _ls.style.removeProperty('color'); } })(); var _fc=qs.querySelector('#formCall'); if(_fc) _fc.style.color=_acc||'var(--pipe)'; })();
        if(Q.eyebrow!=null){ var _qeb=qs.querySelector('.eyebrow'); if(_qeb) _qeb.textContent=Q.eyebrow; }
        if(Q.heading!=null){ var _qh2t=qs.querySelector('h2'); if(_qh2t) _qh2t.textContent=Q.heading; }
        ...
        if(Q.button!=null){ var bt=qs.querySelector('.btn-call'); if(bt) bt.textContent=Q.button; } var _qLbl=function(forId,val){ if(val==null) return; var _l=qs.querySelector('label[for="'+forId+'"]'); if(_l) _l.textContent=val; }; if(Q.formTitle!=null){ var _qh=qs.querySelector('#quoteForm h3'); if(_qh) _qh.textContent=Q.formTitle; } _qLbl('name',Q.lblName); _qLbl('phone',Q.lblPhone); _qLbl('job',Q.lblJob); _qLbl('suburb',Q.lblSuburb); _qLbl('detail',Q.lblDetail); if(Q.suburbPh!=null){ var _qsb=qs.querySelector('#suburb'); if(_qsb) _qsb.setAttribute('placeholder',Q.suburbPh); } if(Q.detailPh!=null){ var _qdt=qs.querySelector('#detail'); if(_qdt) _qdt.setAttribute('placeholder',Q.detailPh); } if(Q.namePh!=null){ var _qnm=qs.querySelector('#name'); if(_qnm) _qnm.setAttribute('placeholder',Q.namePh); } if(Q.phonePh!=null){ var _qpn=qs.querySelector('#phone'); if(_qpn) _qpn.setAttribute('placeholder',Q.phonePh); } (function(){ function _shown(sk){ return Q[sk]!==false; } function _vis(id,sk){ var wrap=qs.querySelector('.ff[data-qf="'+id+'"]')||(qs.querySelector('#'+id)&&qs.querySelector('#'+id).closest('.ff')); if(!wrap) return; var on=_shown(sk); if(on){ wrap.classList.remove('qf-off'); wrap.removeAttribute('hidden'); } else { wrap.classList.add('qf-off'); wrap.setAttribute('hidden',''); } var el=qs.querySelector('#'+id); if(el&&!on){ el.removeAttribute('required'); el.value=''; } } _vis('name','showName'); _vis('phone','showPhone'); _vis('email','showEmail'); _vis('job','showJob'); _vis('suburb','showSuburb'); _vis('detail','showDetail'); function _rq(id,key,df,sk){ var el=qs.querySelector('#'+id); if(!el) return; if(!_shown(sk)){ el.removeAttribute('required'); return; } var on=(Q[key]===true)||(Q[key]==null&&df); if(on) el.setAttribute('required',''); else el.removeAttribute('required'); } _rq('name','reqName',true,'showName'); _rq('phone','reqPhone',true,'showPhone'); _rq('email','reqEmail',true,'showEmail'); _rq('job','reqJob',false,'showJob'); _rq('suburb','reqSuburb',false,'showSuburb'); _rq('detail','reqDetail',false,'showDetail'); var _xh=qs.querySelector('#quoteExtraFields'); if(_xh){ var _xf=(Array.isArray(Q.extraFields)?Q.extraFields:[]).filter(function(x){return x&&x.on!==false&&String(x.label||'').trim();}); _xh.innerHTML=_xf.map(function(x,i){ var lab=esc(String(x.label).trim()); var ph=esc(String(x.placeholder||'').trim()); return '<div class="ff" data-qf-extra="'+i+'"><label for="q-extra-'+i+'">'+lab+'</label><input id="q-extra-'+i+'" type="text" placeholder="'+ph+'" autocomplete="off"></div>'; }).join(''); } })(); if(Q.successTitle!=null){ var _qsc=qs.querySelector('#quoteSuccess .big'); if(_qsc) _qsc.textContent=Q.successTitle; } (function(){ var _fine=qs.querySelector('.q-fine'); if(!_fine) return; var _ph=(typeof SITE_CONFIG!=='undefined'&&SITE_CONFIG.phone)?SITE_CONFIG.phone:''; var _lead=(Q.fineText!=null?Q.fineText:'Genuine emergency? Don’t wait —'); var _ct=(Q.callText!=null&&String(Q.callText).trim())?Q.callText:'call us now'; var _on=(Q.callOn!==false); if(!_on||!_ph){ _fine.textContent=_lead; return; } _fine.innerHTML=esc(_lead)+(_lead?' ':'')+'<a id="formCall" href="tel:'+esc(_ph)+'" style="color:var(--pipe);font-weight:700">'+esc(_ct)+'</a>.'; var _fc=_fine.querySelector('#formCall'); if(_fc) _fc.addEventListener('click',function(){ try{ trackEvent('call_click',{location:'quote_form'}); }catch(_e){} }); })();
      try{
      ...
      var F=(S.footer)||{};
      var ft=document.querySelector('footer.site')||document.querySelector('footer[data-sec="footer"]'); if(ft){
        try{ if(!ft.classList.contains('site')) ft.classList.add('site'); }catch(_fc){}

        if(!ft.querySelector('.footer-top')){
          ft.innerHTML='<div class="wrap-wide"><div class="footer-top"><div class="f-brand"><a class="brand foot-logo-link" href="#top"><span class="foot-logo-wrap" hidden></span><span class="mark"></span><span class="nm" style="color:#fff"></span></a><p class="f-tagline-brand" hidden></p><p class="blurb foot-blurb"></p></div><nav class="f-nav" aria-label="Footer"></nav><div class="f-support"><div class="f-support-copy"><div class="f-support-lead"><h4>Australian support</h4><p class="help">We are here to help.</p></div><div class="f-support-contacts"></div></div><div class="f-support-map" aria-hidden="true"></div></div></div><div class="footer-bottom"><p class="f-tagline"><span class="f-tagline-lead"></span> <span class="f-tagline-accent"></span></p><div class="f-bottom-meta"><div class="f-legal foot-legal"></div><nav class="f-links" aria-label="Legal"></nav></div></div></div>';
        }

        function _fHex(v){ v=String(v||'').trim(); if(/^#?[0-9a-fA-F]{3}$/.test(v)){ v=v.charAt(0)==='#'?v:'#'+v; return '#'+v.charAt(1)+v.charAt(1)+v.charAt(2)+v.charAt(2)+v.charAt(3)+v.charAt(3); } if(/^#?[0-9a-fA-F]{6}$/.test(v)) return v.charAt(0)==='#'?v:'#'+v; return ''; }
        var bg=_fHex(F.bg); if(bg) ft.style.setProperty('--foot-bg',bg); else ft.style.removeProperty('--foot-bg');
        var acc=_fHex(F.accentColor||F.accent); if(acc) ft.style.setProperty('--foot-accent',acc); else ft.style.removeProperty('--foot-accent');
        var ink=_fHex(F.inkColor||F.ink); if(ink){ ft.style.setProperty('--mkt-foot-ink',ink); } else { ft.style.removeProperty('--mkt-foot-ink'); }
        var muted=_fHex(F.mutedColor||F.muted); if(muted){ ft.style.setProperty('--mkt-foot-muted',muted); ft.style.setProperty('--mkt-foot-soft',muted); ft.style.setProperty('--mkt-foot-faint',muted); } else { ft.style.removeProperty('--mkt-foot-muted'); ft.style.removeProperty('--mkt-foot-soft'); ft.style.removeProperty('--mkt-foot-faint'); }
        var mapC=_fHex(F.mapColor||F.map); if(mapC){ ft.style.setProperty('--mkt-foot-map',mapC); } else { ft.style.removeProperty('--mkt-foot-map'); }
        ft.style.setProperty('--foot-col-case', F.columnTitleCaps===true?'uppercase':'none');
        ...
        ft.classList.toggle('foot-hide-support-m', F.showSupportMobile===false);
        ...
        var logoWrap=ft.querySelector('.foot-logo-wrap');
        if(brandA&&!logoWrap){
          brandA.innerHTML='<span class="foot-logo-wrap" hidden></span><span class="mark"></span><span class="nm" style="color:#fff"></span>';
          logoWrap=brandA.querySelector('.foot-logo-wrap');
        }
        var mark=brandA&&brandA.querySelector('.mark');
        ...
        var logoHM=parseInt(F.logoHeightM!=null?F.logoHeightM:Math.round(logoH*0.78),10); if(isNaN(logoHM)) logoHM=Math.round(logoH*0.78); logoHM=Math.max(20,Math.min(120,logoHM));
        var lx=parseInt(F.logoOffsetX!=null?F.logoOffsetX:0,10)||0;
        var ly=parseInt(F.logoOffsetY!=null?F.logoOffsetY:0,10)||0;
        var lxM=(F.logoOffsetXM!=null&&F.logoOffsetXM!=='')?(parseInt(F.logoOffsetXM,10)||0):lx;
        var lyM=(F.logoOffsetYM!=null&&F.logoOffsetYM!=='')?(parseInt(F.logoOffsetYM,10)||0):ly;
        ft.style.setProperty('--foot-logo-h', logoH+'px');
        ft.style.setProperty('--foot-logo-h-m', logoHM+'px');
        ft.style.setProperty('--foot-logo-x', lx+'px');
        ft.style.setProperty('--foot-logo-y', ly+'px');
        ft.style.setProperty('--foot-logo-x-m', lxM+'px');
        ft.style.setProperty('--foot-logo-y-m', lyM+'px');
        if(logoWrap){
          logoWrap.style.setProperty('--foot-logo-h', logoH+'px');
          logoWrap.style.setProperty('--foot-logo-h-m', logoHM+'px');
          logoWrap.style.setProperty('--foot-logo-x', lx+'px');
          logoWrap.style.setProperty('--foot-logo-y', ly+'px');
          logoWrap.style.setProperty('--foot-logo-x-m', lxM+'px');
          logoWrap.style.setProperty('--foot-logo-y-m', lyM+'px');
          logoWrap.style.removeProperty('transform');
          if(logoUrl){
            logoWrap.hidden=false; logoWrap.removeAttribute('hidden');
            logoWrap.innerHTML='<img class="foot-logo-img" src="'+esc(logoUrl)+'" alt="'+esc((C.businessName||C.logo&&C.logo.text)||'')+'">';
        ...
        var brandTag=ft.querySelector('.f-tagline-brand');
        if(brandTag){
          var bt=(F.brandTagline!=null?String(F.brandTagline):'').trim();
          var bta=(F.brandTaglineAccent!=null?String(F.brandTaglineAccent):'').trim();
          if(bt||bta){ brandTag.hidden=false; brandTag.innerHTML=esc(bt)+(bta?(' <span class="acc">'+esc(bta)+'</span>'):''); }
          else { brandTag.hidden=true; brandTag.textContent=''; }
        }
        var blurbText=F.blurb!=null?String(tok(F.blurb)).trim():'';
        var fp=ft.querySelector('.foot-blurb')||ft.querySelector('.blurb');
        if(fp){ fp.textContent=blurbText; fp.hidden=!blurbText; }
        // Nav columns
        var nav=ft.querySelector('.f-nav');
        if(nav){
          var cols=Array.isArray(F.columns)?F.columns:[];
          if(!cols.length && Array.isArray(F.services)){
            cols=[{on:true,title:(F.servicesTitle||'Services'),links:F.services}];
          }
          function _ftResolveHref(l){
            if(!l) return null;
            var act=String(l.action||l.linkAction||'url').toLowerCase();
            if(act==='none'||act==='off') return null;
            if(act==='scroll'||act==='section'){
              var t=String(l.target||l.linkTarget||l.href||'quote').replace(/^#/,'').trim()||'quote';
              return '#'+t;
            }
            if(act==='page'){
              var slug=String(l.page||l.linkPage||'').replace(/^\/+/,'').trim();
              return slug?('/'+slug):null;
            }
            var u=String(l.href||l.url||l.linkUrl||'').trim();
            return u||null;
          }
          nav.innerHTML=cols.filter(function(col){return col&&col.on!==false;}).map(function(col){
            ...
            if(mode==='card'){
              ...
              var bg=(col.cardBg&&String(col.cardBg).trim())||'';
              var sc=(col.cardStrokeColor&&String(col.cardStrokeColor).trim())||'';
              var linkOn=col.cardLinkOn===true;
              var href=linkOn?_ftResolveHref({action:col.cardLinkAction||'url',href:col.cardLinkUrl||col.cardLinkHref||'',page:col.cardLinkPage||'',target:col.cardLinkTarget||''}):null;
              ...
              var btnSw=lpBtnStrokePx(col.cardBtnStrokeWidth||((C.buttonLook&&C.buttonLook.strokeWidth)||'medium'));
              var st=[];
              ...
              if(btnOutline||btnStroke){
                ...
                st.push('--f-card-btn-stroke-w:'+(btnOutline?btnSw:'1.5px'));
              }
              ...
              if(href){
                var _cAct=String(col.cardLinkAction||'url').toLowerCase();
                var _cBlank=col.cardLinkBlank!==false;
                var ext=(_cAct==='url'||_cAct==='link')&&_cBlank;
            ...
            return '<div class="f-col'+spanCls+hideM+'"><h4>'+esc(col.title||'')+'</h4>'+links.map(function(l){
              ...
              var _lAct=String(l.action||l.linkAction||'url').toLowerCase();
              var _lBlank=(l.blank!==false && l.linkBlank!==false);
              var ext=(_lAct==='url'||_lAct==='link')&&_lBlank;
    ...
    (function(){ var S=C.sections||{};
      ...
      var _hdrRoot=document.querySelector('header.site');
      if(_hdrRoot){ _hdrRoot.classList.toggle('lp-hdr-cta-hide-m', H.showOnMobile===false); }
      var _hph=document.querySelector('.head-phone');
      var _telRaw=(C.phone||C.phoneText||((typeof SITE_CONFIG!=='undefined'&&(SITE_CONFIG.phone||SITE_CONFIG.phoneText))||'')||'');
      var _telHref=_telRaw?('tel:'+String(_telRaw).replace(/[^\d+]/g,'')):'';
      if(_hph){
        _hph.style.display=(H.showPhone===false)?'none':'';
        var _phEl=_hph.querySelector('a.head-phone-num,a#navPhone,b#navPhone,b,.head-phone-num')||_hph.querySelector('b,a');
        if(_phEl){
          if(_phEl.tagName!=='A'){
            var _a=document.createElement('a');
            _a.className=(_phEl.className?_phEl.className+' ':'')+'head-phone-num';
            if(_phEl.id) _a.id=_phEl.id; else _a.id='navPhone';
            _a.textContent=_phEl.textContent;
            _phEl.parentNode.replaceChild(_a,_phEl); _phEl=_a;
          } else { _phEl.classList.add('head-phone-num'); if(!_phEl.id) _phEl.id='navPhone'; }
          if(_telHref){ _phEl.setAttribute('href',_telHref); } else { _phEl.removeAttribute('href'); }
        }
      }
      if(hc){
        var _act=String(H.action||'call').toLowerCase();
        var _href='#quote';
        if(_act==='form'||_act==='quote'){ _href='#quote'; }
        else if(_act==='scroll'||_act==='section'){
          var _tg=String(H.linkTarget||H.target||'quote').replace(/^#/,'').trim()||'quote';
          _href='#'+_tg;
        }
        else if(_act==='page'){
          var _slug=String(H.linkPage||H.page||'').replace(/^\/+/,'').trim();
          _href=_slug?('/'+_slug):'#';
        }
        else if(_act==='url'||_act==='link'){
          var _u=String(H.linkUrl||H.href||H.url||'').trim();
          if(_u && !/^https?:\/\//i.test(_u) && _u.charAt(0)!=='/' && _u.charAt(0)!=='#') _u='https://'+_u;
          _href=_u||'#';
        }
        else if(_telHref){ _href=_telHref; }
        else { _href='#quote'; }
        hc.setAttribute('href',_href);
        if((_act==='url'||_act==='link') && H.linkBlank!==false){ hc.setAttribute('target','_blank'); hc.setAttribute('rel','noopener noreferrer'); }
        else { hc.removeAttribute('target'); hc.removeAttribute('rel'); }
        (function(){ function _hx(v){ var m=/^#([0-9a-fA-F]{6})$/.exec(String(v||'').trim()); if(!m) return null; var n=parseInt(m[1],16); return [(n>>16)&255,(n>>8)&255,n&255]; }
          function _sc(v){ v=(v==null?'':String(v)).trim(); return (/^#[0-9a-fA-F]{3,8}$/.test(v)||/^rgba?\([0-9.,%\s]+\)$/.test(v))?v:''; }
          var _hdr=document.querySelector('header.site');
          function _setHdr(name,val){ if(!_hdr) return; if(val) _hdr.style.setProperty(name,val); else _hdr.style.removeProperty(name); }
          var _fill=H.btnFill!==false; var _outline=H.btnOutline===true;
          var _sw=lpBtnStrokePx(H.btnStrokeWidth||((C.buttonLook&&C.buttonLook.strokeWidth)||'medium'));
          _setHdr('--hdr-cta-bg', _fill?(_sc(H.btnBg)||''):'transparent');
          _setHdr('--hdr-cta-fg', _sc(H.btnFg));
          _setHdr('--hdr-cta-border', _sc(H.btnBorder||H.btnStrokeColor)||(_outline?(_sc(H.btnFg)||''):''));
          _setHdr('--hdr-cta-stroke-w', (_outline||_sc(H.btnBorder||H.btnStrokeColor))?_sw:(_fill?'0px':_sw));
          _setHdr('--hdr-phone-fg', _sc(H.phoneFg||H.textColor));
          hc.classList.toggle('lp-btn-outline', _outline);
          hc.classList.toggle('lp-btn-nofill', !_fill && !_outline);
          if(H.glowOn===true){ var _g=_hx(H.glowColor)||_hx(_sc(H.btnBg))||_hx((C.theme&&C.theme.hivis)||'')||[0,0,0]; hc.style.setProperty('--btn-call-glow','rgba('+_g[0]+','+_g[1]+','+_g[2]+',.4)'); hc.classList.add('has-glow'); hc.style.boxShadow=''; }
      ...
      if(qs){
        ...
        if(Q.successTitle!=null){ var bg=qs.querySelector('.q-success .big'); if(bg) bg.textContent=Q.successTitle; } if(Q.successSub!=null||Q.successBody!=null){ var _bs=qs.querySelector('.q-success p'); if(_bs){ var _raw=Q.successSub!=null?Q.successSub:Q.successBody; var _biz=(C&&(C.business||C.businessName))||(typeof SITE_CONFIG!=='undefined'&&(SITE_CONFIG.business||SITE_CONFIG.businessName))||''; var _ph=(C&&(C.phoneText||C.phone))||(typeof SITE_CONFIG!=='undefined'&&(SITE_CONFIG.phoneText||SITE_CONFIG.phone))||''; _bs.textContent=String(_raw).replace(/\{\{businessName\}\}/g,_biz).replace(/\{\{phoneText\}\}/g,_ph).replace(/\{business\}/g,_biz); } }
      }
    ...
    try{
      ...
      if(!__PV){ document.querySelectorAll('.lp-seclabel').forEach(function(__x){ __x.remove(); }); document.documentElement.classList.remove('lp-preview-mode'); }
        ...
        if(!document.getElementById('lp-preview-fit-css')){ var __pf=document.createElement('style'); __pf.id='lp-preview-fit-css'; __pf.textContent='html.lp-preview-mode,html.lp-preview-mode body{min-height:0!important;height:auto!important}html.lp-preview-mode body{padding-top:0!important;padding-bottom:0!important}html.lp-preview-mode .mobile-call,html.lp-preview-mode #pf-lightbox,html.lp-preview-mode .pf-lb,html.lp-preview-mode .promo-fixed,html.lp-preview-mode .promo-pop,html.lp-preview-mode #promo-popup{display:none!important}'; document.head.appendChild(__pf); }
        try{ var __pwin=window.parent; if(__pwin&&__pwin!==window&&typeof __pwin.lpPreviewFitFrameHeight==='function'){ setTimeout(__pwin.lpPreviewFitFrameHeight,0); setTimeout(__pwin.lpPreviewFitFrameHeight,400); } }catch(_pf){}
    ...
    try{ _navMenuRender(C); }catch(_eNm2){}
  }
  function _lpMd(t){
    var s=esc(t);
    ...
    // Collapsible FAQs: lines starting with ??? Question
    s=s.replace(/(?:^|\n)\?\?\?\s+(.+)\n([\s\S]*?)(?=(?:\n\?\?\?\s+)|\n(?:<h[23]>)|$)/g,function(_m,q,a){
      var ans=String(a||'').trim().replace(/\n{2,}/g,'</p><p>').replace(/\n/g,' ');
      return '\n<details class="lp-faq"><summary>'+String(q||'').trim()+'</summary><div class="lp-faq-a"><p>'+ans+'</p></div></details>\n';
    });
    s=s.replace(/\n{2,}/g,'</p><p>').replace(/^(?!<[hud])(.+)$/gm,'$1');
    return s;
  }
  ...
  function _lpArticleBlock(p){ _lpPageStyleOnce(); var _im=(p.imgMode||'wrap-right'); var _ip=(p.imgPad!=null?p.imgPad:16); var _fm=(_im==='wrap-left')?('0 '+_ip+'px '+_ip+'px 0'):(_im==='wrap-right')?('0 0 '+_ip+'px '+_ip+'px'):('0 0 '+_ip+'px'); var _fig=p.img?('<figure class=\"lp-figure\" style=\"margin:'+_fm+'\"><img src=\"'+esc(p.img)+'\" alt=\"'+esc(p.h1||p.title||'')+'\"></figure>'):''; var _cls='lp-img-'+(p.img?_im:'none'); function _lpHex(v){ v=String(v||'').trim(); if(/^#?[0-9a-fA-F]{3}$/.test(v)){ v=v.charAt(0)==='#'?v:'#'+v; return '#'+v.charAt(1)+v.charAt(1)+v.charAt(2)+v.charAt(2)+v.charAt(3)+v.charAt(3); } if(/^#?[0-9a-fA-F]{6}$/.test(v)) return v.charAt(0)==='#'?v:'#'+v; return ''; } var _st=[]; var _h1=_lpHex(p.h1Color); if(_h1) _st.push('--lp-h1:'+_h1); var _h2=_lpHex(p.h2Color); if(_h2) _st.push('--lp-h2:'+_h2); var _pc=_lpHex(p.paragraphColor); if(_pc) _st.push('--lp-p:'+_pc); var _style=_st.length?(' style=\"'+_st.join(';')+'\"'):''; return '<section class=\"lp-page '+_cls+'\"'+_style+' data-sec=\"__lpArticle\"><div class=\"lp-wrap\"><div class=\"lp-text\">'+(p.h1?'<h1>'+esc(p.h1)+'</h1>':'')+_fig+'<div class=\"lp-body\"><p>'+_lpMd(p.body||'')+'</p></div></div>'+'<div class=\"lp-clear\"></div></div></section>'; }
  ...
  function _lpFooterApply(C){
    ...
    var inkExplicit=(/^#[0-9a-fA-F]{6}$/.test(F.ink||''));
    var ink=inkExplicit?F.ink:(_lpfLightBg(bg,op)?'#13161b':'#ffffff');
    var src='/assets/leadpages-logo.svg';
    ...
    function _paint(el){
      ...
      el.classList.add('lp-foot-logo');
      el.setAttribute('data-lp-logo-accent',accent);
      el.setAttribute('data-lp-logo-ink',inkExplicit?'custom':(_lpfLightBg(bg,op)?'custom':'light'));
      el.style.setProperty('--lp-logo-accent',accent,'important');
      el.style.setProperty('--lp-logo-ink',ink,'important');
      _sizeLogo(el);
    }
    ...
    var txt=fn.querySelector('.lp-foot-txt'); if(txt){ if(F.text&&String(F.text).trim()){ txt.textContent=F.text; txt.style.display=''; txt.style.color=ink; } else { txt.textContent=''; txt.style.display='none'; } }
  }
  function _lpBoot(){ // Snapshot #top BEFORE homepage applyCfg so Custom HTML mounts are not cloned pre-filled/bound
    _lpTopTemplate(); applyCfg(SITE_CONFIG); window.__lpLiveCfg=SITE_CONFIG; try{_lpFooterApply(SITE_CONFIG);}catch(_e){} try{ var _p=_lpActivePage(); if(_p) _lpRenderPage(_p); }catch(e){}  try{_lpRichWalk(document.body);}catch(e){} }
  ...
  function _navMenuRender(c){ var node=document.querySelector('[data-sec=\"navMenu\"]'); var nm=(c&&c.sections&&c.sections.navMenu)||{}; var _nmBar=document.querySelector('header.site .bar'); function _nmHeaderClear(){ if(!_nmBar) return; _nmBar.classList.remove('hnav-left','hnav-center','hnav-right','lp-logo-left','lp-logo-center','lp-logo-right','lp-logo-solo'); var _ex=_nmBar.querySelectorAll('.head-nav,.head-menu-btn'); for(var _k=_ex.length-1;_k>=0;_k--){ _ex[_k].parentNode.removeChild(_ex[_k]); } Array.prototype.forEach.call(document.querySelectorAll('.hnm-backdrop,.hnm-panel'),function(_oe){ if(_oe.parentNode) _oe.parentNode.removeChild(_oe); }); try{ document.documentElement.classList.remove('lp-compact-nav','hnm-btn-left','hnm-menu-open'); }catch(_e){} } try{ console.log('[navMenu JUN28-R] style:',nm.style,'icons:',nm.icons,'bg:',nm.bg); }catch(_e){} if(!node) return; if(nm.on!==true){ _nmHeaderClear(); node.style.setProperty('display','none','important'); node.innerHTML=''; return; } function _nmIc(v){ if(!v) return ''; v=String(v); var _i=(window.LP_ICONS&&window.LP_ICONS[v]); if(!_i) return ''; return '<svg class=\"lp-ic\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\">'+_i+'</svg>'; } function _nmChev(){ var _c=(window.LP_ICONS&&(window.LP_ICONS['chevron-down']||window.LP_ICONS.chevronDown))||'<polyline points=\"6 9 12 15 18 9\"></polyline>'; return '<svg class=\"lp-ic hn-chev\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\">'+_c+'</svg>'; } function _nmIsParent(it){ if(!it) return false; if(it.kind==='link') return false; if(it.kind==='parent') return true; return !!(Array.isArray(it.children)&&it.children.length); } function _nmKids(it){ return (it&&Array.isArray(it.children))?it.children.filter(function(ch){ return ch&&(ch.label||ch.icon); }):[]; } function _nmHref(it){ var href='#', blank=false; if(!it) return {href:href,blank:blank}; if(it.target==='external'){ href=it.url||'#'; blank=(it.opens!=='_self'); } else if(it.target&&String(it.target).indexOf('page:')===0){ href=(base+'/'+String(it.target).slice(5)).replace(/\/+/g,'/'); blank=(it.opens==='_blank'); } else if(it.target==='orderStorefront'){ href='#orderStorefront'; } else if(it.target==='orderShop'){ href=(base+'/order').replace(/\/+/g,'/')||'/order'; blank=(it.opens==='_blank'); } else if(it.target==='orderPortal'){ href=(base+'/orders').replace(/\/+/g,'/')||'/orders'; blank=(it.opens==='_blank'); } else { href=base||'/'; blank=(it.opens==='_blank'); } return {href:href,blank:blank}; } var items=(Array.isArray(nm.items)?nm.items:[]); var base=_siteBase(); var _imode=(nm.icons||'both'); var _nmHamOn=nm.mobileHamburger!==false; function _nmMobOk(it){ return !!(it&&it.mobile!==false); } function _nmMobKids(it){ return _nmKids(it).filter(_nmMobOk); } function _nmMobItems(arr){ return (arr||[]).filter(function(it){ return _nmMobOk(it)&&(it.label||it.icon||_nmIsParent(it)); }); } function _nmMountHam(mItems){ var _hnmWasOpen=false; try{ _hnmWasOpen=!!window.__hnmKeepOpen; }catch(_eWo){} try{ document.documentElement.classList.remove('lp-compact-nav','hnm-speed-fast','hnm-speed-normal','hnm-speed-slow','hnm-btn-left','hnm-menu-open'); }catch(_e0){} Array.prototype.forEach.call(document.querySelectorAll('.hnm-backdrop,.hnm-panel'),function(_oe){ if(_oe.parentNode) _oe.parentNode.removeChild(_oe); }); if(!_nmBar) return; var _btns=_nmBar.querySelectorAll('.head-menu-btn'); for(var _bi=_btns.length-1;_bi>=0;_bi--){ _btns[_bi].parentNode.removeChild(_btns[_bi]); } if(!_nmHamOn||!mItems||!mItems.length) return;
  function _scH(v){ v=(v==null?'':String(v)).trim(); return (/^#[0-9a-fA-F]{3,8}$/.test(v)||/^rgba?\([0-9.,%\s]+\)$/.test(v))?v:''; }
  function _setH(el,n,v){ if(!el) return; if(v) el.style.setProperty(n,v); else el.style.removeProperty(n); }
  function _hexRgb(hex){ hex=_scH(hex); if(!hex||hex.charAt(0)!=='#') return null; var h=hex.slice(1); if(h.length===3) h=h[0]+h[0]+h[1]+h[1]+h[2]+h[2]; if(h.length===8) h=h.slice(0,6); if(h.length!==6) return null; return {r:parseInt(h.slice(0,2),16),g:parseInt(h.slice(2,4),16),b:parseInt(h.slice(4,6),16)}; }
  function _rgba(hex,a){ var rgb=_hexRgb(hex); if(!rgb) return ''; return 'rgba('+rgb.r+','+rgb.g+','+rgb.b+','+a+')'; }
  var _hdr=document.querySelector('header.site');
  var _btnStyle=(nm.hamBtnStyle||'outline');
  if(['solid','ghost','outline','pill'].indexOf(_btnStyle)<0) _btnStyle='outline';
  var _btnSize=(nm.hamSize||'md'); if(['sm','md','lg'].indexOf(_btnSize)<0) _btnSize='md';
  var _btnPos=(nm.hamBtnPos==='left')?'left':'right';
  var _side;
  if(nm.hamSide==='left'||nm.hamSide==='right'){ _side=nm.hamSide; }
  else { _side=(_btnPos==='left')?'right':'left'; }
  var _fx=(nm.hamEffect||'slide'); if(['slide','fade','scale','slide-fade'].indexOf(_fx)<0) _fx='slide';
  var _spd=(nm.hamSpeed||'normal'); if(['fast','normal','slow'].indexOf(_spd)<0) _spd='normal';
  var _bd=(nm.hamBackdrop||'dim'); if(['dim','blur','solid'].indexOf(_bd)<0) _bd='dim';
  var _stagger=nm.hamStagger!==false;
  var _iconAnim=nm.hamIconAnim!==false;
  var _expandAll=nm.hamExpandAll===true;
  var _drawerBtn=(nm.hamDrawerBtnStyle||'links');
  if(['links','pills','solid','outline','ghost'].indexOf(_drawerBtn)<0) _drawerBtn='links';
  var _panelOp=nm.hamPanelOpacity; if(_panelOp==null||_panelOp==='') _panelOp=100; _panelOp=Math.max(0,Math.min(100,+_panelOp||100));
  var _panelBg=_scH(nm.hamPanelBg)||'';
  var _panelFg=_scH(nm.hamPanelFg)||'';
  try{ document.documentElement.classList.add('lp-compact-nav'); if(_btnPos==='left') document.documentElement.classList.add('hnm-btn-left'); document.documentElement.classList.add('hnm-speed-'+_spd); }catch(_eSp){}
  if(_hdr){
    _setH(_hdr,'--hn-ham-fg',_scH(nm.hamFg));
    _setH(_hdr,'--hn-ham-bg',_scH(nm.hamBg));
    _setH(_hdr,'--hn-ham-stroke',_scH(nm.hamStroke||nm.hamFg));
    if(nm.hamStrokeWidth!=null&&nm.hamStrokeWidth!=='') _setH(_hdr,'--hn-ham-stroke-w',parseFloat(nm.hamStrokeWidth)+'px'); else _hdr.style.removeProperty('--hn-ham-stroke-w');
    if(nm.hamRadius!=null&&nm.hamRadius!=='') _setH(_hdr,'--hn-ham-radius',(String(nm.hamRadius)==='999'?'999px':(parseFloat(nm.hamRadius)+'px'))); else _hdr.style.removeProperty('--hn-ham-radius');
    _setH(_hdr,'--hn-ham-panel-bg',_panelBg);
    if(_panelBg&&_panelOp<100){ var _pA=_rgba(_panelBg,_panelOp/100); if(_pA) _setH(_hdr,'--hn-ham-panel-bg-a',_pA); else _hdr.style.removeProperty('--hn-ham-panel-bg-a'); }
    else { _hdr.style.removeProperty('--hn-ham-panel-bg-a'); }
    _setH(_hdr,'--hn-ham-panel-fg',_panelFg);
    _setH(_hdr,'--hn-ham-hover-fg',_scH(nm.hamHoverFg));
    _setH(_hdr,'--hn-ham-hover-bg',_scH(nm.hamHoverBg));
    _setH(_hdr,'--hn-ham-sub-fg',_scH(nm.hamSubFg)||_panelFg);
    _setH(_hdr,'--hn-ham-sub-bg',_scH(nm.hamSubBg));
    _setH(_hdr,'--hn-ham-sub-hover-fg',_scH(nm.hamSubHoverFg));
    _setH(_hdr,'--hn-ham-sub-hover-bg',_scH(nm.hamSubHoverBg));
    _setH(_hdr,'--hn-ham-item-bg',_scH(nm.hamItemBg));
    _setH(_hdr,'--hn-ham-item-stroke',_scH(nm.hamItemStroke));
  }
  var _btn=document.createElement('button'); _btn.type='button';
  _btn.className='head-menu-btn ham-'+_btnStyle+(_btnSize!=='md'?' ham-size-'+_btnSize:'')+' ham-pos-'+_btnPos;
  if(_btnStyle==='pill'){ /* radius handled by class */ }
  _btn.setAttribute('aria-label','Menu'); _btn.setAttribute('aria-expanded','false');
  try{ var _bf=_hdr&&(_hdr.style.getPropertyValue('--hn-ham-fg')||_hdr.style.getPropertyValue('--hn-menu-btn-fg')||_hdr.style.getPropertyValue('--hn-fg')||_hdr.style.getPropertyValue('--hdr-phone-fg')||'').trim(); if(_bf) _btn.style.color=_bf; }catch(_eBf){}
  if(_iconAnim){ _btn.innerHTML='<span class="ham-bars" aria-hidden="true"><span></span><span></span><span></span></span>'; }
  else { var _mic=(window.LP_ICONS&&window.LP_ICONS.menu)||''; _btn.innerHTML=_mic?('<svg class="lp-ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">'+_mic+'</svg>'):'<span aria-hidden="true">&#9776;</span>'; }
  var _hc=_nmBar.querySelector('.head-call'); if(_hc) _nmBar.insertBefore(_btn,_hc); else _nmBar.appendChild(_btn);
  var _bdEl=document.createElement('div'); _bdEl.className='hnm-backdrop hnm-bd-'+_bd;
  var _pn=document.createElement('div');
  _pn.className='hnm-panel hnm-side-'+_side+' hnm-fx-'+_fx+' hnm-btn-'+_drawerBtn+(_stagger?' hnm-stagger':'');
  if(_hdr){ ['--hn-ham-panel-bg','--hn-ham-panel-bg-a','--hn-ham-panel-fg','--hn-ham-fg','--hn-ham-bg','--hn-ham-hover-fg','--hn-ham-hover-bg','--hn-ham-sub-fg','--hn-ham-sub-bg','--hn-ham-sub-hover-fg','--hn-ham-sub-hover-bg','--hn-ham-item-bg','--hn-ham-item-stroke'].forEach(function(k){ var v=_hdr.style.getPropertyValue(k); if(v) _pn.style.setProperty(k,v); else _pn.style.removeProperty(k); }); }
  (function(){ var sf=_scH(nm.hamSubFg)||_panelFg; if(sf) _pn.style.setProperty('--hn-ham-sub-fg',sf); var sb=_scH(nm.hamSubBg); if(sb) _pn.style.setProperty('--hn-ham-sub-bg',sb); var shf=_scH(nm.hamSubHoverFg); if(shf) _pn.style.setProperty('--hn-ham-sub-hover-fg',shf); var shb=_scH(nm.hamSubHoverBg); if(shb) _pn.style.setProperty('--hn-ham-sub-hover-bg',shb); var hf=_scH(nm.hamHoverFg); if(hf) _pn.style.setProperty('--hn-ham-hover-fg',hf); var hb=_scH(nm.hamHoverBg); if(hb) _pn.style.setProperty('--hn-ham-hover-bg',hb); })();
  var _ih='';
  if(_side==='right'){
    if(_iconAnim){ _ih='<button type="button" class="hnm-close" aria-label="Close"><span class="ham-bars" aria-hidden="true"><span></span><span></span><span></span></span></button>'; }
    else { var _xic=(window.LP_ICONS&&window.LP_ICONS.x)||''; _ih='<button type="button" class="hnm-close" aria-label="Close">'+(_xic?('<svg class="lp-ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">'+_xic+'</svg>'):'&times;')+'</button>'; }
  }
  _ih+=mItems.map(function(it){ if(!it) return ''; var label=esc(it.label||''); var ic=(_imode!=='text')?_nmIc(it.icon):''; if(!label&&!ic) return ''; if(_nmIsParent(it)){ var kids=_nmMobKids(it); var sub=kids.map(function(ch){ var cl=esc(ch.label||''); var cic=(_imode!=='text')?_nmIc(ch.icon):''; if(!cl&&!cic) return ''; var hr=_nmHref(ch); var ctxt=cl?('<span>'+cl+'</span>'):''; var _sf=_scH(nm.hamSubFg)||_panelFg; var _ssty=_sf?(' style="color:'+_sf+'"'):''; return '<a class="hnm-item hnm-child" href="'+esc(hr.href)+'"'+_ssty+(hr.blank?' target="_blank" rel="noopener"':'')+'>'+cic+ctxt+'</a>'; }).join(''); var ptxt=label?('<span>'+label+'</span>'):''; var _gOpen=_expandAll?' hnm-open':''; var _gExp=_expandAll?'true':'false'; return '<div class="hnm-group'+_gOpen+'"><button type="button" class="hnm-parent" aria-expanded="'+_gExp+'">'+ic+ptxt+_nmChev()+'</button><div class="hnm-children">'+sub+'</div></div>'; } var hr=_nmHref(it); var txt=label?('<span>'+label+'</span>'):''; return '<a class="hnm-item" href="'+esc(hr.href)+'"'+(hr.blank?' target="_blank" rel="noopener"':'')+'>'+ic+txt+'</a>'; }).join('');
  _pn.innerHTML=_ih; document.body.appendChild(_bdEl); document.body.appendChild(_pn);
  var _pinHam=function(){ try{ var r=_btn.getBoundingClientRect(); _btn.classList.add('hnm-pinned'); _btn.style.top=Math.round(r.top)+'px'; _btn.style.left=Math.round(r.left)+'px'; _btn.style.right='auto'; _btn.style.bottom='auto'; }catch(_eP){} };
  var _unpinHam=function(){ try{ _btn.classList.remove('hnm-pinned'); _btn.style.top=''; _btn.style.left=''; _btn.style.right=''; _btn.style.bottom=''; }catch(_eU){} };
  var _open=function(){ try{ window.__hnmKeepOpen=true; document.documentElement.classList.add('hnm-menu-open'); }catch(_eK){} _pinHam(); _bdEl.classList.add('hnm-open'); _pn.classList.add('hnm-open'); _btn.classList.add('is-open'); _btn.setAttribute('aria-expanded','true'); };
  var _close=function(){ try{ window.__hnmKeepOpen=false; document.documentElement.classList.remove('hnm-menu-open'); }catch(_eK2){} _bdEl.classList.remove('hnm-open'); _pn.classList.remove('hnm-open'); _btn.classList.remove('is-open'); _btn.setAttribute('aria-expanded','false'); _unpinHam(); };
  _btn.addEventListener('click',function(){ (_pn.classList.contains('hnm-open')?_close:_open)(); });
  _bdEl.addEventListener('click',_close);
  var _cl=_pn.querySelector('.hnm-close'); if(_cl) _cl.addEventListener('click',_close);
  Array.prototype.forEach.call(_pn.querySelectorAll('.hnm-parent'),function(pb){ pb.addEventListener('click',function(){ var g=pb.parentNode; if(!g) return; var on=g.classList.toggle('hnm-open'); pb.setAttribute('aria-expanded',on?'true':'false'); }); });
  Array.prototype.forEach.call(_pn.querySelectorAll('a.hnm-item'),function(a){ a.addEventListener('click',_close); });
  if(_hnmWasOpen){ try{ setTimeout(_open,0); }catch(_eRe){ _open(); } }
} if((nm.placement||'header')==='header'){ node.style.setProperty('display','none','important'); node.innerHTML=''; if(_nmBar){ _nmHeaderClear(); var _lpos=(c&&c.logo&&c.logo.position)||'left'; var _brand=_nmBar.querySelector('a.brand'); if(_brand){ _brand.style.marginLeft=''; _brand.style.marginRight=''; } var _hbld=function(arr){ return arr.map(function(it){ if(!it) return ''; var label=esc(it.label||''); var ic=(_imode!=='text')?_nmIc(it.icon):''; if(!label&&!ic) return ''; if(_nmIsParent(it)){ var kids=_nmKids(it); var sub=kids.map(function(ch){ var cl=esc(ch.label||''); var cic=(_imode!=='text')?_nmIc(ch.icon):''; if(!cl&&!cic) return ''; var hr=_nmHref(ch); var ctxt=((_imode==='icon'&&cic)?'':'<span class=\"hn-txt\">'+cl+'</span>'); return '<a class=\"hn-sub-link\" href=\"'+esc(hr.href)+'\"'+(hr.blank?' target=\"_blank\" rel=\"noopener\"':'')+'>'+cic+ctxt+'</a>'; }).join(''); var ptxt=((_imode==='icon'&&ic)?'':'<span class=\"hn-txt\">'+label+'</span>'); return '<div class=\"hn-drop\"><button type=\"button\" class=\"hn-link hn-parent\" aria-haspopup=\"true\" aria-expanded=\"false\">'+ic+ptxt+_nmChev()+'</button><div class=\"hn-sub\" role=\"menu\">'+sub+'</div></div>'; } var hr=_nmHref(it); var txt=((_imode==='icon'&&ic)?'':'<span class=\"hn-txt\">'+label+'</span>'); return '<a class=\"hn-link\" href=\"'+esc(hr.href)+'\"'+(hr.blank?' target=\"_blank\" rel=\"noopener\"':'')+'>'+ic+txt+'</a>'; }).join(''); }; var _hvis=items.filter(function(it){ return it && (it.label||it.icon||_nmIsParent(it)); }); var _ico=(_imode==='icon')?' nm-icononly':''; var _stCls=' nm-'+(nm.style||'pills'); _nmBar.classList.add('hnav-'+_lpos); if(_lpos==='center'){ var _mid=Math.ceil(_hvis.length/2); var _nl=document.createElement('nav'); _nl.className='head-nav head-nav-l'+_stCls+_ico; _nl.innerHTML=_hbld(_hvis.slice(0,_mid)); var _nr=document.createElement('nav'); _nr.className='head-nav head-nav-r'+_stCls+_ico; _nr.innerHTML=_hbld(_hvis.slice(_mid)); if(_brand){ _nmBar.insertBefore(_nl,_brand); if(_brand.nextSibling){ _nmBar.insertBefore(_nr,_brand.nextSibling); } else { _nmBar.appendChild(_nr); } } } else { var _nv=document.createElement('nav'); _nv.className='head-nav'+_stCls+_ico; _nv.innerHTML=_hbld(_hvis); if(_lpos==='right'){ _nmBar.insertBefore(_nv,_nmBar.firstChild); } else if(_brand){ if(_brand.nextSibling){ _nmBar.insertBefore(_nv,_brand.nextSibling); } else { _nmBar.appendChild(_nv); } } else { _nmBar.insertBefore(_nv,_nmBar.firstChild); } } }
          (function(){ var _hdr=document.querySelector('header.site'); if(!_hdr) return; function _sc(v){ v=(v==null?'':String(v)).trim(); return (/^#[0-9a-fA-F]{3,8}$/.test(v)||/^rgba?\([0-9.,%\s]+\)$/.test(v))?v:''; } function _set(n,v){ if(v) _hdr.style.setProperty(n,v); else _hdr.style.removeProperty(n); } function _lum(hex){ hex=_sc(hex); if(!hex||hex.charAt(0)!=='#') return null; var h=hex.slice(1); if(h.length===3) h=h[0]+h[0]+h[1]+h[1]+h[2]+h[2]; if(h.length!==6) return null; var r=parseInt(h.slice(0,2),16),g=parseInt(h.slice(2,4),16),b=parseInt(h.slice(4,6),16); return (0.2126*r+0.7152*g+0.0722*b)/255; } function _autoFg(bg,fallback){ var L=_lum(bg); if(L==null) return _sc(fallback)||''; return L>0.62?'#1a2230':'#ffffff'; } function _px(v,defv){ if(v==null||v==='') return defv; var n=parseFloat(v); return isFinite(n)?(n+'px'):defv; } var _st=(nm.style||'pills'); var _navs=_nmBar.querySelectorAll('.head-nav'); for(var _ni=0;_ni<_navs.length;_ni++){   var _el=_navs[_ni];   _el.classList.remove('nm-pills','nm-solid','nm-outline','nm-shadow','nm-buttons','nm-tabs','nm-underline','nm-links','nm-ghost');   _el.classList.add('nm-'+_st); } _set('--hn-fg', _sc(nm.fg||nm.linkColor)); _set('--hn-hover-fg', _sc(nm.hoverFg)); _set('--hn-hover-bg', _sc(nm.hoverBg)); var _fill=_sc(nm.fill); var _stroke=_sc(nm.stroke); if(_fill) _set('--hn-fill', _fill); else _hdr.style.removeProperty('--hn-fill'); if(_stroke) _set('--hn-stroke', _stroke); else _hdr.style.removeProperty('--hn-stroke'); if(nm.strokeWidth!=null&&nm.strokeWidth!=='') _set('--hn-stroke-w', _px(nm.strokeWidth,'0px')); else _hdr.style.removeProperty('--hn-stroke-w'); _set('--hn-hover-stroke', _sc(nm.hoverStroke)); if(nm.hoverStrokeWidth!=null&&nm.hoverStrokeWidth!=='') _set('--hn-hover-stroke-w', _px(nm.hoverStrokeWidth,'')); else _hdr.style.removeProperty('--hn-hover-stroke-w'); var _subBg=_sc(nm.subBg); _set('--hn-sub-bg', _subBg); var _subFg=_sc(nm.subFg)||(_subBg?_autoFg(_subBg, nm.fg||'#ffffff'):''); _set('--hn-sub-fg', _subFg); _set('--hn-sub-hover-fg', _sc(nm.subHoverFg)); _set('--hn-sub-hover-bg', _sc(nm.subHoverBg)); _set('--hn-sub-stroke', _sc(nm.subStroke)); _set('--hn-sub-stroke-w', _px(nm.subStrokeWidth!=null&&nm.subStrokeWidth!==''?nm.subStrokeWidth:'1','1px')); _set('--hn-sub-hover-stroke', _sc(nm.subHoverStroke)); if(nm.subHoverStrokeWidth!=null&&nm.subHoverStrokeWidth!=='') _set('--hn-sub-hover-stroke-w', _px(nm.subHoverStrokeWidth,'0px')); else _hdr.style.removeProperty('--hn-sub-hover-stroke-w'); try{ var _pn=document.querySelector('.hnm-panel'); if(_pn){ ['--hn-fg','--hn-hover-fg','--hn-hover-bg','--hn-fill','--hn-stroke','--hn-stroke-w','--hn-hover-stroke','--hn-hover-stroke-w','--hn-sub-bg','--hn-sub-fg','--hn-sub-hover-fg','--hn-sub-hover-bg','--hn-sub-stroke','--hn-sub-stroke-w','--hn-sub-hover-stroke','--hn-sub-hover-stroke-w'].forEach(function(k){ var v=_hdr.style.getPropertyValue(k); if(v) _pn.style.setProperty(k,v); else _pn.style.removeProperty(k); }); } }catch(_eP){} })();
          try{ var _hdrM=document.querySelector('header.site'); if(_hdrM){ function _scM(v){ v=(v==null?'':String(v)).trim(); return (/^#[0-9a-fA-F]{3,8}$/.test(v)||/^rgba?\([0-9.,%\s]+\)$/.test(v))?v:''; } function _lumM(hex){ hex=_scM(hex); if(!hex||hex.charAt(0)!=='#') return null; var h=hex.slice(1); if(h.length===3) h=h[0]+h[0]+h[1]+h[1]+h[2]+h[2]; if(h.length!==6) return null; var r=parseInt(h.slice(0,2),16),g=parseInt(h.slice(2,4),16),b=parseInt(h.slice(4,6),16); return (0.2126*r+0.7152*g+0.0722*b)/255; } var _bgM=_scM(_hdrM.style.getPropertyValue('--hdr-bg'))||_scM(nm.bg)||''; var _fgM=_scM(nm.hamFg)||_scM(nm.fg||nm.linkColor)||_scM(_hdrM.style.getPropertyValue('--hn-fg'))||_scM(_hdrM.style.getPropertyValue('--hdr-phone-fg')); if(!_fgM){ var L=_lumM(_bgM); _fgM=(L!=null&&L>0.62)?'#1a2230':'#ffffff'; } _hdrM.style.setProperty('--hn-menu-btn-fg', _fgM); } }catch(_eHm){} _nmMountHam(_nmMobItems(items));
 (function(){ function _nmWireDrops(root){ root=root||document; var drops=root.querySelectorAll('.hn-drop,.nm-drop'); for(var i=0;i<drops.length;i++){ (function(drop){ if(drop.__nmWired) return; drop.__nmWired=1; var btn=drop.querySelector('.hn-parent,.nm-parent'); if(!btn) return; btn.addEventListener('click',function(e){ e.preventDefault(); e.stopPropagation(); var open=!drop.classList.contains('hn-open'); Array.prototype.forEach.call(document.querySelectorAll('.hn-drop.hn-open,.nm-drop.hn-open'),function(d){ if(d!==drop){ d.classList.remove('hn-open'); var b=d.querySelector('.hn-parent,.nm-parent'); if(b) b.setAttribute('aria-expanded','false'); } }); drop.classList.toggle('hn-open',open); btn.setAttribute('aria-expanded',open?'true':'false'); }); })(drops[i]); } } if(!window.__nmDropDocClose){ window.__nmDropDocClose=1; document.addEventListener('click',function(e){ if(e.target.closest&&e.target.closest('.hn-drop,.nm-drop')) return; Array.prototype.forEach.call(document.querySelectorAll('.hn-drop.hn-open,.nm-drop.hn-open'),function(d){ d.classList.remove('hn-open'); var b=d.querySelector('.hn-parent,.nm-parent'); if(b) b.setAttribute('aria-expanded','false'); }); }); document.addEventListener('keydown',function(e){ if(e.key!=='Escape') return; Array.prototype.forEach.call(document.querySelectorAll('.hn-drop.hn-open,.nm-drop.hn-open'),function(d){ d.classList.remove('hn-open'); var b=d.querySelector('.hn-parent,.nm-parent'); if(b) b.setAttribute('aria-expanded','false'); }); }); } _nmWireDrops(document); })();
          return; } _nmHeaderClear(); var html=items.map(function(it){ if(!it) return ''; var label=esc(it.label||''); var ic=(_imode!=='text')?_nmIc(it.icon):''; if(!label&&!ic) return ''; if(_nmIsParent(it)){ var kids=_nmKids(it); var sub=kids.map(function(ch){ var cl=esc(ch.label||''); var cic=(_imode!=='text')?_nmIc(ch.icon):''; if(!cl&&!cic) return ''; var hr=_nmHref(ch); var ctxt=((_imode==='icon'&&cic)?'':'<span class=\"nm-txt\">'+cl+'</span>'); return '<a class=\"nm-sub-link\" href=\"'+esc(hr.href)+'\"'+(hr.blank?' target=\"_blank\" rel=\"noopener\"':'')+'>'+cic+ctxt+'</a>'; }).join(''); var ptxt=((_imode==='icon'&&ic)?'':'<span class=\"nm-txt\">'+label+'</span>'); return '<div class=\"nm-drop\"><button type=\"button\" class=\"nm-link nm-parent\" aria-haspopup=\"true\">'+ic+ptxt+_nmChev()+'</button><div class=\"nm-sub\" role=\"menu\">'+sub+'</div></div>'; } var hr=_nmHref(it); var txt=((_imode==='icon'&&ic)?'':'<span class=\"nm-txt\">'+label+'</span>'); return '<a class=\"nm-link\" href=\"'+esc(hr.href)+'\"'+(hr.blank?' target=\"_blank\" rel=\"noopener\"':'')+'>'+ic+txt+'</a>'; }).join(''); var _io=(_imode==='icon'?' nm-icononly':''); node.className='nav-menu-sec nm-'+(nm.style||'pills')+' nm-al-'+(nm.align||'left')+_io; var _bg=(nm.bg||'').trim(); if(_bg){ node.style.setProperty('background',_bg,'important'); } else { node.style.removeProperty('background'); } var _lt=false; if(_bg){ var _hx=_bg.replace('#',''); if(_hx.length===3){ _hx=_hx.charAt(0)+_hx.charAt(0)+_hx.charAt(1)+_hx.charAt(1)+_hx.charAt(2)+_hx.charAt(2); } if(/^[0-9a-fA-F]{6}$/.test(_hx)){ var _r=parseInt(_hx.slice(0,2),16),_g=parseInt(_hx.slice(2,4),16),_b=parseInt(_hx.slice(4,6),16); _lt=(0.2126*_r+0.7152*_g+0.0722*_b)>165; } } node.style.setProperty('--nm-fg',_lt?'#1a2230':'#fff'); node.style.setProperty('--nm-fill',(nm.fill&&nm.fill.trim())?nm.fill.trim():(_lt?'rgba(0,0,0,.07)':'rgba(255,255,255,.13)')); var _stk=(nm.stroke||'').trim(); node.style.setProperty('--nm-stroke',_stk?_stk:(_lt?'rgba(0,0,0,.22)':'rgba(255,255,255,.4)')); var _fg=(nm.fg||nm.linkColor||'').trim(); if(_fg) node.style.setProperty('--nm-fg',_fg); else node.style.removeProperty('--nm-fg'); node.innerHTML='<nav class=\"nm-inner\">'+html+'</nav>'; node.style.setProperty('display','block','important'); try{ var _hdrM=document.querySelector('header.site'); if(_hdrM){ function _scM(v){ v=(v==null?'':String(v)).trim(); return (/^#[0-9a-fA-F]{3,8}$/.test(v)||/^rgba?\([0-9.,%\s]+\)$/.test(v))?v:''; } function _lumM(hex){ hex=_scM(hex); if(!hex||hex.charAt(0)!=='#') return null; var h=hex.slice(1); if(h.length===3) h=h[0]+h[0]+h[1]+h[1]+h[2]+h[2]; if(h.length!==6) return null; var r=parseInt(h.slice(0,2),16),g=parseInt(h.slice(2,4),16),b=parseInt(h.slice(4,6),16); return (0.2126*r+0.7152*g+0.0722*b)/255; } var _bgM=_scM(_hdrM.style.getPropertyValue('--hdr-bg'))||_scM(nm.bg)||''; var _fgM=_scM(nm.hamFg)||_scM(nm.fg||nm.linkColor)||_scM(_hdrM.style.getPropertyValue('--hn-fg'))||_scM(_hdrM.style.getPropertyValue('--hdr-phone-fg')); if(!_fgM){ var L=_lumM(_bgM); _fgM=(L!=null&&L>0.62)?'#1a2230':'#ffffff'; } _hdrM.style.setProperty('--hn-menu-btn-fg', _fgM); } }catch(_eHm){} _nmMountHam(_nmMobItems(items)); (function(){ function _sc(v){ v=(v==null?'':String(v)).trim(); return (/^#[0-9a-fA-F]{3,8}$/.test(v)||/^rgba?\([0-9.,%\s]+\)$/.test(v))?v:''; } function _set(n,v){ if(v) node.style.setProperty(n,v); else node.style.removeProperty(n); } function _lum(hex){ hex=_sc(hex); if(!hex||hex.charAt(0)!=='#') return null; var h=hex.slice(1); if(h.length===3) h=h[0]+h[0]+h[1]+h[1]+h[2]+h[2]; if(h.length!==6) return null; var r=parseInt(h.slice(0,2),16),g=parseInt(h.slice(2,4),16),b=parseInt(h.slice(4,6),16); return (0.2126*r+0.7152*g+0.0722*b)/255; } function _autoFg(bg,fallback){ var L=_lum(bg); if(L==null) return _sc(fallback)||''; return L>0.62?'#1a2230':'#ffffff'; } function _px(v,defv){ if(v==null||v==='') return defv; var n=parseFloat(v); return isFinite(n)?(n+'px'):defv; } var _subBg=_sc(nm.subBg); _set('--nm-sub-bg', _subBg); var _subFg=_sc(nm.subFg)||(_subBg?_autoFg(_subBg, nm.fg||'#ffffff'):''); _set('--nm-sub-fg', _subFg); _set('--nm-sub-hover-fg', _sc(nm.subHoverFg)); _set('--nm-sub-hover-bg', _sc(nm.subHoverBg)); _set('--nm-sub-stroke', _sc(nm.subStroke)); _set('--nm-sub-stroke-w', _px(nm.subStrokeWidth!=null&&nm.subStrokeWidth!==''?nm.subStrokeWidth:'1','1px')); _set('--nm-sub-hover-stroke', _sc(nm.subHoverStroke)); if(nm.subHoverStrokeWidth!=null&&nm.subHoverStrokeWidth!=='') _set('--nm-sub-hover-stroke-w', _px(nm.subHoverStrokeWidth,'0px')); else node.style.removeProperty('--nm-sub-hover-stroke-w'); })(); (function(){ function _nmWireDrops(root){ root=root||document; var drops=root.querySelectorAll('.hn-drop,.nm-drop'); for(var i=0;i<drops.length;i++){ (function(drop){ if(drop.__nmWired) return; drop.__nmWired=1; var btn=drop.querySelector('.hn-parent,.nm-parent'); if(!btn) return; btn.addEventListener('click',function(e){ e.preventDefault(); e.stopPropagation(); var open=!drop.classList.contains('hn-open'); Array.prototype.forEach.call(document.querySelectorAll('.hn-drop.hn-open,.nm-drop.hn-open'),function(d){ if(d!==drop){ d.classList.remove('hn-open'); var b=d.querySelector('.hn-parent,.nm-parent'); if(b) b.setAttribute('aria-expanded','false'); } }); drop.classList.toggle('hn-open',open); btn.setAttribute('aria-expanded',open?'true':'false'); }); })(drops[i]); } } if(!window.__nmDropDocClose){ window.__nmDropDocClose=1; document.addEventListener('click',function(e){ if(e.target.closest&&e.target.closest('.hn-drop,.nm-drop')) return; Array.prototype.forEach.call(document.querySelectorAll('.hn-drop.hn-open,.nm-drop.hn-open'),function(d){ d.classList.remove('hn-open'); var b=d.querySelector('.hn-parent,.nm-parent'); if(b) b.setAttribute('aria-expanded','false'); }); }); document.addEventListener('keydown',function(e){ if(e.key!=='Escape') return; Array.prototype.forEach.call(document.querySelectorAll('.hn-drop.hn-open,.nm-drop.hn-open'),function(d){ d.classList.remove('hn-open'); var b=d.querySelector('.hn-parent,.nm-parent'); if(b) b.setAttribute('aria-expanded','false'); }); }); } _nmWireDrops(document); })(); }
