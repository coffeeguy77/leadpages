  ...
  function applyCfg(C){
    ...
    if(Array.isArray(C.services)){ var grid=document.querySelector('.svcs'); if(grid){
      ...
      grid.innerHTML=on.map(function(s){
        ...
        var fc=_svcHex(s.color); if(fc){ st.push('--svc-body:'+fc); st.push('color:'+fc); }
        ...
        var media=img?('<div class="ic has-img"><img src="'+esc(img)+'" alt=""></div>'):('<div class="ic">'+lpIcon(s.icon)+'</div>');
        return '<div class="svc"'+(st.length?' style="'+st.join(';')+'"':'')+'>'+media+'<h3>'+esc(s.title||'')+'</h3><p>'+esc(s.body||'')+'</p></div>';
      }).join('');
    } }
