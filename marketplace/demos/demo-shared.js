...
(function(){
  ...
  function _lpMergedPageConfig(p){ var base=JSON.parse(JSON.stringify(_lpLiveCfg()||{})); if(!base.sections) base.sections={}; (p.pageApps||[]).forEach(function(a){ var k=a.key||a.section_key; if(!k) return; if(a.mode==='unique'&&p.pageSections&&p.pageSections[k]) base.sections[k]=JSON.parse(JSON.stringify(p.pageSections[k])); else if(_lpLiveCfg()&&_lpLiveCfg().sections&&_lpLiveCfg().sections[k]) base.sections[k]=JSON.parse(JSON.stringify(_lpLiveCfg().sections[k])); if(!base.sections[k]) base.sections[k]={}; base.sections[k].on=true; }); base.sectionOrder=_lpPageLayoutOrder(p); return base; }
