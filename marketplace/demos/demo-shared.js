...
function applyCfg(C){
  ...
  var _side=(nm.hamSide==='left')?'left':'right';
  ...
  try{ document.documentElement.classList.add('hnm-speed-'+_spd); }catch(_eSp){}
  if(_hdr){
  ...
  _btn.className='head-menu-btn ham-'+_btnStyle+(_btnSize!=='md'?' ham-size-'+_btnSize:'')+(_btnStyle==='pill'?'':'');
  ...
  var _xic=(window.LP_ICONS&&window.LP_ICONS.x)||'';
  var _ih='<button type="button" class="hnm-close" aria-label="Close">'+(_xic?('<svg class="lp-ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">'+_xic+'</svg>'):'&times;')+'</button>';
  ...
  var _open=function(){ try{ window.__hnmKeepOpen=true; }catch(_eK){} _bdEl.classList.add('hnm-open'); _pn.classList.add('hnm-open'); _btn.classList.add('is-open'); _btn.setAttribute('aria-expanded','true'); };
  var _close=function(){ try{ window.__hnmKeepOpen=false; }catch(_eK2){} _bdEl.classList.remove('hnm-open'); _pn.classList.remove('hnm-open'); _btn.classList.remove('is-open'); _btn.setAttribute('aria-expanded','false'); };
