...
(function(){
  ...
  function _lpArticleBlock(p){ _lpPageStyleOnce(); var cfg=_lpLiveCfg(), phone=(cfg&&cfg.phone)||''; var _sc=(p.showCall!==false)&&!!phone; var _sq=(p.showQuote!==false); var _ctaIn=(_sc?'<a class=\"lp-btn lp-call\" href=\"tel:'+esc(String(phone).replace(/[^0-9+]/g,''))+'\">Call '+esc(phone)+'</a>':'')+(_sq?'<a class=\"lp-btn lp-quote\" href=\"'+esc(_siteBase()||'/')+'\">Get a free quote</a>':''); var cta=_ctaIn?('<div class=\"lp-cta\">'+_ctaIn+'</div>'):''; var _im=(p.imgMode||'center'); var _ip=(p.imgPad!=null?p.imgPad:16); var _fm=(_im==='wrap-left')?('0 '+_ip+'px '+_ip+'px 0'):(_im==='wrap-right')?('0 0 '+_ip+'px '+_ip+'px'):('0 0 '+_ip+'px'); var _fig=p.img?('<figure class=\"lp-figure\" style=\"margin:'+_fm+'\"><img src=\"'+esc(p.img)+'\" alt=\"'+esc(p.h1||p.title||'')+'\"></figure>'):''; var _cls='lp-img-'+(p.img?_im:'none'); return '<section class=\"lp-page '+_cls+'\" data-sec=\"__lpArticle\"><div class=\"lp-wrap\"><div class=\"lp-text\">'+(p.h1?'<h1>'+esc(p.h1)+'</h1>':'')+_fig+'<div class=\"lp-body\"><p>'+_lpMd(p.body||'')+'</p></div></div>'+cta+'<div class=\"lp-clear\"></div></div></section>'; }
  function _lpMergedPageConfig(p){ var base=JSON.parse(JSON.stringify(_lpLiveCfg()||{})); if(!base.sections) base.sections={}; (p.pageApps||[]).forEach(function(a){ var k=a.key||a.section_key; if(!k) return; if(a.mode==='unique'&&p.pageSections&&p.pageSections[k]) base.sections[k]=JSON.parse(JSON.stringify(p.pageSections[k])); else if(_lpLiveCfg()&&_lpLiveCfg().sections&&_lpLiveCfg().sections[k]) base.sections[k]=JSON.parse(JSON.stringify(_lpLiveCfg().sections[k])); if(!base.sections[k]) base.sections[k]={}; base.sections[k].on=true; }); var ord=(p.pageApps||[]).map(function(a){ return a.key||a.section_key; }).filter(Boolean); if(p.h1||p.body) ord.push('__lpArticle'); base.sectionOrder=ord; return base; }
  function _lpRenderPageHybrid(p){ var main=document.getElementById('top')||document.querySelector('main'); if(!main) return; _lpPageMeta(p); var tpl=_lpTopTemplate(); if(!tpl){ main.innerHTML='<section data-sec=\"navMenu\"></section>'+_lpArticleBlock(p); try{_navMenuRender(_lpMergedPageConfig(p));}catch(e){} try{ window.scrollTo(0,0); }catch(e){} return; } var tmp=document.createElement('div'); tmp.innerHTML=tpl; var html=''; var nm=tmp.querySelector('[data-sec=\"navMenu\"]'); if(nm) html+=nm.outerHTML; (p.pageApps||[]).forEach(function(a){ var k=a.key||a.section_key; var node=tmp.querySelector('[data-sec=\"'+k+'\"]'); if(node) html+=node.outerHTML; }); if(p.h1||p.body) html+=_lpArticleBlock(p); main.innerHTML=html||tpl; var C=_lpMergedPageConfig(p); applyCfg(C); try{_navMenuRender(C);}catch(e){} try{ applySectionAppearances(C); }catch(e){} try{ window.scrollTo(0,0); }catch(e){} }
