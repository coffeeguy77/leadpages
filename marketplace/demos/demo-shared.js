...
function _pgRenderPremiumGallery(PG, node){
  ...
  PG=_pgSeed(PG||{});
  (function ensureIds(){
    function slug(s){ return String(s||'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,40)||('id'+Math.random().toString(36).slice(2,7)); }
    (PG.categories||[]).forEach(function(c,i){ if(c&&!c.id) c.id='cat-'+slug(c.name||i); });
    (PG.albums||[]).forEach(function(a,i){ if(a&&!a.id) a.id='alb-'+slug(a.title||i); });
    (PG.images||[]).forEach(function(im,i){ if(im&&!im.id) im.id='img-'+i+'-'+slug(im.title||''); if(im&&im.featured==='true') im.featured=true; if(im&&typeof im.tags==='string') im.tags=im.tags.split(',').map(function(x){return x.trim();}).filter(Boolean); });
  })();
  ...
  if(showFilters){
    var tags={};
    imgs.forEach(function(im){ (Array.isArray(im.tags)?im.tags:String(im.tags||'').split(',').map(function(x){return x.trim();}).filter(Boolean)).forEach(function(t){ if(t) tags[t]=(tags[t]||0)+1; }); });
