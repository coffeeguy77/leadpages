  ...
  function applyCfg(C){
    ...
    if(Array.isArray(C.services)){ var grid=document.querySelector('.svcs'); if(grid){
      ...
      grid.innerHTML=on.map(function(s){
        ...
        var glowOn=s.glowOn===true; var glow=_svcHex(s.glowColor)||_svcHex((C.theme&&C.theme.pipe)||'')||'#1f7bb8';
        if(glowOn){ var gr=parseInt(glow.slice(1,3),16),gg=parseInt(glow.slice(3,5),16),gb=parseInt(glow.slice(5,7),16); st.push('--svc-glow:rgba('+gr+','+gg+','+gb+',.4)'); }
        var img=(s.image&&String(s.image).trim())||'';
        var icCls='ic'+(img?' has-img':'')+(glowOn?' has-glow':'');
