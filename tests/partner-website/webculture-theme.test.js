...
test('Web Culture build — renders premium sections from profile', function() {
  ...
  assert.ok(html.includes('Average response'));
  assert.ok(/wc-hero-response-value/.test(html));
  assert.ok(!html.includes('data-prm-hero-journey'));
  assert.ok(!html.includes('Visitor lands on website'));
  ...
  assert.ok(html.includes('Canberra Local Support'));
  ...
  assert.ok(html.includes('Can this trades website be customised?'));
  assert.ok(html.includes('prm-demo-trust'));
  assert.ok(html.includes('Live website'));
  assert.ok(html.includes('Built with LeadPages'));
  assert.ok(!html.includes('prm-demo-feature-list'));
  ...
  assert.ok(html.includes('Google Reviews'));
  assert.ok(html.includes('data-prm-review-interval="10000"'));
  assert.ok(html.includes('prm-service-detail'));
  ...
  assert.ok(html.includes('prm-final-cta--premium'));
  assert.ok(html.includes('prm-btn--cta-lg'));
