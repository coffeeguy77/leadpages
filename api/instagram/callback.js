...
module.exports = async (req, res) => {
  // Extract code directly from the raw URL to avoid any framework query-parsing quirks
  const rawUrl  = req.url || '';
  const fullUrl = 'https://www.leadpages.com.au' + rawUrl;
  let parsedCode = null;
  try {
  ...
  try {
    ...
    // Use raw-parsed code; fall back to framework-parsed; strip #_ Meta appends
    let code = (parsedCode || q.code || '').toString().replace(/#_$/, '').trim();
    if(!code) return done(res, slug, 'error', 'no_code');

    console.log('[ig-callback] slug=' + slug + ' codeLen=' + code.length
      + ' codeStart=' + code.slice(0,8) + ' redirect=' + REDIRECT + ' appId=' + APP_ID);

    // ---- step 1: code -> short-lived token ----
    step = 'short_token';
    const body = 'client_id='     + encodeURIComponent(APP_ID)
